...
function setSize(){
	...
	$("wingMap").style.width = winX - 3;
	$("wingMap").style.height = winY - 3;
...
window.onresize = function(){
	setSize();
	richmap.resizeMap(winX - 1,winY - 1);
}
...
function loadSite(){
	...
	loadBaseStationScript.src = "../siteposition.json?action=getCustomerSiteList&callback=resoluteCallBack&req=" + Math.random();
...
function showStationAngels(){
	...
	if(layer >= 13){
		...
		for(var i=0;i<returnStationData.length;i++){
			...
			if(x > MBR[0] && x < MBR[2] && y>MBR[3] && y<MBR[1]){
				if(returnStationData[i][4].length > 0){
					for(var j=0;j<returnStationData[i][4].length;j++){
						//window.status = "2..." + i + "," + j + ".." + returnStationData[i][4][j][0];
						if(returnStationData[i][4][j].length > 0){
							returnStationData[i][4][j].push(x);
							returnStationData[i][4][j].push(y);
							returnStationData[i][4][j].push(5);
							returnStationData[i][4][j].push(returnStationData[i][3]);
							nowAngelArr.push(returnStationData[i][4][j]);
							areaIds += returnStationData[i][4][j][0] + ",";
						}
					}
				}
			}
	...
	if(layer >= 7){
		...
		for(var i=0;i<returnStationData.length;i++){
			...
			y = returnStationData[i][3];
			if(x > MBR[0] && x < MBR[2] && y>MBR[3] && y<MBR[1]){
				nowShowPoint.push(returnStationData[i]);
			}
		}
	}else{
		nowShowPoint = returnStationData;
	}
	showAllCallBack();
}
...
function showAreaInfo(id){
	...
	//if(netype==wlanMarke) {
		loadBaseStationInfoScript.src ="../map/jonjsp/customerInfo.jsp?callback=showAreaBack&cellid=" + id;		
	//} else {
...
function showAllCallBack(){
	...
	var MBR = richmap.getMBR();
	var l = t = 0;
	if(nowShowPoint == null)nowShowPoint = returnStationData;
	if(nowShowPoint.length == 0)nowShowPoint = returnStationData;
	for(var i=0;i<nowShowPoint.length;i++){
		if(richmap.getLayer() > 8){
			tempObj = document.createElement("img");
			tempObj.src = "../images/yjc.gif"
			tempObj.className = "siteImg";
		}else{
		...
		//tempObj.className = "singlePoint";		//
		
		//tempObj.innerHTML = "▲";
		l = richmap.getCoordFromScreenX(tx,nowShowPoint[i][2])-6;
		t = richmap.getCoordFromScreenY(ty,nowShowPoint[i][3])-6;
		//showphoStation(returnStationData[i][0],returnStationData[i][2],returnStationData[i][3]);
		...
		if(richmap.getLayer() > 12){
			//if(returnStationData[i][2] > MBR[0] && returnStationData[i][2] < MBR[2] && returnStationData[i][3]>MBR[3] && returnStationData[i][3]<MBR[1]){
				...
				textObj.style.left = l+12;
				textObj.style.top = t;
				textObj.innerHTML = "<nobr>" + nowShowPoint[i][1] + "</nobr>";
