...
function showCutline(){
	if(cutlineDiv == null){
		cutlineDiv = document.createElement("div");
		cutlineDiv.id="cutline";
		cutlineDiv.style.position="absolute";
		document.body.appendChild(cutlineDiv);
	}
	var outstr = "<b>图例：</b><br>";
	outstr += "<span style='color:#CC6600'>■</span>　自维<br>";
	outstr += "<span style='color:#99FF66'>■</span>　丰祥代维<br>";
	outstr += "<span style='color:#0000FF'>■</span>　国脉代维<br>";
	outstr += "<span style='color:#CC66FF'>■</span>　泉邮代维<br>";
	outstr += "<span style='color:#FFFF00'>■</span>　瑞安代维<br>";
	outstr += "<span style='color:#999999'>■</span>　特力通代维<br>";
	outstr += "<span style='color:#00FFFF'>■</span>　信睿代维<br>";
	outstr += "<span style='color:#FF0000'>■</span>　东信代维<br>";
	outstr += "<span style='color:#660000'>■</span>　富通代维<br>";
	outstr += "<span style='color:#FF6699'>■</span>　一线代维<br>";
	$("cutline").innerHTML = outstr;	
}
...
function setSize(){
	...
	cutlineDiv.style.left = winX - 180 + 5;
	cutlineDiv.style.top = 200;
...
function loadSite(){
	...
	loadBaseStationScript.src = "jonjsp/roleSite.jsp?callback=resoluteCallBack&req=" + Math.random();
	startGetBasePath("loadSite()");
}
var hasReturn = false;
var reDoFun = "";
...
function showAllCallBack(){
	...
	for(var i=0;i<returnStationData.length;i++){
		...
		if(returnStationData[i][4]==1) {
			tempObj.className = "zhiwei" + richmap.getLayer();
		} else if (returnStationData[i][4]==2){
			tempObj.className = "fenxiang" + richmap.getLayer();
		} else if (returnStationData[i][4]==3){
			tempObj.className = "guomai" + richmap.getLayer();
		} else if (returnStationData[i][4]==4){
			tempObj.className = "quanyou" + richmap.getLayer();
		} else if (returnStationData[i][4]==5){
			tempObj.className = "ruian" + richmap.getLayer();
		} else if (returnStationData[i][4]==6){
			tempObj.className = "telitong" + richmap.getLayer();
		} else if (returnStationData[i][4]==7){
			tempObj.className = "xingrui" + richmap.getLayer();
		} else if (returnStationData[i][4]==8){
			tempObj.className = "dongxin" + richmap.getLayer();
		} else if (returnStationData[i][4]==9){
			tempObj.className = "futong" + richmap.getLayer();
		} else if (returnStationData[i][4]==10){
			tempObj.className = "yixian" + richmap.getLayer();
		} else {
