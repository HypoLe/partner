...
function loadMap(){
	//if(countInfoDiv == null){
		...
		//countInfoDiv.innerHTML = "当前区域指标汇总..";
...
function setSize(){
	...
	$("wingMap").style.width = winX - 180-200;
	$("wingMap").style.height = winY - 2-20+20;
	$("lineInfo").style.left = winX - 180 + 5;
	$("lineInfo").style.top = 240;
	$("lineInfo").style.height = winY - 250;
	$("pagetag").style.left = winX - 378;
	$("pagetag").style.top = 1;
	$("pagetag").style.height = winY - 250;
	//$("colordefined").style.left = winX - 180 + 10;
	//$("colordefined").style.top = 240;
	//$("colordefined").style.height = winY - 250;
	//$("pageInfo").style.left = winX - 180 + 10-200;
	//$("pageInfo").style.top = 400 + (winY - 400)*0.5 - 90+200;
	//countInfoDiv.style.left = winX - 180 + 5;
	//countInfoDiv.style.top = 1;
}
window.onresize = function(){
	setSize();
	richmap.resizeMap(winX - 180-200,winY - 2 - 20+20);
}
...
function showStation(){
	...
	tempPic.sImageSrc = "../images/yjc.gif";
...
function showphoStation(n,x,y,t){
	var tempObj = new wingBasePoint(n,x,y);
	//断站原因图标
	//if (p==1) {
	//	var tempPic = new wingPicObj();
	//	tempPic.sImageSrc = "../images/el.gif";
	//	tempPic.nImageWidth = 16;	//图片长
	//	tempPic.nImageHeight = 16;	//图片高
	//	tempPic.sAlt = "电力";
	//	tempPic.nScreenX = -20;			//图片热点相对坐标x
	//	tempPic.nScreenY = -8;			//图片热点相对坐标y
	//	tempObj.addObj(tempPic);
	//}
	//节点站打标
	if (t=="3g") {
		var tempPic = new wingPicObj();
		tempPic.sImageSrc = "../images/3gsite.gif";
		tempPic.nImageWidth = 16;	//图片长
		tempPic.nImageHeight = 16;	//图片高
		tempPic.sAlt = "TD站";
		tempPic.nScreenX = -8;			//图片热点相对坐标x
		tempPic.nScreenY = -8;			//图片热点相对坐标y
		tempObj.addObj(tempPic);
    } else {
		var tempPic = new wingPicObj();
		tempPic.sImageSrc = "../images/2gsite.gif";
		tempPic.nImageWidth = 16;	//图片长
		tempPic.nImageHeight = 16;	//图片高
		tempPic.sAlt = "普通站";
		tempPic.nScreenX = -8;			//图片热点相对坐标x
...
function loadMapSuccess(){
	richmap.showCity("FJ");		//显示城市地图到指定层级
	loadSite(); //打开页面时加载基站 
	//showLine();
	//showStation();
}
...
function getx(x){
	return (x - 73.44695998)*100000
}
function gety(y){
	return (35.39660299 - y)*100000
}
function getLongitude(x){
	return parseInt((73.446960 + x * 0.00001)*100000)/100000;
	//return 73.44695998 + x * 0.00001;
}
function getLatitude(y){
	return parseInt((35.396603 - y * 0.00001)*100000)/100000;
	//return 35.39660299 - y * 0.00001;
}
...
var basePath = "http://10.53.160.91:9202/";
var basePaths = ["http://10.53.160.91:9202/","http://10.209.0.15:8080/"];
var nowPath = 0;
//var basePath = "http://10.46.0.1/f1/";
var nettype = "1";
function loadSite(act){
	...
	//loadBaseStationScript.src = basePath +"taifeng/f1_area/alarm_siteposition.jsp?callback=resoluteCallBack&isincludecells=true&city="+"FJ"+"&req=" + Math.random();
	loadBaseStationScript.src = basePath +"taifeng/f1_area/alarm_siteposition.jsp?callback=resoluteCallBack&isincludecells=true&city="+"FJ"+"&req=" + Math.random();
	//loadBaseStationScript.src = resoluteCallBack([['FJFZ-2311','连江透堡火车站',119.61891666666666,26.336194444444445,'2g',[['FJFZ-23111',60,65],['FJFZ-23112',220,65],['FJFZ-23113',350,65]]],['FJFZ-477','新国际娱乐城',119.31880555555556,26.08363888888889,'2g',[['FJFZ-4771',60,65],['FJFZ-4772',240,65],['FJFZ-4773',310,65]]],['FJND-2627','华为屏南前高溪',118.801528,26.805028,'2g',[['FJND-26271',0,360]]],['FJND-2784','霞浦青岙隧道北',120.101027777778,26.9281111111111,'2g',[['FJND-27841',60,65],['FJND-27842',240,65]]],['FJNP-1877','H松溪水南',118.775972,27.523555,'2g',[['FJNP-18771',0,360],['FJNP-18772',120,65],['FJNP-18773',270,65]]],['FJNP-6005','浦城祝源隧道',118.3541667,27.66944444,'2g',[['FJNP-60051',0,360],['FJNP-60052',0,360]]]]); 
	//lockRegion(6,4289078,957871);
	//richmap.showCity("FJSM");
...
function startGetBasePath(redo){
	...
	hasReturn = false;
	reDoFun = redo;
	setTimeout(checkBaseReturn,3000);
}
...
function resoluteCallBack(returnArr){//alert(returnArr);
	getBaseReturn()
	...
	for(var i=0;i<returnArr.length;i++){
		returnArr[i][2] = getx(returnArr[i][2]);
		returnArr[i][3] = gety(returnArr[i][3]);
	}
...
function showStationAngels(){
	...
	if(layer >= 13){
		...
		if(areaIds != ""){
			...
			ajform.action = basePath + "taifeng/f1_area/btsPInfoList.jsp?callback=countInfo";
...
function showNowAngel(){
	...
	for(var i=0;i<nowAngelArr.length;i++){
		...
		sectorVml += "<v:shape CoordOrig=\"0,0\" id=\"angel_"+nowAngelArr[i][0]+"\" CoordSize=\""+w+","+h+"\" "
					...
					+ " onmousemove=\"showAreaInfo('"+nowAngelArr[i][0]+"')\" onmouseout=\"hideShowArea();\">"
					+""
...
function showAreaInfo(id){
	...
	//}
	//loadBaseStationInfoScript.src = basePath + "taifeng/f1_area/exitBtsVindicateInfo.jsp?callback=showAreaBack&cellid=" + id;
	//alert(loadBaseStationInfoScript.src);
	//showAreaBack({'所属地市':'厦门','网元名称':'嘉湖酒店','所属BSC/PCU':'BSC284','所属MSC/SGSN':'MSC28','LAC':'23010','BTS号':'12','所属CI':'8015','归属角色组':'岛内丰祥代维'});
}
var showInfoDiv = null;
var showInfoX = 0;
var showInfoY = 0;

function showAreaBack(returnData){//alert("aa");
...
function showAllCallBack(){
	...
	for(var i=0;i<returnStationData.length;i++){
		...
		showphoStation (returnStationData[i][0],returnStationData[i][2],returnStationData[i][3],returnStationData[i][4]);
...
function countInfo(returnData){
	var outStr = "<font color='black'><b>当前区域指标汇总：</b></font><br>";
	for(k in returnData){
		outStr += "<nobr><font color=black>" + k + "：</font><font color=green>" + returnData[k] + "</font></nobr><br>";
	}
...
//对所选的基站进行定位
var cityArr=[]; 
cityArr["1281036878"]="FJFZ";
cityArr["-1229189857"]="FJXM"; 
cityArr["-825137106"]="FJND"; 
cityArr["210392078"]="FJPT"; 
cityArr["1248427518"]="FJQZ"; 
cityArr["-752638569"]="FJZZ"; 
cityArr["-1006723975"]="FJLY";
cityArr["-1840837954"]="FJSM"; 
cityArr["1958205703"]="FJNP";
function zoomIntoBts(regionId,longtitude,latitude){
	//alert(cityArr[regionId]+"##"+getx(longtitude)+"##"+gety(latitude));
	//4585637 931852
	var l = r = getx(longtitude);
	var b = t = gety(latitude);
	richmap.setCity(cityArr[regionId]);
	//richmap.zoomToMBR(l,b,r,t);
	richmap.zoomTo(13,r,t);
}
// 显示代维角色下人员设备在图上位置
function showVindicate(vindicatename){
	richmap.setCity("FJ");
	//4477373 931266 
	richmap.zoomTo(1,4477373,931266);
	loadVindicate(vindicatename);
}
var loadBaseVindicateScript=null;
function loadVindicate(vindicatename){
	if(loadBaseVindicateScript == null){
		loadBaseVindicateScript = document.createElement("script");
		loadBaseVindicateScript.setAttribute("type","text/javascript");
		loadBaseVindicateScript.setAttribute("language","javascript");
		document.body.appendChild(loadBaseVindicateScript);
	}
	...
	loadBaseVindicateScript.src = basePath + "taifeng/f1_area/showVindicatePosition.jsp?callback=vindicateCallBack&vindicatename="+ vindicatename +"&req=" + Math.random();
}
function showVindicatePhoto(pid,x,y){
	var tempObj = new wingBasePoint(pid,x,y);
	var tempPic = new wingPicObj();
	tempPic.sImageSrc = "user.gif";
	tempPic.nImageWidth = 16;	//图片长
	tempPic.nImageHeight = 16;	//图片高
	tempPic.sAlt = "代维人员";
	tempPic.nScreenX = -8;			//图片热点相对坐标x
...
function vindicateCallBack(returnArr){
	for(var i=0;i<returnArr.length;i++){
		returnArr[i][2] = getx(returnArr[i][2]);
		returnArr[i][3] = gety(returnArr[i][3]);
		showVindicatePhoto (returnArr[i][0],returnArr[i][2],returnArr[i][3]);
	}
}
// 得到代维人员的路径
var loadCheckLineScript = null; 
function loadCheckLine(checkId){
	if(loadCheckLineScript == null){
		loadCheckLineScript = document.createElement("script");
		loadCheckLineScript.setAttribute("type","text/javascript");
		loadCheckLineScript.setAttribute("language","javascript");
		document.body.appendChild(loadCheckLineScript);
	}
	loadCheckLineScript.src = basePath + "taifeng/f1_area/showCheckGPS.jsp?callback=lineCallBack&checkid="+ checkId +"&req=" + Math.random();
}
function lineCallBack(returnData){
	if(returnData == null)return;
	if(returnData.length == 0)return;
	//richmap.clearAllElement();
	var tempLine = new wingBaseLine();
	for(var i=0;i<returnData.length;i++){
		tempLine.aScreenX.push(getx(returnData[i][1]));
		tempLine.aScreenY.push(gety(returnData[i][2]));
	}
	tempLine.sId = "checkline";
	tempLine.sColor = "red";
	tempLine.nWeight = 1;
	richmap.addWingElement(tempLine);
	
	var x = getx(returnData[returnData.length-1][1]);
	var y = gety(returnData[returnData.length-1][2]);
	var tempObj = new wingBasePoint("checkobj",x,y);
	var tempPic = new wingPicObj();
	tempPic.sImageSrc = "user.gif";
	tempPic.nImageWidth = 16;
	tempPic.nImageHeight = 16;
	tempPic.sAlt = "当前位置";
	tempPic.nScreenX = -8;
	tempPic.nScreenY = -8;
	tempObj.addObj(tempPic);
	richmap.addWingElement(tempObj);
	//richmap.zoomTo((ontime?12:richmap.getLayer()),x,y);
	richmap.zoomTo(8,x,y);
	//ontime = false;
}