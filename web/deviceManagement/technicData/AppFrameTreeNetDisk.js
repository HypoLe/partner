...
var AppFrameTree = function(){
	//private
	var layout,treeroot,treesm,treeCtxMenu,form,formData,sbmBtn,curNode,pageFrame;
	...
	return {
		init : function(){
		    ...
		    treesm = tree.getSelectionModel();
    			treesm.on('selectionchange', innerObj);
    		
    		function innerObj(){
        			curNode = this.getSelectedNode();
        			/*alert(this.curNode);
        			alert(treesm.selNode);
        			//this instanceof treesm?alert("true"):alert("false");
        			alert("eee");
        			alert(this.events.selectionchange.obj.selNode);
        			console.dir(this);
        			console.dir(this.pageFrame);*/
    		}
		 				...
		 				handler: function(){
		 					if(m['isDelete']===true){
	    			              if(confirm("您确定删除这个项目吗")){
									AppFrameTree.openPanel('formPanel',curNode.text+' '+m.text);
		 					        pageFrame.src = m.url+curNode.id+'&nodeType='+curNode.attributes.nodeType;
	    			              }
		 					}
		 					else{
		 					  AppFrameTree.openPanel('formPanel',curNode.text+' '+m.text);
		 	...
		 	**************/
		 	if(config.onClick){
		      tree.on('click',function(node,e){
		    	  ...
		    	  this.openPanel('formPanel',node.text+' '+config.onClick.text);
		    	  pageFrame.src=config.onClick.url+"searchFiles.do?method="+node.id;
		      	}
		      	if(node.attributes.leaf!=null && node.id=="statistics"){
		    	  this.openPanel('formPanel',node.text+' '+config.onClick.text);
		    	  pageFrame.src=config.onClick.url+"statistics.do?method="+node.id;
		      	}
    	...
    	reloadNode : function(node){
    		if(arguments.length==0)node = curNode;
			if(node) {
