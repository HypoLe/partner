...
<html:form action="/yndutyevent.do?method=edit"
	...
	<table border=0 cellspacing="1" cellpadding="1" class="listTable">
		...
		<tr>
			...
			<td COLSPAN="10">
				...
				--%><html:text property="beginTime" styleId="beginTime"
						styleClass="text medium"  onclick="popUpCalendar(this, this,null,null,null,true,-1);" readonly="true" alt="allowBlank:false,vtext:'${eoms:a2u('请填写开始时间')}'"/>
			...
			<html:text property="endtime" styleId="endtime"
						styleClass="text medium"  onclick="popUpCalendar(this, this,null,null,null,true,-1);" readonly="true" alt="allowBlank:false,vtext:'${eoms:a2u('请填写结束时间')}'"/>
		...
		<tr class="tr_show">
			...
			<td COLSPAN="24">
					<input name="eventtitle"   type="text"  class="clstext"  size="70" value="${tawRmDutyEventForm.eventtitle}" alt="allowBlank:false,vtext:'${eoms:a2u('请填写标题时间')}'"/>
	 	</tr>
		<TR>
		 	<td COLSPAN="4" class="label">
			...
			<tr id="pubstatus" style="display:none">
		 	<td COLSPAN="4" class="label">
				<bean:message key="tawRmDutyEvent.pubstatus" /><font color="#ff0000">**</font>
			...
			<td COLSPAN="4" class="label">
				<bean:message key="tawRmDutyEvent.pubflag" /><font color="#ff0000">**</font>
		...
		<tr>
			...
			<input type="button" value="<bean:message key="tawRmDutyEvent.removeEvent" />" Onclick='window.location.href ="${app}/duty/yndutyevent.do?method=deleted&id=${tawRmDutyEventForm.id}"' class="button">
			</td>
		</tr>
		<%} %>
	</table>
<table>
    ...
    requestURI="${app}/duty/yndutyevent.do?method.do?method=search"  >
...
function onsubSave(struts){
        ...
        if(comptime(begin,happentime)<0){
    		alert("${eoms:a2u('发生时间不能小于主事件的开始时间')}");
    		return false; 
    	}
    	...
    	document.forms[0].action ="${app}/duty/yndutyevent.do?method=saveSub&struts="+struts;
