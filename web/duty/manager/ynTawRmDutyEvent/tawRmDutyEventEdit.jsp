...
function validateEventForm(){
   	...
   	if(complateFlag=="") {
   		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}","${eoms:a2u('完成状态不能为空，请选择！')}");
   		alert("${eoms:a2u('完成状态不能为空，请选择！')}");
   	...
   	if(complateFlag==1&&subEventNum<2) {
   		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}","${eoms:a2u('事件记录为完成时,子过程不能少于2条,请增加子过程！')}");
   		alert("${eoms:a2u('事件记录为完成时,子过程不能少于2条,请增加子过程！')}");
 		return false;
 	}

 	if(complateFlag==1&&date2=="") {
 		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}","${eoms:a2u('事件记录为完成，完成时间不能为空，请填写！')}");
 		alert("${eoms:a2u('事件记录为完成，结束时间不能为空，请填写！')}");
   		frm.endtime.focus();
 		return false;
 	}
	...
	if(dateCompare(date1,getDateStr(0))) {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}"	,"${eoms:a2u('发生时间不能是将来时！')}");
		alert("${eoms:a2u('发生时间不能是将来时！')}");
 	...
 	if (complateFlag==1&&frm.flag.value == 3&&(typeName.search("${eoms:a2u('故障')}")> -1)) {
		if (faultCommontCount == 0) {
			...
			alert("${eoms:a2u('请派发任务工单后(任务工单编号不能为空),该事件才可以结束！！')}");
			return false;
		}				
 ...
 function validateSubForm() {
	...
	if(dateCompare(subLastBeginTime,date2)) {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件子过程')}"	,"${eoms:a2u('新增子过程时间要大于最后一个子过程时间！')}");
		alert("${eoms:a2u('新增子过程时间要大于最后一个子过程时间！')}");
	...
	if(date3!=""&&dateCompare(date2,date3)) {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}"	,"${eoms:a2u('开始时间要小于结束时间！')}");
		alert("${eoms:a2u('开始时间要小于结束时间！')}");
		document.getElementById("endtime").focus();
		return false;
	} 
	...
	if(form["happentime"].value == "") {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}"	,"${eoms:a2u('请输入开始时间！')}");
		alert("${eoms:a2u('请输入开始时间！')}");
		form["happentime"].focus();
		return false;
	}
	
	if(form["content"].value == "") {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件记录')}"	,"${eoms:a2u('请输入子过程内容！')}");
		alert("${eoms:a2u('请输入子过程内容！')}");
		form["content"].focus();
		return false;
	}
...
function deleteEvent(id) {	
	if (confirm("${eoms:a2u('您确认要删除故障事件吗？')}")) {
 		var url='${app}/duty/yndutyevent.do?method=deleted&id=' + id;
...
function deleteSub(id) {
	if("${sessionform.isDutyMaster}") {
		//Ext.MessageBox.alert("${eoms:a2u('监控事件子过程')}"	,"${eoms:a2u('你没有删除权限，只有值班长有删除权限！')}");
		alert("${eoms:a2u('你没有删除权限，只有值班长有删除权限！')}");
		return false;
	}
...
function openFault(flag,type) {
	if(flag==1) {
		if(type=="faultCircuit"&&faultEquipmentCount>0) { // 已经生成过设备故障
			// Ext.MessageBox.alert("${eoms:a2u('生成故障记录')}"	,"${eoms:a2u('已经生成过设备故障，不能再新增！')}");
			alert("${eoms:a2u('已经生成过设备故障，不能再新增！')}");
			return false;
		}
		if(type=="faultEquipment"&&faultCircuitCount>0) { // 已经生成过线路故障
			//Ext.MessageBox.alert("${eoms:a2u('生成故障记录')}"	,"${eoms:a2u('已经生成过线路故障，不能再新增！')}");
			alert("${eoms:a2u('已经生成过线路故障，不能再新增！')}");
			return false;
		}
...
<html:form action="/yndutyevent.do?method=save" method="post" styleId="tawRmDutyEventForm">
	...
	<table border=0 cellspacing="1" cellpadding="1" class="listTable">
		...
		<tr class="tr_show">
			...
			<td COLSPAN="3">
					<input name="eventtitle" value='${tawRmDutyEventForm.eventtitle}'  type="text"  class="clstext"  size="80" value="" />
		</tr>
		<tr>
		<td class="label">
		...
		<td colspan="3">
			<input type="button" class="btn" name="query" value=<fmt:message key="faultCircuit.selected" /> onclick="fun_showMyBox(this);" class="cls2">&nbsp;&nbsp;&nbsp;
	...
	<c:choose><c:when test="${tawRmDutyEventForm.roomid==167}">
		<tr height="25">
			<td class="label">${eoms:a2u('操作')}</td>
			<td colspan="3">&nbsp;
				...
				<c:choose><c:when test="${tawRmDutyEventForm.faultCircuitCount==0}">
					...
					<input type=button id="createFaultCircuitButton" class="btn" value="${eoms:a2u('编辑传输线路故障记录')}" onclick="openFault('2','faultCircuit');"><br>&nbsp;
				</c:otherwise></c:choose>
				<c:choose><c:when test="${tawRmDutyEventForm.faultEquipmentCount==0}">
					...
					<input type=button id="createFaultEquipmentButton" class="btn" value="${eoms:a2u('编辑传输设备故障记录')}" onclick="openFault('2','faultEquipment');"><br>&nbsp;
				</c:otherwise></c:choose>
			</td>
		</tr>
	</c:when></c:choose>		
...
<logic:iterate id="TawRmDutyEventSubList" name="TawRmDutyEventSubList" type="com.boco.eoms.duty.model.TawRmDutyEventSub">
	<tr class="tr_show">
		<td width="10%"><eoms:id2nameDB id="${TawRmDutyEventSubList.recordman}" beanId="tawSystemUserDao" /></td>
		<td width="20%">
		...
		<td width="20%"><bean:write name="TawRmDutyEventSubList" property="recordtime" /></td>
		<td width="8%">
