			...
			<table width="100%" height="100%">
						...
						<TABLE cellSpacing="0" cellPadding="0" width="95%"
							...
							<TR>
								<TD colSpan="6" class="label">
									...
									<c:if test="${portalLogin!=null&&portalLogin=='1'}"><!-- 门户登录 -->
									  <IFRAME ID=IFrame2 FRAMEBORDER=0 width="100%" height=100
										SCROLLING=YES SRC='${app }/duty/TawRmRecord/search7days.do'>
									  </IFRAME>
									</c:if>
									<c:if test="${portalLogin==null||portalLogin!='1'}">
									  <IFRAME ID=IFrame2 FRAMEBORDER=0 width="100%" height=100
								...
								<TD colSpan="6" class="clsfth">
									...
									<c:if test="${portalLogin!=null&&portalLogin=='1'}"><!-- 门户登录 -->
										<IFRAME ID=IFrame1 FRAMEBORDER=0 width="100%" height=520
											SCROLLING=NO
											SRC='${app }/duty/TawRmRecord/recordmain.do?WORKSERIAL=<bean:write name="tawRmRecordForm" property="id"/>'>
										</IFRAME>
									</c:if>
									<c:if test="${portalLogin==null||portalLogin!='1'}">
										<IFRAME ID=IFrame1 FRAMEBORDER=0 width="100%" height=520
              ...
              <% if(flag==0){%><font color="red">[</font><%}else { %>[<%} %><a
													href='<%=request.getContextPath()%>/duty/manager/TawRmCycleTable/ntko/showDetail.jsp?id=<%=id%>&url=<%=url%>&sell=<%=sell%>&iterator=<%=iterator%>&path=<%=path%>'><bean:write
														name="tawRmCycleTable" property="name" scope="page" /> </a> <% if(flag==0){%><font color="red">]</font><%}else { %>]<%} %>
														&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </logic:iterate>
										</td>
									</tr>
