...
function changecycle(locationid,monthFlag){
	var list = document.tawwpitemedit.monthFlag;
	if(locationid<5){
	...
	for (i=1;i <= onecount; i++){
		if (subcat[i][1] == locationid){
      		list.options[list.length] = new Option(subcat[i][0], subcat[i][2]);
		}
	}
	list.value = monthFlag;
  ...
  <table class="formTable">
   ...
   <tr>
    ...
    <tr>
      ...
      <td  >
        <input type="text" name="taskid" size="40" class="text" value="<%=tawwpModelExecuteVO.getTaskid() %>">
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
       <eoms:dict key="dict-workplan" dictId="cycle" isQuery="true" defaultId="<%=tawwpModelExecuteVO.getCycle()%>" onchange="changecycle(document.tawwpitemedit.cycle.options[document.tawwpitemedit.cycle.selectedIndex].value)" selectId="cycle" beanId="selectXML"/>
...
<%   } %>
     ...
     <tr>
      ...
      <td>
        <textarea name="remark" class="textarea max"><%=StaticMethod.null2String(tawwpModelExecuteVO.getRemark())%></textarea>
      </td>
    </tr>
     <tr>
      <td class="label">
      ...
      <td>
        <textarea name="note" class="textarea max"><%=StaticMethod.null2String(tawwpModelExecuteVO.getNote())%></textarea>
      </td>
    </tr>
    <tr>
      <td class="label">
...
changecycle(<%=tawwpModelExecuteVO.getCycle()%>,
<%   
  if(tawwpModelExecuteVO.getMonthFlag()==null||tawwpModelExecuteVO.getMonthFlag().equals("")){
  		out.print("0");
  }else{
   		out.print(tawwpModelExecuteVO.getMonthFlag());
  }
%>);
<%
