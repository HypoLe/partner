...
document.tawwpitemedit.monthflag.style.display="block";
  ...
  document.tawwpitemedit.monthflag.value=monthflag;
...
<form name="tawwpitemedit" method="post" action="../tawwpyear/itemmodify.do?yearplanid=<%=request.getParameter("yearplanid")%>&yearexecuteid=<%=tawwpYearExecuteVO.getId()%>" onsubmit='return SubmitCheck()'>
  ...
  <table class="formTable">
    ...
    <tr>
      ...
      <td width="400" colspan="2">
        <input type="text" name="executedeptlevel" size="40" class="text" value="<%=tawwpYearExecuteVO.getExecutedeptlevel()%>">
      </td>
    </tr>
    <tr>
      <td class="label">
    ...
    <tr>
      ...
      <td width="400" colspan="2">
        <input type="text" name="command" size="40" class="text" value="<%=StaticMethod.nullObject2String(tawwpYearExecuteVO.getCommand()) %>">
      </td>
    </tr>
    ...
    <tr>
      ...
      <td width="50"  >
        ...
        <eoms:dict key="dict-workplan" dictId="cycle" isQuery="true" defaultId="<%=tawwpYearExecuteVO.getCycle()%>" onchange="changecycle(document.tawwpitemedit.cycle.options[document.tawwpitemedit.cycle.selectedIndex].value)" selectId="cycle" beanId="selectXML"/>
       	<br>
        ...
        <td width="350">
        <select size="1" name="monthflag" class="text" style="display:none" value="<%=tawwpYearExecuteVO.getMonthFlag()%>">
        </select>
      </td>
    </tr>
    <tr>
...
<%   if(tawwpYearExecuteVO.getFileFlag()!=null&&tawwpYearExecuteVO.getFileFlag().equals("1")) {%>           
        checked
<%   } %>>是
		<input type=radio name='fileFlag' value='0' 
<%   if(tawwpYearExecuteVO.getFileFlag()!=null&&tawwpYearExecuteVO.getFileFlag().equals("0")) {%>           
        checked
<%   } %>>否
      </td>
     </tr>
     ...
     <tr>
      ...
      <td colspan=2 >
        <textarea name="remark" class="textarea max"  ><%=StaticMethod.null2String(tawwpYearExecuteVO.getRemark())%></textarea>
      </td>
    </tr>
     <tr>
       <td class="label">
      ...
      <td colspan=2 >
        <textarea name="note" class="textarea max" ><%=StaticMethod.null2String(tawwpYearExecuteVO.getNote())%></textarea>
      </td>
    </tr>
     <tr>
      <td class="label">
      ...
      <td>
        <eoms:attachment idList="" idField="accessories" appCode="workplan" scope="request" name="yearexecute" property="accessories"/>
      </td>
    </tr>
  </table>
  <br>
  <input type="submit" value="<bean:message key="itemedit.tawwpyear.btnSubmit" />" name="B1"  Class="submit">
  <input type="button" value="<bean:message key="itemedit.tawwpyear.btnBack" />" onclick="javascript:window.history.back();" class="button">
