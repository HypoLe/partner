...
<%@ page import="com.boco.eoms.workplan.util.ComparatorUser"%>
...
<!--  -->
function onExecuteTask(_monthPlanId,_netserialno)
{
  location.href="../tawwpmonth/monthexecuteTask.do?monthplanid=" + _monthPlanId+"&netserialno="+_netserialno;
}
...
<%  		
				    ...
				    ComparatorUser comparator = new ComparatorUser();
         	        Collections.sort(listCheckUser, comparator);
		...
		<tbody>
			...
			<tr>
				...
				<td colspan="20">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<%if("1".equals(switchCheck) && "1".equals(tawwpMonthExecuteVO.getFileFlag())){%>
					质检人：
					<%=StaticMethod.null2String(tawwpMonthExecuteVO.getQualityCheckUserName())%>
					<%}%>
					<p/>
					执行起始时间：
					...
					&nbsp;&nbsp;
					巡检接口：
					<% if(tawwpMonthExecuteVO.getCommand()!= null && !tawwpMonthExecuteVO.getCommand().equals("")){ %>
		...
		<tr>
			<td colspan="35">
				...
				<INPUT type="button"
					value="关联自动巡检"
					name="button"
					Onclick="javascript:onExecuteTask('<%=tawwpMonthPlanVO.getId()%>','<%=tawwpMonthPlanVO.getNetSerialNo()%>');"
					Class="button">
