...
<%@ page import ="com.boco.eoms.workplan.util.WorkplanMgrLocator"%>
...
<%
  ...
  String switchCheck = StaticMethod.null2String(WorkplanMgrLocator.getAttributes().getSwitchQualityCheck());
%>
...
Ext.onReady(function(){
	...
	var config = {
		btnId:'userTreeBtn',	 
		treeDataUrl:userTreeAction,treeRootId:'-1',treeRootText:'部门',treeChkType:'dept',
		saveChkFldId:'userId',panelsConfig:[{title:'人员',dataUrl:'${app}/xtree.do?method=userFromDept',checktype:'user'},
		{title:'机房',dataUrl:'${app}/xtree.do?method=getCptroomTree',checktype:'cptroom'}]
	};
	...
	var	userCheckTreeAction='${app}/xtree.do?method=userFromDept';
	userViewer = new Ext.JsonView("user-list",'<div id="user-{id}" class="viewlistitem-user">{name}</div>',
		{ 
			multiSelect: true,		 
			emptyText : '<div></div>'								 
	...
	var configCheck = {
		btnId:'qualityCheckUserTreeBtn',dlgId:'dlg-user',	 
		treeDataUrl:userCheckTreeAction,treeRootId:'-1',treeRootText:'人员',treeChkMode:'single',treeChkType:'user',
		saveChkFldId:'userId'  		
	};

	configCheck.onOutput = function(textList,dataList,jsonData){
		var deptTemp = [];
		var userTemp = [];
		var subRoleTemp = [];	
		var valueId=textList;
		...
		if(document.forms[0].executeIds){
		    ...
		    for (i=1;i<obj.length+1;i++){
			    if(obj[i-1].checked){
			    	haschecked = true;
					var span = $('sort'+i+'_checkuser');
					var UserHidden = $('qualityCheckUserId'+i);
					span.update(textList);
					var monthExecuteObj = eval("document.getElementById('monthExecute"+i+"')");
				
				   //计划字符串(monthExecuteobj.value)的格式为 执行内容ID#执行人ID字符串#执行人类型#执行日期字符串#质检人
				   var monthExecuteArr = monthExecuteObj.value.split("#");
				   monthExecuteArr[6] = value;
				   monthExecuteObj.value = monthExecuteArr.join("#");
				}
			}
		}
...
function onSubmit(){
  ...
  var size = parseInt("<%=monthExecuteVOList.size()%>");
  
  for(i=1;i<=size;i++){ 
    var monthExecuteObj = eval("document.getElementById('monthExecute"+i+"')");
	var fileflagObj = eval("document.getElementById('fileflag"+i+"')"); 
	var fileflag = fileflagObj.value;
    var monthExecuteArr = monthExecuteObj.value.split("#");
    ...
    if(document.tawwpmonthexecuteform['switch'].value == "1"){
    	if(fileflag == "1"){
     		if(monthExecuteArr[6] == "" || monthExecuteArr.length<7){
      			alert("<bean:message key="monthexecuteedit.title.warnSelPlanContent" /> \""+monthExecuteObj.getAttribute("label")+"\"质检人！");
      			return false;
    		}
    	}
    }
...
<form name="tawwpmonthexecuteform" method="post" action="../tawwpmonth/monthexecuteeditsave.do" onsubmit="return onSubmit();">
<!--   body begin   -->
...
<table class="listTable" id="listTable">
  ...
  <tr>
    <td width="30" nowrap>
      <input type="checkbox" id="" onclick="selectall(this.checked);">
    </td>
    <td width="100" nowrap>
      作业项目
    </td>
    ...
    <td colspan="31">
      ...
      <input type="button" value="<bean:message key="monthexecuteedit.title.btnSetExecuter" />" id="userTreeBtn" class="btn"/>
      <%if(switchCheck.equals("1")){%>
      	<input type="button" value="选择质检人" id="qualityCheckUserTreeBtn" class="btn"/>
      <%}%>
    </td>
  </tr>
  </thead>
  ...
  <tr>
    <td rowspan="3" nowrap>
      ...
      <input type="hidden" name="monthexecuteid<%=i%>" id="monthexecuteid<%=i%>"  value="<%=tawwpMonthExecuteVO.getId()%>">
    </td>
    <td rowspan="3" nowrap>
    ...
    <td colspan="31">
	      ...
	      &nbsp;&nbsp;&nbsp;&nbsp;<bean:message key="monthexecuteedit.title.labCycle" /><%=tawwpMonthExecuteVO.getCycleName()%>	
	      
	      &nbsp;&nbsp;&nbsp;&nbsp;<bean:message key="monthexecuteedit.title.labForm" /><a target="_blank" href="../tawwpaddons/addonsread.do?action=read&window=new&myid=&model=50&addonsid=<%=tawwpMonthExecuteVO.getFormId()%>&reaction=./"><%=tawwpMonthExecuteVO.getFormName()%></a>
	      &nbsp;&nbsp;&nbsp;&nbsp;是否必须上传附件：<%=tawwpMonthExecuteVO.getFileFlagName() %>	
	      <p/>
	      起始时间：
	      <select id="startHH<%=i%>" name="startHH<%=i%>" class="select" style="width:50px" onchange=javascript:changeStartHH('<%=i%>');>
	        <%
	          int sh = 0;
	          if(tawwpMonthExecuteVO.getStartHH()!=null&&!tawwpMonthExecuteVO.getStartHH().equals("")){ 
	            String[] tempSt = tawwpMonthExecuteVO.getStartHH().split(":");
	            sh = StaticMethod.null2int(tempSt[0]);
	          }
              for(int m = 0;m<24;m++){
                if(m==sh){
            %>
	      ...
	      &nbsp;&nbsp;
	      结束时间：
	      <select id="endHH<%=i%>" name="endHH<%=i%>" class="select" style="width:50px" onchange=javascript:changeEndHH('<%=i%>');>
			<%
			  int eh = 23;
			  if(tawwpMonthExecuteVO.getEndHH()!=null&&!tawwpMonthExecuteVO.getEndHH().equals("")){ 
	            String[] tempEt = tawwpMonthExecuteVO.getEndHH().split(":");
			    eh = StaticMethod.null2int(tempEt[0]);
			  }
			  for(int n = 0;n<24;n++){
	      ...
	      <INPUT TYPE="hidden" name="fileflag<%=i%>" id="fileflag<%=i%>" value="<%=tawwpMonthExecuteVO.getFileFlag()%>">
	    <%if(StaticMethod.nullObject2String(tawwpMonthExecuteVO.getFileFlag()).equals("1")){%>
	     质检人：
	      <span id="sort<%=i%>_checkuser"><%=StaticMethod.null2String(tawwpMonthExecuteVO.getQualityCheckUser())%></span>
	    <%}else{%>
	      <span id="sort<%=i%>_checkuser"  style=display:none></span>
	    <%}%> 
	      <INPUT TYPE="hidden" name="qualityCheckUserId<%=i%>" id="qualityCheckUserId<%=i%>" value="">
	     <div id="user-list" class="viewer-list"></div>
	     <div id="qualityCheckUser-list" class="viewer-list"></div>
    </td>
  </tr>
  ...
  <INPUT TYPE="hidden" name="monthExecute<%=i%>" id="monthExecute<%=i%>" value="<%=tawwpMonthExecuteVO.getId()%>#<%=tawwpMonthExecuteVO.getExecuter()%>#<%=tawwpMonthExecuteVO.getExecuterType()%>#<%=tawwpMonthExecuteVO.getExecuteDuty()%>#<%=tawwpMonthExecuteVO.getStartHH()%>#<%=tawwpMonthExecuteVO.getEndHH()%>" label="<%=tawwpMonthExecuteVO.getName()%>">
  ...
  <tr>
    <td width="20%" colspan="35">
	  ...
	  <INPUT type="button" value="<bean:message key="monthexecuteedit.title.btnRemove" />" name="B1" Onclick="javascript:onDeleteExecute();" Class="button">
