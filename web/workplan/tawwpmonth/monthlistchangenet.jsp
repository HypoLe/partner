...
Ext.onReady(function(){
  ...
  {
    var monthId = <%=monthId%>//document.forms[0].monthid.value;
    location.href="../tawwpmonth/netselect.do?yearplanid=" + _yearPlanId + "&nettypeid=" + _netTypeId + "&monthid=" + monthId + "&flag=1";
  }
  function changeYear(){
     if(document.monthplan.yearid.value==""){
	     ...
	     document.monthplan.flag.value="1";
...
<table class="listTable" id="list-table">
    ...
    <caption>月计划列表
       <eoms:dict key="dict-workplan" dictId="yearFlag" isQuery="false" defaultId="<%=yearId%>"  onchange="changeYear()" selectId="yearid" beanId="selectXML" />
       <eoms:dict key="dict-workplan" dictId="monthFlag" isQuery="false" defaultId="<%=monthId%>"  onchange="changeYear()" selectId="monthid" beanId="selectXML" />
    ...
    %>
	<tr>
		<td rowspan="<%=monthPlanVOList.size()%>">
    ...
    %>
        ...
        <td align="center">
          <a href="javascript:onAdd('<%=tawwpYearPlan.getId()%>','<%=tawwpYearPlan.getNetTypeId()%>');">
            <img src="${app }/images/icons/layout_add.png" alt="新增网元计划">
          </a>
        </td>
    <%
    		}else{
    ...
    %>
        ...
        <td><%=tawwpMonthPlanVO.getDeptName()%></td>
        <td><%=tawwpMonthPlanVO.getCrtime()%></td>
        <td><%=tawwpMonthPlanVO.getConstituteStateName()%></td>
        <td>
          <%if((tawwpMonthPlanVO.getExecuteState().equals("0") || tawwpMonthPlanVO.getExecuteState().equals("2")) && (tawwpMonthPlanVO.getConstituteState().equals("1") || tawwpMonthPlanVO.getConstituteState().equals("2")) && (tawwpMonthPlanVO.getNetId() != null || !tawwpMonthPlanVO.getNetId().equals(""))){%>
          <a href="../tawwpmonth/monthdelete.do?monthplanid=<%=tawwpMonthPlanVO.getId()%>">
