...
function Transfer() {
	...
	callerWindowObj.document.forms[0].<%=name%>.value='<%=resulturl%>';
	callerWindowObj.document.forms[0].<%=numname%>.value=tawRmCycleTableForm.num.value;
    callerWindowObj.document.forms[0].<%=executeContentUserIdName%>.value=tawRmCycleTableForm.executecontentuserid.value;
  ...
  for(i=0;i<arrLink.length;i++){
    if(arrLink[i]!=null&&arrLink[i]!=""){
      var arrLinkInfo = arrLink[i].split("@");
	  	strTemp += "<li><a  href='"+arrLinkInfo[1]+"'>"+arrLinkInfo[0]+"</a><br>";
      }
...
<form method="post"  target="mywindow" name="tawRmCycleTableForm" action="../tawwpexecute/uploadfile.do" enctype="multipart/form-data">
 <fmt:bundle basename="config/ApplicationResources-workplan" >	
 
<table class="formTable">
   <caption>附件上传</caption>
   <tr>
	  <td width="30%" class="label"> <%if (!action.equals("read")){%>文件列表<%}else{%>>文件列表<%}%></td>
	<td>
		<%
        String[] n=null;
		...
		for(int i=0;i<a.length;i++)
		{
			...
			if (!action.equals("read")){
                        ...
                        if(!n[0].equals("")||!n[1].equals("")||n[0]!=null||n[1]!=null){
                          out.println("<input type=\"checkbox\" name=ListDelete value=\""+a[i]+"\" onclick=\"checkdelete()\"><a  href=\""+n[1]+"\">"+n[0]+"</a><br>");
                          }
                        }
			}else{
                          n=a[i].split("@");
                        if(!n[0].equals("")||!n[1].equals("")||n[0]!=null||n[1]!=null){
                          out.println("<li><a  href=\""+n[1]+"\">"+n[0]+"</a>");
                        }
  			}
		//附件数量
		num++;
...
<tr>
	<td colspan="2" align=center>
		...
		<input type="button" name="submitBut" value="完成" class="button" onclick="Transfer();WinClose();">
	</td>
</tr>
<%}%>
