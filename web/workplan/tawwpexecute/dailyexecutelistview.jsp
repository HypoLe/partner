...
<table class="formTable large">
  <thead>	
    <tr>
      ...
      %>
      <td>
        <a href="../tawwpexecute/executeaddsnew.do?date=<%=i%>"><%=i%></a>
      </td>
      <%
        }
      %>
  ...
  <tr>
	...
	%>
	<td>
	  <a href="javascript:onExecuteAdd('<%=tawwpExecuteContentVO.getId()%>','<%=tawwpmonthplanvo.getId()%>','<%=tawwpmonthplanvo.getUserByStub()%>','<%=tawwpmonthplanvo.getExecuteType() %>');">
	    <img src="${app }/images/icons/yes.gif">
	  </a>
	</td>
	<%
  ...
  <tr class="complete">
    ...
    <%
            ...
            } else if(Integer.parseInt(year+month+xx)<day){
  ...
  <tr>
    <td colspan="<%=dayCount+3 %>">
      [<%=tawwpmonthplanvo.getName() %>]&nbsp;&nbsp;&nbsp;&nbsp;网元:<%=tawwpmonthplanvo.getNetName() %><font color="red">*代理</font>
    </td>
  </tr>
  <%
   }
