...
function OnSubmit(formindex,flag, btn){
     ...
     for(i=0;i<size;i++){
     	...
     	alert('此作业计划中有执行项选择了必须上传附件,请上传附件');
  ...
  <table>
    ...
    <tr>
      <td>
        ...
        int formindex = 0;
        int count=monthplanvoHash.size();
        int tmpcount=0;
        while(i.hasNext()){
          ...
          <tr class="tr_show">
            <td width="15%" class="clsthd2">
              ...
              <input type="hidden" name="fileFlag<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>" value="<%=tawwpExecuteContentVO.getFileFlag()%>">
        		<input type="hidden" name="name<%=strArray[0]%>_<%=formindex %>_<%=stubFlag %>" value="<%=tawwpExecuteContentVO.getName()%>">
      ...
      %>
        ...
        --%>
        <input type="submit" value="<bean:message key="executeadds.title.btnSubmit" />" name="B1" class="submit" onclick="return OnSubmit('<%=formindex %>','<%=stubFlag %>',this);"> 
    ...
    <%     
      ...
      %>
    <table class="listTable">
        <caption><font color="red">没有可执行的值班执行内容</font></caption>
