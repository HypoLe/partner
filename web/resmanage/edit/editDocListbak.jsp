			...
			for(int a = 0; a < colNum; a ++)
			{
				...
				if(colData.getCol_flag() == 1)
				{
					...
					else
					{
						...
						//out.println("<td align=center><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'><a href=+"</a></font></td>");
					}
					tempFlag = tempFlag + 1;
				}
				else
					...
					switch(a)
					{
						case 0:
							...
							else
							{
								...
								out.println("<td align=center><a href='../document/"+rs.getString(a+2)+"'>");
							}
							break;
						case 1:
...
<%if(rs.getCurrentPageIndex() != rs.getFirstPageId())
				  out.println("<font face=webdings color=#06b020>4</font><a href=editDocList.jsp?id="+sId+"&pageid="+rs.getFirstPageId()+"&cityid="+cityId+">首页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getFirstPageId())
				  out.println("<font face=webdings color=#06b020>4</font><a href=editDocList.jsp?id="+sId+"&pageid="+rs.getPreviewPageId()+"&cityid="+cityId+">上一页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount() > 15)
				  out.println("<font face=webdings color=#06b020>4</font><a href=editDocList.jsp?id="+sId+"&pageid="+rs.getNextPageId()+"&cityid="+cityId+">下一页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount() > 15)
				  out.println("<font face=webdings color=#06b020>4</font><a href=editDocList.jsp?id="+sId+"&pageid="+rs.getLastPageId()+"&cityid="+cityId+">末页</a>");%>
				  </td>
<td></td>
<td></td>
</table>
