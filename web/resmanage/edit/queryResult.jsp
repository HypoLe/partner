...
if(colNum != 0)
{
	...
	%>
	<td align=center class=ttTable><b><font size=2 color=#000000 face='Verdana, Arial, Helvetica, sans-serif'>²éÑ¯³öµÄÉè±¸ÊýÁ¿Îª<%=rs.getRowCount()%></font></b>
	</td>
	<br><br>
...
<%if(rs.getCurrentPageIndex() != rs.getFirstPageId())
	 out.println("<font face=webdings color=#06b020>4</font><a href=queryResult.jsp?id="+typeid+"&pageid="+rs.getFirstPageId()+"&classtype="+classid+"&typeid="+typeid+"&manuf="+manufid+"&city="+cityid+arg+"&region="+regionid+"&orderColName="+orderColName+"&orderflag="+orderflag+">Ê×Ò³</a>");%>
<%if(rs.getCurrentPageIndex() != rs.getFirstPageId())
	  out.println("<font face=webdings color=#06b020>4</font><a href=queryResult.jsp?id="+typeid+"&pageid="+rs.getPreviewPageId()+"&classtype="+classid+"&typeid="+typeid+"&manuf="+manufid+"&city="+cityid+arg+"&region="+regionid+"&orderColName="+orderColName+"&orderflag="+orderflag+">ÉÏÒ»Ò³</a>");%>
<%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount() > 15)
	  out.println("<font face=webdings color=#06b020>4</font><a href=queryResult.jsp?id="+typeid+"&pageid="+rs.getNextPageId()+"&classtype="+classid+"&typeid="+typeid+"&manuf="+manufid+"&city="+cityid+arg+"&region="+regionid+"&orderColName="+orderColName+"&orderflag="+orderflag+">ÏÂÒ»Ò³</a>");%>
<%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount() > 15)
	  out.println("<font face=webdings color=#06b020>4</font><a href=queryResult.jsp?id="+typeid+"&pageid="+rs.getLastPageId()+"&classtype="+classid+"&typeid="+typeid+"&manuf="+manufid+"&city="+cityid+arg+"&region="+regionid+"&orderColName="+orderColName+"&orderflag="+orderflag+">Ä©Ò³</a>");%>
</td>
<td></td>
<td></td>
</table>
