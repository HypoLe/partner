...
<table width='100%'>
<tr>
	<td align=center width='100%' colspan=2 class="table_title">查询条件选择</td></tr>
</table>
...
for(int a = 0; a < manuV.size(); a ++)
{
	...
	if(manu.getId() == manufid)
		out.println("<option value="+manu.getId()+" selected>"+StaticMethod.dbNull2String(manu.getName())+"</option>");
	else
		out.println("<option value="+manu.getId()+">"+StaticMethod.dbNull2String(manu.getName())+"</option>");
}
...
{
		out.println("<tr class=tr_show><td></td><td align=right width='33%'><font size=2>请您选择一个城市:</font></td><td align=left><select name=city><option value=0>--全部--</option>");
}
else
{
	out.println("<tr class=tr_show><td></td><td align=right width='33%'><font size=2>请您选择一个城市:</font></td><td align=left><select name=city>");
}
...
if (queryVec.size()>0)
{
	for(int i = 1; i < queryVec.size(); i ++)
	{
		...
		out.println("<TR class=tr_show id=info  align=center>");
		...
		out.println("<font size=2>"+StaticMethod.dbNull2String(colindex.getCc_name())+"</font>:</td>");
		  ...
		  for (int k=0;k<refVec.size();k++)
			{
			   ...
			   out.println("<option value='"+coldata.getPi_idvalue()+"'>"+StaticMethod.dbNull2String(coldata.getCc_namevalue())+"</option>");
			}
		  out.println("</select>");
		}
		else
		...
		out.println("<TD width='27%' align=left><select name='disp'><option value=1 selected>显示</option><option value=0 >不显示</option></select></TD>");
	}
}
//out.println("</table>");
...
<!-- <td width='50%' align=right colspan=2><input type=button name=go value='增加查询条件' onclick='javascript:addInput()'></td>
 --><td width='100%' align=right height="32"><input type=submit name=go value='查询' onclick='javascript:goSearch()' class="clsbtn2"></td>
</tr>
</table>
</form>
