...
out.println("<td class='table_title' align=left   width='70%'><font size=2>"+cc_location+"->"+tabname+"列表显示</font></td>");
...
if(colNum != 0)
{
	...
	<%
	String strSql = Entity.getDataNSql(EntVect,cityVec,systemId,orderColName,orderflag);	//得到查询的SQL语句
	//out.println(strSql);
	/******************  构造分页显示  *************/
...
<%if(rs.getCurrentPageIndex() != rs.getFirstPageId())
				  out.println("<font face=webdings color=#06b020>4</font><a href=dataNList.jsp?id="+sId+"&pageid="+rs.getFirstPageId()+"&orderColName="+orderColName+"&orderflag="+orderflag+">首页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getFirstPageId() && rs.getRowCount()>15)
				  out.println("<font face=webdings color=#06b020>4</font><a href=dataNList.jsp?id="+sId+"&pageid="+rs.getPreviewPageId()+"&orderColName="+orderColName+"&orderflag="+orderflag+">上一页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount()>15)
				  out.println("<font face=webdings color=#06b020>4</font><a href=dataNList.jsp?id="+sId+"&pageid="+rs.getNextPageId()+"&orderColName="+orderColName+"&orderflag="+orderflag+">下一页</a>");%>
				  <%if(rs.getCurrentPageIndex() != rs.getLastPageId() && rs.getRowCount()>15)
				  out.println("<font face=webdings color=#06b020>4</font><a href=dataNList.jsp?id="+sId+"&pageid="+rs.getLastPageId()+"&orderColName="+orderColName+"&orderflag="+orderflag+">末页</a>");%>
				  </td>
<td></td>
<td></td>
</table>
