...
var AppFrameTree = function() {
	...
	return {
		...
		init : function() {
            ...
            west.createToolButton({cls:'expand',fn:this.expandAll,scope:this,text:'展开树图'});
            west.createToolButton({cls:'collapse',fn:this.collapseAll,scope:this,text:'收起树图'});
            west.createToolButton({cls:'refresh',fn:this.refreshTree,scope:this,text:'刷新树图'});
            west.collapsedEl.setStyle({'margin-left':3});
			...
			var tree = new xt.TreePanel('treePanel-body', {
				...
				lines : true,
				rootVisible : (config.rootVisible===false) ? false : true 
			});
			...
			treeLoader.on('beforeload', function(treeLoader, node, callback) {
				treeLoader.baseParams['dataType'] = (node.attributes.dataType || '');
				treeLoader.baseParams['sonType'] = (node.attributes.sonType || '');
			});
		...
		onClick : function(node, e) {
			...
			if(this.selNode.expanded){
				treesm.selNode.collapse();
			}else{
				treesm.selNode.expand();
			}
		},
		/**
		 * 树图点击或右键菜单点击时，提交请求
		...
		submit : function(menu, m, e) {
			...
			form['dataType'].value = this.selNode.attributes.dataType;
			form['sonType'].value = this.selNode.attributes.sonType;
			form['node'].value = this.selNode.attributes.id;
			form.action = this.fixUrl(m.url);
		...
		fixUrl : function(url) {
			if (url.substr(url.length - 1) == "=") {
				return url + this.selNode.id+"&dataType="+this.selNode.attributes.dataType;
		...
		reloadNode : function(node) {
			...
			//	node = (node.attributes.leaf == true || node.attributes.leaf == null) ? node.parentNode : node;
				treesm.select(node);
				node.reload();
			}
		},
		/*delReloadNode : function(node) {
		node = this.selNode;
