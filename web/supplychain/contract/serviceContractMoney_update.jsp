...
<%@page import="com.boco.supplychain.util.StaticMethod,com.boco.supplychain.baseinfo.util.vo.DeviceVo"%>
  ...
  a.clicked {color: darkblue; font-size: 12; font-weight:bold;TEXT-DECORATION: underline}
		...
		*/
		...
		function getDeviceInSelect(){
		  ...
		  //这里面的设备ID不是Device中的ID了，而是变成(WorkAroundId/deviceTypeId)
		  	...
		  	var action="<%=webapp%>/contract/makeServiceFee.do?contractId=" + contractId + "&deviceIds="+paramUrl;
		  	document.getElementById("contractMoneyFrm").src=action;
		  }
		  function init(){
		   	if(document.forms[0].serviceWorkArounds.options.length>0)
	 		 	showServiceFees(getDeviceInSelect());
	 	 	}
	</script>
	...
	<body onload="init();">
		...
		<table width="100%" height="100%" border="0" >
					...
					<table>
						...
						<td>
							<select size="15" name="serviceWorkArounds" style="width:280">
								<logic:notEmpty name="deviceVoList">
									<logic:iterate id="row" name="deviceVoList">
										<option value='<bean:write name="row" property="workAroundId"/>/<bean:write name="row" property="deviceTypeId"/>'>
											<bean:write name="row" property="workAroundName"/>/<bean:write name="row" property="deviceTypeName"/>
										</option>
									</logic:iterate>
								</logic:notEmpty>
							</select>
						...
						<td>
							<table cellpadding="1" cellspacing="2">
								...
								<tr><td>
									<input type="button" class="clsbigbtn" value='保存维护对象' onclick="showServiceFees(getDeviceInSelect());"/>
								</td></tr>
							</table>
			...
			<tr>
				<td colspan="2">
					<iframe name="contractMoneyFrm" id="contractMoneyFrm" src="serviceFee.jsp" style="position:absolute;HEIGHT: 100%;WIDTH: 100%; Z-INDEX: 0" frameborder=0 scrolling="auto"></iframe>
				</td>
			</tr>
		</table>
