...
<html:form method="post" action="/project/taskupdatedone">
<SCRIPT language=javascript>
<!--
//以下四行用于日历显示
var outObject;
...
<html:hidden name="projectForm" property="parent_id" />
  ...
  <tr>
    <td width="106%" class="table_title" align="center"><center><%=project_name%>项目----修改<%=task_name%>子任务信息(剩余可分配比例<font color="red">  <%=rest%>%</font>)</center></td>
  </tr>
</table>
    ...
    <td width="70%" height="25">
      ...
      <tr class="tr_show">
		...
		<td width="75%" height="25">
	      ...
	      不得早于  <font  color="red"><%=pro_exec_time.substring(0,10)%></font></td>
       ...
       <tr class="tr_show">
		...
		<td width="75%" height="25">
          <html:text styleClass="clstext" property="task_comp_time" readonly="true" size="20" onfocus="setday(this)"  /><font color=crimson>*</font>
               不得晚于  <font color="red"><%=pro_comp_time.substring(0,10)%></font></td>
...
<html:hidden property="task_dept_id"/><font color=crimson>*</font>
  ...
  <tr class="tr_show">
   ...
   <%if(rest==0) {%>
   <td width="70%" height="25">
      <html:text property="task_scale"  size="2" styleClass="clstext"/>%(修改不要超过<%=task_scale%>%)</td>

  <%}  else {%>
  <td width="70%" height="25">
      <html:text property="task_scale"  size="2" styleClass="clstext"/>%(修改不要超过<%=Integer.parseInt(task_scale.toString())+rest%>%)</td>
<%}%>
  </tr>
   <tr class="tr_show">
		<td width="25%" height="25" class="clsfth"><center>
