/*
 * ! PING.JS VERSION 2.51 (23-OCT-2012) @REQUIRES JQUERY V1.3 OR LATER @AUTHOR
 * HUHAO @COMPANY BOCO
 */var intStartTime;
var arrDelays = [];
var intSent;
...
var bolIsTimeout;
var strUrlFlag = connetFlag;
var strURL;
var intTimeout;
var intTimerID;
var urlNet = "http://211.137.243.1";
var urlNet1 = "http://10.110.138.164";
var $ = function(v) {
...
function handleBtnClick() {
	var objBtn = document.getElementById("btnSwitch");
	var objTxtURL;
	if (strUrlFlag) {
		objTxtURL = urlNet;
	} else {
	...
	if (bolIsRunning) {
		...
		bolIsRunning = false;
		println("　");
		println("请求连接地址为：" + strURL);
		println("　　信息: 发送 = " + intSent + ", 接收 = " + intRecv + ", 丢失 = "
				+ intLost + " (" + Math.floor(intLost / intSent * 100)
				+ "% 丢失率),");
		if (intRecv == 0)
			return;
		println("Approximate round trip times in milli-seconds:");
		println("　　最小连接时长 = " + Math.min.apply(this, arrDelays)
				+ "ms, 最大连接时长 = " + Math.max.apply(this, arrDelays)
				+ "ms, 平均连接时长 = " + Math.floor(sum / intRecv) + "ms");
		if (Math.floor(sum / intRecv) > 200
				|| Math.floor(intLost / intSent * 100) > 10) {
			alert("网络较差延迟为：" + Math.floor(sum / intRecv) + "ms 丢包率为"
					+ Math.floor(intLost / intSent * 100) + "%，可能导致地图无法加载！");
		} else {
			alert("网络正常延迟为：" + Math.floor(sum / intRecv) + "ms 丢包率为"
		...
		if (strURL.substring(0, 7).toLowerCase() != "http://")
			strURL = "http://" + strURL;
		intTimeout = parseInt(document.getElementById("txtTimeout").value, 10);
		if (isNaN(intTimeout))
			intTimeout = 2000;
		if (intTimeout < 1000)
			intTimeout = 1000;
