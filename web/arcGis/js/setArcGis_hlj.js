var map=null;var siteGraphics=[];var toDrawLine=[];var toDrawLineFlag=[];var mapUrl=connetFlag;var cityData=["哈尔滨","齐齐哈尔","牡丹江","佳木斯","大庆","绥化","鸡西","伊春","黑河","双鸭山","七台河","鹤岗 ","大兴安岭 "];var cityForUrl=["haerbin","qiqihaer","mudanjiang","jiamusi","daqing","suihua","jixi","yichun","heihe","shuangyashan","qitaihe","hegang","daxinganling"];var cityUrl={helongjiang:["128.047414","47.356592"],haerbin:["126.657717","45.773225"],qiqihaer:["123.987289","47.3477"],mudanjiang:["129.608035","44.588521"],jiamusi:["130.273614","46.811078 "],daqing:["125.02184","46.596709"],suihua:["126.989095","46.646064"],jixi:["130.941767","45.32154"],yichun:["128.910766","47.734685"],heihe:["127.50083","50.25069"],shuangyashan:["131.138209","46.765249"],qitaihe:["131.019048","45.775005"],hegang:["130.292472","47.338666"],daxinganling:["122.772849","52.229257"]};function cityAdapter(car,oil,flag){try{if(flag){removeGraphics(flag);}
...
return icon;}
function cityMarker(point,setCarInformation,setOilInformation,setCityInformation,setCarType,setOilType){try{var picSymbol=new esri.symbol.PictureMarkerSymbol(iconConfig.biaoshi,40,40);var cpPoint=new esri.geometry.Point(point[0],point[1],new esri.SpatialReference({wkid:4326}));var gra=new esri.Graphic(cpPoint,picSymbol);var infoTemplate=new esri.InfoTemplate();infoTemplate.setTitle(setCityInformation);var tempContent="<table style='width: 100%;border-collapse: collapse;font-size: 12px;'>";tempContent=tempContent
...
function loadMap(){try{var initExtent=new esri.geometry.Extent({"xmin":120.2918833182248,"ymin":42.73471804221281,"xmax":136.55740230349178,"ymax":54.22205964799385,"spatialReference":{"wkid":2311}});map=new esri.Map("map",{extent:initExtent});var tiledLayer;if(mapUrl){tiledLayer=new esri.layers.ArcGISTiledMapServiceLayer("http://211.137.243.1:8399/arcgis/rest/services/hlj_gis_service/MapServer");}
else{tiledLayer=new esri.layers.ArcGISTiledMapServiceLayer("http://10.110.180.166:8399/arcgis/rest/services/hlj_gis_service/MapServer");}
map.addLayer(tiledLayer);if(map.loaded){mapOnLoad(map);}else{dojo.connect(map,"onLoad",mapOnLoad);}}catch(err){alert("地图加载失败，请重试！  可能出现的原因为：地图地址无效");}};function mapOnLoad(map){try{if(map!=null){var resizeTimer;dojo.connect(dijit.byId('map'),'resize',function(){clearTimeout(resizeTimer);resizeTimer=setTimeout(function(){map.resize();map.reposition();},500);});dojo.connect(map.graphics,"onMouseOver",graphicsOnMouseOver);}}catch(err){alert("地图加载失败，请重试！");}}
...
var location=null;if(flag1){location=new esri.geometry.Point(eval(cityUrl.helongjiang[0]),eval(cityUrl.helongjiang[1]),new esri.SpatialReference({wkid:2311}));map.setLevel(0);}else{location=new esri.geometry.Point(eval(cityUrl.helongjiang[0]),eval(cityUrl.helongjiang[1]),new esri.SpatialReference({wkid:2311}));map.setLevel(0);}}
catch(err){alert("地图加载失败，请重试！");}}
...
var icon=iconAdapter(data_a[0].resType,typeFlag);var picSymbol=new esri.symbol.PictureMarkerSymbol(icon,25,25);var infoTemplate=new esri.InfoTemplate();var px=data_a[0].resLongitude;var py=data_a[0].resLatitude;var cpPoint=new esri.geometry.Point(px,py,new esri.SpatialReference({wkid:4326}));var gra=new esri.Graphic(cpPoint,picSymbol);var infoTemplate=new esri.InfoTemplate();infoTemplate.setTitle(data_a[0].resName);var getResourceInformation=getResourceMarkerTemplateContent(data_a,otherInformation,pathUrl,detailFlag);infoTemplate.setContent(getResourceInformation[0]);gra.setInfoTemplate(infoTemplate);map.centerAt(cpPoint);setTimeout(function(){map.setLevel(8);map.graphics.add(gra);if(type=="selected"){map.infoWindow.setTitle(data_a[0].resName);map.infoWindow.setContent(getResourceInformation[0]);map.infoWindow.show(cpPoint);}
map.infoWindow.resize(245,getResourceInformation[1]*33);},500);}catch(err){alert("地图渲染资源失败，请重试！");}}
...
content+="</table>";infoTemplate.setContent(content);gra.setInfoTemplate(infoTemplate);map.infoWindow.resize(245,350);map.graphics.add(gra);var textSym=new esri.symbol.TextSymbol();textSym.setText(i+1);textSym.setFont(new esri.symbol.Font("12pt"));textSym.setColor(new dojo.Color([128,0,0]));textSym.setOffset(20,0);var graText=new esri.Graphic(cpPoint,textSym);map.graphics.add(graText);allData.push(gra);}else{}}}
...
function addCarMarker(data_a,pathUrl,flag){try{removeGraphics(flag);var picSymbol=new esri.symbol.PictureMarkerSymbol(iconConfig.car,25,25);var infoTemplate=new esri.InfoTemplate();var px=data_a[0].longitude;var py=data_a[0].latitude;if(px==null||py==null){alert("无法获取车辆坐标信息，请重试！");}else{var cpPoint=new esri.geometry.Point(px,py,new esri.SpatialReference({wkid:4326}));var gra=new esri.Graphic(cpPoint,picSymbol);var infoTemplate=new esri.InfoTemplate();infoTemplate.setTitle(data_a[0].carNumber);var content="<table style='width: 100%;border-collapse: collapse;font-size: 12px;'>";content=content
...
+data_a[0].driverContact+"</td></tr>";content=content+"</table>";content=content
+"</br><div><a style='cursor:pointer' onclick= \"showDialog('"
+data_a[0].carUrl+"','"+pathUrl+"')\""
+">查看详情</a></div>";infoTemplate.setContent(content);gra.setInfoTemplate(infoTemplate);map.centerAt(cpPoint);setTimeout(function(){map.setLevel(8);map.graphics.add(gra);map.infoWindow.setTitle(data_a[0].carNumber);map.infoWindow.setContent(content);map.infoWindow.show(cpPoint);map.infoWindow.resize(245,260);},500);}}catch(err){alert("地图渲染车辆信息失败，请重试！");}}
function addUserMarker(data_a,pathUrl,flag){try{removeGraphics(flag);var picSymbol=new esri.symbol.PictureMarkerSymbol(iconConfig.people,25,25);var infoTemplate=new esri.InfoTemplate();var px=data_a[0].longitude;var py=data_a[0].latitude;if(px==null||py==null){alert("无法获取人员坐标信息！");}else{var cpPoint=new esri.geometry.Point(px,py,new esri.SpatialReference({wkid:4326}));var gra=new esri.Graphic(cpPoint,picSymbol);var infoTemplate=new esri.InfoTemplate();infoTemplate.setTitle(data_a[0].carNumber);var content="<table style='width: 100%;border-collapse: collapse;font-size: 12px;'>";content=content
...
+">查看详情</a></div>";infoTemplate.setContent(content);gra.setInfoTemplate(infoTemplate);map.centerAt(cpPoint);setTimeout(function(){map.setLevel(8);map.graphics.add(gra);map.infoWindow.setTitle(data_a[0].userName);map.infoWindow.setContent(content);map.infoWindow.show(cpPoint);map.infoWindow.resize(245,220);},500);}}catch(err){alert("地图渲染人员信息失败，请重试！");}}
function addErrorMarker(data_a,pathUrl,flag){try{if(data_a.length!=0&&data_a!=null){removeGraphics(flag);var icon=iconAdapter(data_a[2],null);var picSymbol=new esri.symbol.PictureMarkerSymbol(icon,25,25);var infoTemplate=new esri.InfoTemplate();var px=data_a[7];var py=data_a[8];var cpPoint=new esri.geometry.Point(px,py,new esri.SpatialReference({wkid:4326}));var gra=new esri.Graphic(cpPoint,picSymbol);var infoTemplate=new esri.InfoTemplate();infoTemplate.setTitle(data_a[0]);var content="<table style='width: 100%;border-collapse: collapse;font-size: 12px;'>";content=content
+"<tr><td style='border: 1px solid #C9DEFA;padding: 6px 6px;background-color:#CDE0FC'> 资源名称：</td><td style='border: 1px solid #C9DEFA;padding: 6px 6px;background-color:#FFFFFF;'>"
...
+data_a[9]+","+data_a[10]+"</td></tr>";content=content+"</table>";infoTemplate.setContent(content);gra.setInfoTemplate(infoTemplate);map.centerAt(cpPoint);setTimeout(function(){map.setLevel(8);map.graphics.add(gra);map.infoWindow.setTitle(data_a[0]);map.infoWindow.setContent(content);map.infoWindow.show(cpPoint);map.infoWindow.resize(245,320);},500);var picSymbol1=new esri.symbol.PictureMarkerSymbol(iconConfig.biaoshi,25,25);var px=null;var py=null;var cpPoint1;var flag;if(data_a[9]!=null&&data_a[10]!=null){px=data_a[9];py=data_a[10];flag=true;}else{px=data_a[7];py=data_a[8];flag=false;}
cpPoint1=new esri.geometry.Point(px,py,new esri.SpatialReference({wkid:4326}));if(flag){var gra1=new esri.Graphic(cpPoint1,picSymbol1);map.graphics.add(gra1);}
