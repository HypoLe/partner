...
if(bolIsRunning)
{
   ...
   */
   println("　");
   println("请求连接地址为：http:// map.baidu.com");
   println("　　信息: 发送 = " +
     intSent +
     ", 接收 = " +
     intRecv +
   ...
   println("　　最小连接时长 = " +
     ...
     "ms");
   if( Math.floor(sum/intRecv)>200|| Math.floor(intLost / intSent * 300)>10){
	   alert("网络较差延迟为："+Math.floor(sum/intRecv)+"ms 丢包率为"+Math.floor(intLost / intSent * 300)+"%，可能导致地图无法加载！");
   }
   else{
	   alert("网络正常延迟为："+Math.floor(sum/intRecv)+"ms 丢包率为"+Math.floor(intLost / intSent * 300)+"%");
   }
}
else
{
   ...
   */
   ...
   println("　");
   println("Pinging http://map.baidu.com :");
   println("　");
   ping();
}
}
