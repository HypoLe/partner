...
eoms.Calendar = function() {
    ...
    var m = ["<div id='calendar' style='display:none'>"];
    ...
    eoms.Element.on.call(window,'load',function(){cal.initCalendar();});
    eoms.Element.on.call(document,'click',function(){cal.hide();});
    ...
    return {
        ...
        s : eoms.$('selectsecond'),
        hide: function() {
            if (!bShow && this.el.style.display!="none") {
                try {
                    this.el.hide();
                    this.popDownYear();
                    this.popDownMonth();
        ...
        show: function(handler, field, format, top, left, showTime, minDate, maxDate) {
            ...
            if (typeof maxDate == "number") {
                max = new Date();
                max.setDate(today.getDate() + parseInt(maxDate));
            }
            ...
            timeFormat = splitFormat[1] || 'hh:ii:ss';
            
            isShowTime = showTime===false ? false : true;
            this.timePicker[isShowTime ? 'show' : 'hide']();
            this.s[timeFormat.indexOf('s')!=-1?'show':'hide']();
            this.i[timeFormat.indexOf('i')!=-1?'show':'hide']();
            this.h[timeFormat.indexOf('h')!=-1?'show':'hide']();
            
            if (isShowTime) {
                var t = field['defaultTime'] || field.getAttribute('defaultTime')|| this.defaultTime;
                if(t) t = t.split(':');
                if(field.value != ""){
                    t = field.value.split(' ')[1].split(':');
                }
            ...
            if (field.value != "") {
                curYear = parseInt(field.value.split("-")[0], 10);
                curMonth = parseInt(field.value.split("-")[1], 10) - 1;
                curDate = parseInt(field.value.split("-")[2], 10);
            }
        ...
        closeCalendar: function() {
            ...
            var constructDate = function(d, m, y) {
                ...
                if (isShowTime) {
                	var t = timeFormat;
                	t = t.replace('hh',_this.h.value);
                	t = t.replace('ii',_this.i.value);
                	t = t.replace('ss',_this.s.value);
                    return s.replace("yyyy", y) + " " + t;
                } else {
            ...
            if (t > _max || t < _min) {
                ...
                this.onAfterClose(ctlToPlaceValue.value, ctlToPlaceValue);
         ...
         * @param {HTMLElement} 回填的input元素
         */
        onAfterClose:function(value, el){},
        popUpYear: function() {
            this.popDownMonth();
            ...
            if (!yearConstructed) {
                ...
                sHTML = "<tr><td align='center' onmouseover='this.style.backgroundColor=\"#FFCC99\"' " 
                  + "onmouseout='clearInterval(intervalID1);this.style.backgroundColor=\"\"' style='cursor:pointer' " 
                  + "onmousedown='clearInterval(intervalID1);intervalID1=setInterval(\"cal.decYear()\",30)' " 
 ...
 * @param {String} format 格式字符串，缺省为yyyy-mm-dd hh:ii:ss
