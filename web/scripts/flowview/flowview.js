...
var FlowView = function(el) {
	Page = el;
	if(!eoms.isIE){
		$('workflowCanvas').innerHTML="<canvas id='canvas' width='1000' height='1000'></canvas>";		
	...
	actions.each(function(action) {
		...
		if (sNode != null && getCellNode(action.from) != null
			...
			if (getCellNode(action.to) == null)
				missCells.push("Step:<action id=" + action.id + ">的开始节点ID:"
						+ action.to);
		}
	});
...
*/
	...
	if(eoms.isIE){
		this.txtCell = Toolkit.newElement("<div class=\"TXT\"></div>");
		this.txtCell.innerText = this.name;
		this.el.appendChild(this.txtCell);
	}else{
		this.txtCell = Toolkit.newElement("div");
		this.txtCell.className = "TXT"
		this.txtCell.innerHTML = this.name;
		this.el.appendChild(this.txtCell);
	}
