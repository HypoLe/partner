 ...
 * @example
 *  日期控件与某一日期字符串比较：
 *  &lt;input id="time1" type="text" alt="compare:'>2008-02-14 23:59:59|回复时限必须晚于2月14日'"&gt;
 * @example
 *  正则表达式验证：
 *  &lt;input type="text" alt="re:/^(\d+)(\.\d+)?$/,re_vt:'请输入整数或小数'"/&gt;
 * @example
...
eoms.form.Validation = function(config) {
	...
	this.mask = this.mask || 'true';//add by fengguangping 解决蒙版效果，默认值为"true"字符串；
	this.vAttr = 'alt';
	this.form = $(this.form);
	this.init();
    this.initPreCommit();
...
eoms.form.Validation.prototype = {
	...
	isVElement : function(el) {
		...
		if (!el[this.vAttr] && !el.getAttribute(this.vAttr)) {
			...
			if (config.allowBlank == false || config.vtype || config.maxLength || config.compare || config.re) {
				return true;
			}
			return false;
		}
    ...
    /**
     * form标签中含有preCommit="true"时，自动启用preCommit
     */
    initPreCommit : function(){ 
        this.preCommitQueried = false; // 是否请求过preCommit后台
        this.preCommitUrl = ''; 
        if((this.form['preCommit'] || this.form.getAttribute('preCommit')) && this.form.action){
            this.preCommitUrl = this.form.action.replace(/method=([\w]+)([&]?)/, function($0,$1,$2){
                return $0.replace($1,'performPreCommit');
            });
        };
    },
	/**
	...
	onSubmit : function(form, event) {
		...
		//add by fengguangping  
		//解决问题：
		//在下载模板文件时需要通过下拉框选择字段来拼接文件名而又需要对下拉框验证是否为选择时，提交表单后蒙版效果一直存在页面中，
		//页面一直要显示“'数据验证中，请稍候...'”，此时页面一直在等待后台响应，但是后台输出的是文件流，所以一直处于蒙版效果
		//解决方法：new eoms.form.Validation({form:'downloadForm',mask:'sdfsdf'});添加mask：只要改字符串不为“true”即可去掉蒙版效果，不添加时默认为"true",和以前一样；
		if(this.mask=='true') {
	...
	validate : function(el) {
		...
		if (el.disabled || this.validateValue(el.value.trim(), el, config)) {
	...
	validateValue : function(value, el, config) {
		var blankText = config.vtext || "请输入内容";
        ...
        //只用于日期字符串比较，格式为 alt="compare:'>2008-02-14 23:59:59|回复时限必须晚于2月14日'"
        if (config.compare && !this.passing){
            var cpre = /^(>|<|=)([\w-: ]+)\|(.+)/;
            var q = config.compare.match(cpre);
            if(q && this.operators[q[1]](value, q[2])===false){
                this.markInvalid(el, q[3]);
                return false;
            }
        }
        if (config.re && config.re instanceof RegExp && !this.passing){
            if(!config.re.test(value)) {
            	this.markInvalid(el, config.re_vt || '请检查输入格式');
                return false;
            }
        }
	...
	alertInvalid : function(el) {
		var config = this.getConfig(el);
		var info = el.invalidText || config.vtext || config.blankText || '请检查表单项';
		info += "\n\n...\n表单至少有1项不符合要求，请检查表单项。"
		alert(info);
		Ext.get(this.form).unmask();
		setTimeout(function() {
...
eoms.form.VTypes = function() {
	...
	var phonemobile =  /^(\d{3,4}\-)?-?[0-9]*$/;
	//modify by Wuchunhui
	var float = /((\d+\.\d*[1-9]\d*)|(\d*[1-9]\d*\.\d+)|(\d*[1-9]\d*))$/;
	/** @scope eoms.form.VTypes */
	return {
		'float':function(v){
			return float.test(v);
		},
		'floatText':'请输入数字或者带小数点得数字',
	//end modify
		'number' : function(v) {
		...
		'phonemobile' : function(v) {
			return phonemobile.test(v);
		},
		/**
		 * 数字类型验证提示
		 * @type {String}
		 */
		'phonemobileText' : '请输入正确电话号码',
		/**
		 * 整数验证
		 */
		'integer' : function(v) {
