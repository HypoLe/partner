...
<html>
  ...
  <form id="form"></form>
   ...
   <% 
   		List list = (List)request.getAttribute("myexamlist");
   	...
   	%>		
	   	 <tr height="35">
		      ...
		      <%
			      ...
			      if(TimeUtil.timeCompareto(c,StaticMethod.getTimestampString(ec.getStartTime()))>0&&TimeUtil.timeCompareto(c,StaticMethod.getTimestampString(ec.getEndTime()))<0){
		       %>
		      	<input name="button" type="button" class="add_btn_04"  value=" 开始考试" onclick="star('<%=ec.getIssueId()%>')" />
		      <%}else if(TimeUtil.timeCompareto(c,StaticMethod.getTimestampString(ec.getStartTime()))<0) {%>
		      	<img src="<%=basePath%>examonline/manage/images/button_25.png" /> 
		      <%} else{%>
		     	 <img src="<%=basePath%>examonline/manage/images/button_24.png" />
		      <%} %>
		      </td>
    		</tr>
   	<%
       ...
       <TBODY>
					<TR>
						<TD height="28" align=right vAlign=center noWrap>
							&nbsp;&nbsp;
							<!-- 可以在这里插入分页导航条 -->
							<pg:pager url="myexamList.do" items="${resultSize }"
