...
<script language="javascript">
  ...
  function batchDel(){
    if( confirm( "是否删除批次：" +
        document.forms[0].batchSel[document.forms[0].batchSel.selectedIndex].text ) )
      return true;
    return false;
  }
function view(image){
...
%>
<body bgcolor="#ffffff" >
<form action="${pageContext.request.contextPath}/examonline/batchDelete.do" >
<html:hidden property="checkSel" value=""/>
<table cellpadding="0" cellspacing="0" width="95%" align="center" border="1" class="table_show">
  <tr class="tr_show" align="center">
    <td class="td_label" colspan="2">
      批次删除
    </td>
    <td align="left" colspan="4">
      <html:select property="batchSel" value="0" style="width:350">
        <html:options collection="INFOLABLIST" property="value" labelProperty="label"/>
      </html:select>

      <html:submit styleClass="clsbtn2" value="删除批次" onclick="return batchDel()"/>
    </td>
  </tr>
  <tr>
...
<center>
  <div id="divTable" style="position: relative; align: center; top: 0px;width:  95%; height:  80%; z-index: 1; overflow: auto; overflow-x: hidden"> 
...
%>
      <tr class="tr_show" align="center"> 
        <td width="6%"> <input type="checkbox" name="<%=onlineWarehouse.getSubId()%>"> 
        </td>
        <td width="6%"> <%=++i%> </td>
        <td width="70%" align="left"> <bean:write name="onlineWarehouse" property="title"/> 
        </td>
        <td width="6%" align="left"> <logic:equal name="onlineWarehouse" property="issueType" value="1"> 
          ...
          考试 </logic:equal> </td>
        <td width="6%"> <logic:notEmpty name="onlineWarehouse" property="image"> 
          <img src="<%=request.getContextPath()%>/images/bottom/an_xs.gif" border="0" onclick="view('<%=urlImage%>')"> 
          </logic:notEmpty> </td>
        <td width="6%"> <html:link href="<%=urlM%>" name="map"> <img src="<%=request.getContextPath()%>/images/bottom/an_bj.gif" border="0"> 
          </html:link> </td>
      </tr>
      </logic:iterate> 
    </table>
