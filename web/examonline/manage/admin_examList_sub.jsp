...
<html>
  ...
  function star(issueId)
  {
  window.location.href="<%=basePath%>examonline/examine.do?issueid="+issueId;
  }
  </script>
    <base href="<%=basePath%>">
  ...
  <body>
  <form action="<%=basePath%>examonline/getAdminExamList.do?examType=prov"></form>
   ...
   <% 
   	...
   	%>		
   		<tr>
	    	...
	    	<td><%=m.get("testerCount")%></td>
	    	<td><%=m.get("startTime")%>--<%=m.get("endTime")%></td>
	    	<td><%=m.get("tester")%></td>
	    	<td><%=m.get("noTester")%></td>
	    	<td>
	    	<%
	    	int markFlag = Integer.parseInt(m.get("markFlag").toString());
	    	
		    String c = StaticMethod.getCurrentDateTime();
		    if(TimeUtil.timeCompareto(c,StaticMethod.getTimestampString((java.sql.Timestamp)m.get("endTime")))>0
		    	&& markFlag == 1){
	    	%><input name="button" type="button" class="add_btn_05"  value="阅卷" 
  	onclick="window.open('<%=basePath%>examonline/findQaMarkList.do?issueId=<%=m.get("issueId")%>')"><% 	
	    	}else if(TimeUtil.timeCompareto(c,StaticMethod.getTimestampString((java.sql.Timestamp)m.get("endTime")))<0
			    	&& markFlag == 1){
		    %>待阅卷<% 	
		    }else if(markFlag == 2){
	    	%>阅卷完成<%
       ...
       <TBODY>
					<TR>
						<TD height=28 align=right vAlign=center noWrap>
							&nbsp;&nbsp;
							<!-- 可以在这里插入分页导航条 -->
							<pg:pager url="getAdminExamList.do" items="${resultSize }"
