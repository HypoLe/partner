  ...
  <form id="form"></form>
   ...
   <% 
   		...
   		for(int j=0;j<list.size();j++){
   			ExamConfig exam=(ExamConfig)list.get(j);
   	%>		
	   	 <tr height="35">
		      <td><%=j+1%></td>
		      <td><%=exam.getTitle()%></td>
		      <td><eoms:id2nameDB id="<%=exam.getIssueUser()%>" beanId="tawSystemUserDao" /></td>
		      <td><eoms:id2nameDB id="<%=exam.getCompanyId()%>" beanId="partnerDeptDao"/></td>
		      <td><%=exam.getStartTime()%></td>
    		  <td><%=exam.getEndTime()%></td>
    		  <td><%=exam.getTypesel_fk()%></td>
		      <td>
		      <c:if test="${assign=='false'}" var="result">
		      <a href="${app}/examonline/examPartner.do?method=findQualifierList&issueId=<%=exam.getIssueId()%>">查看</a>
		      </c:if>
		      <c:if test="${!result}">
		       <a href="${app}/examonline/examResult.do?method=findExamResultList&issueId=<%=exam.getIssueId()%>">指定</a>	
		      </c:if>
		      </td>
    		</tr>
   	<%
       ...
       <TBODY>
					<TR>
						<TD height="28" align=right vAlign=center noWrap>
							&nbsp;&nbsp;
							<!-- 可以在这里插入分页导航条 -->
							<pg:pager url="${app}/examonline/examPartner.do" items="${resultSize }"
								export="currentPageNumber=pageNumber" maxPageItems="10">
								<pg:param name="method" value="findExamConfigList"/>
								<pg:first>
