...
<%@ page import="com.boco.eoms.examonline.model.ExamNotice"%>
  ...
  <form id="form"></form>
   ...
   <% 
   		...
   		for(int j=0;j<list.size();j++){
   			ExamNotice en=(ExamNotice)list.get(j);
   	%>		
	   	 <tr height="35">
		      <td><%=j+1%></td>
		      <td><%=en.getTitle()%></td>
		      <td><eoms:id2nameDB id="<%=en.getCreator()%>" beanId="tawSystemUserDao" /></td>
		      <td><%=en.getCreateTime()%></td>
		      <td>
		      <a href="<%=basePath%>examonline/examNotice.do?method=showDetail&id=<%=en.getId()%>">查看</a>
		      </td>
    		</tr>
   	<%
   	}
       ...
       <TBODY>
					<TR>
						<TD height="28" align=right vAlign=center noWrap>
							&nbsp;&nbsp;
							<!-- 可以在这里插入分页导航条 -->
							<pg:pager url="examonline/examNotice.do" items="${resultSize }"
								export="currentPageNumber=pageNumber" maxPageItems="10">
								<pg:param name="method" value="findExamNoticeList"/>
								<pg:first>
