...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
 <script type="text/javascript" src="${app}/scripts/base/prototype.js"></script>
  <script type="text/javascript">
	  function deptQuery(value){
	  	$('showcfg').src="<%=basePath%>examonline/examDeptQuery.do?issueId="+value;
	  }
	  function testerQuery(value){
	  	$('showcfg').src="<%=basePath%>examonline/examTesterQuery.do?issueId="+value;
	  }
  </script>
...
<body>
  ...
  <% 
        ...
        if(pm!=null){
   		List list = (List)pm.getDatas();
   		
   		for(int j=0;j<list.size();j++){
   			ExamConfig m=(ExamConfig)list.get(j);
   	%>		
   		<tr height="35">
	   		<td><%=j+1%></div></td>
	    	<td><%=m.getTitle()%></div></td>
	    	<td><%=m.getStartTime()%></div></td>
	    	<td><%=m.getEndTime()%></div></td>
	    	<td><%=m.getCompanyId()%></div></td>
	    	<td>
	    		<input name="button" type="button" class="add_btn_05"  value="按单位查看" onclick="deptQuery('<%=m.getIssueId()%>')" />
	    		<input name="button" type="button" class="add_btn_05"  value="按成绩查看" onclick="testerQuery('<%=m.getIssueId()%>')" />
	    	</td>
    	</tr>
   	<%
   	}
...
<table width="100%" class="table_2" height="20" border="0" cellpadding="0" cellspacing="0">
       <TBODY>
			<TR>
				<TD height=28 align=right vAlign=center noWrap>
					&nbsp;&nbsp;
					<!-- 可以在这里插入分页导航条 -->
					<pg:pager url="examQuerySub.do" items="${pm.total }"
	...
	<%if(list.size()>0){ %>
		<iframe name="showcfg"  id="showcfg" height="500" width="100%"></iframe>
	<%} }%>
