...
<form name="addEForm" method="post" action="<%=request.getContextPath()%>/sheetnameservlet"  onsubmit="javascript: return checkInput();" >
<div align="center">
  <center>
<br>
<table cellSpacing=0 cellPadding=0  border=1 width="760" bordercolordark="#FFFFFF" bordercolorlight="#66CCFF" bgcolor=#F3F3F3 align="center">
 ...
 <tr>
      ...
      <td width="23%">
          <fmt:message key="autosheet.yes"/><input type="radio" name="ifsetpath" value="0" onClick="selectTr0()" checked="true">
	  ...
	  <select name="module" id="s1" onchange="this.form.action='';this.form.submit();">
			...
			while(rs.next()){
                  	if (module == null) {
                        ...
                        %>
				<option value="<%=rs.getString(1)%>"<%if(module.equals(rs.getString(1))) {out.print("selected");}%>><%=rs.getString(2)%></option>
			<%
                       }
                      }
		%>
          ...
          <option></option>
                ...
                for(int i=0;i<alltree.size();i++){
                	Module = alltree.get(i).toString().split("_");
                        %>
                        <option value="<%=Module[0]%>"><%=Module[2]%></option>
                        <%
                }
               %>
          </select>
	  ...
	  <select name="modulemk">
			...
			while(rs1.next()){
                  	if (module == null){
                        ...
                        %>
				<option value="<%=rs1.getString(1)%>"<%if(module.equals(rs1.getString(1))) {out.print("selected");}%>><%=rs1.getString(2)%></option>
			<%
                       }
                      }
		%>
