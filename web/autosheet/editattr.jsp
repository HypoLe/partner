...
<td nowrap height="34"><a href="./sheetvalue.jsp?sheetID=<%out.print(sheet_id);%>&reFactor=reFactor">增加输入框</a></td>
    ...
    <table id=table1 cellspacing=0 cellpadding=0  border=1 width="779" bordercolordark="#FFFFFF" bordercolorlight="#66CCFF" bgcolor=#F3F3F3 align="center">
       <thead>
      ...
      <tr>
        ...
        <td>&nbsp;<%
              if(rs.getString(15).equals("nowrap"))
                ...
                out.print("是");%></td>
        <td><a href="./editAttributeDetail.jsp?attr_id=<%out.print(rs.getString(1));%>&sheet_id=<%out.print(sheet_id);%>">修改</a></td>
        <td><a href="<%=request.getContextPath()%>/sheetattrservlet?flag=delete&attr_id=<%=rs.getString(1)%>&sheet_id=<%=sheet_id%>" onClick = "javascript: return ConfirmDel();">删除</a></td>
      </tr>
        </tbody>
      <%}%>
    </table>
