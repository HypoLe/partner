...
<div id="loadIndicator" class="loading-indicator" style="display:none">载入详细数据中，请稍候</div>
 ...
 <logic:present
	...
	<display:table name="publicizeArticleApprovalList" cellspacing="0" cellpadding="0"
		...
		requestURI="{app }/deviceManagement/publicizeArticle/publicizeArticleApproval.do" sort="list" partialList="true"
		size="${size}">
		<display:column sortable="true" headerClass="sortable" title="项目名称">
			${publicizeArticleApprovalList.projectName}
		</display:column>
		<display:column sortable="true" headerClass="sortable" title="宣传单/宣传品发放地点">
		...
		<display:column sortable="false" headerClass="sortable" title="审批"
			paramProperty="id" url="/deviceManagement/publicizeArticle/publicizeArticle.do?method=goToApproval"
		...
		<display:column sortable="false" headerClass="sortable" title="详情"
			...
			<a id="${publicizeArticleApprovalList.id }"
				href="${app }/deviceManagement/publicizeArticle/publicizeArticle.do?method=goToDetail&id=${publicizeArticleApprovalList.id}"
