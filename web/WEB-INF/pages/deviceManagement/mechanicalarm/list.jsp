...
<html:form  action="mechanicalArmManagement.do?method=list" styleId="MechanicalArmList" method="post">
	...
	<display:table name="MechanicalArmList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true" headerClass="sortable" title="状态">
			...
			<display:column sortable="false" headerClass="sortable" title="操作"
				...
				<c:if test="${MechanicalArmList.approvingresult eq '驳回'}">
				<a id="${MechanicalArmList.id }"
					href="${app }/mechanicalArm/mechanicalArmManagement.do?method=gotoEdit&id=${MechanicalArmList.id}&state=${MechanicalArmList.state}">
...
<div>
    <c:if test="${MechanicalArmList.state eq '待审核'}">
	<input type="button" onclick="jumpToAdd()" value="添加" class="btn" /></c:if>
	<c:if test="${MechanicalArmList.state eq '归档'}">
	<input type="button" onclick="deleteSome()" value="删除" class="btn" /></c:if>
</div>
