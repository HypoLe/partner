...
function deleteInfo(id) {
			if(confirm("确定要删除吗？")){
				Ext.Ajax.request({
					url:"${app}/advertisementPlan/advertisementPlanAction.do",
					params:{method:"delete",id:id},
					success:function(res) {
						Ext.Msg.alert("提示：","删除成功！",function() {location.href='<html:rewrite page='/advertisementPlanAction.do?method=listReject'/>'});
					},
...
<div id="listQueryObject" 
	...
	<form action="advertisementPlanAction.do?method=list" method="post" id="checkSegmentFindForm" name="checkSegmentFindForm">
		<fieldset>
			<legend>快速查询</legend>
...
<logic:notEmpty  name="advertisementPlanList" scope="request">
	<display:table name="advertisementPlanList" cellspacing="0" cellpadding="0"
		...
		<%--<display:column sortable="true" headerClass="sortable" title="驳回人">
			
			<eoms:id2nameDB id="${advertisementPlanList.reviewer}" beanId="tawSystemUserDao" />
		</display:column>--%>
<display:column sortable="true" headerClass="sortable" title="状态">
			<c:if test="${advertisementPlanList.status eq '1'}">
			待审核
			</c:if>
			<c:if test="${advertisementPlanList.status eq '2'}">
			归档
			</c:if>
			<c:if test="${advertisementPlanList.status eq '3'}">
			驳回
			</c:if>
		</display:column>
		<display:column sortable="false" headerClass="sortable" title="操作"
				media="html">
				<c:if test="${advertisementPlanList.status eq '3'}">
				<a id="${advertisementPlanList.id }"
					href="${app }/advertisementPlan/advertisementPlanAction.do?method=goToReSubmit&id=${advertisementPlanList.id}">
		...
		<%--<display:column sortable="false" headerClass="sortable" title="删除"
			 media="html">
			 <a href="javascript:void(0)" onclick="deleteInfo('${advertisementPlanList.id}')">
