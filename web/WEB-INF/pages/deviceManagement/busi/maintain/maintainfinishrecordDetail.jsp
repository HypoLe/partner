...
<table id="stylesheet" class="formTable">
	...
	<c:out value="割接详情页面" />
		...
		<tr>
			<td class="label">割接名称a*</td>
			<td class="content" colspan="3">
			${maintain.maintainName}
			</td>
		</tr>
		<tr><td class="label">光缆线段*</td>
			<td>${maintain.fiberSection }</td>
  ...
  <div id="history-info" class="tabContent">
    <display:table name="maintainApprovalList" cellspacing="0" cellpadding="0"  class="table advertisementListApproval"  
        id="maintainApprovalList" export="false" sort="list" partialList="true" size="${maintainApprovalListsize}" pagesize="${pagesize}">
        ...
        <display:column headerClass="sortable" title="审批状态">
			${ApprovaType[maintainApprovalList.approvalType]}
        </display:column>
   ...
   <div id="check-info" class="tabContent">
    <display:table name="maintainFinishList" cellspacing="0" cellpadding="0"  class="table advertisementListApproval"  
        id="maintainFinishList" export="false" sort="list" partialList="true" size="${maintainFinishListsize}" pagesize="${pagesize}">
		<display:column headerClass="sortable" title="验收结果">
		${checkresult[maintainFinishList.finishResult]}
		</display:column>
		<display:column headerClass="sortable" title="开始验收时间	">
			${maintainFinishList.startTime}
        </display:column>
        <display:column headerClass="sortable" title="结束验收时间">
			${maintainFinishList.endTime}
        </display:column>
	</display:table>
  </div>
