 ...
 <logic:present
	...
	<display:table name="maintainList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true" headerClass="sortable" title="状态">
			${characterId2TYPEMap[maintainList.maintainFinish]}
		</display:column>
		<c:if test="publicizeArticleList.approvalType"></c:if>
		<display:column sortable="false" headerClass="sortable" title="归档"
			 media="html">
			 <c:set value="${maintainList.isRecord}" var="isRecord"></c:set>
			 <c:choose>
			 <c:when test="${isRecord == 1}">
			 已归档
			 </c:when>
			 <c:when test="${isRecord != 1}">
			<a href="${app }/deviceManagement/line/maintainfinish.do?method=goToAddMaintainFinishRecord&id=${maintainList.id }">归档<img src="${app }/images/icons/edit.gif"><a>
			 </c:when>
			 </c:choose>
		</display:column>
		<display:column sortable="false" headerClass="sortable" title="详情"
			...
			<a id="${maintainList.id }"
				href="${app }/deviceManagement/line/maintainfinish.do?method=goToMaintainFinishRecordDetail&id=${maintainList.id}"
