...
<div id="listQueryObject" 
	...
	<html:form action="advertisement.do?method=list" method="post">
 ...
 <logic:present
	name="advertisementList" scope="request">
	<display:table name="advertisementList" cellspacing="0" cellpadding="0"
		id="advertisementList" pagesize="${pagesize}"
		...
		<display:column sortable="true" headerClass="sortable"
			title="墙体广告数量（份）">
			${advertisementList.advertisementAmount}
		</display:column>
		<display:column sortable="true" headerClass="sortable" title="完成时间">
			${advertisementList.finishTime}
		</display:column>
	
		<display:column sortable="true" headerClass="sortable" title="审核人">
			 <eoms:id2nameDB beanId="tawSystemUserDao" id="${advertisementList.approvalUser}"/>
		</display:column>
		<display:column sortable="true" headerClass="sortable" title="状态">
			${advertisementType[advertisementList.approvalType]}
		</display:column>
		...
		media="html">
			...
			<a id="${mediaPublicityList.id }"
				href="${app }/deviceManagement/advertisement/advertisement.do?method=goToEdit&id=${advertisementList.id}"
				shape="rect"> <img
				src="${app }/images/icons/edit.gif"> </a>
				</c:if>
				<c:if test="${advertisementType[advertisementList.approvalType]=='待审核'}">
				无
				</c:if>
		...
		<display:column sortable="false" headerClass="sortable" title="详情"
			media="html">
			<a id="${advertisementList.id }"
				href="${app }/deviceManagement/advertisement/advertisement.do?method=goToDetail&id=${advertisementList.id}"
