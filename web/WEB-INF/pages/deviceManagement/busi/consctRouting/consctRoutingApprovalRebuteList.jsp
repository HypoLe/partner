...
已审核列表页
	...
	<%-- <form action="consctRouting.do?method=approvalRebuteList" method="post" name="searchForm" id="searchForm">
		<fieldset>
			<legend>快速查询</legend>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app }/consctRouting/consctRouting.do?method=approvalRebuteList'"/>
						</div>
					</td>
				</tr>
			</table>
...
<logic:present name="consctRoutingList" scope="request">
	...
	<%-- 	<display:column sortable="false" headerClass="sortable" title="操作"
			...
			<a id="${consctRoutingList.id}"
				...
				<c:if test="${consctRoutingList.status eq '2'}">
				无
			</c:if>
		</display:column>--%>
