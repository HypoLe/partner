     ...
     <c:if test="${type eq '1' }" var="return"> 
		...
		<display:column  headerClass="sortable" title="操作">
			...
			<a id="${warningBoardList.id }"
				href="${app }/deviceManagement/warningboard/warningboard.do?method=goToEdit&id=${warningBoardList.id}"
				 >
				<img src="${app }/images/icons/edit.gif">
				</a>
			</c:if><c:if test="${(1!=(warningBoardList.status))&&(5!=(warningBoardList.status))}">
			无
			</c:if>
		</display:column>
		<display:column  headerClass="sortable" title="详情">
			 <%--<c:if test="${(1<(warningBoardList.status))&&(5>(warningBoardList.status))}" > 
			---%><a id="${warningBoardList.id }" target="blank" shape="rect"
