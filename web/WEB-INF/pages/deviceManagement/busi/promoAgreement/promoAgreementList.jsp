...
<%-----
		...
		<display:column sortable="true" headerClass="sortable" title="填报人">
			...
			<eoms:id2nameDB beanId="tawSystemUserDao"
				id="${promoAgreementList.personnelId}" />
		</display:column>
		<%-----
...
---%>
		...
		<display:column sortable="true" headerClass="sortable" title="状态">
			...
			<c:if test="${5==(promoAgreementList.status)}">驳回</c:if>
			<c:if test="${2==(promoAgreementList.status)}">审核中</c:if>
			<c:if test="${3==(promoAgreementList.status)}">归档</c:if>
			<c:if test="${4==(promoAgreementList.status)}">归档</c:if>
		</display:column>
		<display:column headerClass="sortable" title="操作">
		<c:if test="${5==(promoAgreementList.status)}">
			<a id="${promoAgreementList.id }"
				href="${app }/deviceManagement/promoagreement/promoagreement.do?method=goToEdit&id=${promoAgreementList.id}&type=${type}">
				<img src="${app }/images/icons/edit.gif"> </a></c:if>
			<c:if test="${5!=(promoAgreementList.status)}">无</c:if>
		</display:column>
		<display:column headerClass="sortable" title="详情">
			<a id="${promoAgreementList.id }" 
				href="${app }/deviceManagement/promoagreement/promoagreement.do?method=goToDetail&id=${promoAgreementList.id}&type=${type}" target="blank" shape="rect">
