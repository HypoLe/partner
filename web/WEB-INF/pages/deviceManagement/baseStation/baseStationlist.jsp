	...
	<display:table name="baseStationList" cellspacing="0" cellpadding="0"
		...
		size="${size}">
		  <display:column  title="<input type='checkbox' onclick='javascript:chooseAll();'>" >
        	 <input type="checkbox" name="checkbox11" id="checkbox11"  value="${baseStationList.id }"/>
   		 </display:column>
		<%--<display:column  sortable="true"
			headerClass="sortable" title="序号" >
			<a href="${app}/baseinfo/baseStation.do?method=goToDetail&id=${baseStationList.id }"><%=i++ %>
			href="${app}/baseinfo/baseStation.do?method=goToDetail"
			</a>
			</display:column>--%>
		...
		<%--<display:column property="remark" sortable="true"
			headerClass="sortable" title="备注" >${baseStationList.remark}</display:column>>--%>
	</display:table>
</logic:present>
	<input class="button" type="button" onclick="addBaseStation()" value="添加基站信息"/>&emsp;&emsp;
	<input class="button" type="button" onclick="delSelBaseStation()" value="删除所选基站息"/>
