...
var myJ = $.noConflict();
 myJ(function(){
 	...
 	if(!window.opener){
		myJ('table#electricity').find('tr:gt(0)')
		.css('cursor','pointer')
...
function goAdd(){
	myJ("form#mainForm").get(0).action = "ElectricityManagement.do?method=add";
	myJ("form#mainForm").submit();
}
</script>
<input type="button" id="showSearch" value="快速查询" class="btn" />
	<form action="${app}/outlay/ElectricityManagement.do?method=list" method="post" id="searchForm">
		<table id="sheet" class="formTable">
			<tr>
				...
				<td>
					...
					<input type="hidden" id="partnerName2search" name ="partnerName2search" value="${electricity.partnerName}"/></td>
...
<form id="mainForm" action="/ElectricityManagement.do?method=list" method="post"> 
<logic:notEmpty name="electricityList">
<c:set var="myCheckboxAllBtn" scope="page">
	<input type="checkbox" id="myCheckbox" />
</c:set>
<display:table name="electricityList" class="table" id="electricity" export="true" sort="list"
		partialList="true" size="${size}" requestURI="ElectricityManagement.do">  
<display:column media="html" title="${myCheckboxAllBtn}">
	<input type="checkbox" class="checkAble" value="${electricity.id}" id="${electricity.id}" />
</display:column>
<display:column title="合作伙伴" headerClass="sortable" sortable="true">
	<eoms:id2nameDB id='${electricity.partnerName}' beanId='partnerDeptDao'/>
</display:column>
<display:column property="areaName" title="地市" headerClass="sortable" sortable="true"/>
<display:column property="date" title="时间" headerClass="sortable" sortable="true"/>
<display:column property="electricQuantity" title="月度电量" headerClass="sortable" sortable="true"/>
<display:column sortable="true" headerClass="sortable" title="查看"
	paramProperty="id" url="/outlay/ElectricityManagement.do?method=detail"
	paramId="id" media="html">
	<img src="${app}/images/icons/search.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="编辑"
	paramProperty="id" url="/outlay/ElectricityManagement.do?method=edit"
	paramId="id" media="html">
	<img src="${app}/nop3/images/edit.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="删除"
	paramProperty="id" url="/outlay/ElectricityManagement.do?method=remove"
	paramId="id" media="html">
