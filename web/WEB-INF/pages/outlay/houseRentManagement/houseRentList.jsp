...
var myJ = $.noConflict();
 myJ(function(){
 	...
 	if(!window.opener){
		myJ('table#houseRent').find('tr:gt(0)')
		.css('cursor','pointer')
...
function goAdd(){
	myJ("form#mainForm").get(0).action = "HouseRentManagement.do?method=add";
	myJ("form#mainForm").submit();
}
</script>
<input type="button" id="showSearch" value="快速查询" class="btn" />
	<form action="${app}/outlay/HouseRentManagement.do?method=list" method="post" id="searchForm">
...
<form id="mainForm" action="/HouseRentManagement.do?method=list" method="post"> 
<logic:notEmpty name="houseRentList">
<c:set var="myCheckboxAllBtn" scope="page">
	<input type="checkbox" id="myCheckbox" />
</c:set>
<display:table name="houseRentList" class="table" id="houseRent" export="true" sort="list"
		partialList="true" size="${size}" requestURI="HouseRentManagement.do">  
<display:column media="html" title="${myCheckboxAllBtn}">
	<input type="checkbox" class="checkAble" value="${houseRent.id}" id="${houseRent.id}" />
</display:column>
<display:column title="合作伙伴" headerClass="sortable" sortable="true">
	<eoms:id2nameDB id='${houseRent.partnerName}' beanId='partnerDeptDao'/>
</display:column>
...
<display:column property="country" title="区县" headerClass="sortable" sortable="true"/>
<display:column property="houseRentFee" title="房租费" headerClass="sortable" sortable="true"/>
<display:column sortable="true" headerClass="sortable" title="查看"
	paramProperty="id" url="/outlay/HouseRentManagement.do?method=detail"
	paramId="id" media="html">
	<img src="${app}/images/icons/search.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="编辑"
	paramProperty="id" url="/outlay/HouseRentManagement.do?method=edit"
	paramId="id" media="html">
	<img src="${app}/nop3/images/edit.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="删除"
	paramProperty="id" url="/outlay/HouseRentManagement.do?method=remove"
	paramId="id" media="html">
