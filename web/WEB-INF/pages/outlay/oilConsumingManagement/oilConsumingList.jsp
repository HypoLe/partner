...
var myJ = $.noConflict();
 myJ(function(){
 	...
 	if(!window.opener){
		myJ('table#oilConsuming').find('tr:gt(0)')
		.css('cursor','pointer')
...
function goAdd(){
	myJ("form#mainForm").get(0).action = "OilConsumingManagement.do?method=add";
	myJ("form#mainForm").submit();
}
</script>
<input type="button" id="showSearch" value="快速查询" class="btn" />
	<form action="${app}/outlay/OilConsumingManagement.do?method=list" method="post" id="searchForm">
...
<form id="mainForm" action="/OilConsumingManagement.do?method=list" method="post"> 
<logic:notEmpty name="oilConsumingList">
<c:set var="myCheckboxAllBtn" scope="page">
	<input type="checkbox" id="myCheckbox" />
</c:set>
<display:table name="oilConsumingList" class="table" id="oilConsuming" export="true" sort="list"
		partialList="true" size="${size}" requestURI="OilConsumingManagement.do">  
<display:column media="html" title="${myCheckboxAllBtn}">
	<input type="checkbox" class="checkAble" value="${oilConsuming.id}" id="${oilConsuming.id}" />
</display:column>
<display:column title="合作伙伴" headerClass="sortable" sortable="true">
<eoms:id2nameDB id='${oilConsuming.partnerName}' beanId='partnerDeptDao'/>
</display:column>
<display:column property="oilConsumingCount" title="油耗" headerClass="sortable" sortable="true"/>
