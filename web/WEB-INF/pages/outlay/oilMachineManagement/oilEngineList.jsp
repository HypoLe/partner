...
<input type="button" id="showSearch" value="快速查询" class="btn" />
	<form action="${app}/outlay/OilEngineManagement.do?method=search" method="post" id="searchForm">
		<table id="sheet" class="formTable">
			<tr>
				...
				<td>
				<input type="text" id="partnerName_CN" name="partnerName_CN" value="<eoms:id2nameDB id='${partnerName2search}' beanId='tawSystemDeptDao'/>" alt="allowBlank:false"/>
...
<form id="mainForm" action="/OilEngineManagement.do?method=list" method="post"> 
<logic:notEmpty name="oilEngineList">
<c:set var="myCheckboxAllBtn" scope="page">
	<input type="checkbox" id="myCheckbox" />
</c:set>
<display:table name="oilEngineList" class="table" id="oilEngine" export="true" sort="list" pagesize="15" 
		partialList="true" size="${size}" requestURI="OilEngineManagement.do">  
<display:column media="html" title="${myCheckboxAllBtn}">
	<input type="checkbox" class="checkAble" value="${oilEngine.id}" id="${oilEngine.id}" />
</display:column>
<display:column title="代维公司" headerClass="sortable" sortable="true">
${oilEngine.monitorCompany}
</display:column>

<display:column property="beginTime" title="油机开始使用时间" headerClass="sortable" sortable="true"/>
<display:column property="endTime" title="油机使用结束时间" headerClass="sortable" sortable="true"/>
<display:column property="recordTime" title="油机费用填写时间" headerClass="sortable" sortable="true"/>
<display:column property="station" title="维护站名称" headerClass="sortable" sortable="true"/>
<display:column property="flag" title="是否应急发电" decorator="com.boco.eoms.partner.oilmachine.util.Decorate_OilEngine" headerClass="sortable" sortable="true"/>
<display:column sortable="true" headerClass="sortable" title="查看"
	paramProperty="id"  url="/partner/oilmachine/OilEngineManagement.do?method=detail"
	paramId="id" media="html">
	<img src="${app}/images/icons/search.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="编辑"
	paramProperty="id" url="/partner/oilmachine/OilEngineManagement.do?method=edit"
	paramId="id" media="html">
		<img src="${app}/nop3/images/edit.gif">
</display:column>
<display:column sortable="true" headerClass="sortable" title="删除"
	paramProperty="id" url="/partner/oilmachine/OilEngineManagement.do?method=remove"
	paramId="id" media="html">
