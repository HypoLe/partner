...
<table class="formTable">
	...
	<display:table name="resultList" cellspacing="0" cellpadding="0"
		id="resultList"  class="table resultList"
		export="false" 
		requestURI="${app}/partner/oilmachine/oilEngineStatistics.do?method=statisticsTimes"
...
<center>
	...
	<c:if test="${not empty generateTime}">
	<display:column property="generateTime" sortable="true"
			headerClass="sortable" title="动环油机告警时长"  paramId="id" paramProperty="id"/>
	</c:if>		
	<c:if test="${not empty alarmTime}">
	<display:column property="alarmTime" sortable="true"
			headerClass="sortable" title="代维油机发电时长"  paramId="id" paramProperty="id"/>
	</c:if>		
...
<script>
  var myjs=jQuery.noConflict();
      ...
      var monitorCompanyid = "${monitorCompanyid}"; //代维公司id 
            if(monitorCompanyid != ''){
            alert('monitorCompanyid:'+monitorCompanyid);
