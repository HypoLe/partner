...
<%
String userJosn = (String) request.getAttribute("userJosn");
%>
<!--根据给定的实例名生成标题 -->
<title><fmt:message key="basemetermgrDetail.title"/></title>
<!-- <content tag="heading"><fmt:message key="basemetermgrDetail.heading"/></content> -->
...
<script type="text/javascript">
	function onSubmit(){
		...
		if(checknumber(document.forms[0].faxVal.value)) 
			{ 
				alert('${eoms:a2u(" 必须填写数字!")}'); 
				document.all.faxVal.focus();
				return false; 
			}else if(checknumber(document.forms[0].desktopComputerVal.value)) 
			{ 
				alert('${eoms:a2u(" 必须填写数字!")}'); 
...
<ul>
  <input name="newSave"  type="hidden" id="newSave" value="edit"/>
    <!--表示对所有的域有�? -->
	 ...
	 <table class="formTable middle" cellspacing="3" >	       
	   ...
	   <tr class="tr_show">
		    ...
		    <td>
			        <html:textarea property="mobileGps" styleId="mobileGps" styleClass="text medium"/>
		    </td>
	</tr>
	   <tr class="tr_show">
	 	   <td>     
