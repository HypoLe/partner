...
<%
	...
	String receiveTime_hour = String.valueOf(request.getAttribute("receiveTime_hour"));
	String receiveTime_min = String.valueOf(request.getAttribute("receiveTime_min"));
%>
          ...
          <td class="label" nowrap="nowrap"  width="15%">
    	       ...
    	       <td width="500" colspan="5">
        			<select name="receiveTime_hour">
        				...
        				{
        					String select = "";
         					if(hours[i].equalsIgnoreCase(receiveTime_hour))
        					{
        						select = "Selected";
        						
        					}
        				%>
        					<option value=<%=hours[i]%> <%=select %>><%=hours[i]%></option>
        				<%
        				}
        				%>	
		    	   	</select>时
		    	   	<select name="receiveTime_min">
        				...
        				{
        					String select = "";
         					if(mins[j].equalsIgnoreCase(receiveTime_min))
        					{
        						select = "Selected";
        						
        					}
        				%>
        					<option value=<%=mins[j]%> <%=select %>><%=mins[j]%></option>
