...
<td>
	<display:table name="taskAuditList" cellspacing="0" cellpadding="0" id="taskAuditList" pagesize="${pageSize}" class="table auditInfoList" export="false"
		...
		<display:column sortable="true" headerClass="sortable" title="模板名称">
		 	<a href="${app}/eva/evaTaskAudit.do?method=auditDetail&id=${taskAuditList.id}&taskId=${taskAuditList.taskId}&templateId=${taskAuditList.templateId}&timeStr=${taskAuditList.auditTime}&partner=${taskAuditList.partner}" target="_blank" >${taskAuditList.templateName}</a>
		</display:column>
		...
		<display:column sortable="true" headerClass="sortable" titleKey="eva.auditInfo.status">
		 <eoms:dict key="dict-eva" dictId="auditFlag" itemId="${taskAuditList.auditResult}" beanId="id2nameXML" />
			...
			<a href="javascript: var url=eoms.appPath+'/eva/evaTaskAudit.do?method=auditDetail&id=${taskAuditList.id}&taskId=${taskAuditList.taskId}&templateId=${taskAuditList.templateId}&timeStr=${taskAuditList.auditTime}&partner=${taskAuditList.partner};
