...
<%@page import="com.boco.eoms.partner.eva.mgr.*"%>
<%@page import="com.boco.eoms.partner.eva.util.*"%>
<%@page import="com.boco.eoms.partner.eva.model.*"%>
<style type="text/css">
body {
...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/evaKpis.do?method=saveKpi" styleId="pnrEvaKpiForm">
			<table align="center">
				<tr>
					<td>
						指标名称
				...
				<tr>
					...
					<td colspan="3">
						<input type="text" id="weight" name="weight" class="text medium"
							value="${pnrEvaKpiForm.weight}" />
						<font color="red">*可分配分数范围：${minWt}~${maxWt}</font>
					</td>
				</tr>
				<!--  tr>
					<td>
					...
					<td colspan="3">
						<input type="textarea" id="defaultValue" name="defaultValue"
							class="text medium"
							value="${pnrEvaKpiForm.defaultValue}" />
					</td>
				</tr-->
				<input type="hidden" id="defaultValue" name="defaultValue" value="0" />
				<tr>
					<td>
						算法描述
					</td>
				...
				<tr>
					...
					<td colspan="3">
						<input type="textarea" id="remark" name="remark" class="textarea"
							style="width: 88%" value="${pnrEvaKpiForm.remark}" />
					</td>
				</tr>
				<tr>
					<td colspan="4">
			...
			<input type="hidden" id="id" name="id" value="${pnrEvaKpiForm.id}" />
		...
		<caption>
			<div class="header center">
				<%
					...
					IPnrEvaTreeMgr evaTreeMgr = (IPnrEvaTreeMgr) ApplicationContextHolder
					...
					//if (PnrEvaConstants.NODETYPE_TEMPLATETYPE.equals(nodeType)) {
						tempname = evaTreeMgr.id2Name(parentNodeId);
					//}else{
					//	PnrEvaTree evaTree = evaTreeMgr.getTreeNodeByNodeId(parentNodeId);
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a
							href="${app}/partner/eva/evaKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}&areaId=${areaId}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url=eoms.appPath+'/partner/eva/evaKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
