					...
					<c:when test="${item.colDictType == 0}">
						<c:choose>
							<c:when test="${item.colType == 1}">								
								<%-- 不绑定_普通文本 --%>								
								${CtContentsMap[item.colName]}
							</c:when>
							<c:when test="${item.colType == 2}">
								<%-- 不绑定_大文本域 --%>
								<textarea type="_moz" class="textarea max" cols="50" readonly="readonly">${CtContentsMap[item.colName]}</textarea>
							</c:when>
							...
							<c:when test="${item.colType == 7}">
								<%-- 不绑定_附件上传 --%>
								<eoms:attachment name="CtContents" property="${item.colName}" scope="request" idField="${item.colName}" appCode="contract" viewFlag="Y"/>																
							</c:when>
						</c:choose>
					</c:when>
					<c:when test="${item.colDictType == 1}">
						<c:choose>
							<c:when test="${item.colType == 5}">
								<%-- 普通字典_单选字典 --%>
								<eoms:id2nameDB id="${CtContentsMap[item.colName]}" beanId="ctSystemDictTypeDaoHibernate" />
							</c:when>
