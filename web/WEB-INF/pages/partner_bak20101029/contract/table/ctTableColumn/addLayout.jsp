...
<%@ page import="com.boco.eoms.partner.contract.table.model.CtTableColumn"%>

<% 
List tmp = (List)request.getAttribute("CtTableColumnList");
...
<form id="adjustForm" method="post" action="${app}/partner/contract/ctTableColumns.do?method=saveLayout"> 
<table class="formTable" id="adjustTable" >
	...
	<c:if test="${item.mark == 3}"> <%-- 有右半行，填充左半行为空行--%>
	  <td class="label"><%=steerer_blank%></td>
	...
	<c:if test="${item.isOpen == 1}">
	  <td class="label">
	  	${item.colChname}<%=steerer%><c:if test="${item.isNullable == 0}">&nbsp;<font color='red'>*</font></c:if>
	  </td>
	  <td class="content" <c:if test="${item.mark == 1}">colspan="3"</c:if> role="${item.mark}">
		    ...
		    <c:when test="${item.colDictType == 0}">
		        ...
		        <c:when test="${item.colType == 1}">
		         <%-- 不绑定_普通文本 --%>
		        <input type="text" name="TableInfo/${item.id}" id="${item.id}" value="${item.colDefault}" maxLength="${item.colSize}" class="text medium" 
		        ...
		        <%-- 不绑定_数字类型 --%>
		        <input type="text" name="TableInfo/${item.id}" id="${item.id}" value="${item.colDefault}" class="text medium" 
		        ...
		        <%-- 不绑定_日期时间 --%>
		        <input type="text" name="TableInfo/${item.id}" id="${item.id}" value="${item.colDefault}" size="20" onclick="popUpCalendar(this, this,null,null,null,true,-1);" readonly="true" class="text" 
		        ...
		        <%-- 不绑定_附件上传 --%>
		        <eoms:attachment name="" property="" scope="request" idField="TableInfo/${item.id}" appCode="contract"  startsWith="0"/>
		        </c:when>
		        </c:choose>
		    </c:when>
		    ...
		    <c:when test="${item.colDictType == 1}">
		        ...
		        <%-- 普通字典_单选字段 --%>
		        <eoms:comboBox name="TableInfo/${item.id}" id="${item.id}" initDicId="${item.colDictId}" defaultValue="" 
