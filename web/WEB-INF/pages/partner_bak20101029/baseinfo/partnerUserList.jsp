...
<c:if test="${in==null}">
<c:set var="buttons">
   <a href='partnerUsers.do?method=add&nodeId=${treeNodeId }'><fmt:message key="button.add"/></a>	   
...
<html:form action="partnerUsers.do?method=search&in=${in }&nodeId=${nodeId}" method="post"  styleId="partnerUserForm">
<c:if test="${in!=null}">
...
<table class="formTable">
	...
	<tr>
		...
		<c:otherwise>
					<html:select property="areaId" styleId="areaId" disabled="true" size="1" onchange="changeDep();">
					 <%
					for(int i=0;i<listId.size();i++){
	...
	<tr>	
		...
		<c:when test="${in=='factory'||in==null}"> 
				<html:select property="deptId" disabled="true" styleId="deptId" size="1">
				</html:select>	
		</c:when>
		</c:choose>	
		</td>
...
<content tag="heading">
	...
	<display:column property="name" sortable="true"      
			headerClass="sortable" titleKey="partnerUser.name" href="${app}/partner/baseinfo/resumes.do?method=newExpert&nodeId=${nodeId}" paramId="id" paramProperty="id"/>
