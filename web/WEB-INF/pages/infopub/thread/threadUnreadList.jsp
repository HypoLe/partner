...
<html:form action="/thread.do" method="post" styleId="threadForm">
	...
	<html:hidden property="forumsId" />
		...
		<display:table name="threadList" cellspacing="0" cellpadding="0"
			...
			export="true"
			requestURI="${app}/workbench/infopub/thread.do?method=listUnread&forumsId=${forumsId }"
			sort="external" defaultsort="1" partialList="true" size="resultSize"
...
<display:setProperty name="export.rtf" value="false"></display:setProperty>
			...
			<display:column property="title" sortable="true" sortName="title"
				headerClass="sortable" titleKey="infopub.threadForm.title" paramId="id"
				paramProperty="id"
				href="${app}/workbench/infopub/thread.do?method=detail" />

			<display:column property="threadCount" sortable="true" sortName="threadCount"
				headerClass="sortable" titleKey="infopub.threadForm.num"
				href="${app}/workbench/infopub/threadHistory.do?method=search"
				paramId="threadId" paramProperty="id" />

			<display:column property="createrName" sortable="true" sortName="createrName"
				headerClass="sortable" titleKey="infopub.threadForm.createrId" />

			<display:column property="createTime" sortable="true" sortName="createTime"
				headerClass="sortable" titleKey="infopub.threadForm.createTime"
				format="{0,date,yyyy-MM-dd HH:mm:ss}" />

			<display:column property="editTime" sortable="true" sortName="editTime"
				headerClass="sortable" titleKey="infopub.threadForm.editTime"
				format="{0,date,yyyy-MM-dd HH:mm:ss}" />
			...
			<display:column sortable="true" sortName="threadTypeId" headerClass="sortable"
				...
				<eoms:dict key="dict-workbench-infopub" dictId="threadType"
					itemId="${threadList.threadTypeId}" beanId="id2nameXML" />
			</display:column>
	...
	<priv:region
		...
		<html:submit styleClass="button" property="method.delChoose"
			onclick="return ConfirmDel()">
			<fmt:message key="button.delete" />
		</html:submit>

		<html:submit styleClass="button" property="method.sort"
			onclick="return sort()">
			<bean:message key="threadList.sort" />
		</html:submit>

		<input type="text" id="forumsName" name="forumsName" class="text"
			readonly="readonly"
