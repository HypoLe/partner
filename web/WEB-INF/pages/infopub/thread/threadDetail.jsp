...
<%@ page import="com.boco.eoms.util.BulletinMgrLocator"%>
...
%>
<%
	String forumsId=StaticMethod.nullObject2String(request.getAttribute("forumsId"));
 %>
...
<b> </content> <script type="text/javascript">
	...
	try{
		if(flag==0){		
    		...
    		//	return false; 
    		//}
    		//alert(document.forms[0].reader.value);
...
</script> <html:form action="/thread.do?method=rotationRead" method="post"
		styleId="threadForm">
		<table class="formTable" width="98%">
			...
			<tr>
				...
				<td class="content" >
					<eoms:id2nameDB id="${threadForm.createrId }"
			...
			<tr>
				...
				<td class="content">
					<eoms:dict key="dict-workbench-infopub" dictId="threadType"
						itemId="${threadForm.threadTypeId }" beanId="id2nameXML" />
				</td>
				...
				<td class="content">
					<eoms:dict key="dict-workbench-infopub" dictId="reply"
						itemId="${threadForm.reply }" beanId="id2nameXML" />
				</td>
			</tr>
			<tr>
			<td class="label">
			...
			<tr>
				<%	
				String defaultForumsId = BulletinMgrLocator.getAttributes()
						.getBulletinForumsId();
				if(forumsId.equals(defaultForumsId)){
				
			 %>
			 	<td class="label">
			 		<eoms:label styleClass="desc" key="threadForm.companyName"/>
			 	</td>
			 	<td class="content">
			 		<eoms:dict key="dict-workbench-infopub" dictId="serSupplier"
			 			itemId="${threadForm.serSupplier}" beanId="id2nameXML"/>
			 	</td>
			 <%} %>
			</tr>
			...
			<%
				if (!status.equals("6") && !status.equals("7")
						&& !status.equals("4") && !status.equals("1")) {
			%>
			<tr>
				<td class="label">
			...
			<tr>
				...
				<td class="content" colspan="3">
					<input type="text" readonly id="showRead" name="showRead"
						display="none" />
					<input type="button" id="clkRead" name="clkRead"
						value="<bean:message key='threadForm.tree.rotationRead'/>"
						display="none" />
			...
			<c:if test="${threadForm.reply=='1' }">
				<tr>
					...
					<td class="content">
						<eoms:dict key="dict-workbench-infopub" dictId="replyresult"
							selectId="replyresult" beanId="selectXML" alt="allowBlank:false"
							defaultId="${threadForm.replyresult}" />
					</td>
				</tr>
			</c:if>
			<%
			...
			<tr>
				...
				<td class="content" colspan="3">
					<html:text property="comments" styleId="comments"
						styleClass="textarea medium" style="width:98%"/>
					<p />
						<input type="button"
							value="<bean:message key="threadHistoryList.savenote" />"
							onclick="javascript:onSubmit(1);" class="button" />
						<input type="button"
							value="<bean:message key='threadForm.button.rotationRead'/>"
							onClick="javascript:onSubmit(0);" class="button" />
						<eoms:attachment idList="" idField="accessories" appCode="9"
							scope="request" name="threadForm" property="replyAccessories" />
				</td>
			</tr>
			<%
				}
			%>
			<tr>
				<td class="content"  align="center" colspan="4">
					<c:choose>
						<c:when test="${threadForm.createrId==sessionform.userid}">
							<input type="button" value="<fmt:message key='button.edit'/>"
								onclick="javascript:window.location='<html:rewrite page='/thread.do?method=edit&id=${threadForm.id}'/>'"
								class="button" />
						</c:when>
						<c:otherwise>
							<priv:region
								url="<%=InfopubConstants.WORKBENCH_INFOPUB_PERMISSION_THREAD_MGR%>">
								<input type="button" value="<fmt:message key='button.edit'/>"
									onclick="javascript:window.location='<html:rewrite page='/thread.do?method=edit&id=${threadForm.id}'/>'"
									class="button" />
							</priv:region>
		...
		<div id="info-page">
			<div id="count-info" class="tabContent">
				<display:table name="threadHistoryCountList" cellspacing="0"
					cellpadding="0" id="threadHistoryCountList"
					pagesize="${pageSizeCount}" class="table threadHistoryCountList"
					export="false"
					requestURI="${app}/workbench/infopub/threadHistory.do?method=search&threadId=${id}"
					sort="list" partialList="true" size="resultSizeCount"
					decorator="com.boco.eoms.workbench.infopub.displaytag.support.ThreadCountHistoryListDisplaytagDecorator">
					<display:column property="userId" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.user" />
					<display:column property="count" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.num"
						href="${app}/workbench/infopub/threadHistory.do?method=search&threadId=${threadForm.id}&userId="
			...
			<div id="comments-info" class="tabContent">
				<display:table name="threadHistoryCommentsList" cellspacing="0"
					cellpadding="0" id="threadHistoryCommentsList"
					pagesize="${pageSizeComments }"
					class="table threadHistoryCommentsList" export="false"
					requestURI="${app}/workbench/infopub/threadHistory.do?method=search&threadId=${threadForm.id}"
					sort="list" partialList="true" size="resultSizeComments"
					decorator="com.boco.eoms.workbench.infopub.displaytag.support.ThreadBrowseHistoryListDisplaytagDecorator">
					<display:column property="userId" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.user" />
					<display:column property="ip" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.ip" />
					<display:column property="userDept" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.dept" />
					<display:column property="replyAccessories" sortable="false" headerClass="sortable"
						titleKey="infopub.threadForm.replyAccessories">
					</display:column>
					<display:column property="readTime" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.readTime"
						format="{0,date,yyyy-MM-dd HH:mm:ss}" />
					<display:column property="comments" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.comments" />
					<display:column property="replyresult" sortable="false"
						headerClass="sortable" titleKey="infopub.threadForm.replyresult" />
					<display:setProperty name="paging.banner.item_name"
						value="threadHistory" />
					<display:setProperty name="paging.banner.items_name"
						value="threadHistorys" />
	...
	</html:form> <%@ include file="/common/footer_eoms.jsp"%>