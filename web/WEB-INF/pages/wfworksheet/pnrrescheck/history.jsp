...
<logic:present name="HISTORY" scope="request">  
      ...
      <logic:iterate id="baselink" name="HISTORY" type="com.boco.eoms.partner.sheet.pnrrescheck.model.PnrResCheckLink" >     
       <%         
        jNum += 1;       
       %>
      <div class="history-item">&nbsp;   
	      ...
	      <div class="history-item-title">
			 ...
			 <bean:write name="baselink" property="operateTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="page"/>&nbsp; 
	  	     ...
	  	     <eoms:dict key="dict-sheet-pnrrescheck" dictId="activeTemplateId" itemId="${baselink.activeTemplateId}" beanId="id2descriptionXML" />
	  	     <bean:message bundle="pnrrescheck" key="pnrrescheck.operateType"/>:
	  	     <eoms:dict key="dict-sheet-pnrrescheck" dictId="mainOperateType" itemId="${baselink.operateType}" beanId="id2descriptionXML" />
	 ...
	 <!-- 流程中的历史页面 -->
		  		...
		  		<%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("CheckTask")) {%>
		  				...
		  				<%if(baselink.getOperateType() != null && (baselink.getOperateType().intValue() == 93 || baselink.getOperateType().intValue() == 11)){ %>
		 					...
		 					<td class="label">
								<!-- 审批是否通过 -->
								<bean:message bundle="pnrrescheck" key="instrumentStorageLink.linkIfCheckPass"/>
							</td>
     ...
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 93||baselink.getOperateType().intValue() == 94||baselink.getOperateType().intValue() == 97)){%> 
	  ...
	  <%} %>
	 <!-- 受理、完成确认 -->
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 99 || baselink.getOperateType().intValue() == 96)){%> 
			<tr>
     ...
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 54)){%> 
	  ...
	  <!-- 处理回复不通过 或 处理回复通过 -->
	 <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 6 || baselink.getOperateType().intValue() == 10||baselink.getOperateType().intValue() == 30)){%> 
               <tr>
  				  <td class="label">
  				     <bean:message bundle="sheet" key="linkForm.remark"/>
  				  </td>
...
<script type="text/javascript">
 ...
 function copy(id){
    ...
    Ext.Ajax.request({
		method:"get",
		url: "pnrrescheck.do?method=getJsonLink&id="+id+"&beanName=PnrResCheck",
		success: function(x){
