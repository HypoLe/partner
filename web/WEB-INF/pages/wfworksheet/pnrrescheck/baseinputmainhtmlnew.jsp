<%@ page language="java" errorPage="/error.jsp" pageEncoding="UTF-8" contentType="text/html;charset=utf-8" %>
		...
		<input type="hidden" name="sendRoleId" id="sendRoleId" value="${sheetMain.sendDeptId}"/>
		...
		<input type="hidden" name="taskAcceptLimit" id="taskAcceptLimit" value="${eoms:date2String(sheetMain.sheetAcceptLimit)}"/>
		<input type="hidden" name="taskCompleteLimit" id="taskCompleteLimit" value="${eoms:date2String(sheetMain.sheetCompleteLimit)}"/>
		...
		<input type="hidden" name="linkBeanId" id="linkBeanId" value="iInstrumentStorageLinkManager"/>
...
<table class="formTable">
	<tr>
	  ...
	  <c:if test="${empty sheetMain.title}">
	    <input type="text" name="title" class="text max" id="title" 
			value="${sheetMain.title}" alt="allowBlank:false,maxLength:200,vtext:'请输入工单主题，最大长度为100个汉字！'"/>
	  </c:if>
	  <c:if test="${!empty sheetMain.title}">
	...
	<tr>
	   ...
	   <td class="label"></td>
	   <td >
	    <input type="hidden"  name="sendTime" id="sendTime" value="${eoms:date2String(sheetMain.sendTime)}"  /> 
	   </td>
	 </tr>
