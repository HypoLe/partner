...
<c:url var="urlDeal" value="/sheet/pnrfaultdeal/pnrfaultdeal.do">
...
Ext.onReady(function(){
	...
	var taskName = "${taskName}";
   	if (dealTemplateId != null && dealTemplateId != "" && taskName != "draft") {	
   		strUrl = '${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=showTemplateDealInputSheetPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&taskStatus=${taskStatus}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&dealTemplateId='+dealTemplateId;
	}
	var config = {
		url:strUrl,
		callback : initPage
    ...
    var form = document.forms[0];
   	...
   	if(v.check()) {
	    ...
	    Ext.Ajax.request({
		   		form: ajaxForm,
				method:"post",
				url: "${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=performSaveInfo",
				success: function(){
		        	alert("保存成功！");
	 		}
	    });
...
<div id="sheetform">
  <html:form action="/pnrfaultdeal.do?method=performDeal" styleId="theform">
   ...
   <c:choose> 
    <c:when test="${operateType=='61'}">
		<div class="form-btns" id="btns">
		  <input type="submit" class="submit" onclick="this.form.action='${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=performClaimTask'" 
    ...
    <c:when test="${taskName=='DraftTask'}">
		...
		<html:submit styleClass="btn" property="method.save"  styleId="submitone">
		 <bean:message bundle="sheet" key="button.send"/>
	      ...
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=performSaveInfoAndExit&draft=1'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=showDraftList'" value="<bean:message bundle='sheet' key='button.back'/>" />
		</div>	 
   	</c:when>
   	<c:when test="${taskName=='RejectTask'}">    
		<div class="form-btns" id="btns">
		<html:submit styleClass="btn" property="method.save"  styleId="submitone" onclick="javascript:changeType3();">
		 <bean:message bundle="sheet" key="button.send"/>
          ...
          <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=performSaveInfoAndExit'" value="<bean:message bundle='sheet' key='button.saveback'/>" />
	      <input type="submit" class="submit" onclick="v.passing=true;this.form.action='${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=showListsendundo'" value="<bean:message bundle='sheet' key='button.back'/>" />
	    </div>	 
   	</c:when>
