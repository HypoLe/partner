...
<%@page import="com.boco.eoms.partner.sheet.faultdeal.model.PnrFaultDealTask" %>
...
PnrFaultDealTask task = new PnrFaultDealTask();
String operaterType = "";
//String ifsub = "";
//String ifwaitfor="";
if(request.getAttribute("task")!=null){
	 task = (PnrFaultDealTask)request.getAttribute("task");
...
Ext.onReady(function() {
	var tabConfig = {
		items : [{
			...
			url : 'pnrfaultdeal.do?method=showSheetDealList&sheetKey=${sheetMain.id}&taskName=${taskName}&ifWaitForSubTask=${task.ifWaitForSubTask}'
		}, {
			text : '<bean:message bundle="sheet" key="sheet.flowchar"/>',
			url : 'pnrfaultdeal.do?method=showWorkFlow&linkServiceName=iPnrFaultDealLinkManager&dictSheetName=dict-sheet-pnrfaultdeal&description=mainOperateType&sheetKey=${sheetMain.id}',
			isIframe : true
		  }
		,{
			text : '<bean:message bundle="sheet" key="sheet.filesView"/>',
			url : 'pnrfaultdeal.do?method=showSheetAccessoriesList&id=${sheetMain.id}',
			isIframe : true
		}
		<c:if test="${sheetMain.status ==1 && sheetMeteringMenuRight }">
		 ,{
		 	text : '计次明细',
			url : '${app}/netresource/pnrResMeteringAction.do?method=pnrResMeteringDetail&eventId=${sheetMain.id}&sheet=001',
			isIframe : true
		 }
		 </c:if>
		]
    ...
    <%if(taskStatus.equals("2") || taskStatus.equals("3") || taskStatus.equals("8")){%>
    	<%if(taskName.equals("cc")){ %>
			var url2="pnrfaultdeal.do?method=newShowCCPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=-10&taskStatus=${taskStatus}";
			eoms.util.appendPage("sheet-deal-content",url2);	
	<%}%>
	<%if(taskName.equals("reply")){ %>
		var url2="pnrfaultdeal.do?method=showRemarkPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}";
		eoms.util.appendPage("sheet-deal-content",url2);	
	<%}%>
...
function forceOperation(obj){
	if(obj == 1){
		var url2="pnrfaultdeal.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=forceHold";
		eoms.util.appendPage("sheet-deal-content",url2);	
	}else if(obj == 2){
	    
		var url2="pnrfaultdeal.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=nullity";
		eoms.util.appendPage("sheet-deal-content",url2);	
	}else{
	   
	    var url2="pnrfaultdeal.do?method=showForceHoldPage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&taskName=${taskName}&correlationKey=${sheetMain.correlationKey}&operateType=forceNullity";
		eoms.util.appendPage("sheet-deal-content",url2);	
    }
   }
    function eventOperation(obj){
	if(obj == 1){
	     var url2="pnrfaultdeal.do?method=showPhaseAdvicePage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&taskName=advice&operateFlag=driveForward&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}";
...
<div id="sheet-detail-page">
  ...
  <div id="sheetinfo">
	...
	<%@ include file="/WEB-INF/pages/wfworksheet/pnrfaultdeal/basedetailnew.jsp"%>
	<br/>
	<table id="sheet" class="formTable" > 
		...
		<tr>
			...
			<td class="content"> 
				<eoms:id2nameDB id="${sheetMain.mainIsGroupBusi}" beanId="ItawSystemDictTypeDao" />
			</td>
		...
		<tr>
			...
			<td class="content"> 
				<eoms:id2nameDB id="${sheetMain.mainNetType}" beanId="ItawSystemDictTypeDao" />
			</td>
		...
		<tr>
			...
			<td class="content"> 
				${sheetMain.mainFaultNet}
			</td>
		</tr>
		<tr>
			<td class="label">业务影响</td>
			<td class="content"> 
				${sheetMain.mainBusiEffect}
			</td>
			<td class="label">站点</td>
			<td class="content"> 
				<c:if test="${! empty sheetMain.mainResId}">
					<a href="${app}/partner/res/PnrResConfig.do?method=detial&&seldelcar=${sheetMain.mainResId}" target="pnrfaultdeal">${sheetMain.mainResName}</a>
				</c:if>
			</td>
		</tr>
			<tr>
	  ...
	  <tr>
		    ...
		    <eoms:attachment name="sheetMain" property="sheetAccessories" 
		            scope="request" idField="sheetAccessories" appCode="pnrfaultdeal" 
		             viewFlag="Y"/> 
		    </td>
	  </tr>
</table>
...
<!-- 下面是流程中的步骤URL -->
 ...
 <!-- 回复到派单人 -->
<c:url var="urlShowReplyTaskPage" value="pnrfaultdeal.do">
 ...
 <!-- 派发 -->
<c:url var="urlSendTaskPage" value="pnrfaultdeal.do">
	<c:param name="method" value="showDealPage"/>
	<c:param name="sheetKey" value="${sheetMain.id}"/>
	<c:param name="piid" value="${sheetMain.piid}"/>
	<c:param name="taskId" value="${taskId}"/>
 ...
 <!-- 待归档 -->
<c:url var="urlShowHoldTaskPage" value="pnrfaultdeal.do">
	<c:param name="method" value="showDealPage"/>
	<c:param name="sheetKey" value="${sheetMain.id}"/>
	<c:param name="piid" value="${sheetMain.piid}"/>
	<c:param name="taskId" value="${taskId}"/>
...
<!-- 下面是公共的URL -->
<!-- 退回 -->
<c:url var="urlShowRejectDealPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<c:url var="urlShowTransferkPage" value="pnrfaultdeal.do">
...
<c:url var="urlShowPhaseReplyPage" value="pnrfaultdeal.do">
...
<!--任务分派-->
<c:url var="urlShowInputSplit" value="pnrfaultdeal.do">
...
<!-- 处理回复通过 -->
<c:url var="urlShowDealReplyAcceptPage" value="pnrfaultdeal.do">
...
<!-- 处理回复不通过 -->
<c:url var="urlShowDealReplyRejectPage" value="pnrfaultdeal.do">
...
<!-- 驳回上一级 -->
<c:url var="urlShowRejectBackPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 确认受理 -->
<c:url var="urlShowAcceptDealPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 被驳回 -->
<c:url var="urlBackDealPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 草稿 -->
<c:url var="urlShowDraftPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 分派回复 -->
<c:url var="urlShowDispatchPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 会审 -->
<c:url var="urlShowInputSplitAudit" value="pnrfaultdeal.do">
  <c:param name="method" value="showInputSplit"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 会审回复 -->
<c:url var="urlShowDispatchAuditPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 受理工单 -->
<c:url var="urlShowAcceptTaskPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 处理工单 -->
<c:url var="urlShowFaultDealTaskPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 完成确认 -->
<c:url var="urlShowConfirmTaskPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 驳回到派发人 -->
<c:url var="urlShowReject2SendPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 驳回到受理人 -->
<c:url var="urlShowReject2AcceptPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 驳回到处理人 -->
<c:url var="urlShowReject2DealPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
...
<!-- 退回到任务完成确认 -->
<c:url var="urlShowReject2ConfirmPage" value="pnrfaultdeal.do">
  <c:param name="method" value="showDealPage"/>
  <c:param name="sheetKey" value="${sheetMain.id}"/>
  <c:param name="piid" value="${sheetMain.piid}"/>
  <c:param name="taskId" value="${taskId}"/>
			...
			<tr>
				 <td width="50%">  
					...
					<% }if(taskName.equals("HoldTask")){%>	
							<select id="dealSelector">
								<option value="${urlShowHoldTaskPage}"><bean:message bundle="pnrfaultdeal" key="operateType.holdTask"/> </option>
					...
					<% }else if(taskName.equals("RejectTask")){%>	
			  		...
			  		<% }else if(taskName.equals("DraftTask")){%>	
						<select id="dealSelector">
						  <option value="${urlShowDraftPage}"><bean:message bundle="pnrfaultdeal" key="operateType.DraftTask"/></option>
						</select>
					<!-- 受理工单 -->
			   		<% } else if(taskName.equals("AcceptTask")){%>	
						<select id="dealSelector">
						  <option value="${urlShowAcceptTaskPage}">受理故障</option>
						  <option value="${urlShowReject2SendPage}">驳回</option>
						</select>
			   		<% } else if(taskName.equals("FaultDealTask")){%>	
						<select id="dealSelector">
							<option value="${urlShowFaultDealTaskPage}">处理故障</option>
...
<script type="text/javascript">
	Ext.onReady(function(){
        ...
        if (dealTemplateId != null && dealTemplateId != "") {
   			url = "pnrfaultdeal.do?method=referenceTemplate&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+dealTemplateId
        }
		eoms.util.appendPage("sheet-deal-content",url);		
	});
</script>
