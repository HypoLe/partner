...
function check() {
	...
	if (dealTemplateId == "") {
		...
		if (parent == null)
			return;
		var url = "${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+dealTemplateId;
...
<fmt:bundle basename="config/ApplicationResources-sheet">
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true" headerClass="sortable" titleKey="template.templateName">
			<a href="${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=openDealTemplateInfo&dealTemplateId=${process.id}&type=${templateManage}">
