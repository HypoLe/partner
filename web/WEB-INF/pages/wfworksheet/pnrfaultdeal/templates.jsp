...
function check() {
	...
	if (templateId == "") {
		...
		if (taskName != null && taskName == "DraftTask") {
			window.opener.location.href = "${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+templateId ;
		} else {
			window.opener.location.href = "${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=referenceTemplate&templateId="+templateId ;
		}
...
<bean:define id="url" value="pnrfaultdeal.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		export="false" requestURI="pnrfaultdeal.do" 
		...
		<display:column sortable="true" headerClass="sortable" title="${templateName}">
			<a href="${app}/sheet/pnrfaultdeal/pnrfaultdeal.do?method=editTemplateInfo&templateId=${taskList.id}&type=${templateManage}">
