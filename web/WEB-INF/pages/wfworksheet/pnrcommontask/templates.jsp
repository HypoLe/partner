...
function check() {
	...
	if (templateId == "") {
		...
		if (taskName != null && taskName == "DraftTask") {
			window.opener.location.href = "${app}/sheet/pnrcommontask/pnrcommontask.do?method=showDetailPage&sheetKey=${sheetKey}&piid=${piid}&taskId=${taskId}&taskName=${taskName}&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}&operateType=${operateType}&taskStatus=${taskStatus}&dealTemplateId="+templateId ;
		} else {
			window.opener.location.href = "${app}/sheet/pnrcommontask/pnrcommontask.do?method=referenceTemplate&templateId="+templateId ;
		}
...
<bean:define id="url" value="pnrcommontask.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		id="taskList" pagesize="${pageSize}" class="table resourceAssessTable"
		export="false" requestURI="pnrcommontask.do" 
		...
		<display:column sortable="true" headerClass="sortable" title="${templateName}">
			<a href="${app}/sheet/pnrcommontask/pnrcommontask.do?method=editTemplateInfo&templateId=${taskList.id}&type=${templateManage}">
