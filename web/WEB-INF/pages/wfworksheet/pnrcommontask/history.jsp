...
<logic:present name="HISTORY" scope="request">  
      ...
      <logic:iterate id="baselink" name="HISTORY" type="com.boco.eoms.partner.sheet.commontask.model.PnrCommonTaskLink" >     
       <%         
        jNum += 1;       
      ...
      <div class="history-item">&nbsp;   
	      ...
	      <div class="history-item-title">
			 ...
			 <bean:write name="baselink" property="operateTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="page"/>&nbsp; 
	  	     ...
	  	     <eoms:dict key="dict-sheet-pnrcommontask" dictId="activeTemplateId" itemId="${baselink.activeTemplateId}" beanId="id2descriptionXML" />
	  	     <bean:message bundle="pnrcommontask" key="pnrcommontask.operateType"/>:
	  	     <eoms:dict key="dict-sheet-pnrcommontask" dictId="mainOperateType" itemId="${baselink.operateType}" beanId="id2descriptionXML" />
	 ...
	 <!-- 流程中的历史页面 -->
		  		...
		  		<%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("CheckTask")) {%>
		  				...
		  				<%if(baselink.getOperateType() != null && (baselink.getOperateType().intValue() == 93 || baselink.getOperateType().intValue() == 11)){ %>
		 					...
		 					<td class="label">
								<!-- 审批是否通过 -->
								<bean:message bundle="pnrcommontask" key="instrumentStorageLink.linkIfCheckPass"/>
							</td>
     ...
     <!-- 下面是公共的 -->
     <!-- 审核 -->
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 93||baselink.getOperateType().intValue() == 94)){%> 
			<tr>
...
<script type="text/javascript">
 ...
 function copy(id){
    ...
    Ext.Ajax.request({
		method:"get",
		url: "pnrcommontask.do?method=getJsonLink&id="+id+"&beanName=InstrumentStorage",
		success: function(x){
