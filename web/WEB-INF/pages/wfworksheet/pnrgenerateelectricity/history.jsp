...
<logic:present name="HISTORY" scope="request">  
      ...
      <logic:iterate id="baselink" name="HISTORY" type="com.boco.eoms.partner.sheet.generateelectricity.model.PnrGenerateElectricityLink" >     
       <%         
        jNum += 1;       
      ...
      <div class="history-item">&nbsp;   
	      ...
	      <div class="history-item-title">
			 ...
			 <bean:write name="baselink" property="operateTime" formatKey="date.formatDateTimeAll" bundle="sheet" scope="page"/>&nbsp; 
	  	     ...
	  	     <eoms:dict key="dict-sheet-pnrgenerateelectricity" dictId="activeTemplateId" itemId="${baselink.activeTemplateId}" beanId="id2descriptionXML" />
	  	     <bean:message bundle="pnrgenerateelectricity" key="pnrgenerateelectricity.operateType"/>:
	  	     <eoms:dict key="dict-sheet-pnrgenerateelectricity" dictId="mainOperateType" itemId="${baselink.operateType}" beanId="id2descriptionXML" />
	 ...
	 <!-- 流程中的历史页面 -->
		  		...
		  		<%if(baselink.getActiveTemplateId()!=null && baselink.getActiveTemplateId().equals("CheckTask")) {%>
		  				...
		  				<%if(baselink.getOperateType() != null && (baselink.getOperateType().intValue() == 93 || baselink.getOperateType().intValue() == 11)){ %>
		 					...
		 					<td class="label">
								<!-- 审批是否通过 -->
								<bean:message bundle="pnrgenerateelectricity" key="instrumentStorageLink.linkIfCheckPass"/>
							</td>
     ...
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 54)){%> 
	  ...
	  <%} %>
     <%if(baselink.getOperateType()!=null && (baselink.getOperateType().intValue() == 14)){%> 
			<tr>
				<td class="label">开始发电时间</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkStartGenerateTime" format="yyyy-MM-dd HH:mm:ss" scope="page"/>
			    </td>	  				                  
				<td class="label">发电结束时间</td>
				<td class="content" colspan=1>
				   <bean:write name="baselink" property="linkEndGenerateTime" format="yyyy-MM-dd HH:mm:ss" scope="page"/>
			    </td>	  				                  
			</tr>
			<tr>
				<td class="label">市电恢复时间</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkResumeTime" format="yyyy-MM-dd HH:mm:ss" scope="page"/>
			    </td>	  				                  
				<td class="label">工作电流</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkWorkingCurrent"  scope="page"/>
			    </td>	  				                  
			</tr>
			<tr>
				<td class="label">油机编号/型号</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkOilEngineSerialNumber" scope="page"/>
			    </td>	  				                  
				<td class="label">耗油(升)</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkFuelConsumption" scope="page"/>
			    </td>	  				                  
			</tr>
			<tr>
				<td class="label">零星耗材记录</td>
				<td class="content" colspan=1>
				    <bean:write name="baselink" property="linkConsumptiveMaterial" scope="page"/>
			    </td>	  				                  
				<td class="label">用时</td>
				<td class="content" colspan=1>
				   <bean:write name="baselink" property="linkGenerateTime" scope="page"/>
			    </td>	  				                  
			</tr>
	  <%} %>
...
<script type="text/javascript">
 ...
 function copy(id){
    ...
    Ext.Ajax.request({
		method:"get",
		url: "pnrgenerateelectricity.do?method=getJsonLink&id="+id+"&beanName=InstrumentStorage",
		success: function(x){
