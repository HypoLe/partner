...
<content tag="heading"><bean:message key="tawWorkbenchReportList.auditHeading"/></content>
</c:if>
<c:if test="${flag!=null&&flag=='week'}">
 <content tag="heading"><bean:message key="tawWorkbenchReportList.weekauditHeading"/></content> 
</c:if>

<c:set var="buttons">
    <input type="button" style="margin-right: 5px"
        onclick="location.href='<c:url value="/workbench/report/tawWorkbenchReports.do?method=add&flag=${flag }" />'"
...
<fmt:setBundle basename="com.boco.eoms.workbench.report.config.ApplicationResources-workbench-report"/>
<display:table name="tawWorkbenchReportList" cellspacing="0" cellpadding="0"
    ...
    export="false" requestURI="/workbench/report/tawWorkbenchReports.do?method=listAudited" sort="external" partialList="true" size="resultSize">
...
<c:if test="${flag==null||flag!='week'}">
    ...
    <display:column property="auditTime" sortable="true" headerClass="sortable"
         titleKey="tawWorkbenchReportForm.auditTime"/>  
