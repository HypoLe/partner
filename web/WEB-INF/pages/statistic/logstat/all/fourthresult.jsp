...
<script language="JavaScript">
	  function GoBack(){
	   ...
	   colorRows('list-table');
		...
		String aurl="/statistic/logstat/stat.do?method=performStatistic&excelConfigURL=logstat_all&findListForward=all_statlist&reportIndex=3&beginTime="+beginTime+"&endTime="+endTime;
		String burl="/statistic/logstat/stat.do?method=performStatistic&excelConfigURL=logstat_all&findListForward=all_statlist&reportIndex=4&beginTime="+beginTime+"&endTime="+endTime;
		           %>
  ...
  <tr class="tr_show" >    
	...
	<tr class="tr_show">
	  ...
	  <td nowrap align="center">
	   ...
	   <td nowrap align="center">
	  	  <a href="#" onclick="javascript:window.open('${app}<%=aurl%>&revdeptid=<%=s1%>&operid=<%=s2%>');">
		        <bean:write name="STATLIST" property="s22" scope="page"/>
		    </a>
	  </td> 	 		 	
	  <td nowrap align="center">
	      <a href="#" onclick="javascript:window.open('${app}<%=burl%>&revdeptid=<%=s1%>&operid=<%=s2%>');">
	 	     	  <bean:write name="STATLIST" property="f1" scope="page"/>
	 	   </a>
		</td>
		<td nowrap align="center">
				<% if(!"0".equals(f2)){ %>
		        <a href='<%=dataUrl%>&s1=<%=s1%>&s2=<%=s2%>&fieldId=f2' target="_blank">
