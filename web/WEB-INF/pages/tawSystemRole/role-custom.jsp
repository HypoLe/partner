...
Ext.onReady(function(){
	...
	xbox({
		btnId:'depttree',dlgId:'dlg-dept',
		treeDataUrl:deptTreeAction,treeRootId:'-1',treeRootText:'${eoms:a2u("部门")}',treeChkMode:'',treeChkType:'dept',
	...
	xbox({
		...
		callback:function(json,ids){
			Ext.lib.Ajax.request(
	    		...
	    		{success: function(){userViewer.load(gridAction_getUsers+Ext.get('subRoleId').dom.value);},
	    		 failure: function(){}
	    		},
	    		"id="+Ext.get('subRoleId').dom.value
				+"&userId="+ids
	    	);
		}
	});
...
function pregrid(){
	...
	if(dupeRecords.length>0){
		msg(dupeRecords.length+'${eoms:a2u("条重复的记录已被覆盖:")}'+dupeRecords.toString());
	}
...
function crtSubRoles(){
	if(subRolesPrestore.data.items.length==0){
		msg("${eoms:a2u('请先生成你要创建的子角色预览，再提交。')}");
		return;
	}
	else{
	...
	Ext.lib.Ajax.request(
		...
		{success: function(){
				unmask();
				msg("${eoms:a2u('子角色提交成功')}");
				openPanel('gridPanel',selectNode.text+'${eoms:a2u("的子角色列表")}');
			},
		 failure: function(){unmask();msg("${eoms:a2u('提交失败')}")}
		},
		data
	);
}
	...
	Ext.MessageBox.confirm('${eoms:a2u("确认:")}','${eoms:a2u("您确定删除这个项目吗？")}',function(btn){
		if(btn=="yes"){
			...
			Ext.lib.Ajax.request(
				...
				{success: function(){gridds.reload();msg("${eoms:a2u('删除子角色成功。')}");},
...
function resetDistincter(){
	...
	Options.add('selDistincter',"${eoms:a2u('一级专业')}","t1");
	Options.add('selDistincter',"${eoms:a2u('一级专业和二级专业')}","t2 t3");
	Options.add('selDistincter',"${eoms:a2u('级别')}","t4");
...
<div id="headerPanel"><img src="${app}/styles/default/images/header-role.gif"></div>
<div id="rolePanel">
	<div id="rolePanel-form"></div>
	<div id="rolePanel-users">
		<div style="margin:5px;">${eoms:a2u("人员列表:")}<div id="role-user-list" style="padding:10px;width:90%"></div></div>
	  	<input type="button" value="${eoms:a2u('更改人员列表')}" id="role-userTreeBtn" class="btn">
	</div>
</div>
<div id="main-ct" style="width:800px;height:600px;"></div>
...
<div id="gridPanel" style="padding:10px;">
	<div id="subRoleList" style="width:90%;border:1px solid #6593cf"></div>
	
	<div id="gridBtns" style="width:100%;padding:10px 0;">
		<input type="button" value="${eoms:a2u('删除选中的子角色')}" id="delSubRoleBtn" 
			class="btn" onclick="javascript:delSubRole();">
	</div>
	
	<div id="subRoleUsers" style="margin:10px 0;padding:10px;background-color:#f1f1f1;border:1px solid #dadada">
	  <input type="hidden" id="subRoleId" name="subRoleId">
	  <div style="margin:5px;">
		<span id="user-subrolename" class="viewlistitem-role"></span>
	  </div>
	  
	  <div style="margin:5px;">${eoms:a2u("人员列表:")}<div id="user-list" style="padding:10px;width:90%"></div></div>
	  <input type="button" value="${eoms:a2u('更改人员列表')}" id="userTreeBtn" class="btn">
	  
	</div>
</div>

<div id="crtSubRolesPanel" style="padding:10px;overflow:auto">
	...
	<div class="preHelp"><div class="preHeader">${eoms:a2u('1. 添加区分度选择器:')}</div>
	  <select class="select-class" id="selDistincter">
		<option value="">${eoms:a2u('请选择')}</option>
		<option value="t1">${eoms:a2u('一级专业')}</option>
		<option value="t2 t3">${eoms:a2u('一级专业和二级专业')}</option>
		<option value="t4">${eoms:a2u('级别')}</option>
	  </select>
	  <input id="addDistincter" type="button" value="${eoms:a2u('添加')}" onclick="javascript:doselect();"/>
	  <input type="button" value="${eoms:a2u('重置')}" onclick="javascript:resetDistincter();"/>
	  <br/>
	  <input type="checkbox" value="" onclick="javascript:defaultDistincter(this)"/>${eoms:a2u('使用系统默认的区分度')}
	</div>
	<table width="100%" cellspacing="3" border="1">
		<tr>
			<td class="preHelp">
			<div class="preHeader">${eoms:a2u('2. 选择区分度:')}</div>
			<div><label>${eoms:a2u('区分度 1')}</label><div id="distinction0">
				<label>${eoms:a2u('选择部门')}:</label>
				<table width="100%" cellspacing="3" border="1">
				<tr>
					<td width="180">
						<textarea class="ta" readonly="readonly" style="height: 60px; width: 180px;" name="deptname" id="deptname"></textarea>					
						<input type="hidden" name="deptid" id="deptid"/>
					</td>
					<td valign="top" align="left">
						<input type="button" name="depttree" id="depttree" value="${eoms:a2u('部门树')}" class="btn"/>
					</td>
				</tr>
				</table>			
			</div></div>
			<div style="margin-bottom:10px"><label>${eoms:a2u('区分度 2')}</label><div id="distinction1"></div></div>
			<div style="margin-bottom:10px"><label>${eoms:a2u('区分度 3')}</label><div id="distinction2"></div></div>
			<div style="margin-bottom:10px"><label>${eoms:a2u('区分度 4')}</label><div id="distinction3"></div></div>
			
			<div id="templates" style="display:none">
				
			  <div id="t1"><label>${eoms:a2u('选择一级专业')}:</label><br/>
				<eoms:comboBox name="class1m" id="{id}" initDicId="1020101" multiple="multiple" size="4" styleClass="select-class"/>
			  </div>
			
			  <div id="t2"><label>${eoms:a2u('选择一级专业')}:</label><br/>
				<eoms:comboBox name="class1" id="{id}" initDicId="1020101" sub="{sub}" styleClass="select-class"/>
			  </div>
			  
			  <div id="t3"><label>${eoms:a2u('选择二级专业')}:</label><br/>
				<select id="{id}" multiple="multiple" size="4" class="select-class"></select>
			  </div>
			  
			  <div id="t4"><label>${eoms:a2u('选择级别')}:</label><br/>
				<eoms:comboBox name="jibie" id="{id}" initDicId="10102" multiple="multiple" size="4" styleClass="select-class"/>
			  </div>
		...
		<td class="preHelp">
			...
			<input type="button" style="display:none" value="${eoms:a2u('提交')}" id="submitBtn" onclick="javascript:crtSubRoles();" class="btn"/>
			<!--
