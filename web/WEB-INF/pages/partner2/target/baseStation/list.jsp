...
<div id="searchForm"  >
<form action="${app}/partner2/target/baseStation.do?method=list" method="post">
	<fieldset><br />
	<table id="sheet" class="formTable">
		...
		<td class="label">代维公司*</td>
			...
			<input class="text popupMonitorCompanyTree" style="width:300" type="text" readonly="readonly" id="showMonitorCompany" 
				...
				<input type="hidden" name="monitorCompanyStringEqual" id="monitorCompanyStringEqual" /> 
			</td>
		</tr>
		<tr>
...
<logic:present name="${listName }" scope="request">
	<display:table name="${listName }" cellspacing="0" cellpadding="0"
		...
		export="false" requestURI="${app}/partner2/target/baseStation.do?method=list&listType=list"
		sort="list" partialList="true" size="${size }">
		<display:column sortable="true"  title="分公司">
			<eoms:id2nameDB id="${baseStationTargetList.city}"
				beanId="tawSystemAreaDao" />
		</display:column>
		<display:column sortable="true"  title="归属县公司">
			<eoms:id2nameDB id="${baseStationTargetList.country}"
				beanId="tawSystemAreaDao" />
		</display:column>
		<display:column sortable="true"  title="代维公司">
			<eoms:id2nameDB id="${baseStationTargetList.monitorCompany}"
				beanId="tawSystemAreaDao" />
		</display:column>
		<display:column sortable="false" title="月份">
			${baseStationTargetList.monthFlag}月份
		</display:column>
		<display:column sortable="false" property="myText1" title="小区完好性"></display:column>
		<display:column sortable="false" property="myText2" title="基站话务损失率"></display:column>
		<display:column sortable="false" property="myText3" title="基站故障投诉比率"></display:column>
		<display:column sortable="false" property="myText4" title="动环可用率"></display:column>
		<display:column sortable="false" property="myText5" title="基站巡检完成率"></display:column>
		<display:column sortable="false" title="删除"
				url="/partner2/target/baseStation.do?method=delete" paramProperty="id"
...
myJ(function() {
	...
	myJ("input.popupMonitorCompanyTree").bind("click",function(event) {
			...
			var saveChkFldId = "monitorCompanyStringEqual";
