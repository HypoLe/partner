...
<fmt:bundle basename="com/boco/eoms/commons/system/role/config.ApplicationResources-role">
	<content tag="heading">
	...
	<c:set var="buttons">
		<input type="button" style="margin-right: 5px" class="button"
			...
			<input type="button" class="button" name="save" value="下载导入模板" Onclick="window.location.href ='${app}/role/systemRoleImport.do?method=outPutModel'" >
	</c:set>
	</fmt:bundle>
	<!-- <c:out value="${buttons}" escapeXml="false"/> -->

	<display:table name="tawSystemRoleImportList" cellspacing="0"
		cellpadding="0" id="tawSystemRoleImportList" pagesize="${pageSize }"
		class="table tawSystemRoleImportList" export="false"
		requestURI="${app}/role/systemRoleImport.do?method=list" sort="list"
		...
		<display:column property="versionAt" sortable="true"
			headerClass="sortable" titleKey="tawSystemRoleImportForm.versionAt" format="{0,date,yyyy-MM-dd HH:mm:ss}"/>
		...
		<display:column sortable="true" headerClass="sortable"
				titleKey="tawSystemRoleImportForm.operation.import" url="/role/systemRoleImport.do?method=importrole" paramProperty="id"  paramId="id">
			<fmt:message key="tawSystemRoleImportForm.operation.import"/>
		</display:column>
		
		<display:column sortable="true" headerClass="sortable"
				titleKey="tawSystemRoleImportForm.operation.restore" url="/role/systemRoleImport.do?method=restore" paramProperty="id"  paramId="id">
			<fmt:message key="tawSystemRoleImportForm.operation.restore"/>
		</display:column>
		
		<display:column sortable="true" headerClass="sortable"
				titleKey="tawSystemRoleImportForm.operation.delete" url="/role/systemRoleImport.do?method=delete" paramProperty="id"  paramId="id">
