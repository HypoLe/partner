...
<html:form  action="/tawzjweek?method=save" method="post" styleId="tawzjWeekForm">
	...
	<!-- 如果是新提交审核或者是被上级领导驳回的。要重新审核-->
	 <%if("1".equals(week.getState()) || "5".equals(week.getState()) ||"6".equals(week.getState())){%>
		<input type="button" value="审核通过" name="B1" class="submit" onclick="sendaudit(2)">
		<input type="button" value="驳回" name="B1" class="submit" onclick="sendaudit(3)">
	 <%}else if("2".equals(week.getState())){%>
	 	<input type="button" value="审核通过" name="B1" class="submit" onclick="sendaudit(4)">
		<input type="button" value="驳回" name="B1" class="submit" onclick="sendaudit(5)">
	 <%}%>
</html:form>
