	...
	var config = {
		...
		ctxMenu:[
			{id:'cm-priv', text:"${eoms:a2u('分配权限')}",cls:'edit-mi',handler:function(){
				...
				document.getElementById("canfp").style.display = 'block' ;
				document.getElementById("add").style.display = 'block' ;
				document.getElementById("del").style.display = 'block' ;
				document.getElementById("tdprivSource").style.display = 'block' ;		
			}},
			...
			{id:'cm-look', text:"${eoms:a2u('查看')}",cls:'edit-mi',handler:function(){
			    var node = PrivAssigner.getSelNode();
				var nodeType = node.attributes.nodeType;
				eoms.log(node.attributes.nodeType);
				tabs.tabPanel.activate(0); 
				...
				PrivAssigner.doEdtNode();
				document.getElementById("canfp").style.display = 'none' ;
				document.getElementById("add").style.display = 'none' ;
				document.getElementById("del").style.display = 'none' ;
				document.getElementById("tdprivSource").style.display = 'none' ;
			}}
		],
		...
		deptCtxMenu:[
			{id:'cm-deptpriv',text:"${eoms:a2u('分配部门权限')}",cls:'edit-mi',handler:function(){
				var node = PrivAssigner.getSelNode();
				var nodeType = node.attributes.nodeType;
				eoms.log(node.attributes.nodeType);
			...
			{id:'cm-deptprivlook',text:"${eoms:a2u('查看')}",cls:'edit-mi',handler:function(){
				var node = PrivAssigner.getSelNode();
				var nodeType = node.attributes.nodeType;
				eoms.log(node.attributes.nodeType);
...
<div id="helpPanel" class="app-panel">
	...
	<dl>
		...
		<dt style="color:red">${eoms:a2u('权限')}</dt>
		<dd>
			<div class="viewer-list">
			  <div class="viewlistitem-dept">${eoms:a2u('表示对部门下的人员有管理权限;')}</div>
			  <div class="viewlistitem-dept-readonly">${eoms:a2u('表示对部门下的人员仅有查看权限.')}</div>			  
			</div>
		</dd>
	</dl>
</div>
	...
	<div id="formPanel-body" class="app-panel">
	  ...
	  <div id="tab" style="height:400px;">
		...
		<tr>
		 ...
		 -->
	  	<tr>
	  		<td class="textHeader" id="canfp" >${eoms:a2u("可分配的菜单方案")}</td>
			<td></td>
	  		<td class="textHeader">${eoms:a2u("已分配的菜单方案")}</td>
	  	</tr>
	  	<tr>
	  		<td valign="top" id="tdprivSource" >
			...
			<td>
			  <input type="button" id="add" value="${eoms:a2u('添加')}&gt;&gt;" 
			  	onclick="javascript:PrivAssigner.addPriv();" class="button"/>
			  <br/><br/>
			  <input type="button" id="del" value="&lt;&lt;${eoms:a2u('删除')}" 
	    ...
	    <div id="tab-deptAsg" class="tabContent"><select id="deptAsg" size="10" class="menuBox"></select></div>
	     <div id="tab-alldeptAsg" class="tabContent"><select id="alldeptAsg" size="10" class="menuBox"></select></div>
...
<div id="regionPanel" class="app-panel">
	<iframe id="regionPanel-page" name="regionPanel-page" frameborder="no" style="width:100%;height:100%" src=""></iframe>
</div>
