...
Ext.onReady(function(){
	...
	tplTab.on('activate',function(){
	   	//location.href = "${app}/assEva/assEvaTemplates.do?method=newTemplate&nodeId="
	});
	kpiTab.on('activate',function(){
	   	//location.href = "${app}/assEva/assEvaKpis.do?method=newKpi&nodeId="
	});
	var tplId = document.forms[0].id.value;
	if ("" != tplId) {
	   	//$('kpiList').src="${app}/assEva/assEvaTemplateKpis.do?method=listKpiOfTemplate&templateId=${assEvaTemplateForm.id}";
...
function activeTemplate() {
	var msg = "模板下属指标权重和值为"+'${sessionScope.assEvaTW}'+"，确认激活？";
		if (confirm(msg)==true){
			var url = "${app}/assEva/assEvaTemplates.do?method=activeTemplate&templateId=${assEvaTemplateForm.id}&nodeId=${requestScope.parentNodeId}";
			location.href = url;
		}	
}

function selectChange(templateTypeId) {
	document.forms[0].action = "assEvaTemplates.do?method=newTemplate";
...
<html:form action="/assEvaTemplates.do?method=saveTemplate" styleId="assEvaTemplateForm" method="post"> 
<table class="formTable" id="tplForm" style="width:88%" align="center">
	...
	<tr>
		...
		<td class="content" colspan="3">
			<html:text property="templateName" styleId="templateName" style="width:88%" 
						...
						alt="allowBlank:false,vtext:'请输入模版名称'" value="${assEvaTemplateForm.templateName}" />
		</td>
	</tr>
	<c:if test="${not empty assEvaTemplateForm.id}">
	<tr>
		<td class="label">
			创建人
		</td>
		<td class="content">
			<eoms:id2nameDB id="${assEvaTemplateForm.creator}" beanId="tawSystemUserDao" />
		</td>
	...
	<tr>
		...
		<td class="content" colspan="3">
			<eoms:dict key="dict-assEva" dictId="activated" itemId="${assEvaTemplateForm.activated}" beanId="id2nameXML" />
		</td>
	</tr>
	</c:if>
	<tr>
		...
		<td class="content" >
			<html:text property="professional" styleId="professional" style="width:88%" 
						styleClass="text medium"
						value="${assEvaTemplateForm.professional}" />
		</td>
		<td class="label">
			考核周期
		</td>
		<td class="content">
			<c:choose>
				<c:when test="${assEvaTemplateForm.cycle != null}">
					<eoms:dict key="dict-assEva" dictId="cycle" defaultId="${assEvaTemplateForm.cycle}"
						 selectId="cycle" beanId="selectXML" alt="allowBlank:false,vtext:'请选择考核周期'" />
				</c:when>
				<c:otherwise>
					<eoms:dict key="dict-assEva" dictId="cycle" defaultId="month"
						 selectId="cycle" beanId="selectXML" alt="allowBlank:false,vtext:'请选择考核周期'" />
				</c:otherwise>
			</c:choose>
		</td>		
	...
	<tr>
		...
		<td class="content">
			<c:choose>
				<c:when test="${assEvaTemplateForm.orgType != null}">
					<eoms:dict key="dict-assEva" dictId="orgType" defaultId="${assEvaTemplateForm.orgType}"
						 selectId="orgType" beanId="selectXML" alt="allowBlank:false,vtext:'请选择组织类型'" />
				</c:when>
				<c:otherwise>
					<eoms:dict key="dict-assEva" dictId="orgType" defaultId="dept"
						 selectId="orgType" beanId="selectXML" alt="allowBlank:false,vtext:'请选择组织类型'" />
				</c:otherwise>
			</c:choose>
		</td>
	...
	<tr>
		...
		<td class="content" colspan="3">
			<html:textarea property="remark" styleId="remark" style="width:88%" 
						styleClass="textarea"  value="${assEvaTemplateForm.remark}" />
		</td>
	</tr>
	<c:if test="${empty assEvaTemplateForm.id}">
	<tr>
		<td class="label">
			复制模板指标
		</td>
		...
		<td class="content" colspan="3">
			...
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="assEvaTreeDao" />
					</option>
				</logic:iterate>
			</select>
			&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<option value="">--请选择--</option>
				<logic:iterate id="tList" name="tList">
					<option value="${tList.nodeId}">
						<eoms:id2nameDB id="${tList.nodeId}" beanId="assEvaTreeDao" />
					</option>
				</logic:iterate>
			</select>
		</td>
		...
		<td class="content" colspan="3">
			...
			<option value=""><eoms:id2nameDB id="${requestScope.templateTypeId}" beanId="assEvaTreeDao" /></option>	
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="assEvaTreeDao" />
					</option>
	...
	<tr>
		...
		<c:if test="${not empty assEvaTemplateForm.id 
			&& not empty requestScope.parentNodeId 
				&& assEvaTemplateForm.activated == 0}">
		<td>
...
<input type="hidden" id="id" name="id" value="${assEvaTemplateForm.id}" />
</html:form>
</div>
...
<div id="kpi" class="tab-content">
	...
	<iframe id="fileList" name="fileList" frameborder="0" width="100%" height="500" 
		src="${app}/assEva/assEvaKpis.do?method=listNextLevelKpi&parentNodeId=${requestScope.parentNodeId}">
	</iframe>
	</c:if>
</div>
</div>
