...
<%@page import="com.boco.eoms.assEva.webapp.form.AssEvaKpiInstanceForm"%>
<%@page import="com.boco.eoms.base.util.StaticMethod"%>
<%@ include file="/common/taglibs.jsp"%>
<%@ include file="/common/header_eoms_form.jsp"%>
<%@ page language="java" import="java.util.*"%>
<%@ page language="java" import="com.boco.eoms.assEva.util.AssEvaStaticMethod"%>

<script type="text/javascript">
Ext.onReady(function(){
	v = new eoms.form.Validation({form:'assEvaKpiInstanceForm'});
})
</script>

<html:form action="/assEvaTasks.do?method=saveTaskDetail" styleId="assEvaKpiInstanceForm" method="post">
	...
	<tr>
		...
		<% 
		   ...
		   for(int i=0;i<parIdss.size();i++){
			   String parName = AssEvaStaticMethod.orgId2Name(String.valueOf(parIdss.get(i)), "dept");
		...
		<tr>
			<% 
			    ...
			    for(int k=0;k<rowList.size();k++){
			    	AssEvaKpiInstanceForm ekif = (AssEvaKpiInstanceForm) rowList.get(k);
			%>			
				<td rowspan="<%=ekif.getRowspan()%>" colspan="<%=ekif.getColspan()%>" style="vertical-align:middle;text-align:left">
					<eoms:id2nameDB id="<%=ekif.getKpiId()%>" beanId="assEvaKpiDao" />(<%=ekif.getWeight()%>分)
				</td>
				<%if (AssEvaConstants.NODE_LEAF.equals(ekif.getLeaf())) { %>
					<td>
						<%=ekif.getAlgorithm()%>
					</td>
					<% 
					   for(int j=0;j<parIdss.size();j++){
						   ...
						   String parName = AssEvaStaticMethod.orgId2Name(parIds, "dept");
					...
					%>			
						<%if(AssEvaConstants.TASK_PUBLISHED.equals(ekif.getIsPublish())){%>
					<td>
...
function  save()
    {
      document.forms[0].action="assEvaTasks.do?method=saveTaskDetail";
    };
function  commit()
    {
      document.forms[0].action="assEvaTasks.do?method=commitTaskDetail";
    };
function  runBack()
    {
   	  v.passing="true";
      ...
      document.forms[0].action="assEvaTasks.do?method=xquery&searchType=1";
    };
function  queryBack()
    {
      v.passing="true";
      document.getElementById("taskId").value = '' ;
      document.forms[0].action="assEvaTasks.do?method=xquery&searchType=0";
    };
</script>