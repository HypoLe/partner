...
function changeTemplateType()
		{    
			...
			var url="<%=request.getContextPath()%>/assEva/assEvaTasks.do?method=changeTemplateType&templateTypeId="+templateTypeId;
...
function isSubmit(){
    if(checkAssEvaTime())return true;//document.getElementById("assEvaKpiInstanceForm").submit();
    else if(!checkAssEvaTime()){
       alert("考核月份在模板激活时间之后！");
       return false;
    }
}
...
<html:form action="/assEvaTasks.do?method=taskDetailList" styleId="assEvaKpiInstanceForm" method="post" > 
<table class="formTable" id="form">
			...
			<select name="templateTypeId" id="templateTypeId"
					...
					<logic:iterate id="templateType" name="templateType">
						<option value="${templateType.nodeId}">
							<eoms:id2nameDB id="${templateType.nodeId}" beanId="assEvaTreeDao" />
						</option>
					</logic:iterate>
				</select>
	...
	<tr>
		...
		<td class="content">
			<eoms:dict key="dict-assEva" dictId="cycle" defaultId="month" onchange="changeCycle();"
