			...
			<td colspan="4">
				...
				<select name="itemTypes">
					...
					<option value="<%=itemType%>">
						<eoms:id2nameDB id="<%=itemType%>" beanId="IItawSystemDictTypeDao" />
					</option>
					<%
					}
					%>
				...
				<logic:notEmpty name="tawSupplierkpiInstanceList">
					<logic:iterate id="aal" name="tawSupplierkpiInstanceList"
						...
						<tr class="normal">
							...
							<td>
								<eoms:dict key="dict-supplierkpi" dictId="statictsCycle" itemId="<%=statictsCycle%>" beanId="id2nameXML" />
							</td>
							<td>
							...
							<td>
								...
								<input type="hidden" name="isImpersonality" value='<bean:write name="aal" property="isImpersonality" />'> 
  ...
  for(i=0;i<infoId.length;i++){
  	...
  	if(!isNaN(exam[i].value)){
  	   if(unit[i].value=="106010502"){//判断单位是否百分比
  	     if(exam[i].value>100||exam[i].value<0){alert('${eoms:a2u("单位是百分比的评估数据范围在0-100")}');flag=false;}
  	   }
  	   else if(exam[i].value<0){alert('${eoms:a2u("数字大于0")}');flag=false;}
  	}
  	else{ alert('${eoms:a2u("评估数据请输入数字！")}');flag=false; } 	
  	}
	ids+="@@"+infoId[i].value;
