...
<div style="width:90%;text-align:right;margin-top:5px;">
	<input type="button" class="btn" style="font-size:15px;margin-right:20px;" value="提交"  onclick="javascript:doSubmit();"/>	
</div>
		...
		<div id="part1" style="padding-bottom:20px;">
		  <table width="100%">
			...
			<c:if test="${item1.questionsID==item.questionID}">
			  <tr>
			    <td colspan="2" align="center">
					<div style="width:95%;text-align:left;margin-top:2px;">${item1.orderBy}.&nbsp;<input type="radio" id="${item.questionID}" name="${item.questionID}" value="${item1.orderBy}" disabled="true"/>&nbsp;${item1.content}</div>
				</td>					
			  </tr>
			  </c:if>
			</c:forEach>
			  ...
			  <tr>
				...
				<div style="width:95%;text-align:left;padding-top:10px;padding-bottom:10px;margin-bottom:10px;margin-top:10px;border-top:1px #cacaca dotted;border-bottom:1px dotted #cacaca;">
					...
					<c:if test="${item0.questionId==item.questionID}">
						...
						评分&nbsp;<font color='red'>*</font> ：<input type="text" id="${item.questionID}" name="${item.questionID}" onchange="if(!/^(\d+)(\.\d+)?$/.test(this.value)){alert('只能输入数字');this.value='';}"><br>
							<input type="hidden" id="${item.questionID}1" value="${item0.referenceScore}">
					</c:if>
					</c:forEach>
