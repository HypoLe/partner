...
<html>
	...
	<script type="text/javascript">
		function showPicture(obj)
		{
	        ...
	        pDiv.innerHTML = '<div id="close" style="width:100%;text-align:right;cursor:pointer;padding-top:4px;padding-bottom:2px;" onclick="hiddenP(this)"><b>关闭</b></div>';
	        pDiv.style.display = "";
...
<html:form action="/kmExamAttends.do?method=isPublic" styleId="kmExamTestForm" method="post"> 
...
<div style="color:#818181;">
	<b>答题人姓名：</b><eoms:id2nameDB id="${attendUser}" beanId="tawSystemUserDao" />&nbsp;&nbsp;&nbsp; |&nbsp;&nbsp;&nbsp;<b>考试时间：</b><font color="red">${kmExamTest.testDuration }</font> 分钟&nbsp;&nbsp;&nbsp;<b>考试总分：</b><font color="red">${kmExamTest.totalScore }</font> 分&nbsp;&nbsp;&nbsp; 
</div>
	...
	<tr>
		<td align="center">
			<div style="border:1px #98c0f4 solid;width:90%;height:60px;">
				<div style="text-align:left;margin:5px auto auto 5px;"><b>阅卷结果</b></div>
				<div style="float:left;margin:5px auto auto 18px;">阅卷人：<b><eoms:id2nameDB id="${kmExamAttend.readUser}" beanId="tawSystemUserDao" /></b>&nbsp;&nbsp;&nbsp;&nbsp;得分：<font color='red'><b>${kmExamAttend.score}</b></font></div>
			</div>
		</td>
	</tr>	
</table>
...
<div style="text-align:left;width:90%;margin-top:10px;">
	...
	%>
	<%=size1+size2+size3+size4+size5 %> </font>道题目</div>
</div>
<div style="width:90%;text-align:right;margin-top:5px;">
	<c:if test="${kmExamAttend.isPublic==0}">
		<input type="button" class="btn" style="font-size:15px;margin-right:20px;" value="发布该试卷成绩"  onclick="javascript:doSubmit();"/>
	</c:if>	
</div>
		...
		<div id="part1" style="padding-bottom:20px;">
		  <table width="100%">
			...
			<c:if test="${item1.questionsID==item.questionID}">
			  <tr>
			    <td colspan="2" align="center">
					<div style="width:95%;text-align:left;margin-top:2px;">${item1.orderBy}.&nbsp;<input type="radio" disabled="true" id="${item.questionID}" name="${item.questionID}" value="${item1.orderBy}" />&nbsp;${item1.content}</div>
				</td>					
			  </tr>
			  </c:if>
			</c:forEach>
	...
	<div id="part2" style="padding-bottom:20px;">
		  <table width="100%">
			...
			<c:if test="${item1.questionsID==item.questionID}">
			  <tr>
			    <td colspan="2" align="center">
				<div style="width:95%;text-align:left;margin-top:2px;">${item1.orderBy}.&nbsp;<input type="checkbox" disabled="true" id="${item.questionID}" name="${item.questionID}" value="${item1.orderBy}" />&nbsp;${item1.content}</div>
				</td>					
			  </tr>
			 </c:if>
			</c:forEach>	
			...
			<tr>
			  ...
			  <tr>
				...
				<div style="width:95%;text-align:left;padding-top:10px;padding-bottom:10px;margin-bottom:10px;margin-top:10px;border-top:1px #cacaca dotted;border-bottom:1px dotted #cacaca;">
					...
					<c:if test="${item0.questionId==item.questionID}">
						得分: <font color='red'><b>${item0.score}</b></font><br/><br/>
						用户答题答案：<b>
									<script type="text/javascript">
										var strs = '${item0.answer}'.split("#");
										for(var i=0;i<strs.length-1;i++)
											document.write("<U>&nbsp;&nbsp;&nbsp;"+strs[i]+"&nbsp;&nbsp;&nbsp;</U>&nbsp;&nbsp;");
									</script>
								</b><br/><br/>
						参考答案：${item0.referenceAnswer}<br>
					</c:if>
					</c:forEach>
