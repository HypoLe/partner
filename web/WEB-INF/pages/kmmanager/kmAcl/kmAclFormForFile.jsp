...
<script type="text/javascript">
	...
	function saveOrUpdateKmAcl(my) {
		...
		if(($(moduleId+"_R").checked)||($(moduleId+"_C").checked)||($(moduleId+"_U").checked)||($(moduleId+"_D").checked)||($(moduleId+"_G").checked)||($(moduleId+"_CA").checked)){
			url = '${app}/kmmanager/kmAcls.do?method=saveOrUpdateKmAcl&moduleType='+moduleType 
					+'&principalType='+principalType +'&principalId='+principalId +'&moduleId='+moduleId
						+ '&flag='+flag + '&permission='+permission +'&grade='+grade;
		}else{
			url ='${app}/kmmanager/kmAcls.do?method=removeKmAcl&moduleType='+moduleType 
					+'&principalType='+principalType +'&principalId='+principalId +'&moduleId='+moduleId;
		}
	...
	function initDate(){
		createXMLHttpRequest();
		var url = '${app}/kmmanager/kmAcls.do?method=searchCheck&moduleType='+moduleType 
	...
	function changeGrade(my1){
		...
		if(($(id+"_R").checked)||($(id+"_C").checked)||($(id+"_U").checked)||($(id+"_D").checked)||($(id+"_G").checked)||($(id+"_CA").checked)){
			...
			createXMLHttpRequest();
			var url = '${app}/kmmanager/kmAcls.do?method=saveKmAclGrade&moduleType='+moduleType 
...
<html:form action="/kmAcls.do?method=save" styleId="kmAclForm"
	method="post">
		...
		<table class="tableEdit" style="width: 560px;" cellspacing="0"
			...
			<c:forEach items="${list1}" var="Pnode">
				<tr>
					...
					<td>
						<!-- 
				<input type="checkbox" id="${Pnode.nodeId }_R" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="0">查看
				<input type="checkbox" id="${Pnode.nodeId }_C" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="1" >增加
				<input type="checkbox" id="${Pnode.nodeId }_U" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="2" >修改
				<input type="checkbox" id="${Pnode.nodeId }_D" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="3" >删除
				<input type="checkbox" id="${Pnode.nodeId }_G" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="4" >授予
				<input type="checkbox" id="${Pnode.nodeId }_CA" onclick="saveOrUpdateKmAcl(this)" moduleId="${Pnode.nodeId}" permission="5" >排除
				 -->
					</td>
					<td></td>
					<td>
						...
						<tr>
							...
							<td>
								<input type="checkbox" id="${Cnode.nodeId }_R"
									onclick="saveOrUpdateKmAcl(this)" moduleId="${Cnode.nodeId}"
									permission="0">
								查看
	...
	<html:hidden property="id" value="${kmAclForm.id}" />
</html:form>
