...
<html:form action="/kmExpertAnswer.do?method=selectUrl" styleId="KmExpertAnswerForm" method="post"  onsubmit=""> 
		...
		<eoms:comboBox name="type" id="type" 
            	      initDicId="1010104" defaultValue="" alt="allowBlank:false,vtext:''" styleClass="select-class" onchange=""/>	
		</td>
...
<fmt:bundle basename="com/boco/eoms/km/config/applicationResource-kmmanager">
	...
	<display:column sortable="true" headerClass="sortable" title="主题" >
		<a href="${app}/kmmanager/kmExpertAnswer.do?method=detailQuestion&id=${kmExpertAnswerList.id}" target=_blank>${kmExpertAnswerList.title}</a> 
	</display:column>
	...
	<display:column sortable="true" headerClass="sortable" title="问题分类" >
			<eoms:id2nameDB id="${kmExpertAnswerList.type}" beanId="IItawSystemDictTypeDao" />
	</display:column>	
...
<script>
	function changeUrl(id){
		 var url = "${app}/kmmanager/kmExpertAnswer.do?id="+id+"&method=detailQuestion";
