...
function setFormValue(content){
  ...
  window.opener.smsSendForm.content.value =arr[0].trim();
  window.opener.smsSendForm.contentRemark.value =arr[1].trim();
}
//-->
</script>
<!--根据给定的实例名生成标题 -->
...
<html:form action="/smsContentTemplates.do?method=forward2New" method="post" styleId="smsContentTemplateForm"> 
	<table class="formTable">
		...
		<logic:iterate id="contentList" name="contentList">
			<tr ondblclick="setFormValue('<bean:write name="contentList" property="content"/>#<bean:write name="contentList" property="remark" />');"  title="<bean:message key='smsContentTemplate.alert'/>" onMouseOver="this.className='tr-over'" onMouseOut="this.className=''">
				<td>
