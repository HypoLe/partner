...
<%@page import="com.boco.eoms.partner.chanEva.mgr.*"%>
<%@page import="com.boco.eoms.partner.chanEva.util.*"%>
<%@page import="com.boco.eoms.partner.chanEva.model.*"%>
<style type="text/css">
body {
...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/chanEvaKpis.do?method=saveKpi" styleId="chanEvaKpiForm">
			<table align="center">
				<tr>
					<td>
						指标名称
				...
				<tr>
					...
					<td colspan="3">
						<input type="textarea" id="algorithm" name="algorithm"
							...
							value="${chanEvaKpiForm.algorithm}" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td colspan="3">
						<input type="textarea" id="remark" name="remark" class="textarea"
							style="width: 88%" value="${chanEvaKpiForm.remark}" />
					</td>
				</tr>
				<tr>
					<td colspan="4">
			...
			<input type="hidden" id="id" name="id" value="${chanEvaKpiForm.id}" />
		...
		<caption>
			<div class="header center">
				<%
					...
					IChanEvaTreeMgr chanEvaTreeMgr = (IChanEvaTreeMgr) ApplicationContextHolder
					...
					//if (ChanEvaConstants.NODETYPE_TEMPLATETYPE.equals(nodeType)) {
						tempname = chanEvaTreeMgr.id2Name(parentNodeId);
					//}else{
					//	ChanEvaTree chanEvaTree = chanEvaTreeMgr.getTreeNodeByNodeId(parentNodeId);
					//	ChanEvaKpi kpi = new ChanEvaKpi();
					//	kpi = kpiMgr.getKpi(chanEvaTree.getObjectId());
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a
							href="${app}/partner/chanEva/chanEvaKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url='${app}/partner/chanEva/chanEvaKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
