...
<%@page import="com.boco.eoms.partner.chanEva.webapp.form.ChanEvaKpiInstanceForm"%>
<%@page import="com.boco.eoms.base.util.StaticMethod"%>
<%@ include file="/common/taglibs.jsp"%>
<%@ include file="/common/header_eoms_form.jsp"%>
<%@ page language="java" import="java.util.*"%>
<%@ page language="java" import="com.boco.eoms.partner.chanEva.util.ChanEvaStaticMethod"%>
...
<html:form action="/chanEvaTasks.do?method=saveTaskDetail" styleId="chanEvaKpiInstanceForm" method="post">
	...
	<tr>
		...
		<% 
		   ...
		   for(int i=0;i<parIdss.size();i++){
			   String parName = ChanEvaStaticMethod.orgId2Name(String.valueOf(parIdss.get(i)), "dept");
		%>
		...
		<tr>
			<% 
			    ...
			    for(int k=0;k<rowList.size();k++){
			    	ChanEvaKpiInstanceForm ekif = (ChanEvaKpiInstanceForm) rowList.get(k);
			%>			
				<td rowspan="<%=ekif.getRowspan()%>" colspan="<%=ekif.getColspan()%>" style="vertical-align:middle;text-align:left">
					<eoms:id2nameDB id="<%=ekif.getKpiId()%>" beanId="chanEvaKpiDao" />(<%=ekif.getWeight()%>分)
				</td>
				<%if (ChanEvaConstants.NODE_LEAF.equals(ekif.getLeaf())) { %>
					<td>
						<%=ekif.getAlgorithm()%>
					</td>
					<% 
					   for(int j=0;j<parIdss.size();j++){
						   ...
						   String parName = ChanEvaStaticMethod.orgId2Name(parIds, "dept");
					...
					%>			
						<%if(ChanEvaConstants.TASK_PUBLISHED.equals(ekif.getIsPublish())){%>
					<td>
...
function  save()
    {
      document.forms[0].action="chanEvaTasks.do?method=saveTaskDetail";
    };
function  commit()
    {
      document.forms[0].action="chanEvaTasks.do?method=commitTaskDetail";
    };
function  runBack()
    {
   	  v.passing="true";
      ...
      document.forms[0].action="chanEvaTasks.do?method=xquery&searchType=1";
    };
function  queryBack()
    {
      v.passing="true";
      document.getElementById("taskId").value = '' ;
      document.forms[0].action="chanEvaTasks.do?method=xquery&searchType=0";
    };
</script>