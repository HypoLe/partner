...
<html>
	...
	<body>
		<form action="storeOut.do?method=selectBy" method="post">
			
		<p><center><span style="font-size:20px">出库管理</span></center></p>
			...
			<input type="button" style="margin-right: 5px"
				onclick="location.href='<c:url value="/materials/storeOut.do?method=showAddStoreOutView"/>'"
				value="新增出库单" />
		</form>
		<br />
...
<br />
		<display:table name="list" cellspacing="0" cellpadding="0" id="list"
			...
			<display:column sortable="true" headerClass="sortable" title="操作"
				media="html">
			 <!-- <html:link
					href="${app}/materials/storeOut.do?method=showAlterView"
					paramId="id" paramProperty="id" paramName="list">
						修改
					</html:link>
				<html:link href="${app}/materials/storeOut.do?method=delete"
					paramId="id" paramProperty="id" paramName="list">
						删除
					</html:link>
				<html:link href="${app}/materials/storeOut.do?method=detail"
					paramId="id" paramProperty="id" paramName="list">
						详情
					</html:link>
				<html:link href="${app}/materials/storeOut.do?method=goAuditPage"
					paramId="id" paramProperty="id" paramName="list">
						审核
					</html:link>
			  	<html:link href="${app}/materials/storeOut.do?method=goDeAuditPage"
					paramId="id" paramProperty="id" paramName="list">
						反审
					</html:link>-->
			...
			<c:if test="${ '3' == mateper}">
				<a
					href="${app}/materials/storeOut.do?method=showAlterView&id=${list.id}" >修改</a>
				
				<a
					href="${app}/materials/storeOut.do?method=delete&id=${list.id}"
					onClick="if(confirm('确定要删除吗？'))return true;return false;">删除</a>
				<a
					href="${app}/materials/storeOut.do?method=detail&id=${list.id}">详情</a>
				<c:if test="${list.storeExamineStatus=='未审核'}">
				<a
					href="${app}/materials/storeOut.do?method=goAuditPage&id=${list.id}">审核</a>
				</c:if>
			</c:if>
			<!-- 财务人员 -->
			<c:if test="${ '2' == mateper}">
