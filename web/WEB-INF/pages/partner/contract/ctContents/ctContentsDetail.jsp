...
<table>
	<tr>
		<td>
		    <input type="button" class="btn" value="修改" onclick="onSubmitEdit();"/>
		    <!--&nbsp;
		    <c:if test="${CtContentsMap.CONTRACT_STATUS != 3}"> 可以将其它状态的合同失效 
		    <input type="button" class="btn" value="失效" onclick="onSubmitOver();"/>&nbsp;
		    </c:if>
		    
		    <c:if test="${CtContentsMap.CONTRACT_STATUS == 3}"> 可以删除失效的合同 
		    <input type="button" class="btn" value="删除" onclick="onSubmitDele();"/>&nbsp;
		    </c:if>
		    -->
		    
		    
		    <c:if test="${CtContentsMap.CONTRACT_STATUS != 3}"><!-- 可以删除合同 -->
		    <input type="button" class="btn" value="删除" onclick="onSubmitDele();"/>&nbsp;
		    </c:if>
