...
function onSubmit(){ 
		var arr = [<c:forEach items="${ctContentsSubscribeList}" var="obj" varStatus="status"><c:if test="${status.count > 1}">, </c:if>'${obj.createUser}'</c:forEach>];
...
<html:form action="/ctContentsSubscribes.do?method=saveSubscribe" method="post" styleId="ctContentsForm">
	...
	<display:table name="ctContentsSubscribeList" cellspacing="0" cellpadding="0"
		id="ctContentsList" pagesize="${pageSize}" class="table ctContentsCollectList"
		export="false"
		requestURI="${app}/partner/contract/ctContentsSubscribes.do"
		>
		...
		<display:column title="删除订阅" headerClass="imageColumn">
		    <a href="javascript:if(confirm('确定要删除该订阅？')){
		                        ...
		                        var url='${app}/partner/contract/ctContentsSubscribes.do?method=removeCreateUser';
		                        url = url + '&ID=' + id ;
