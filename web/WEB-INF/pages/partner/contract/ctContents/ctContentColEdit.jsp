						...
						<c:choose>
							<c:when test="${item.colType == 1}">
								...
								<input type="text" class="text medium"
									...
									id="${item.colName}" 
									value="${CtContentsMap[item.colName]}"
							...
							<c:when test="${item.colType == 2}">
								...
								<textarea type="_moz" class="textarea max" 
									...
									alt="allowBlank:<c:if test="${item.isNullable == 1}">true</c:if><c:if test="${item.isNullable == 0}">false</c:if>,vtext:'请输入${item.colChname}...'">${CtContentsMap[item.colName]}</textarea>
							</c:when>
