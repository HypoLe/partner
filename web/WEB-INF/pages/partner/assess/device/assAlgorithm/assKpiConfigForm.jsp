...
<html:form action="/deviceAssKpiConfigs.do?method=save" styleId="assKpiConfigForm" method="post"> 
<fmt:bundle basename="com/boco/eoms/partner/assess/config/applicationResources-assess">
<table class="formTable">
	<caption>
		<div class="header center"><fmt:message key="assKpiConfig.form.heading"/>${parentWeight}</div>
	</caption>
	<%
...
<c:if test="${assKpiConfigForm.parentNodeId==1}">
	...
	<tr>		
		...
		<td class="content">
			<input id="numRelationOne" styleClass="text medium" style="width:50px;" alt="vtype:'number'" onblur="changeRate(document.getElementById('numRelationOne').value,document.getElementById('numRelationTwo').value)"/>~<input id="numRelationTwo" styleClass="text medium" style="width:50px;" alt="vtype:'number'" value=""  onblur="changeRate(document.getElementById('numRelationOne').value,document.getElementById('numRelationTwo').value)"/>		
		</td>		
	</tr>	
</c:if>	
	<%
			...
			<c:if test="${not empty assKpiConfigForm.id}">
				<input type="button" class="btn" value="<fmt:message key="button.delete"/>" 
					onclick="javascript:if(confirm('你确定删除?')){
						var url='${app}/partner/assess/deviceAssKpiConfigs.do?method=remove&nodeId=${assKpiConfigForm.nodeId}';
						location.href=url}"
						/>
			</c:if>
		</td>
