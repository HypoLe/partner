...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/deviceAssKpis.do?method=saveKpi" styleId="assKpiForm">
			<table align="center">
				<tr>
					<td>
						指标名称
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a href="${app}/partner/assess/deviceAssKpiConfigs.do?method=tree&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}" target="_blank">打分配置</a>
					</td>					
					<td>
						<a
							href="${app}/partner/assess/deviceAssKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url='${app}/partner/assess/deviceAssKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
