...
Ext.onReady(function(){
	...
	tplTab.on('activate',function(){
	   	//location.href = "${app}/partner/assess/deviceAssTemplates.do?method=newTemplate&nodeId="
	});
...
function activeTemplate() {
	var msg = "模板下属指标权重和值为"+'${sessionScope.assTW}'+"，确认激活？";
		if (confirm(msg)==true){
			var url = "${app}/partner/assess/deviceAssTemplates.do?method=activeTemplate&templateId=${assTemplateForm.id}&nodeId=${requestScope.parentNodeId}";
			location.href = url;
		}	
}

function selectDevice(templateTypeId) {
	document.forms[0].action = "deviceAssTemplates.do?method=newTemplate";
...
<html:form action="/deviceAssTemplates.do?method=saveTemplate" styleId="assTemplateForm" method="post"> 
<table class="formTable" id="tplForm" style="width:88%" align="center">
	...
	<input type="hidden" id="orgIds" name="orgIds" value = "中国联通山东江公司" alt="allowBlank:false,vtext:'请选择所属地市'" />
	...
	<tr>
		...
		<td class="content" colspan="3">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			模板分类：
			<select name="templateTypeId" id="templateTypeId" disabled="disabled" onchange="selectDevice(this.options[this.options.selectedIndex].value)">
			<option value="">--请选择--</option>	
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="assTreeDao" />
					</option>
				</logic:iterate>
			</select>
			&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<option value="">--请选择--</option>
				<logic:iterate id="tList" name="tList">
					<option value="${tList.nodeId}">
						<eoms:id2nameDB id="${tList.nodeId}" beanId="assTreeDao" />
					</option>
				</logic:iterate>
			</select>
		</td>
		...
		<td class="content" colspan="3">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			模板分类：
			<select name="templateTypeId" id="templateTypeId"  onchange="selectDevice(this.options[this.options.selectedIndex].value)">
			<option value=""><eoms:id2nameDB id="${requestScope.templateTypeId}" beanId="assTreeDao" /></option>	
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="assTreeDao" />
					</option>
...
<div id="kpi" class="tab-content">
	...
	<iframe id="fileList" name="fileList" frameborder="0" width="100%" height="500" 
		src="${app}/partner/assess/deviceAssKpis.do?method=listNextLevelKpi&parentNodeId=${requestScope.parentNodeId}">
	</iframe>
	</c:if>
</div>
</div>
