				...
				%>
					...
					<%
							...
							<%
							 ...
							 %>
							 <td>
								<a href="${app}/partner/assess/deviceAssReportInfo.do?method=nodeQuarter&taskId=${taskId}&kpiId=<%=ekif.getKpiId()%>&year=${year}&quarter=${quarter}&month=${month}&timeType=${timeType}&partnerId=<%=parIds%>&kpiInstanceId=<%=ekif.getId() %>" target='_blank'>
								<%=realScore%></a>
							 </td>
							 <%
							 }
