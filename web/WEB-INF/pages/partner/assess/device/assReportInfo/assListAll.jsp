		...
		<tr>
			...
			<%
				for(int k = 0;k<partnerList.size();k++){
					...
					if(report.getTotalScore()==null||"".equals(report.getTotalScore())){
			%>
				<td>
			...
			%>
				<td>
					<center><a href="${app}/partner/assess/deviceAssReportInfo.do?method=taskList&month=${month}&taskId=${taskId}&areaId=<%=areaList.get(i)%>&year=${year}&quarter=${quarter}&timeType=${timeType}&partnerId=<%=partnerList.get(k)%>" target='_blank'>
					<%=report.getTotalScore() %></a></center>
				</td>
			<%		
					}
				}
