				...
				%>
					...
					<%
							...
							<%
							 ...
							 %>
							 <td>
								<a href="${app}/partner/assess/tranAssReportInfo.do?method=nodeQuarter&taskId=${taskId}&kpiId=<%=ekif.getKpiId()%>&year=${year}&quarter=${quarter}&month=${month}&timeType=${timeType}&partnerId=<%=parIds%>&kpiInstanceId=<%=ekif.getId() %>&treeNodeId=<%= ekif.getNodeId() %>&weight=<%=ekif.getWeight()%>" target='_blank'>
								<%=realScore%></a>
							 </td>
							 <%
							 }
