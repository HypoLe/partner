	...
	<tr>
		...
		<td class="content">
			<html:text property="minValue" styleId="minValue"
						styleClass="text medium"  style="width:100px;" 
						alt="re:/^[0-9]+\.{0,1}[0-9]{0,2}$/,allowBlank:false,vtext:''" value="${assKpiConfigForm.minValue}" />
		~
			<html:text property="maxValue" styleId="maxValue" style="width:100px;" 
			...
			<c:if test="${not empty assKpiConfigForm.id}">
				<input type="button" class="btn" value="<fmt:message key="button.delete"/>" 
					onclick="javascript:if(confirm('你确定删除?')){
						var url='${app}/partner/assess/assKpiConfigs.do?method=remove&nodeId=${assKpiConfigForm.nodeId}';
						location.href=url}"
						/>
			</c:if>
		</td>
