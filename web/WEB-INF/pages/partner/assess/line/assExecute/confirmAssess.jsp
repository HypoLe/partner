...
<table class="formTable">
	...
	<tr>
		...
		<td class="content" colspan="3">
			<span id="messageValue" style="display:none"><input type="text" id = "megValue" value="" onblur="validateMes()"></span><span id="messageButton"><input type="button" value="获得短信确认码" onclick="getMsg()" ></span>
			<font color='blue'><span id="messageText">*短信确认通过后才可以提交</span></font>
		</td>	
	</tr>
	<tr>
		<td class="label">
...
function getMsg()
{
	var url = eoms.appPath+"/partner/assess/lineAssOperateStep.do?method=sendMessage";
		Ext.Ajax.request({
				url : url ,
				method: 'POST',
				success: function ( result, request ) { 
					...
					var json = eval(res);
						if(json[0].validateNum!=""){
							validateNum = json[0].validateNum;
							document.getElementById("messageButton").style.display ='none'
							document.getElementById("messageValue").style.display ='';
						}
				},
...
function validateMes(){
	var value = document.getElementById("megValue").value;
	if(value==validateNum){
		document.getElementById("messageText").innerHTML = '短信确认通过';
		document.getElementById("messageValue").style.display ='none';;
		submit = 'true';
	}else{
		document.getElementById("messageText").innerHTML = "对不起,你输入的验证码不匹配，请重新输入";
		document.getElementById("megValue").focus();
	}
}

function sub(){
	if(submit!=''){
	 	$("assOperateStepForm").submit();
	}else{
		alert("请进行短信认证");
	}
}
