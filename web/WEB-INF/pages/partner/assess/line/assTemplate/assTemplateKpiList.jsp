...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/lineAssKpis.do?method=saveKpi" styleId="assKpiForm">
			<table align="center">
				<tr> 
					<td>
						指标名称
				...
				<tr>
					...
					<td colspan="3">
						<c:if test="${isTotal == 'Y'}">
					       <INPUT type="radio" name="isTotal" value="Y" checked="true" >是
					       <INPUT type="radio" name="isTotal" value="N" >否
					    </c:if>
						<c:if test="${isTotal != 'Y'}">
					       <INPUT type="radio" name="isTotal" value="Y">是
					       <INPUT type="radio" name="isTotal" value="N"  checked="true" >否
					    </c:if>					    
					</td>
				</tr>			
				<tr>
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a href="${app}/partner/assess/lineAssKpiConfigs.do?method=tree&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}" target="_blank">打分配置</a>
					</td>					
					<td>
						<a
							href="${app}/partner/assess/lineAssKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url='${app}/partner/assess/lineAssKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
