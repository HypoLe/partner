...
Ext.onReady(function(){
	...
	tplTab.on('activate',function(){
	   	//location.href = "${app}/partner/assess/changeAssTemplates.do?method=newTemplate&nodeId="
	});
...
function activeTemplate() {
	var msg = "模板下属指标权重和值为"+'${sessionScope.assTW}'+"，确认激活？";
		if (confirm(msg)==true){
			var url = "${app}/partner/assess/changeAssTemplates.do?method=activeTemplate&templateId=${assTemplateForm.id}&nodeId=${requestScope.parentNodeId}";
			location.href = url;
		}	
}

function selectChange(templateTypeId) {
	document.forms[0].action = "changeAssTemplates.do?method=newTemplate";
...
<html:form action="/changeAssTemplates.do?method=saveTemplate" styleId="assTemplateForm" method="post"> 
...
<div id="kpi" class="tab-content">
	...
	<iframe id="fileList" name="fileList" frameborder="0" width="100%" height="500" 
		src="${app}/partner/assess/changeAssKpis.do?method=listNextLevelKpi&parentNodeId=${requestScope.parentNodeId}">
	</iframe>
	</c:if>
</div>
</div>
