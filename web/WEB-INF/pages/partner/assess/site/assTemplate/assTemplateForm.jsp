...
Ext.onReady(function(){
	...
	tplTab.on('activate',function(){
	   	//location.href = "${app}/partner/assess/siteAssTemplates.do?method=newTemplate&nodeId="
	});
...
function activeTemplate() {
	var msg = "模板下属指标权重和值为"+'${sessionScope.assTW}'+"，确认激活？";
		if (confirm(msg)==true){
			var url = "${app}/partner/assess/siteAssTemplates.do?method=activeTemplate&templateId=${assTemplateForm.id}&nodeId=${requestScope.parentNodeId}";
			location.href = url;
		}	
}

function selectSite(templateTypeId) {
	document.forms[0].action = "siteAssTemplates.do?method=newTemplate";
...
<html:form action="/siteAssTemplates.do?method=saveTemplate" styleId="assTemplateForm" method="post"> 
<table class="formTable" id="tplForm" style="width:88%" align="center">
	...
	<tr>
		...
		<td class="content" colspan="3">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			模板分类：
			<select name="templateTypeId" id="templateTypeId" disabled="disabled" onsite="selectSite(this.options[this.options.selectedIndex].value)">
		...
		<td class="content" colspan="3">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			模板分类：
			<select name="templateTypeId" id="templateTypeId"  onsite="selectSite(this.options[this.options.selectedIndex].value)">
...
<div id="kpi" class="tab-content">
	...
	<iframe id="fileList" name="fileList" frameborder="0" width="100%" height="500" 
		src="${app}/partner/assess/siteAssKpis.do?method=listNextLevelKpi&parentNodeId=${requestScope.parentNodeId}">
	</iframe>
	</c:if>
</div>
</div>
