...
<%@page import="com.boco.eoms.partner.siteEva.mgr.*"%>
<%@page import="com.boco.eoms.partner.siteEva.util.*"%>
<%@page import="com.boco.eoms.partner.siteEva.model.*"%>
<style type="text/css">
body {
...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/siteEvaKpis.do?method=saveKpi" styleId="siteEvaKpiForm">
			<table align="center">
				<tr>
					<td>
						指标名称
				...
				<tr>
					...
					<td colspan="3">
						<input type="textarea" id="algorithm" name="algorithm"
							...
							value="${siteEvaKpiForm.algorithm}" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td colspan="3">
						<input type="textarea" id="remark" name="remark" class="textarea"
							style="width: 88%" value="${siteEvaKpiForm.remark}" />
					</td>
				</tr>
				<tr>
					<td colspan="4">
			...
			<input type="hidden" id="id" name="id" value="${siteEvaKpiForm.id}" />
		...
		<caption>
			<div class="header center">
				<%
					...
					ISiteEvaTreeMgr siteEvaTreeMgr = (ISiteEvaTreeMgr) ApplicationContextHolder
					...
					//if (SiteEvaConstants.NODETYPE_TEMPLATETYPE.equals(nodeType)) {
						tempname = siteEvaTreeMgr.id2Name(parentNodeId);
					//}else{
					//	SiteEvaTree siteEvaTree = siteEvaTreeMgr.getTreeNodeByNodeId(parentNodeId);
					//	SiteEvaKpi kpi = new SiteEvaKpi();
					//	kpi = kpiMgr.getKpi(siteEvaTree.getObjectId());
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a
							href="${app}/partner/siteEva/siteEvaKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url='${app}/partner/siteEva/siteEvaKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
