...
<%@page import="com.boco.eoms.partner.siteEva.webapp.form.SiteEvaKpiInstanceForm"%>
<%@page import="com.boco.eoms.base.util.StaticMethod"%>
<%@ include file="/common/taglibs.jsp"%>
<%@ include file="/common/header_eoms_form.jsp"%>
<%@ page language="java" import="java.util.*"%>
<%@ page language="java" import="com.boco.eoms.partner.siteEva.util.SiteEvaStaticMethod"%>

<script type="text/javascript">
Ext.onReady(function(){
	v = new eoms.form.Validation({form:'siteEvaKpiInstanceForm'});
})
</script>

<html:form action="/siteEvaTasks.do?method=saveTaskDetail" styleId="siteEvaKpiInstanceForm" method="post">
	...
	<tr>
		...
		<% 
		   ...
		   for(int i=0;i<parIdss.size();i++){
			   String parName = SiteEvaStaticMethod.orgId2Name(String.valueOf(parIdss.get(i)), "dept");
		%>
		...
		<tr>
			<% 
			    ...
			    for(int k=0;k<rowList.size();k++){
			    	SiteEvaKpiInstanceForm ekif = (SiteEvaKpiInstanceForm) rowList.get(k);
			%>			
				<td rowspan="<%=ekif.getRowspan()%>" colspan="<%=ekif.getColspan()%>" style="vertical-align:middle;text-align:left">
					<eoms:id2nameDB id="<%=ekif.getKpiId()%>" beanId="siteEvaKpiDao" />(<%=ekif.getWeight()%>分)
				</td>
				<%if (SiteEvaConstants.NODE_LEAF.equals(ekif.getLeaf())) { %>
					<td>
						<%=ekif.getAlgorithm()%>
					</td>
					<% 
					   for(int j=0;j<parIdss.size();j++){
						   ...
						   String parName = SiteEvaStaticMethod.orgId2Name(parIds, "dept");
						   ...
						   reduceReason = StaticMethod.null2String(realScore);
					%>			
						<%if(SiteEvaConstants.TASK_PUBLISHED.equals(ekif.getIsPublish())){%>
					<td>
...
function  save()
    {
      document.forms[0].action="siteEvaTasks.do?method=saveTaskDetail";
    };
function  commit()
    {
      document.forms[0].action="siteEvaTasks.do?method=commitTaskDetail";
    };
function  runBack()
    {
   	  v.passing="true";
      ...
      document.forms[0].action="siteEvaTasks.do?method=xquery&searchType=1";
    };
function  queryBack()
    {
      v.passing="true";
      document.getElementById("taskId").value = '' ;
      document.forms[0].action="siteEvaTasks.do?method=xquery&searchType=0";
    };
</script>