...
function changeTemplateType()
		{    
			...
			var url="<%=request.getContextPath()%>/partner/siteEva/siteEvaTasks.do?method=changeTemplateType&templateTypeId="+templateTypeId;
...
function isSubmit(){
    if(checkSiteEvaTime())return true;//document.getElementById("siteEvaKpiInstanceForm").submit();
    else if(!checkSiteEvaTime()){
       alert("考核月份在模板激活时间之后！");
       return false;
    }
}
...
<html:form action="/siteEvaTasks.do?method=taskDetailList" styleId="siteEvaKpiInstanceForm" method="post" > 
<table class="formTable" id="form">
			...
			<select name="templateTypeId" id="templateTypeId"
					...
					<logic:iterate id="templateType" name="templateType">
						<option value="${templateType.nodeId}">
							<eoms:id2nameDB id="${templateType.nodeId}" beanId="siteEvaTreeDao" />
						</option>
					</logic:iterate>
				</select>
	...
	<tr>
		...
		<td class="content">
			<eoms:dict key="dict-partner-siteEva" dictId="cycle" defaultId="month" onchange="changeCycle();"
