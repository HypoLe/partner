...
function sendBox() {
	...
	function changeCheckBox(){
		...
		var app0type=myjs('#app0typeT').val();
		...
		if(app0type&&app0type!=''){
			myjs('#app0type').attr('checked',true);
			myjs('#app0type').attr('disabled','disabled');
			}
			else{
				if((myjs('#app0type').attr('disabled'))){
			myjs('#app0type').attr('checked',false);
			myjs('#app0type').attr('disabled','');
			}
			}
...
<eoms:xbox id="tree1" dataUrl="${app}/xtree.do?method=area" rootId="26"
	rootText='所属地市' valueField="d0area_nameId" handler="d0area_nameT" textField="d0area_nameT"
	checktype="area" single="false"></eoms:xbox>
	<eoms:xbox id="tree2" dataUrl="/partner/statistically/paternerStaff.do?method=dept&type=parent" rootId="2"
	rootText='合作伙伴公司（省）' valueField="dt0nameTId" handler="dt0nameT" textField="dt0nameT"
	checktype="dept" single="true"></eoms:xbox>
	<eoms:xbox id="tree3" dataUrl="/partner/statistically/paternerStaff.do?method=dept" rootId="2"
	rootText='合作伙伴（地市州）分公司' valueField="d0nameId" handler="d0nameT" textField="d0nameT"
	checktype="dept" single="true"></eoms:xbox>
<form  id="checkAndSearchFrom" action="paternerStaff.do?method=apparatusList" method="post">
...
<table class="formTable">
		...
		<tr>
		<td class="label">仪表类型</td>
			<td>
			<eoms:comboBox name="app0typeT" id="app0typeT"  initDicId="11204" defaultValue=""
			    alt="allowBlank:false" onchange="changeCheckBox()" />
			</td>	
		</tr>
...
<tr>
			...
			<td class="label">所属地市</td>
				...
				value="d0name" /></td>
				<td class="label">仪表类型</td>
				<td><input  id="app0type" type="checkbox" name="cardNumber"
				value="app0typeTypeLikedict" /></td>
