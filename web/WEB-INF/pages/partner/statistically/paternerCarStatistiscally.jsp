...
function sendBox() {
	...
	function changeCheckBox(){
		...
		if(c0use&&c0use!=''){
			myjs('#c0use').attr('checked',true);
			myjs('#c0use').attr('disabled','disabled');
			}
			else{
				if((myjs('#c0use').attr('disabled'))){
			myjs('#c0use').attr('checked',false);
			myjs('#c0use').attr('disabled','');
			}
			}
...
<eoms:xbox id="tree1" dataUrl="${app}/xtree.do?method=area" rootId="26"
	...
	checktype="dept" single="true"></eoms:xbox>
<form  id="checkAndSearchFrom" action="paternerStaff.do?method=carList" method="post">
...
<table class="formTable">
		...
		<td class="label">所属地市</td>    
			...
			<td><input type="text" class="text"
						...
						<td class="label">车辆用途</td>
			<td>
			<eoms:comboBox name="c0useT" id="c0useT"  initDicId="1121306" defaultValue=""
...
<tr>
			...
			<td class="label">所属地市</td>
				...
				value="d0name" /></td>
				<td class="label">车辆用途</td>
				<td><input  id="c0use" type="checkbox" name="cardNumber"
				value="c0useTypeLikedict" /></td>
