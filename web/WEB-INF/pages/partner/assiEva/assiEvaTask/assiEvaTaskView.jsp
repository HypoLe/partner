...
function changeTemplateType()
		{    
			...
			var url="<%=request.getContextPath()%>/partner/assiEva/assiEvaTasks.do?method=changeTemplateType&templateTypeId="+templateTypeId;
...
function isSubmit(){
    if(checkAssiEvaTime())return true;//document.getElementById("assiEvaKpiInstanceForm").submit();
    else if(!checkAssiEvaTime()){
       alert("考核月份在模板激活时间之后！");
       return false;
    }
}
...
<html:form action="/assiEvaTasks.do?method=taskDetailList" styleId="assiEvaKpiInstanceForm" method="post" > 
<table class="formTable" id="form">
			...
			<select name="templateTypeId" id="templateTypeId"
					...
					<logic:iterate id="templateType" name="templateType">
						<option value="${templateType.nodeId}">
							<eoms:id2nameDB id="${templateType.nodeId}" beanId="assiEvaTreeDao" />
						</option>
					</logic:iterate>
				</select>
	...
	<tr>
		...
		<td class="content">
			<eoms:dict key="dict-partner-assiEva" dictId="cycle" defaultId="month" onchange="changeCycle();"
