...
<%@page import="com.boco.eoms.partner.assiEva.webapp.form.AssiEvaKpiInstanceForm"%>
<%@page import="com.boco.eoms.base.util.StaticMethod"%>
<%@ include file="/common/taglibs.jsp"%>
<%@ include file="/common/header_eoms_form.jsp"%>
<%@ page language="java" import="java.util.*"%>
<%@ page language="java" import="com.boco.eoms.partner.assiEva.util.AssiEvaStaticMethod"%>

<script type="text/javascript">
Ext.onReady(function(){
	v = new eoms.form.Validation({form:'assiEvaKpiInstanceForm'});
})
</script>

<html:form action="/assiEvaTasks.do?method=saveTaskDetail" styleId="assiEvaKpiInstanceForm" method="post">
	...
	<tr>
		...
		<% 
		   ...
		   for(int i=0;i<parIdss.size();i++){
			   String parName = AssiEvaStaticMethod.orgId2Name(String.valueOf(parIdss.get(i)), "dept");
		%>
		...
		<tr>
			<% 
			    ...
			    for(int k=0;k<rowList.size();k++){
			    	AssiEvaKpiInstanceForm ekif = (AssiEvaKpiInstanceForm) rowList.get(k);
			%>			
				<td rowspan="<%=ekif.getRowspan()%>" colspan="<%=ekif.getColspan()%>" style="vertical-align:middle;text-align:left">
					<eoms:id2nameDB id="<%=ekif.getKpiId()%>" beanId="assiEvaKpiDao" />(<%=ekif.getWeight()%>分)
				</td>
				<%if (AssiEvaConstants.NODE_LEAF.equals(ekif.getLeaf())) { %>
					<td>
						<%=ekif.getAlgorithm()%>
					</td>
					<% 
					   for(int j=0;j<parIdss.size();j++){
						   ...
						   String parName = AssiEvaStaticMethod.orgId2Name(parIds, "dept");
					...
					%>			
						<%if(AssiEvaConstants.TASK_PUBLISHED.equals(ekif.getIsPublish())){%>
					<td>
...
function  save()
    {
      document.forms[0].action="assiEvaTasks.do?method=saveTaskDetail";
    };
function  commit()
    {
      document.forms[0].action="assiEvaTasks.do?method=commitTaskDetail";
    };
function  runBack()
    {
   	  v.passing="true";
      document.forms[0].action="assiEvaTasks.do?method=xquery&searchType=1";
    };
function  queryBack()
    {
      v.passing="true";
      document.getElementById("taskId").value = '' ;
      document.forms[0].action="assiEvaTasks.do?method=xquery&searchType=0";
    };
</script>