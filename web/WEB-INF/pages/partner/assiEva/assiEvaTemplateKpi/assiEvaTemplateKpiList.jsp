...
<%@page import="com.boco.eoms.partner.assiEva.mgr.*"%>
<%@page import="com.boco.eoms.partner.assiEva.util.*"%>
<%@page import="com.boco.eoms.partner.assiEva.model.*"%>
<style type="text/css">
body {
...
<div style="height: 400px">
	...
	<div id="kpiContent">
		<html:form action="/assiEvaKpis.do?method=saveKpi" styleId="assiEvaKpiForm">
			<table align="center">
				<tr>
					<td>
						指标名称
				...
				<tr>
					...
					<td colspan="3">
						<input type="textarea" id="algorithm" name="algorithm"
							...
							value="${assiEvaKpiForm.algorithm}" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td colspan="3">
						<input type="textarea" id="remark" name="remark" class="textarea"
							style="width: 88%" value="${assiEvaKpiForm.remark}" />
					</td>
				</tr>
				<tr>
					<td colspan="4">
			...
			<input type="hidden" id="id" name="id" value="${assiEvaKpiForm.id}" />
		...
		<caption>
			<div class="header center">
				<%
					...
					IAssiEvaTreeMgr assiEvaTreeMgr = (IAssiEvaTreeMgr) ApplicationContextHolder
					...
					//if (AssiEvaConstants.NODETYPE_TEMPLATETYPE.equals(nodeType)) {
						tempname = assiEvaTreeMgr.id2Name(parentNodeId);
					//}else{
					//	AssiEvaTree assiEvaTree = assiEvaTreeMgr.getTreeNodeByNodeId(parentNodeId);
					//	AssiEvaKpi kpi = new AssiEvaKpi();
					//	kpi = kpiMgr.getKpi(assiEvaTree.getObjectId());
		...
		<tbody>
			<logic:iterate id="kpiIt" name="kpiIt" indexId="index">
				<tr>
					...
					<td>
						<a
							href="${app}/partner/assiEva/assiEvaKpis.do?method=editKpi&parentNodeId=${requestScope.parentNodeId}&kpiId=${kpiIt.id}"
							title="编辑这个项目"> <img src="${app}/images/icons/edit.gif" /> </a>
					</td>
					<td>
						<a
							href="javascript:if(confirm('确定要从模板中移除该指标?')){var url='${app}/partner/assiEva/assiEvaKpis.do?method=delKpiFromTree&kpiId=${kpiIt.id}&parentNodeId=${requestScope.parentNodeId}';location.href=url}"
							title="删除这个项目"> <img
