...
Ext.onReady(function() {
	v = new eoms.form.Validation({form:'factoryDisposeForm'});
	setTabs();
});
	...
	function changeFacility(con){
			...
			Ext.Ajax.request({
								...
								success: function ( result, request ) { 
									...
									if(con==1){
										var equipmentType = '${factoryDisposeForm.equipmentType}';
										if(equipmentType!=''){
...
window.onload = function(){
    var speciality = '${factoryDisposeForm.speciality}';
	if(speciality!=''){
...
function saveImport() {
	...
	method : 'post',
	url : "${app}/partner/deviceAssess/factoryDisposes.do?method=importData",
	  	waitTitle : '请稍后...',
		waitMsg : '正在导入数据,请稍后...',
...
<div id="info-page">
	<div id="base-info" class="tabContent">
		...
		<div id="listQueryObject"
			...
			<form action="factoryDisposes.do?method=importData" method="post"
						...
						<tr>
							...
							<td>
								<input type="button" class="btn" value="下载导入文件模板" 
										onclick="javascript:location.href='${app}/partner/deviceAssess/factoryDisposes.do?method=download'"/>
							</td>
						</tr>
						<td class="label">
							选择审批人*
		...
		<br />
		<html:form action="/factoryDisposes.do?method=save" styleId="factoryDisposeForm" method="post"> 
			...
			<table id="sheet" class="formTable">
				...
				<tr>
					...
					<td class="content">
						<html:text property="sheetNum" styleId="sheetNum"
							...
							value="${factoryDisposeForm.sheetNum}" />
					</td>
					<td class="label">
						建单时间*
					</td>
					<td class="content">
						<html:text property="createTime" styleId="createTime"
							...
							alt="vtype:'lessThen',link:'changeTime',vtext:'建单时间不能晚于转派厂家时间',allowBlank:false"
							value="${factoryDisposeForm.createTime}" readonly="true" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content">
						<html:text property="changeTime" styleId="changeTime"
							styleClass="text medium"
							onclick="popUpCalendar(this,this,null,null,null,true,-1);"
							alt="vtype:'moreThen',link:'createTime',vtext:'建单时间不能晚于转派厂家时间',allowBlank:false"
							value="${factoryDisposeForm.changeTime}" readonly="true" />
					</td>
					<td class="label">
						归档时间*
					</td>
					<td class="content">
						<html:text property="pigeonholeTime" styleId="pigeonholeTime"
							styleClass="text medium"
							onclick="popUpCalendar(this,this,null,null,null,true,-1);"
							alt="vtype:'moreThen',link:'changeTime',vtext:'转派厂家时间不能晚于归档时间',allowBlank:false"
							value="${factoryDisposeForm.pigeonholeTime}" readonly="true" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content">
						<html:text property="affairName" styleId="affairName"
							styleClass="text medium" alt="allowBlank:false,vtext:''"
							value="${factoryDisposeForm.affairName}" />
					</td>
					<td class="label">
						事件级别*
					</td>
					<td class="content">
						<eoms:comboBox name="affairLevel" id="affairLevel"
							styleClass="select max" initDicId="1121501"
							defaultValue="${factoryDisposeForm.affairLevel}"
				...
				<tr>
					...
					<td class="content">
						<html:text property="province" styleId="province"
							styleClass="text medium" alt="allowBlank:false,vtext:''"
							value="${factoryDisposeForm.province}" />
					</td>
					<td class="label">
						发生地市*
					</td>
				...
				<tr>
					...
					<td class="content">
						<eoms:comboBox name="factory" id="factory" initDicId="1121502"
							...
							defaultValue="${factoryDisposeForm.factory}"
					...
					<td class="content">
						<eoms:comboBox name="speciality" id="speciality"
							...
							defaultValue="${factoryDisposeForm.speciality}"
				...
				<tr>
					...
					<td class="content">
						<html:text property="equipmentName" styleId="equipmentName"
							styleClass="text medium" alt="allowBlank:false,vtext:''"
							value="${factoryDisposeForm.equipmentName}" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content" colspan="3">
						<html:text property="equipmentVersion" styleId="equipmentVersion"
							styleClass="text medium" alt="allowBlank:false,vtext:''"
							value="${factoryDisposeForm.equipmentVersion}" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content">
						<html:text property="beginTime" styleId="beginTime"
							...
							value="${factoryDisposeForm.beginTime}" />
					</td>
					<td class="label">
						业务恢复时间*
					</td>
					<td class="content">
						<html:text property="resumeTime" styleId="resumeTime"
							...
							value="${factoryDisposeForm.resumeTime}" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content" colspan="3">
						<html:text property="removeTime" styleId="removeTime"
							...
							onclick="popUpCalendar(this,this,null,null,null,true,-1);"
							alt="vtype:'moreThen',link:'beginTime',vtext:'故障开始时不能晚于业务恢复时间或故障消除时间',allowBlank:false"
							value="${factoryDisposeForm.removeTime}" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="content" colspan="3">
						<html:text property="satisfaction" styleId="satisfaction"
									styleClass="text medium" alt="re:/^([1]{1}[0]{1}[0]{1}([.][0]{1,2})?)$|^([(0-9)]{1,2}([.][0-9]{1,2})?)$/,re_vt:'请输入小于等于100的正整数或小数(2位)',allowBlank:false,maxLength:5"
									value="${factoryDisposeForm.satisfaction}" />
					</td>	
				</tr>
				...
				<tr>
					...
					<td class="content" colspan="3">
						<input type="text" name="textReviewer" id="textReviewer" value="<eoms:id2nameDB id="${factoryDisposeForm.deviceAssessApprove.approveUser }" beanId="tawSystemUserDao" />"
							class="text" readonly="readonly" alt="allowBlank:false" />
						<input type="button" name="approvalButton" id="approvalButton"
							value="请选择审核人" class="btn" alt="allowBlank:false" />
						<input type="hidden" name="approvalUser" id="approvalUser" value="${factoryDisposeForm.deviceAssessApprove.approveUser }"/>
			...
			<html:hidden property="faultLong" value="${factoryDisposeForm.faultLong}" />
			<html:hidden property="resumeLong" value="${factoryDisposeForm.resumeLong}" />
			<html:hidden property="amount" value="1" />
			<html:hidden property="id" value="${factoryDisposeForm.id}" />
		...
		<logic:notEmpty name="dacList" scope="request">
			<display:table name="dacList" cellspacing="0" cellpadding="0"
				...
				<display:column headerClass="sortable" title="事件名">
					<eoms:id2nameDB id="${dacList.assessId}" beanId="factoryDisposeDao" />
				</display:column>
