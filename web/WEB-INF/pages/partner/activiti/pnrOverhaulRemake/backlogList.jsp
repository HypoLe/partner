...
<script type="text/javascript">
   ...
   function archive(){
       if(confirm("确定不需要手机端抽查，直接归档吗？")){
		...
		<div id="sheetform">
			<html:form action="/pnrOverhaulRemake.do?method=listBacklog"
				styleId="theform">
		...
		<h6><font color="red">总金额：${sumProjectAmout}</font></h6>
		<br/>
		...
		<display:table name="taskList" cellspacing="0" cellpadding="0"
			...
			<display:column 
				...
				<c:if test="${taskList.taskDefKey eq 'cityLineDirectorAudit' ||taskList.taskDefKey eq 'cityManageExamine' || taskList.taskDefKey eq 'cityManageDirectorAudit'||taskList.taskDefKey eq 'cityViceAudit'||taskList.taskDefKey eq 'provinceLineExamine'||taskList.taskDefKey eq 'provinceLineViceAudit'||taskList.taskDefKey eq 'provinceManageExamine'||taskList.taskDefKey eq 'provinceManageViceAudit'}">
			...
			<display:column sortable="false" headerClass="sortable" title="线路级别">
				<eoms:id2nameDB id="${taskList.lineType}" beanId="ItawSystemDictTypeDao" />
			</display:column>
...
<c:if test="${!empty taskList}">
	<eoms:excelExportToProcess modelName="com.boco.activiti.partner.process.po.TaskModel" serviceBeanId="pnrTransferOverhaulRemakeService" title="待回复工单" queryFlag="listBacklog" processKey="overHaulNewProcess" flag="backlog">		
			...
			<eoms:rowToProcess name="线路级别" value="lineType" dictDaoName="ItawSystemDictTypeDao"/>
