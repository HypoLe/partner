...
var v;
  ...
  function selectRes(){
		...
		var url = '${app}/activiti/pnrOltBbuRoom/pnrOltBbuRoom.do?method=showCommonRejectPage&sign=above';
		//var _sHeight = 180;
		var _sHeight = 200;
	    var _sWidth = 820;
...
<html:form action="/pnrOltBbuRoom.do?method=transferProgram" styleId="theform" >
			<!-- 带注释为驳回时需要的参数 -->
			<input type="hidden" id="marker" name="marker" value="provinceManageExamine" /><!-- 流程标识 -->	
            ...
            <input type="hidden" id="rejectState" name="rejectState" value="provinceManageChargeState#rollback"><!--驳回状态名值对  -->
			...
			<input type="hidden" name="interfaceCharge" value="superUser"><!-- 接口状态的工单放到superUser用户下 -->
			<input type="hidden" name="nextPerson" value="${pnrTransferOffice.oneInitiator}"><!-- 调用接口之前的最后一个环节给工单发起人发短信 -->
			<input type="hidden" id="nextTaskLinkName" name="nextTaskLinkName" value="等待接口调用"><!-- 发短信指定派发到的环节名 -->
			 		...
			 		<tr>
						...
						<td class="content" >
							<eoms:attachment name="sheetMain" property="sheetAccessories" 
					            scope="request" idField="provinceManageExamine" appCode="pnrActTransferOffice" alt="allowBlank:false;请选择保存的附件"/> 
						</td>
			 			<td class="label">
							审批人
						</td>
