...
<table id="sheet1" class="formTable" >
		...
		<tr>
			...
			<td class="content" >
				<c:if test="${item.linkName eq 'need'}">区县公司发起</c:if>
				...
				<c:if test="${item.linkName eq 'waitInterface'}">等待接口调用</c:if>
				<c:if test="${item.linkName eq 'sendOrder'}">派发代维</c:if>
				<c:if test="${item.linkName eq 'worker'}">施工队施工回单</c:if>
				<c:if test="${item.linkName eq 'orderAudit'}">区县公司验收</c:if>
				<c:if test="${item.linkName eq 'daiweiAudit'}">市线路主管审批</c:if>			
				<c:if test="${item.linkName eq 'cityManagefile'}">市运维主管审核归档</c:if>	
			</td>
