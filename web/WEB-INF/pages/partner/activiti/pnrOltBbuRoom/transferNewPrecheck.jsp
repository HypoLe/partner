...
var v;
  ...
  Ext.onReady(function(){
  	//起点照片回显
 	var valuesStart = "${startPhotoList}";
	if (valuesStart!=null){
	var startPhotoDiv =  document.getElementById("startPhotoDiv");  	
		startPhotoDiv.style.display = "block";
	}
	
	//终点照片回显
 	var valuesEnd = "${endPhotoList}";
	if (valuesEnd!=null){
	var endPhotoDiv =  document.getElementById("endPhotoDiv");  	
		endPhotoDiv.style.display = "block";
	}
	
   v = new eoms.form.Validation({form:'theform'});
   
     v.custom = function(){ 
     	var tempProjectAmount=jq("#projectAmount").val();//项目金额估算
     	//alert("tempProjectAmount="+tempProjectAmount);
     	var tempLineTotalInvestment=jq("#lineTotalInvestment").val();//线路总投资估算
     	//alert("tempLineTotalInvestment="+tempLineTotalInvestment);
     	if(parseFloat(tempProjectAmount) > parseFloat(tempLineTotalInvestment)){
     		alert("项目金额估算不能大于线路总投资估算");
     		return false;
     	}
     
	   if(jq(window.frames["UIFrame1-newLineRoutingDiagram"].document).find("input:checkbox[name='files']").length < 1){
  			alert("请上传新建线路路由简图");
  			return false;
		}
	     return true;
...
<script type="text/javascript">
	//工单类型变化函数
	function changeJobOrderType() {
		var value = jq("#jobOrderType").val();
		//alert(value);
		if (value == "olt") {
			//当选择OLT机房优化清单，显示 机房内OLT数量 、总用户数 、语音用户 、宽带用户、IPTV用户 、机房内有无BBU
			jq("tr[id='jfBasicInforTwoTr']").css('display', 'block');
			jq("tr[id='jfBasicInforTwoTr']").find("input").val("");
			jq("tr[id='jfBasicInforThreeTr']").css('display', 'block');
			jq("tr[id='jfBasicInforThreeTr']").find("input").val("");

			jq("td[id='broadbandUserTextTd']").css('display', 'block');
			jq("td[id='broadbandUserInputTd']").css('display', 'block');
			jq("#broadbandUser").val("");

			jq("td[id='iptvUserTextTd']").css('display', 'block');
			jq("td[id='iptvUserInputTd']").css('display', 'block');
			jq("#iptvUser").val("");

			jq("td[id='isNoBbuTextTd']").css('display', 'block');
			jq("td[id='isNoBbuInputTd']").css('display', 'block');
			jq("#isNoBbu").val("");

			//隐藏 机房内BBU数量,并清空值
			jq("td[id='bbuNumberTextTd']").css('display', 'none');
			jq("td[id='bbuNumberInputTd']").css('display', 'none');
			jq("#bbuNumber").val("");//清空值
			
			jq("td[id='boardDemandTextTd']").css('display', 'block');
			jq("td[id='boardDemandInputTd']").css('display', 'block');
			jq("#boardDemand").val("");
			jq("td[id='lightModuleDemandTextTd']").css('display', 'block');
			jq("td[id='lightModuleDemandInputTd']").css('display', 'block');
			jq("#lightModuleDemand").val("");
			
			jq("td[id='transBoardDemandTextTd']").css('display', 'none');
			jq("td[id='transBoardDemandInputTd']").css('display', 'none');
			jq("#transBoardDemand").val("空");
			jq("td[id='transLightModuleDemandTextTd']").css('display', 'none');
			jq("td[id='transLightModuleDemandInputTd']").css('display', 'none');
			jq("#transLightModuleDemand").val("空");
			
			
			
			//jq("td[id='bbuNumberInputTd']").removeAttr('colspan');
			
		} else if (value == "bbu") {
			//当选择BBU机房优化清单，显示 机房内BBU数量
			jq("tr[id='jfBasicInforThreeTr']").css('display', 'block');
			jq("tr[id='jfBasicInforThreeTr']").find("input").val("");
			jq("td[id='bbuNumberTextTd']").css('display', 'block');
			jq("td[id='bbuNumberInputTd']").css('display', 'block');
			jq("#bbuNumber").val("");//清空值

			//隐藏 机房内OLT数量 、总用户数 、语音用户 、宽带用户、IPTV用户 、机房内有无BBU
			jq("tr[id='jfBasicInforTwoTr']").css('display', 'none');
			jq("tr[id='jfBasicInforTwoTr']").find("input").val("");

			jq("td[id='broadbandUserTextTd']").css('display', 'none');
			jq("td[id='broadbandUserInputTd']").css('display', 'none');
			jq("#broadbandUser").val("");

			jq("td[id='iptvUserTextTd']").css('display', 'none');
			jq("td[id='iptvUserInputTd']").css('display', 'none');
			jq("#iptvUser").val("");

			jq("td[id='isNoBbuTextTd']").css('display', 'none');
			jq("td[id='isNoBbuInputTd']").css('display', 'none');
			jq("#isNoBbu").val("");
			
			jq("td[id='boardDemandTextTd']").css('display', 'none');
			jq("td[id='boardDemandInputTd']").css('display', 'none');
			jq("#boardDemand").val("空");
			jq("td[id='lightModuleDemandTextTd']").css('display', 'none');
			jq("td[id='lightModuleDemandInputTd']").css('display', 'none');
			jq("#lightModuleDemand").val("空");
			
			jq("td[id='transBoardDemandTextTd']").css('display', 'block');
			jq("td[id='transBoardDemandInputTd']").css('display', 'block');
			jq("#transBoardDemand").val("");
			jq("td[id='transLightModuleDemandTextTd']").css('display', 'block');
			jq("td[id='transLightModuleDemandInputTd']").css('display', 'block');
			jq("#transLightModuleDemand").val("");
			
			
			//jq("td[id='bbuNumberInputTd']").attr('colspan', '5');

		} else if (value = "oltbbu") {
			//当选择BBU机房优化清单，显示 机房内OLT数量 、总用户数 、语音用户 、宽带用户、IPTV用户 、机房内BBU数量
			jq("tr[id='jfBasicInforTwoTr']").css('display', 'block');
			jq("tr[id='jfBasicInforTwoTr']").find("input").val("");
			jq("tr[id='jfBasicInforThreeTr']").css('display', 'block');
			jq("tr[id='jfBasicInforThreeTr']").find("input").val("");
		...
		//清空文本框
			...
			jq("#jfCity").val("");
			jq("#jfCountry").val("");
			jq("#jfAddressName").val("");
	}
	//按照工单类型初始化机房基本信息字段
	function initJobOrderType() {
		var value = jq("#jobOrderType").val();
		//alert(value);
		if (value == "olt") {
			//当选择OLT机房优化清单，显示 机房内OLT数量 、总用户数 、语音用户 、宽带用户、IPTV用户 、机房内有无BBU
...
<!--
		//选择局址名称
	function selectOltBbuRool() {
		var themeInterface=document.getElementById("jobOrderType").value;
		var oltBbuRoomId=document.getElementById("oltBbuRoomId").value;
		//var tempAssetIds=document.getElementById("assetIds").value;
	    var url = '${app}/activiti/pnrOltBbuRoom/pnrOltBbuRoom.do?method=selectOltBbuRool&themeInterface='+themeInterface+'&oltBbuRoomId='+oltBbuRoomId;
	    var _sHeight = 600;
	    var _sWidth = 820;
	    var sTop = (window.screen.availHeight - _sHeight) / 2;
	    var sLeft = (window.screen.availWidth - _sWidth) / 2;
	...
	function setOltBbuRoom(selectedOltBbuRoomlist){
		for(var i=0;i<selectedOltBbuRoomlist.length;i++){
		
			
			var id = selectedOltBbuRoomlist[i].id;
			var themeInterface = selectedOltBbuRoomlist[i].themeInterface;
			var city = selectedOltBbuRoomlist[i].city;
			var country = selectedOltBbuRoomlist[i].country;
			var addressName = selectedOltBbuRoomlist[i].addressName;
			var oltNumber = selectedOltBbuRoomlist[i].oltNumber;
			var totalUserNumber = selectedOltBbuRoomlist[i].totalUserNumber;
			var voiceUser = selectedOltBbuRoomlist[i].voiceUser;
			var broadbandUser = selectedOltBbuRoomlist[i].broadbandUser;
			var iptvUser = selectedOltBbuRoomlist[i].iptvUser;
			var isNoBbu = selectedOltBbuRoomlist[i].isNoBbu;
			var bbuNumber = selectedOltBbuRoomlist[i].bbuNumber;
			var lineTotalInvestment = selectedOltBbuRoomlist[i].lineTotalInvestment; 
			if(themeInterface=='olt'){
				document.getElementById("oltBbuRoomId").value=id; 
				document.getElementById("jfCity").value=city; 
				document.getElementById("jfCountry").value=country;
				document.getElementById("jfAddressName").value=addressName;
				document.getElementById("oltNumber").value=oltNumber;
				document.getElementById("totalUserNumber").value=totalUserNumber;
				document.getElementById("voiceUser").value=voiceUser;
				document.getElementById("broadbandUser").value=broadbandUser;
				document.getElementById("iptvUser").value=iptvUser;
				document.getElementById("isNoBbu").value=isNoBbu;
				document.getElementById("lineTotalInvestment").value=lineTotalInvestment;
			}else if(themeInterface=='bbu'){
				document.getElementById("oltBbuRoomId").value=id; 
				document.getElementById("jfAddressName").value=addressName;
				document.getElementById("jfCity").value=city; 
				document.getElementById("jfCountry").value=country;
				document.getElementById("bbuNumber").value=bbuNumber;
				document.getElementById("lineTotalInvestment").value=lineTotalInvestment;
			}else if(themeInterface=='oltbbu'){
				document.getElementById("oltBbuRoomId").value=id; 
				document.getElementById("jfCity").value=city; 
				document.getElementById("jfCountry").value=country;
				document.getElementById("jfAddressName").value=addressName;
	...
	//验证发起人电话
	function checkInitiatorMobilePhone(value){
		//手机号
		var reMobile=/^0?(13[0-9]|15[012356789]|18[0-9]|17[0-9]|14[57])[0-9]{8}$/;
		//固话 
		var rePhone=/^0\d{2,3}-?\d{7,8}$/;
		if(value==''){
			alert("发起人电话不可以为空!");
			return;
		}else if(!reMobile.test(value)&&!rePhone.test(value)){
			alert("电话号码格式不正确!");
			document.getElementById("initiatorMobilePhone").value="";
			return;
		}
	}
	
	//验证新建杆路长度
	function checkNewBuiltRodLength(value){
		var a=/^(\d+)(\.\d+)?$/;
		if(value==''){
			alert("新建杆路长度不可以为空!");
			return;
		...
		if(!a.test(value)){
			alert("新建杆路长度请输入整数或小数");
			document.getElementById("newBuiltRodLength").value=""; 
			return;
		}
		//if(value>1000){
			//alert("新建杆路长度不能大于1000M");
			//document.getElementById("newBuiltRodLength").value="";
			//return;
		//}
	}
	...
	function checkCableClothLength(value){
		var a=/^(\d+)(\.\d+)?$/;
		if(value==''){
			alert("光缆布放长度不可以为空!");
			return;
		...
		if(!a.test(value)){
			alert("光缆布放长度请输入整数或小数");
			document.getElementById("cableClothLength").value=""; 
			return;
		}
		//if(value>5){
			//alert("光缆布放长度不能大于5KM");
...
<html:form action="/pnrOltBbuRoom.do?method=performAdd" styleId="theform" >
    ...
    <input id="themeInterface" type="hidden" name="themeInterface" value="oltBbuProcess"><!-- 流程标识 -->
    ...
    <input type="hidden" id="nextTaskLinkName" name="nextTaskLinkName" value="市线路主管审核"><!-- 发短信指定派发到的环节名 -->
    <!-- 新建派单环节的乙方费用总额 -->
    <input type="hidden" id="sumCostOfPartyB"  name="sumCostOfPartyB"  value="<fmt:formatNumber value='${pnrTransferOffice.sumNeedCostOfPartyB!=null&&pnrTransferOffice.sumNeedCostOfPartyB!=""?pnrTransferOffice.sumNeedCostOfPartyB:"0"}' pattern='##.##' maxFractionDigits='2' minFractionDigits='0' />" />
    <!-- 基本信息 -->
		...
		<table id="sheet" class="formTable">
			<tr>
				...
				<td colspan="5">
					<c:if test="${pnrTransferOffice.theme ==null}">
						<input type="text" name="title" class="text max" id="title" value="" alt="allowBlank:false,maxLength:120,vtext:'请输入工单名称，最大长度为60个汉字！'" />
					</c:if>
					<c:if test="${pnrTransferOffice.theme !=null}">
			...
			<tr>
				...
				<td class="content" style="width: 24%">
					<input type="text" class="text" id="initiatorMobilePhone" name="initiatorMobilePhone" value="${pnrTransferOffice.initiatorMobilePhone}"  alt="allowBlank:false,vtext:'请输入发起人电话！'" onblur="checkInitiatorMobilePhone(this.value)" />
				</td>
			</tr>
			...
			<tr>
				...
				<td class="content" style="width:15%">
					<select id="jobOrderType" name="jobOrderType" class="select" onchange="changeJobOrderType()">
						<option value="olt" <c:if test="${pnrTransferOffice.jobOrderType eq 'olt'}">selected="selected"</c:if>>
							OLT机房优化清单
						</option>
						<option value="bbu" <c:if test="${pnrTransferOffice.jobOrderType eq 'bbu'}">selected="selected"</c:if>>
							BBU机房优化清单
						</option>
						<option value="oltbbu" <c:if test="${pnrTransferOffice.jobOrderType eq 'oltbbu'}">selected="selected"</c:if>>
							OLT、BBU共站址机房
						</option>
					</select>
				</td>
	...
	<!-- 机房基本信息 -->
	<fieldset style="border-width: 2px; border-color: #98C0F4;">
		<input type="hidden" id="oltBbuRoomId" name="oltBbuRoomId" value="${pnrTransferOffice.roomId}">
		<legend>
			<B>机房基本信息</B>
		</legend>
		<table id="sheet" class="formTable">
			<tr>
				<td class="label" style="width: 10%">
					地市*${pnrTransferOffice.jfCity}
				</td>
				<td class="content" style="width: 23%">
					<input type="text" readonly="readonly" class="text" id="jfCity"  name="jfCity" value="${pnrTransferOffice.jfCity}" />
				</td>
				<td class="label" style="width: 10%">
					区县*
				</td>
				<td class="content" style="width: 23%">
					<input type="text" readonly="readonly" class="text" id="jfCountry"  name="jfCountry" value="${pnrTransferOffice.jfCountry}"/>
				</td>
				<td class="label" style="width: 10%">
					设备位置/局址名称*
				</td>
				<td class="content" style="width: 24%">
					<input type="text" readonly="readonly" class="text" id="jfAddressName"  name="jfAddressName" value="${pnrTransferOffice.jfAddressName}" alt="allowBlank:flase,vtext:'请选择设备位置/局址名称！'" />
					<input type="button" class="btn" id="assetsButton" name="assetsButton" value="选择" onclick="selectOltBbuRool()" />
				</td>			
			</tr>
		
			 <tr id="jfBasicInforTwoTr" style="display:block">
				<td class="label" style="width: 10%" id="oltNumberTextTd" style="display:block">
					机房内OLT数量*
				</td>
				<td class="content" style="width: 23%" id="oltNumberInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="oltNumber"  name="oltNumber" value="${pnrTransferOffice.oltNumber}" />
				</td>
				<td class="label" style="width: 10%" id="totalUserNumberTextTd" style="display:block">
					总用户数*
				</td>
				<td class="content" style="width: 23%" id="totalUserNumberInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="totalUserNumber"  name="totalUserNumber" value="${pnrTransferOffice.totalUserNumber}" />
				</td>
				<td class="label" style="width: 10%" id="voiceUserTextTd" style="display:block">
					语音用户数*
				</td>
				<td class="content" style="width: 24%" id="voiceUserInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="voiceUser"  name="voiceUser" value="${pnrTransferOffice.voiceUser}" />
				</td>
			</tr>
			<tr id="jfBasicInforThreeTr" style="display:block">
				<td class="label" style="width: 10%" id="broadbandUserTextTd" style="display:block">
					宽带用户数*
				</td>
				<td class="content" style="width: 23%" id="broadbandUserInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="broadbandUser"  name="broadbandUser" value="${pnrTransferOffice.broadbandUser}" />
				</td>
				<td class="label" style="width: 10%" id="iptvUserTextTd" style="display:block">
					IPTV用户数*
				</td>
				<td class="content" style="width: 23%" id="iptvUserInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="iptvUser"  name="iptvUser" value="${pnrTransferOffice.iptvUser}" />
				</td>
				<td class="label" style="width: 10%" id="isNoBbuTextTd" style="display:block">
					机房内有无BBU*
				</td>
				<td class="content" style="width: 24%" id="isNoBbuInputTd" style="display:block">
					<input type="text" readonly="readonly" class="text" id="isNoBbu"  name="isNoBbu" value="${pnrTransferOffice.isNoBbu}" />
				</td>
				
				<td class="label" style="width: 10%" id="bbuNumberTextTd" style="display:none">
					机房内BBU数量 *
				</td>
				<td class="content" style="width: 24%" id="bbuNumberInputTd" style="display:none">
					<input type="text" readonly="readonly" class="text" id="bbuNumber"  name="bbuNumber" value="${pnrTransferOffice.bbuNumber}" />
				</td>
	...
	<fieldset style="border-width: 2px; border-color: #98C0F4;">
		...
		<table id="sheet" class="formTable">
			<tr>
				...
				<td class="content" style="width: 23%">
					<input type="text" id="isNeedRodInvestment" name="isNeedRodInvestment" class="text" value="${pnrTransferOffice.isNeedRodInvestment}" alt="allowBlank:false,maxLength:60,vtext:'请输入是否需要杆路投资，最大长度为30个汉字！'"/>
				</td>
				...
				<td class="content" style="width: 23%">
					<input type="text" id="isNeedCableInvestment" name="isNeedCableInvestment" class="text" value="${pnrTransferOffice.isNeedCableInvestment}" alt="allowBlank:false,maxLength:60,vtext:'请输入是否需要光缆投资，最大长度为30个汉字！'"/>
				</td>
				<td class="label" style="width: 10%">
					新建杆路长度*
				</td>
				<td class="content" style="width: 24%">
					<input type="text" id="newBuiltRodLength" name="newBuiltRodLength" class="text" value="${pnrTransferOffice.newBuiltRodLength}" alt="allowBlank:false,vtext:'请输入新建杆路长度！'" onblur="checkNewBuiltRodLength(this.value)" />
				</td>				
			</tr>
			...
			<tr>
				...
				<td class="content" style="width: 23%">
					<input type="text" id="newBuiltRodInvestment" name="newBuiltRodInvestment" class="text" value="${pnrTransferOffice.newBuiltRodInvestment}"  alt="allowBlank:false,maxLength:20,vtext:'请输入新建杆路投资！'" onblur="checkMainEngineeringQuantity(this,'新建杆路投资');" />
				</td>
				<td class="label" style="width: 10%">
					原光缆路由简述*（如A-B-C-D）
				</td>
				<td class="content" style="width: 23%">
					<input type="text" id="originalCableRoute" name="originalCableRoute" class="text" value="${pnrTransferOffice.originalCableRoute}" alt="allowBlank:false,maxLength:200,vtext:'请输入原光缆路由简述，最大长度为100个汉字！'"/>
				</td>
				<td class="label" style="width: 10%">
					新光缆路由简述*（如A-E-F-D）
				</td>
				<td class="content" style="width: 24%">
					<input type="text" id="newCableRoute" name="newCableRoute" class="text" value="${pnrTransferOffice.newCableRoute}" alt="allowBlank:false,maxLength:200,vtext:'请输入新光缆路由简述，最大长度为100个汉字！'"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="content" style="width: 23%">
					<input type="text" id="newParagraph" name="newParagraph" class="text" value="${pnrTransferOffice.newParagraph}" alt="allowBlank:true,maxLength:100,vtext:'请输入新光缆路由简述，最大长度为50个汉字！'"/>
				</td>
				<td class="label" style="width: 10%">
					光缆布放芯数*
				</td>
				<td class="content" style="width: 23%">
					<input type="text" id="cableClothCoreNumber" name="cableClothCoreNumber" class="text" value="${pnrTransferOffice.cableClothCoreNumber}" alt="allowBlank:false,maxLength:20,vtext:'请输入光缆布放芯数！'" onblur="checkMainEngineeringQuantity(this,'光缆布放芯数');"/>
				</td>
				<td class="label" style="width: 10%">
					光缆布放长度*
				</td>
				<td class="content" style="width: 24%">
					<input type="text" id="cableClothLength" name="cableClothLength" class="text" value="${pnrTransferOffice.cableClothLength}" alt="allowBlank:false,maxLength:20,vtext:'请输入光缆布放长度！'" onblur="checkCableClothLength(this.value)"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="content" style="width: 23%">
					<input type="text" id="cableInvestment" name="cableInvestment" class="text" value="${pnrTransferOffice.cableInvestment}" alt="allowBlank:false,maxLength:20,vtext:'请输入光缆投资估算！'" onblur="checkMainEngineeringQuantity(this,'光缆投资估算');"/>
				</td>
				<td class="label" style="width: 10%">
					线路总投资估算*
				</td>
				<td class="content" style="width: 23%">
					<input type="text" readonly="readonly" id="lineTotalInvestment" name="lineTotalInvestment" class="text" value="${pnrTransferOffice.lineTotalInvestment}" alt="allowBlank:false,maxLength:20,vtext:'请输入线路总投资估算！'" onblur="checkMainEngineeringQuantity(this,'线路总投资估算');"/>
				</td>
				
				 <td class="label" style="width: 10%" id="boardDemandTextTd" style="display:block">
					设备板卡需求*
				</td>
				<td class="content" style="width: 24%" id="boardDemandInputTd" style="display:block">
					<input type="text" id="boardDemand" name="boardDemand" class="text" value="${pnrTransferOffice.boardDemand}" alt="allowBlank:false,maxLength:200,vtext:'请输入设备板卡需求，最大长度为100个汉字！'"/>
				</td>
				
				<td class="label" style="width: 10%" id="transBoardDemandTextTd" style="display:none">
					传输设备板卡需求*
				</td>
				<td class="content" style="width: 24%" id="transBoardDemandInputTd" style="display:none">
					<input type="text" id="transBoardDemand" name="transBoardDemand" class="text" value="${pnrTransferOffice.transBoardDemand}" alt="allowBlank:false,maxLength:200,vtext:'请输入传输设备板卡需求，最大长度为100个汉字！'"/>
				</td>
			</tr>
			<tr>
				 <td class="label" style="width: 10%" id="lightModuleDemandTextTd" style="display:block">
					设备光模块需求*
				</td>
				<td class="content" style="width: 23%" id="lightModuleDemandInputTd" style="display:block">
					<input type="text" id="lightModuleDemand" name="lightModuleDemand" class="text" value="${pnrTransferOffice.lightModuleDemand}" alt="allowBlank:false,maxLength:200,vtext:'请输入设备光模块需求，最大长度为100个汉字！'"/>
				</td> 
				
				<td class="label" style="width: 10%" id="transLightModuleDemandTextTd" style="display:none">
					传输设备光模块需求*
				</td>
				<td class="content" style="width: 23%" id="transLightModuleDemandInputTd" style="display:none">
					<input type="text" id="transLightModuleDemand" name="transLightModuleDemand" class="text" value="${pnrTransferOffice.transLightModuleDemand}" alt="allowBlank:false,maxLength:200,vtext:'请输入传输设备光模块需求，最大长度为100个汉字！'"/>
				</td>
				...
				<td class="content" style="width: 23%" colspan="3">
					<input type="text" id="equipmentInvestment" name="equipmentInvestment" class="text" value="${pnrTransferOffice.equipmentInvestment}" alt="allowBlank:false,maxLength:20,vtext:'请输入设备类投资估算！'" onblur="checkMainEngineeringQuantity(this,'设备类投资估算');"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" style="width:90%">
					<textarea id="jfRemark" name="jfRemark" class="textarea max" alt="allowBlank:true,maxLength:1000,vtext:'请输入备注，最大长度为500个汉字！'">${pnrTransferOffice.jfRemark}</textarea>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5">
					<eoms:attachment name="newLineRoutingDiagram" property="sheetAccessories"
						scope="request" idField="newLineRoutingDiagram"
			...
			<tr>
				...
				<td colspan="5">
					<eoms:attachment name="orderAttachment" property="sheetAccessories"
						scope="request" idField="orderAttachment"
						appCode="pnrActTransferOffice" alt="allowBlank:false;请选择保存的附件" />
		    ...
		    <tr>
				...
				<td colspan="5" style="width:90%">
					<div id="startPhotoDiv" style="display: none">
						<table id="startPhotoTable">
							${startPhotoList}
						</table>
					</div>
					<input class="text" type="hidden" name="startPhotoIds" readonly="true"
						id="startPhotoIds" alt="allowBlank:false" value="${startPhotoIds}" />
					<!-- <input type="hidden" name="mainResId" id="mainResId" value="" /> -->
					<input type="button" class="btn" id="startPhotoButton" name="startPhotoButton" value="选择" onclick="selectPhoto('startPhoto','start')" />
				</td>
			</tr>
			<tr>
				...
				<td colspan="5" style="width:90%">
					<div id="endPhotoDiv" style="display: none">
						<table id="endPhotoTable">
							${endPhotoList}
						</table>
					</div>
					<input class="text" type="hidden" name="endPhotoIds" readonly="true"
						id="endPhotoIds" alt="allowBlank:false" value="${endPhotoIds}" />
					<!-- <input type="hidden" name="mainResId" id="mainResId" value="" /> -->
					<input type="button" class="btn" id="endPhotoButton" name="endPhotoButton" value="选择" onclick="selectPhoto('endPhoto','end')" />
				</td>
		    </tr>
		</table>
	</fieldset>
...
<br/>
	...
	*  打开图片选择页面
	*/
	function selectPhoto(tagName,photoSubType){	
		var selectedPhotoIds=jq("#"+tagName+"Ids").val();
		var photoType="oltBbuProcess";	//oltbbu
		var url ='${app}/activiti/pnrTransferPrecheck/pnrTransferNewPrecheck.do?method=conditionSelectPhoto&selectedPhotoIds='+selectedPhotoIds+'&tagName='+tagName+'&photoType='+photoType+'&photoSubType='+photoSubType;
		//var url = '${app}/activiti/pnrOltBbuRoom/pnrOltBbuRoom.do?method=conditionSelectPhoto&selectedPhotoIds='+selectedPhotoIds+'&tagName='+tagName+'&photoType='+photoType+'&photoSubType='+photoSubType;
		var _sHeight = 600;
	    var _sWidth = 820;
	    var sTop=(window.screen.availHeight-_sHeight)/2;
	    var sLeft=(window.screen.availWidth-_sWidth)/2;
	...
	* 设置图片名称和ID
	*/
	function setPhoto(photoes,photoIds,tagName,photoSubType){
	var photoId = document.getElementById(tagName+"Ids");
	photoId.value=photoIds;
		if(photoes.length>0&&photoes.length<=2){
				alert("您选择的图片数量不足三张，请重新选择！");
				}else if(photoes.length>2){
				var photoDiv = document.getElementById(tagName+"Div");
					photoDiv.style.display="block";
					deleteAllChild(tagName,photoSubType);
				for(var i=0;i<photoes.length;i++){
					...
					addMess(longitude,latitude,createtime,userId,tagName,photoSubType);
					}
				}
			}
	
	
	function addMess(longitude,latitude,createtime,userId,tagName,photoSubType){
		
		var myTable = document.getElementById(tagName+"Table");
	...
	function deleteAllChild(tagName,photoSubType){
