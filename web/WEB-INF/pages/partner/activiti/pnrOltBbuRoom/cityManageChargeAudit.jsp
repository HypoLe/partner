...
var v;
   ...
   //市运维主管审批的时候,对项目金额进行判断	
	function validateBudgetAmount(){
		...
		jq.ajax({ 
			...
			data:{"processInstanceId":tempProcessInstanceId},
			dataType:"json",
			success:function(data){
				if(data.result=="exceed"){//超出	审核不通过
				   alert("无法审批通过，已临近本月预算金额封顶值");
				   rel= false;
				}
  ...
  function refresh(){//此刷新函数被弹出的子模态窗口调用。
	    ...
	    window.location.href = "${app}/activiti/pnrOltBbuRoom/pnrOltBbuRoom.do?method="+method+condition;
  }
...
<html:form action="/pnrOltBbuRoom.do?method=transferProgram" styleId="theform" >
			...
			<input type="hidden" name="nextPerson" value="${pnrTransferOffice.provinceLineCharge}"><!-- 发短信指定派发到的人 -->
			<input type="hidden" id="nextTaskLinkName" name="nextTaskLinkName" value="省线路主管审核"><!-- 发短信指定派发到的环节名 -->
