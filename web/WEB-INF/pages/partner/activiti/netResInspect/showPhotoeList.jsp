...
<br/><br/><br/>
	<display:table name="list" cellspacing="0" cellpadding="0"
			...
			<display:column property="systemTime" sortable="false"
			...
			<display:column  sortable="false" headerClass="sortable" title="环节">
				<c:choose>
					<c:when test="${list.photoSubType eq 'starflow'}">
						隐患上报
					</c:when>
					<c:when test="${list.photoSubType eq 'siteCheck'}">
						现场核实
					</c:when>
					<c:when test="${list.photoSubType eq 'defectTreatment'}">
						隐患处理
					</c:when>
				</c:choose>
			</display:column>
