...
<script type="text/javascript">
 Ext.onReady(function(){
		...
		function newReset(){
			...
			document.getElementById("roomPici").value="";	       
   ...
   function refresh(){
	...
	function archive(){
       if(confirm("确定手动归档吗？")){
		 	...
		 	var url = "${app}/activiti/roomDemolition/roomDemolition.do?method=doOneWorkOrderArchiving&taskId="+arguments[0]+"&processInstanceId="+arguments[1]+"&titleId="+arguments[2]+"&title="+arguments[3]+"&linkName=manualArchive"+condition;
		...
		<div id="sheetform">
			<html:form action="/roomDemolition.do?method=queryWorkOrder"
				styleId="theform">
			...
			<td class="content">
					...
					<tr>
						...
						<td class="content" style="width:20%">
							<eoms:comboBox name="roomPici" id="roomPici"
								defaultValue="${roomPici}" initDicId="12706"
								alt="allowBlank:false" styleClass="select" />
						 </td>
		<!-- 	<td class="label">
				工单类别
		...
		<display:table name="taskList" cellspacing="0" cellpadding="0"
			...
			<display:column property="sendTime" sortable="true"	headerClass="sortable" title="派发时间"
				format="{0,date,yyyy-MM-dd HH:mm:ss}" />
			<display:column sortable="false" headerClass="sortable"  title="批次">
				<eoms:id2nameDB id="${taskList.roomPici}" beanId="ItawSystemDictTypeDao" />
			</display:column>	
...
<c:if test="${!empty taskList}">
	<eoms:excelExportToProcess modelName="com.boco.activiti.partner.process.po.RoomDemolitionModel" serviceBeanId="roomDemolitionService" title="机房拆除工单查询" queryFlag="getQueryWorkOrder" processKey="roomDemolition" flag="">		
			...
			<eoms:rowToProcess name="合同到期日期" value="contractTime" />
			...
			<eoms:rowToProcess name="批次" value="roomPici" />
