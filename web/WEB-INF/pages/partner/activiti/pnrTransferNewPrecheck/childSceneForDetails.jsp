...
//}
	...
	//定额不唯一时使用 查看主材
function checkMainMaterialForDetails(itemNo, processInstanceId,uniqueId,linkType) {
        //alert(processInstanceId+"--"+itemNo);
        var url = '${app}/activiti/pnrTransferPrecheck/pnrTransferNewPrecheck.do?method=checkMaterialForDetails&materialType=main&processInstanceId=' + processInstanceId + '&itemNo=' + itemNo+'&uniqueId='+uniqueId+'&linkType='+linkType;
        var _sHeight = 300;
        var _sWidth = 820;
        var sTop = (window.screen.availHeight - _sHeight) / 2;
        var sLeft = (window.screen.availWidth - _sWidth) / 2;
	...
	//定额不唯一时使用 查看辅材
function checkAssistMaterialForDetails(itemNo, processInstanceId,uniqueId,linkType) {
        //alert(processInstanceId+"--"+itemNo);
        var url = '${app}/activiti/pnrTransferPrecheck/pnrTransferNewPrecheck.do?method=checkMaterialForDetails&materialType=assist&processInstanceId=' + processInstanceId + '&itemNo=' + itemNo+'&uniqueId='+uniqueId+'&linkType='+linkType;
        var _sHeight = 300;
        var _sWidth = 820;
        var sTop = (window.screen.availHeight - _sHeight) / 2;
        var sLeft = (window.screen.availWidth - _sWidth) / 2;
...
<!--
	...
	function scenarioTemplateExcel(processInstanceId,linkType){
		window.location.href="${app}/activiti/pnrTransferPrecheck/pnrTransferNewPrecheck.do?method=scenarioTemplateExcel&processInstanceId="+processInstanceId+"&linkType="+linkType;
	}
...
<c:if test="${!empty sceneTableList}">			
		   <input type="button" id="export" name="export" value="导出到Excel" onclick="javaScript:scenarioTemplateExcel('${processInstanceId}','${linkType}')">		
</c:if>
  
<c:forEach  items="${sceneTableList}" var="sceneTable" >
	<div style="margin-top:10px;margin-bottom:5px;">子场景-${sceneTable.childSceneName}</div>
	<table class="formTable" style="margin-bottom:20px;">
	<tr>
		<c:forEach items="${sceneTable.tableHeader}" var="tableHeader">
			<td align='center' style="background-color: #f7f7f7;white-space:nowrap;">${tableHeader}</td>
		</c:forEach>
	</tr>
