...
<script type="text/javascript">
 Ext.onReady(function(){
		...
		function newReset(){
			...
			document.getElementById("status").value="";	//当前环节
			document.getElementById("reportUser").value="";	//隐患上报人
		}
		// 地区、区县连动
   ...
   function archive(){
       if(confirm("确定不需要手机端抽查，直接归档吗？")){
					...
					<tr>
						...
						<td class="content" style="width:20%">
							<select id="status" name="status" class="text" 
								...
								<option value="综合调度核实" ${wsStatus == "need" ? 'selected="selected"':'' }>
									综合调度核实
								</option>
								<option value="综合调度转派" ${wsStatus == "workOrderCheck" ? 'selected="selected"':'' }>
									综合调度转派
								</option>
								<option value="维护中心核实" ${wsStatus == "cityLineExamine" ? 'selected="selected"':'' }>
									维护中心核实
								</option>
								<option value="维护中心隐患处理" ${wsStatus == "cityLineDirectorAudit" ? 'selected="selected"':'' }>
									维护中心隐患处理
								</option>
								<option value="归档" ${wsStatus == "cityManageExamine" ? 'selected="selected"':'' }>
									归档
								</option>
							</select>
						</td>
						...
						<td class="content" style="width:20%">
							<input type="text" name="reportUser" class="text" id="reportUser"
								value="${reportUser}" />
						</td>
					</tr>
			<tr>
		...
		<display:table name="taskList" cellspacing="0" cellpadding="0"
			...
			<a href="${app}/activiti/pnrHiddendangerReported/pnrHiddendangerReported.do?method=gotoDetail&processInstanceId=${taskList.processInstanceId}" target="_blank">
			...
			<display:column sortable="false" headerClass="sortable" title="专业" maxLength="15">
			</display:column>
			...
			<display:column sortable="false" headerClass="sortable" title="隐患上报">
			</display:column>
			<display:column sortable="false" headerClass="sortable" title="是否隐患">
			</display:column>
			<display:column sortable="false" headerClass="sortable" title="是否派发线路工单" media="html">
				<c:if test="${1==1}">
					--
				</c:if>
				<c:if test="${1==2}">
...
<c:if test="${!empty taskList}">
	<eoms:excelExportToProcess modelName="com.boco.activiti.partner.process.po.TaskModel" serviceBeanId="pnrTransferNewPrecheckService" title="待回复工单" queryFlag="listBacklog" processKey="transferNewPrechechProcess" flag="backlog">		
			...
			<eoms:rowToProcess name="隐患上报" value=""/>
			<eoms:rowToProcess name="是否隐患" value=""/>
