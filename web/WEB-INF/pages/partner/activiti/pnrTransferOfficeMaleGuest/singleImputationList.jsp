...
inputs[i].checked = true;  
 ...
 inputs[i].checked = false;  
 	...
 	<script type="text/javascript">
		...
		<div id="sheetform">
			<html:form action="/pnrTransferOfficeMaleGuest.do?method=querySingleImputationList"
				styleId="theform">
...
<bean:define id="url" value="pnrTransferOfficeMaleGuest.do"/>
	...
	<display:table name="taskList" cellspacing="0" cellpadding="0"
       ...
       <display:column  sortable="true"
    	...
    	<display:column sortable="false" headerClass="sortable" title="查看" media="html">
				<a href="javascript:void(0);" onclick="querySingleImputationDetail(&quot;${taskList.processInstanceId}&quot;,&quot;${taskList.siteCd}&quot;)">查看</a>
		</display:column>
...
<script type="text/javascript">
		function querySingleImputationDetail(processInstanceId,siteCd){
			var url = '${app}/activiti/pnrTransferMaleGuest/pnrTransferOfficeMaleGuest.do?method=querySingleImputationDetail&processInstanceId='+processInstanceId+'&siteCd='+siteCd;
			var _sHeight = 603;
		    var _sWidth = 1158;
	...
	//子窗口点击主工单的工单号，跳转到处理界面
	function jumpTodo(processInstanceId,taskId,flag){
		window.location.href = '${app}/activiti/pnrTransferMaleGuest/pnrTransferOfficeMaleGuest.do?method=todo&taskId='+taskId+'&queryFlag='+flag;
	}
	
	//子窗口中修改主工单的主题，修改成功后，刷新父窗口列表中的工单主题
	function updateTheme(processInstanceId,theme){
		var tdObj;
		$("#taskList tbody tr ").find("td:eq(0)").each(function(){
			var text = $(this).text();
			if(text.indexOf(processInstanceId)!=-1){
				tdObj = $(this);
				return false;
			}	
 		});
 		var themeTdObj = tdObj.parent("tr").find("td:eq(1)");
 		var themeHtml = themeTdObj.html();
 		if(themeHtml.indexOf("FONT")!= -1){
 			themeTdObj.html("<font class='order'>"+theme+"</font>");
 		}else{
 			themeTdObj.html(theme);
 		}
	}
