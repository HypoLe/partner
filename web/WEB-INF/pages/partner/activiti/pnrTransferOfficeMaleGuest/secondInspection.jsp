...
inputs[i].checked = true;  
 ...
 inputs[i].checked = false;  
 	...
 	<script type="text/javascript">
		...
		<div id="sheetform">
			<html:form action="/pnrTransferOfficeMaleGuest.do?method=secondInspection"
				styleId="theform">
...
<bean:define id="url" value="pnrTransferOfficeMaleGuest.do"/>
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true" headerClass="sortable" title="审批">
				<c:if test="${taskList.approveFlag eq '0' && operateFlag eq 'yes' && taskList.changeColor eq 'false'}">
				...
				<c:if test="${taskList.approveFlag eq '0' && operateFlag eq 'no' && taskList.changeColor eq 'false'}">
					未审批
				</c:if>
				<c:if test="${taskList.approveFlag eq '0' && taskList.changeColor eq 'true'}">
					<font color="red">已超时，不能抽检</font>
				</c:if>
		...
		<display:column sortable="true" headerClass="sortable" title="场景模板" >
			<a href="javascript:void(0);" onclick="viewUpdateSceneTemplate(this,&quot;${taskList.processType}&quot;,&quot;${taskList.processInstanceId}&quot;,'twoSpotChecks',&quot;${operateFlag}&quot;,&quot;${taskList.handleFlag}&quot;)">查看</a>
		</display:column>
		<display:column  sortable="true"
...
<c:if test="${!empty taskList}">
	<eoms:excelExportToProcess modelName="com.boco.activiti.partner.process.po.TaskModel" serviceBeanId="pnrTransferOfficeService" title="二次抽检" queryFlag="second" processKey="myMaleGuestProcess" flag="" hideFlag="hide">
...
jq(document).delegate("select[name='approveOperate']","change", function(){
	...
	if(approveVal!="0"){
		if(confirm("确定要"+tsStr+"吗？"+tsStr+"后，不可撤销！")){
	  		...
	  		var url="${app}/activiti/pnrTransferMaleGuest/pnrTransferOfficeMaleGuest.do?method=secondInspectionApprove&approvePId="+approvePId+"&approveTaskId="+approveTaskId+"&approveFlag="+approveVal+"&tdate="+tdate+"&tcountry="+tcountry+"&approveProcessType="+approveProcessType;
			jq.ajax({
				type: "post",
		 		url: url,
		 		//async: false,//控制同步        
...
jq(document).delegate("#signBtn","click", function(){
	...
	var isMob=/^((\+?86)|(\(\+86\)))?(13[012356789][0-9]{8}|15[012356789][0-9]{8}|18[02356789][0-9]{8}|147[0-9]{8}|1349[0-9]{7})$/;//验证手机号
		...
		var url="${app}/activiti/pnrTransferMaleGuest/pnrTransferOfficeMaleGuest.do?method=submitSecondInspectionApprovalSign&tdate="+tdate+"&tcountry="+tcountry+"&approvalSign="+temp+"&approvalPhone="+approvalPhone;
		jq.ajax({
			type: "post",
	 		url: url,
	 		//async: false,//控制同步        
...
jq(document).delegate("#exportDetails","click", function(){
	var tdate = jq("#tdate").val();
	var tcountry = jq("#tcountry").val();
	window.location.href="${app}/activiti/pnrTransferMaleGuest/pnrTransferOfficeMaleGuest.do?method=exportDetails&reportFlag=twoSpotChecks&country="+tcountry+"&sheetAcceptLimit="+tdate;
});
</script>
