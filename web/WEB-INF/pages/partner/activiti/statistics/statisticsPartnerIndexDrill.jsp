...
<bean:define id="url" value="pnrStatistics.do" />
	<display:table name="taskList" cellspacing="0" cellpadding="0"
		...
		<display:column sortable="true"
			...
			<a target="_blank" href="${app}/activiti/pnrTroubleTicket/pnrTroubleTicket.do?method=gotoDetail&processInstanceId=${taskList.processInstanceId}">
			${taskList.processInstanceId}
			</a>
			</c:if>
			<c:if test="${taskList.flag == 2}">
			<a target="_blank" href="${app}/activiti/pnrTaskTicket/pnrTaskTicket.do?method=gotoDetail&processInstanceId=${taskList.processInstanceId}">
			${taskList.processInstanceId}
			</a>
			</c:if>
		</display:column>	
