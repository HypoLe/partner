...
<bean:define id="url" value="pnrTransferOffice.do"/>
	...
	<display:column sortable="true"
			...
			${taskList.personStatus}--${taskList.taskDefKey}
			<c:if test="${taskList.personStatus eq 'createPerson' && taskList.taskDefKey eq 'cityLineExamine'}">
				&nbsp;&nbsp;
	           		 <a href="${app}/activiti/pnrTransferPrecheck/pnrTransferPrecheck.do?method=newPrecheckRollback&handle=cityLineExamine&taskId=${taskList.taskId}&initiator=${taskList.createPerson}" title="回退">
					抓回
					</a>
			</c:if>
			<c:if test="${taskList.personStatus eq 'createPerson' && taskList.taskDefKey eq 'cityManageExamine'}">
				&nbsp;&nbsp;
	           		 <a href="${app}/activiti/pnrTransferPrecheck/pnrTransferPrecheck.do?method=newPrecheckRollback&handle=cityManageExamine&taskId=${taskList.taskId}&initiator=${taskList.cityLineExamine}" title="回退">
					抓回
					</a>
			</c:if>
			<c:if test="${taskList.personStatus eq 'daiwei' && taskList.taskDefKey eq 'worker'}">
				&nbsp;&nbsp;
	           		 <a href="${app}/activiti/pnrTransferPrecheck/pnrTransferPrecheck.do?method=newPrecheckRollback&handle=transferHandle&taskId=${taskList.taskId}&initiator=${taskList.daiwei}" title="回退">
					抓回
					</a>
			</c:if>
			<c:if test="${(taskList.personStatus ne 'createPerson' || taskList.taskDefKey ne 'cityLineExamine')&&(taskList.personStatus ne 'createPerson' && taskList.taskDefKey ne 'cityManageExamine')&&(taskList.personStatus ne 'daiwei' && taskList.taskDefKey ne 'worker')}">
			--
			</c:if>
		</display:column>
