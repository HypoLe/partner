...
var v;
  ...
  function refresh(){//此刷新函数被弹出的子模态窗口调用。
	    ...
	    window.location.href = "${app}/activiti/pnrTransferPrecheck/pnrTransferArteryPrecheck.do?method="+method+condition;
  }
</script>
...
<html:form action="/pnrTransferArteryPrecheck.do?method=transferProgram" styleId="theform" >
			<!-- 带注释为驳回时需要的参数 -->
			<!-- <input type="hidden" id="marker" name="marker" value="provinceLineCheck" />流程标识 -->	
			<input type="hidden" id="marker" name="marker" value="cityManageDirectorAudit" /><!-- 流程标识 -->	
...
<script type="text/javascript">
 approveSwitcher= new detailSwitcher();
   ...
   function waitWorkOrder(){
   		var condition=document.getElementById("condition").value;
  		window.location.href="${app}/activiti/pnrTransferPrecheck/pnrTransferArteryPrecheck.do?method=waitWorkOrder&processInstanceId=${processInstanceId}"+condition;
  }
  ...
  function changeDegree(processInstanceId)
		{    
			...
			var url="${app}/activiti/pnrTransferPrecheck/pnrTransferArteryPrecheck.do?method=changeDegree&degree="+degree+"&processInstanceId="+processInstanceId;
