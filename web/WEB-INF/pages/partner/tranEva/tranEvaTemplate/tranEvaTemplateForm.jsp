...
Ext.onReady(function(){
	...
	tplTab.on('activate',function(){
	   	//location.href = "${app}/partner/tranEva/tranEvaTemplates.do?method=newTemplate&nodeId="
	});
	kpiTab.on('activate',function(){
	   	//location.href = "${app}/partner/tranEva/tranEvaKpis.do?method=newKpi&nodeId="
	});
	var tplId = document.forms[0].id.value;
	if ("" != tplId) {
	   	//$('kpiList').src="${app}/partner/tranEva/tranEvaTemplateKpis.do?method=listKpiOfTemplate&templateId=${tranEvaTemplateForm.id}";
...
function activeTemplate() {
	var msg = "模板下属指标权重和值为"+'${sessionScope.tranEvaTW}'+"，确认激活？";
		if (confirm(msg)==true){
			var url = "${app}/partner/tranEva/tranEvaTemplates.do?method=activeTemplate&templateId=${tranEvaTemplateForm.id}&nodeId=${requestScope.parentNodeId}";
			location.href = url;
		}	
}

function selectChange(templateTypeId) {
	document.forms[0].action = "tranEvaTemplates.do?method=newTemplate";
...
<html:form action="/tranEvaTemplates.do?method=saveTemplate" styleId="tranEvaTemplateForm" method="post"> 
<table class="formTable" id="tplForm" style="width:88%" align="center">
	...
	<tr>
		...
		<td class="content" colspan="3">
			<html:text property="templateName" styleId="templateName" style="width:88%" 
						...
						alt="allowBlank:false,vtext:'请输入模版名称'" value="${tranEvaTemplateForm.templateName}" />
		</td>
	</tr>
	<c:if test="${not empty tranEvaTemplateForm.id}">
	<tr>
		<td class="label">
			创建人
		</td>
		<td class="content">
			<eoms:id2nameDB id="${tranEvaTemplateForm.creator}" beanId="tawSystemUserDao" />
		</td>
		...
		<td class="content">
			${tranEvaTemplateForm.createTime}
		</td>
	</tr>   
	...
	<tr>
		...
		<td class="content" colspan="3">
			<eoms:dict key="dict-partner-tranEva" dictId="activated" itemId="${tranEvaTemplateForm.activated}" beanId="id2nameXML" />
		</td>
	</tr>
	</c:if>
	<tr>
		...
		<td class="content" >
			<html:text property="professional" styleId="professional" style="width:88%" 
						styleClass="text medium"
						value="${tranEvaTemplateForm.professional}" />
		</td>
		<td class="label">
			考核周期
		</td>
		<td class="content">
			<c:choose>
				<c:when test="${tranEvaTemplateForm.cycle != null}">
					<eoms:dict key="dict-partner-tranEva" dictId="cycle" defaultId="${tranEvaTemplateForm.cycle}"
						 selectId="cycle" beanId="selectXML" alt="allowBlank:false,vtext:'请选择考核周期'" />
				</c:when>
				<c:otherwise>
					<eoms:dict key="dict-partner-tranEva" dictId="cycle" defaultId="month"
						 selectId="cycle" beanId="selectXML" alt="allowBlank:false,vtext:'请选择考核周期'" />
				</c:otherwise>
			</c:choose>
		</td>		
	...
	<tr>
		...
		<td class="content">
			<c:choose>
				<c:when test="${tranEvaTemplateForm.orgType != null}">
					<eoms:dict key="dict-partner-tranEva" dictId="orgType" defaultId="${tranEvaTemplateForm.orgType}"
						 selectId="orgType" beanId="selectXML" alt="allowBlank:false,vtext:'请选择组织类型'" />
				</c:when>
				<c:otherwise>
					<eoms:dict key="dict-partner-tranEva" dictId="orgType" defaultId="dept"
						 selectId="orgType" beanId="selectXML" alt="allowBlank:false,vtext:'请选择组织类型'" />
				</c:otherwise>
			</c:choose>
		</td>
	...
	<tr>
		...
		<td class="content" colspan="3">
			<html:textarea property="remark" styleId="remark" style="width:88%" 
						styleClass="textarea"  value="${tranEvaTemplateForm.remark}" />
		</td>
	</tr>
	<c:if test="${empty tranEvaTemplateForm.id}">
	<tr>
		<td class="label">
			复制模板指标
		</td>
		...
		<td class="content" colspan="3">
			...
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="tranEvaTreeDao" />
					</option>
				</logic:iterate>
			</select>
			&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<option value="">--请选择--</option>
				<logic:iterate id="tList" name="tList">
					<option value="${tList.nodeId}">
						<eoms:id2nameDB id="${tList.nodeId}" beanId="tranEvaTreeDao" />
					</option>
				</logic:iterate>
			</select>
		</td>
		...
		<td class="content" colspan="3">
			...
			<option value=""><eoms:id2nameDB id="${requestScope.templateTypeId}" beanId="tranEvaTreeDao" /></option>	
			<logic:iterate id="ttList" name="ttList">
					<option value="${ttList.nodeId}">
						<eoms:id2nameDB id="${ttList.nodeId}" beanId="tranEvaTreeDao" />
					</option>
	...
	<tr>
		...
		<c:if test="${not empty tranEvaTemplateForm.id 
			&& not empty requestScope.parentNodeId 
				&& tranEvaTemplateForm.activated == 0}">
		<td>
...
<input type="hidden" id="id" name="id" value="${tranEvaTemplateForm.id}" />
</html:form>
</div>
...
<div id="kpi" class="tab-content">
	...
	<iframe id="fileList" name="fileList" frameborder="0" width="100%" height="500" 
		src="${app}/partner/tranEva/tranEvaKpis.do?method=listNextLevelKpi&parentNodeId=${requestScope.parentNodeId}">
	</iframe>
	</c:if>
</div>
</div>
