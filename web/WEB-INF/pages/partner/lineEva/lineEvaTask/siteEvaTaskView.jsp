...
function changeTemplateType()
		{    
			...
			var url="<%=request.getContextPath()%>/partner/lineEva/lineEvaTasks.do?method=changeTemplateType&templateTypeId="+templateTypeId;
...
function isSubmit(){
    if(checkLineEvaTime())return true;//document.getElementById("lineEvaKpiInstanceForm").submit();
    else if(!checkLineEvaTime()){
       alert("考核月份在模板激活时间之后！");
       return false;
    }
}
...
<html:form action="/lineEvaTasks.do?method=taskDetailList" styleId="lineEvaKpiInstanceForm" method="post" > 
<table class="formTable" id="form">
			...
			<select name="templateTypeId" id="templateTypeId"
					...
					<logic:iterate id="templateType" name="templateType">
						<option value="${templateType.nodeId}">
							<eoms:id2nameDB id="${templateType.nodeId}" beanId="lineEvaTreeDao" />
						</option>
					</logic:iterate>
				</select>
	...
	<tr>
		...
		<td class="content">
			<eoms:dict key="dict-partner-lineEva" dictId="cycle" defaultId="month" onchange="changeCycle();"
