...
<html:form action="/lineEvaTasks.do?method=saveTaskDetail"
	styleId="lineEvaKpiInstanceForm" method="post">
		...
		<tbody>
			<logic:iterate id="taskDetail" name="taskDetailList" indexId="index">
				<tr>
					...
					<td>
						<eoms:dict key="dict-partner-lineEva" dictId="cycle" itemId="${taskDetail.timeType}" beanId="id2nameXML" />
					</td>
					<td>
					...
					<td>
						...
						<%if(LineEvaConstants.TASK_PUBLISHED.equals(isPublish)){%>
						已发布
						<%}else{%>
						未发布
						<%}%>
					...
					<td>
						<a href="${app}/partner/lineEva/lineEvaTasks.do?method=taskDetailList&taskId=${taskDetail.taskId}&partnerId=${taskDetail.partnerId}&time=${taskDetail.time}&cycle=${taskDetail.timeType}&queryType=${isPublish }&region=${taskDetail.city}&specialty=${specialty} " title="查看" />查看
					</td>
				</tr>
			</logic:iterate>
		</tbody>
