...
Ext.onReady(function(){
		...
		leakListJSON = eval('('+'${leakListJSON}'+')');
		jq("#btnShowAllLeak").bind("click",showAllLeak);
		jq("#btnCleanAllLeak").bind("click",cleanAllLeak);
		
		if(parent.frames["gis-frame"] != undefined) {
			parent.frames["gis-frame"].setAllLeakGraphics(leakListJSON);
		}
		
		function showAllLeak(e) {
			//parent.frames["gis-frame"].paramConfig.leakListJSON = leakListJSON;
			parent.frames["gis-frame"].Ext.get("map").mask("正在加载漏检点数据，请稍等......");
			parent.frames["gis-frame"].setAllLeakGraphics(leakListJSON);
			parent.frames["gis-frame"].addAllLeakGraphics();
			parent.frames["gis-frame"].Ext.get("map").unmask();
		}
		
		function cleanAllLeak(e) {
			parent.frames["gis-frame"].cleanAllLeakGraphics();
		}
		...
		jq("#leakList tbody tr").bind("mouseover",function(e) {
			jq(this).addClass("trMouseOver");
		});
		
		jq("#leakList tbody tr").bind("mouseout",function(e) {
			jq(this).removeClass("trMouseOver");
		});
		
		jq("#leakList tbody tr").bind("click",function(e) {
			var siteId = jq(this).find("input:hidden").val();
			parent.frames["gis-frame"].locationLeak(siteId);
		});
});
//设置查询条件
...
<div id="listQueryObject" 
	...
	<form action="gisLeakReq.do?method=showLeakList" method="post" id="searchForm" name="searchForm">
			<fieldset>
			<legend>操作选项</legend>
			<table id="sheet" class="formTable">
				...
				<tr>
					<td class="label">
						<input type="button" value="本页漏检数据" id="btnShowAllLeak" class="btn"/> 
					</td>
					<td class="label">
						<input type="button" value="清除本页数据" id="btnCleanAllLeak" class="btn"/> 
					</td>
				</tr>
...
<br/>
<display:table name="leakList" cellspacing="0" cellpadding="0"
	id="leakList" pagesize="${pageSize}" class="table leakList"
	export="false" 
	requestURI="${app}/partner/gis/gisLeakReq.do?method=showLeakList"
	sort="list" partialList="true" size="resultSize" >
	<display:column  headerClass="sortable" title="站点名称">
			${leakList.siteName }<input type="hidden" id="dataId_${leakList_rowNum }" value="${leakList.id}"/>
	</display:column>
	...
	<display:column  sortable="true" headerClass="sortable" title="维护人员">
		<eoms:id2nameDB id="${leakList.inspectUser}" beanId="tawSystemUserDao" />
	</display:column>			
