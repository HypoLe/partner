...
<script type="text/javascript">
  ...
  function delTmpl(prctmplid){
     if(confirm("确定删除")){
       ...
       Ext.Ajax.request({
	 	            url: '${app}/partner/feeManage/evaluFee.do?method=evalu_feeprctmpl_del', 
                    params:{   
       	  				prctmplid:prctmplid        	  				
       				}, 
  ...
  function addTmpl(){
    ...
    // ahref.click();
       ...
       aform.action="${app}/partner/feeManage/evaluFee.do?method=evalu_feeprctmpl";
       aform.method="post";
...
<div id="listQueryObject" 
	...
	<form action="${app}/partner/feeManage/evaluFee.do?method=tmplList" method="post" id="tmplListConditionForm">
...
<logic:present name="tmplList" scope="request">
    <display:table name="tmplList" cellspacing="0" cellpadding="0"
		id="tmplListEL" pagesize="${pageSize}" class="table" 
		export="false" 
		requestURI="${app}/partner/feeManage/evaluFee.do?method=tmplList"
		...
		<display:column   title="专业" >
				<eoms:id2nameDB id="${tmplListEL.majorid}" beanId="ItawSystemDictTypeDao" />
		</display:column>					
	
		<display:column  title="地区" >
		   <eoms:id2nameDB id='${tmplListEL.areaid}' beanId='tawSystemAreaDao'/>
		</display:column>		
	
		<display:column  title="代维公司" >
			<eoms:id2nameDB id='${tmplListEL.compid}' beanId='partnerDeptDao'/>
		</display:column>		
				
		<display:column title="创建人" >
			 <eoms:id2nameDB id="${tmplListEL.creatorid}" beanId="tawSystemUserDao" />
		</display:column>
		<display:column title="创建时间" >
		  <fmt:formatDate value="${tmplListEL.createdttm}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>
		</display:column> 
		<display:column  title="备注" >
		 <pre>${tmplListEL.remark}</pre>
		</display:column>
			
		<!-- 查看、修改在新页面，删除后依旧返回列表 -->		
		<display:column  title="查看">
		    <a  href="${app}/partner/feeManage/evaluFee.do?method=evalu_feeprctmpl_view&prctmplid=${tmplListEL.id}"
				target="view" shape="rect"> 
				<img src="${app}/images/icons/search.gif" style="cursor:hand;display:block" /> 
			</a>
		</display:column>	
    	<display:column  title="修改">    
		    <a  href="${app}/partner/feeManage/evaluFee.do?method=evalu_feeprctmpl_edit&prctmplid=${tmplListEL.id}"
				target="modify" shape="rect"> 
				<img src="${app}/images/icons/edit.gif" style="cursor:hand;display:block" /> 
			</a>
    	</display:column>
    	<display:column  title="删除">
				<img src="${app}/images/icons/delete.gif" style="cursor:hand;display:block" onClick="javascript:delTmpl('${tmplListEL.id}');" /> 		        	
    	</display:column>						
    </display:table>                
</logic:present>         
