...
<form
	action="${app}/partner/feeManage/feeCountManage.do?method=saveFeeCountDetailTemplate"
...
<tr>
<td class="label"><font color='red'> * </font>模板名称：</td> <td><input type="text" class="text" name="prcTmplNm" id="prcTmplNm" alt="allowBlank:false"/></td>
...
pageInit=false;
     ...
     var tableIds= document.getElementById("majorSelect").options;
     for(var i=0;i<tableIds.length;i++){
     if(tableIds[i].value.trim()==majorId.trim()){
     tableIds[i].selected='selected';
     }
     }
        return ;
     } 
...
function loadFeeFilterByCntTyp(select){
	...
	*/
	...
	var ajax = createXmlHttpRequest();
	ajax.open("POST","${app}/partner/feeManage/feeCountManage.do?method=feeFilterSelectAndBindeEvent",true);
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	ajax.onreadystatechange=function(){
		if(ajax.readyState==4 && ajax.status==200){
			var result = ajax.responseText;
...
if(isHasRecord()==true){
 ...
 rowId++;
  ...
  var sb="";
  for(var i=0;i<fields.length;i++){
  sb=sb+"<input type='hidden' name='"+cntTypId+"_tableField"+(i+1)+"'id='"+cntTypId+"_tableField"+(i+1)+"' value='"+fields[i].value+"'/>"
  }
  sb=sb+"<td>"+rowId+"</td><td>"+majorName+"</td><td>"+cntTypName+"</td>";
  for(var i=0;i<fields.length;i++)
  {
  sb=sb+"<td id='"+cntTypId+"_tableFiled'"+(i+1)+">"+fields[i].value+"</td>"
  
  }
  sb=sb+"<td><input type='text' id='"+cntTypId+"_tablePrice' name='"+cntTypId+"_tablePrice' class='text' /></td><td><img src='${app}/images/icons/delete.gif' onclick='delRow(this)' style='cursor:hand;'/></td><input type='hidden' name='rowId' id='rowId' value='"+rowId+"'/>"
   tr.innerHTML = sb;
   */
