		...
		<table id="sheet" class="listTable">
			<tr>		
				...
				<td>
				  ...
				  <eoms:xbox id="areaTree" dataUrl="${app}/xtree.do?method=areaTree&areaMaxLevel=2"  
           ...
           <c:forEach items="${tdList}" var="td" varStatus="status">
             <c:if test="${td.show}">             
                <td rowspan="${td.rowspan}" colspan="${td.colspan}">
                 <c:choose>
                  <c:when test="${status.index==0 or status.index==1}"><eoms:id2nameDB id='${td.name}' beanId='partnerDeptDao'/></c:when>
...
<c:if test="${isSubDeptEvaluStatistics}">	
   <span style="color:red; font-size:12pt;line-height:150%;">
    <logic:notEmpty name="notCmpltEvalTargetList" scope="request">还未完成考核的下级地市级代维机构：</logic:notEmpty>
