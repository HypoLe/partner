...
<script type="text/javascript">
	function add(){
	window.location.href="<%=request.getContextPath()%>/partner/resourceInfo/mobileTerminal.do?method=goToAdd";
	}
	function openImport(){
	...
	function del(id){
		if(confirm("确定要删除？")){
		 	Ext.Ajax.request({
		 		url:"${app}/partner/resourceInfo/mobileTerminal.do",
		 		...
		 		success:function(res,opt){
		 			...
		 			var config={
		 				title:"提示信息",
		 				msg:Ext.util.JSON.decode(res.responseText).info,
		 				width:200,
		 				closable:true,
...
<form action="${app}/partner/resourceInfo/mobileTerminal.do?method=search" id="mobileTerminalForm" method="post">
	...
	<display:column sortable="false" headerClass="sortable" title="编辑" url="/partner/resourceInfo/mobileTerminal.do?method=edit"
	...
	<display:column sortable="false" headerClass="sortable" title="删除" media="html">
			<img class="delete" src="${app}/images/icons/icon.gif" onclick="del('${mobileTerminalList.id}')"/>
	</display:column>	
