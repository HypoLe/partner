...
<script type="text/javascript">
	function add(){
		window.location.href="<%=request.getContextPath()%>/partner/resourceInfo/apparatus.do?method=goToAdd";
	}
	...
	function del(id){
		if(confirm("确定要删除？")){
		 	Ext.Ajax.request({
		 		url:"${app}/partner/resourceInfo/apparatus.do",
...
<%--<div id="searchDiv" class="x-layout-panel-hd" style="cursor:pointer;border: 1px solid rgb(152, 192, 244); padding: 5px; width: 98%;">
快速查询</div>
...
<form action="${app}/partner/resourceInfo/apparatus.do?method=search" id="apparatusForm" method="post">
	...
	<display:column  sortable="true" headerClass="sortable" title="代维公司" maxLength="20">
			<eoms:id2nameDB id="${apparatusList.maintenanceCompany}"	beanId="tawSystemDeptDao"  />
	</display:column>
	<display:column  sortable="true" headerClass="sortable" title="专业" maxLength="12">
	...
	<display:column sortable="false" headerClass="sortable" title="编辑"
				url="/partner/resourceInfo/apparatus.do?method=edit"
	...
	<display:column sortable="false" headerClass="sortable" title="删除" media="html">
			<img class="delete" src="${app}/images/icons/icon.gif" onclick="del('${apparatusList.id}')"/>
	</display:column>
</display:table>
