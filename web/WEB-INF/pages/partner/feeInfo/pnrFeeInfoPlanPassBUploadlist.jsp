...
<%@page import="com.boco.eoms.partner.feeInfo.model.PnrFeeInfoMain"%>
<%@page import="com.boco.eoms.partner.feeInfo.model.PnrFeeInfoPay"%>
<%@page import="com.boco.eoms.partner.feeInfo.mgr.IPnrFeeInfoMainMgr"%>
...
<%
List planList = (List)request.getAttribute("pnrFeeInfoPlanPassBUploadlist");
...
String planNumStr ="";
String buttomName = "付款完成";
String buttonName1 = "阶段回复";
Map feeMainMap = new Hashtable();
TawSystemSessionForm sessionform = (TawSystemSessionForm) request
.getSession().getAttribute("sessionform");
IPnrFeeInfoMainMgr pnrFeeInfoMainMgr = (IPnrFeeInfoMainMgr) ApplicationContextHolder.getInstance().getBean("pnrFeeInfoMainMgr");
...
function showPlanStepMsg(trId)
{
  ...
  else
  {
	...
	<body>
		...
		<caption>
		<div class="header center"  style="text-align:center;font-weight:bold;">正在付款任务列表</div>
	</caption>
	...
	(<font color="red"><%=planNumStr %>个正在付款</font>) 
	</td>
	</tr>
	...
	<tr>
		...
		if (feeMainMap.get(pnrFeeInfoPlan.getFeeId()) == null) {
			feeMainMap.put(pnrFeeInfoPlan.getFeeId(),pnrFeeInfoMainMgr.getPnrFeeInfoMain(pnrFeeInfoPlan.getFeeId()));
		} 
		pnrFeeInfoMain =  (PnrFeeInfoMain)feeMainMap.get(pnrFeeInfoPlan.getFeeId());
		if(pnrFeeInfoMain.getPayMoneyUser().equals(userId)){
	
		
		//if(pnrFeeInfoMain.getPayMoneyUser().equals(userId)){
		%>
			<td rowspan="2">
			 	<input  type="button" class="btn" value="<%=buttomName%>"onclick="window.open('${app}/partner/feeInfo/pnrFeeInfoPays.do?method=planPay&planId=<%=pnrFeeInfoPlan.getId() %>');"	/>
			 	</br>
			 	<!-- 
			 	<input  type="button" class="btn" value="<%=buttonName1%>"onclick="javascript:showPlan('<%=pnrFeeInfoPlan.getFeeId() %>Msg');"	/>
				 -->
				<input  type="button" class="btn" value="<%=buttonName1%>"onclick="window.open('${app}/partner/feeInfo/pnrFeeInfoPays.do?method=planPayReply&planId=<%=pnrFeeInfoPlan.getId() %>');"	/>
			</td>
			 
		<%} %>		
	</tr>
	<tr>	
		<td style="vertical-align:middle;" class="label">
			阶段付款描述：
		</td>
	...
	<tr id="<%=pnrFeeInfoPlan.getFeeId() %>Msg" style="display:none;" >	
		<td class="label" style="vertical-align:middle;">
			阶段回复信息：
		</td>
	<td class="content" colspan="4">
		this is step message:<%= pnrFeeInfoPlan.getRemark() %></br>
		<html:textarea property="pnrpayMsg" value="who ?"></html:textarea>
	</td>
	</tr>
	<%
