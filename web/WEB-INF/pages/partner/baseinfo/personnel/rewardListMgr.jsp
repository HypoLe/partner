	...
	<script type="text/javascript">
		<!--
		  var checkflag = "false";
			...
			function add(){
				location.href="${app}/personnel/reward.do?method=getjsp&operationType=toadd&workerid=${workerid}&workername=${workername}";
			}
			function del(){
				var string="";
		        ...
		        if(confirm("确认要删除吗？")){
				 	location.href="${app}/personnel/reward.do?method=remove&selectids="+string;
				 }else{
				 	return false;
				 }
			}
...
<div align="center"><b>人员奖励信息管理-人员奖励信息列表</div><br><br/>
			...
			<form id="select_from" action="${app}/personnel/reward.do?method=search" method="post">
				<table class="listTable" >
					<tr>		
						...
						<td><input type="text" name="awarddeptStringLike" class="text" value="${awarddeptStringLike }"/></td>
	...
	<display:table name="rewardList" cellspacing="0" cellpadding="0"
		...
		<display:column  title="<input type='checkbox' onclick='javascript:chooseAll();'>" media="html">
	         <input type="checkbox" name="checkbox11" value="<c:out value='${rewardList.id}'/>" >
	    </display:column>
	<c:if test="${empty workerid}">    
	    ...
	    <display:column sortable="true" headerClass="sortable" title="修改" media="html">					 
			<a id="${rewardList.id}"
			   href="${app}/personnel/reward.do?method=getjsp&operationType=toedit&id=${rewardList.id}&workerid=${workerid}&workername=${workername}">
	...
	<input type="hidden" value="tomgr" name = "operationType"/>
		...
		<input type="button" class="btn" value="刷新" onclick="javascript:window.location.href='<%=request.getContextPath()%>/personnel/reward.do?method=search&operationType=tomgr&personCardNo=${personCardNo}'"/>
