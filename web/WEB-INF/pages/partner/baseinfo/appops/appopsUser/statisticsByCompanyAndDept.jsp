...
<html:form action="/partnerAppOpsUsers.do?method=statisticsAppopsUserBySpeciallty&flag=2" styleId="partnerUserAndDeptForm" method="post">
...
<!-- 给导出用 -->
		<input type="hidden" name="workerStateTemp" value="${workerState }">
		<input type="hidden" name="highAgreeTemp" value="${highAgree }">
		<input type="hidden" name="zhichengTemp" value="${zhicheng }">
		<input type="hidden" name="operatingPostZTemp"
			value="${operatingPostZ }">
		<input type="hidden" name="operatingPostJTemp"
			value="${operatingPostJ }">

</html:form>	
</div>
<br/><br/>
<display:table name="taskList" id="taskList" class="table businessdesignMain" export="true" requestURI="partnerAppOpsUsers.do">
	<display:column title="分公司" style="text-align:center;">
	<c:if test="${taskList.company eq '合计'}">
	合计
	</c:if>
	<c:if test="${taskList.company ne '合计'}">
	<a href="${app}/partner/appops/partnerAppOpsUsers.do?method=statisticsAppopsUserBySpeciallty&flag=4&city=${taskList.company}&workerState=${workerState }&highAgree=${highAgree }&zhicheng=${zhicheng }&operatingPostZ=${operatingPostZ }&operatingPostJ=${operatingPostJ }" target="_blank" title="回复">
				<eoms:id2nameDB id="${taskList.company}" beanId="ItawSystemDictTypeDao"/>
			     </a>
	</c:if>
	</display:column>
	<display:column title="运行维护部" property="dept_one" style="text-align:center;"/>
	<display:column title="网络维护中心" property="dept_two" style="text-align:center;"/>
	<display:column title="网络优化中心" property="dept_three" style="text-align:center;"/>
	<display:column title="公众客户响应中心" property="dept_forth" style="text-align:center;"/>
	<display:column title="集团客户响应中心" property="dept_five" style="text-align:center;"/>
	<display:column title="资源管理中心" property="dept_sex" style="text-align:center;"/>
	<display:column title="线路维护中心" property="dept_seven" style="text-align:center;"/>
	<display:column title="省级平台维护中心" property="dept_ength" style="text-align:center;"/>
	<display:column title="国际海缆监控中心" property="dept_nine" style="text-align:center;"/>
	<display:column title="县（区）维护中心" property="dept_ten" style="text-align:center;"/>
