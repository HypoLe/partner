...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff" >
	<form action="partnerDepts.do?method=findCompanyBaseInfo&isPartner=${isPartner}" method="post" id="partnerDeptForm">
	...
	<tr>
		...
		<td class="content">
			<input type="text" name="managerSearch" id="manager" class="text medium" value="${managerSearch}"/>
		</td>
	</tr>
	</table>
		<html:submit styleClass="btn" property="method.list" styleId="method.iist" value="提交"></html:submit>
...
<c:set var="buttons">
	...
	<c:if test="${requestScope.hasRightForAdd=='1'}">
		<a
			href='${app}/partner/baseinfo/partnerDepts.do?method=toCompanyBaseInfoForm'>
			添加省下属公司 <a>
	</c:if>
  <a href="javascript:;">删除</a>
	...
	<html:form action="partnerDepts" method="post" styleId="partnerDeptForm">
		...
		<display:table name="partnerDeptList" cellspacing="0" cellpadding="0" id="partnerDeptList" pagesize="${pageSize}"
			class="table partnerDeptList" export="true" requestURI="${app}/partner/baseinfo/partnerDepts.do?method=findCompanyBaseInfo"
			sort="list" partialList="true" size="resultSize" decorator="com.boco.eoms.partner.baseinfo.displaytag.support.PartnerDeptDisplaytagDecorator">
			<c:if test="${requestScope.hasRightForAdd=='1'}">
			...
			<display:column property="phone" sortable="true"
				...
				paramProperty="id" />
			<display:column sortable="true" headerClass="sortable" title="入围级别"
				paramId="id" paramProperty="id">
				<eoms:id2nameDB id="${partnerDeptList.selectedLevel}" beanId="ItawSystemDictTypeDao" />
			</display:column>
			<display:column property="registerTime" sortable="true"
				headerClass="sortable" title="注册日期" paramId="id" paramProperty="id" />
			<display:column title="查看下属组织" media="html">
					<a href="${app}/partner/baseinfo/partnerDepts.do?method=findSubCompanyBaseInfo&id=${partnerDeptList.id }&parentname=${partnerDeptList.name }&isPartner=${isPartner}">
						查看</a>
			</display:column>
			<display:column title="添加下属组织" media="html">
				<a href="${app}/partner/baseinfo/partnerDepts.do?method=toCompanyBaseInfoSubForm&id=${partnerDeptList.id }&parentDeptname=${partnerDeptList.name }&isPartner=${isPartner} ">
                    <c:choose>
		...
		<c:if test="${hasRightForAdd=='1'}">
            <c:choose>
                <c:when test="${isPartner eq '1'}">
                    <input type="button" class="btn" value="添加巡检单位" onclick="javascript:window.location='${app}/partner/baseinfo/partnerDepts.do?method=toCompanyBaseInfoForm&isPartner=${isPartner}'">
                </c:when>
                <c:otherwise>
                    <input type="button" class="btn" value="添加代维公司" onclick="javascript:window.location='${app}/partner/baseinfo/partnerDepts.do?method=toCompanyBaseInfoForm&isPartner=${isPartner}'">
                </c:otherwise>
            </c:choose>
