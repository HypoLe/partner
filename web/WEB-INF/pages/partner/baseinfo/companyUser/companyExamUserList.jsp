...
<c:set var="buttons">
   ...
   <a href="partnerUsers.do?method=addCompanyUser&pnrType=partner">添加</a>	   
...
<html:form action="partnerUsers.do?method=findExamUser" method="post"  styleId="partnerUserForm">
<c:if test="${in!=null}">
...
<content tag="heading">
	...
	<display:table name="partnerUserList" cellspacing="0" cellpadding="0"
		...
		export="false"
		requestURI="${app}/partner/baseinfo/partnerUsers.do?method=findExamUser"
	...
	<display:column property="name" sortable="true"      
			headerClass="sortable" titleKey="partnerUser.name" href="${app}/partner/baseinfo/resumes.do?method=newExpert&pnrType=partner" paramId="id" paramProperty="id"/>
	...
	<display:column  sortable="true"
		...
		<display:column  sortable="true"
			...
			<c:if test="${partnerUserList.approveStatus ==1}">
			审批通过
			</c:if>
			<c:if test="${partnerUserList.approveStatus ==2}" >
			审批不通过
			</c:if>
	</display:column>
