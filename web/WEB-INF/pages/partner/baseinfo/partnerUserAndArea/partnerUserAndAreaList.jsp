...
<c:set var="buttons">
	...
	<input type="button" class="btn" value="<fmt:message key="button.add" />"  
		onclick="javascript:
						var url=eoms.appPath+'/partner/baseinfo/partnerUserAndAreas.do?method=add';
						location.href=url"/>
...
<content tag="heading" >
	...
	<display:column property="userId" sortable="true"
			headerClass="sortable" titleKey="partnerUserAndArea.userId" href="${app}/partner/baseinfo/partnerUserAndAreas.do?method=edit" paramId="id" paramProperty="id"/>

	<display:column  titleKey="partnerUserAndArea.areaNames">
	
			<c:forTokens items="${partnerUserAndAreaList.areaId}" delims="," var="area_ID" varStatus="status"><eoms:id2nameDB id="${area_ID}" beanId="tawSystemAreaDao"/><c:choose><c:when test="${status.last}"></c:when><c:otherwise>,</c:otherwise></c:choose></c:forTokens>
	</display:column>
