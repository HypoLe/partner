...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsWlanRepeater"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>直放站-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.wlan.IrmsWlanRepeater" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<eoms:comboBox id="nodetype" name="nodetypeStringEqual" initDicId="11228030101" defaultValue="${nodetypeStringEqual}" styleClass="input select" />
						</td>
						</tr>
						<tr>
						<td class="label">
						...
						<td class="content" >
								<input class="text" type="text" name="relatedCellCuidStringEqual" value="${relatedCellCuidStringEqual}" />
						</td>
						...
						<td class="content" >
								<input class="text" type="text" name="relatedVendorStringLike" value="${relatedVendorStringLike}" />
						</td>
						</tr>
						<tr>
						<td class="label">
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.wlan.IrmsWlanRepeater'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.wlan.IrmsWlanRepeater" 
							 serviceBeanId="eomsService"   title="网络资源--直放站室分及WLAN--直放站" >
		<eoms:row name="直放站站点名称" value="labelCn"/>
		<eoms:row name="状态" value="status"/>
		<eoms:row name="直放站站点类型" value="nodetype"/>
		<eoms:row name="所属小区" value="relatedCellCuid"/>
		<eoms:row name="覆盖类型" value="coverageType"/>
		<eoms:row name="经度" value="longitude"/>
		<eoms:row name="纬度" value="latitude"/>
		<eoms:row name="设备提供商名称" value="relatedVendor"/>
		<eoms:row name="信号接收方式" value="signalReceivetype"/>
		<eoms:row name="供电方式" value="powerType"/>
...
<logic:present name="irmsWlanRepeaterList" scope="request">
	<display:table id="irmsWlanRepeaterList"
					name="irmsWlanRepeaterList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsWlanRepeaterList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="直放站站点名称">
						${irmsWlanRepeaterList.labelCn}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="状态">
						<eoms:id2nameDB id="${irmsWlanRepeaterList.status}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="直放站站点类型">
						<eoms:id2nameDB id="${irmsWlanRepeaterList.nodetype}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属小区">
						${irmsWlanRepeaterList.relatedCellCuid}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="覆盖类型">
						<eoms:id2nameDB id="${irmsWlanRepeaterList.coverageType}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="经度">
						${irmsWlanRepeaterList.longitude}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="纬度">
						${irmsWlanRepeaterList.latitude}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="设备提供商名称">
						${irmsWlanRepeaterList.relatedVendor}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="信号接收方式">
						<eoms:id2nameDB id="${irmsWlanRepeaterList.signalReceivetype}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="供电方式">
						<eoms:id2nameDB id="${irmsWlanRepeaterList.powerType}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="代维公司">
						${irmsWlanRepeaterList.manageCompany}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="备注">
						${irmsWlanRepeaterList.remark}
				</display:column>
