...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsWlanHot"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>热点-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.wlan.IrmsWlanHot" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" colspan="3">
								<eoms:comboBox id="relatedVendor" name="relatedVendorStringLike" initDicId="112280702" defaultValue="${relatedVendorStringLike}" styleClass="input select" />
						</td>
						</tr>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.wlan.IrmsWlanHot'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.wlan.IrmsWlanHot" 
							 serviceBeanId="eomsService"   title="网络资源--直放站室分及WLAN--热点" >
		<eoms:row name="热点名称" value="labelCn"/>
		<eoms:row name="地市" value="relatedDistrict"/>
		<eoms:row name="NAS_ID" value="nasId"/>
		<eoms:row name="热点类型或覆盖类型" value="hotpointType"/>
		<eoms:row name="热点地理位置经度" value="longitude"/>
		<eoms:row name="热点地理位置纬度" value="latitude"/>
		<eoms:row name="传输类型" value="transType"/>
		<eoms:row name="设备厂家" value="relatedVendor"/>
		<eoms:row name="AP数量" value="apCount"/>
		<eoms:row name="热点详细地址" value="address"/>
		<eoms:row name="WLAN网络覆盖区域" value="wlanCoverageArea"/>
