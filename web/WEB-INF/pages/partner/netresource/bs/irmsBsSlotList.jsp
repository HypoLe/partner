...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsBsSlot"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>机槽-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.IrmsBsSlot" method="post">
		<fieldset>
			<legend>快速查询</legend>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.IrmsBsSlot'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.bs.IrmsBsSlot" 
							 serviceBeanId="eomsService"   title="网络资源--基站设备及配套--机槽" >
		<eoms:row name="机槽名称" value="labelCn"/>
		<eoms:row name="机槽状态" value="status"/>
		<eoms:row name="所属机框" value="relatedShelfCuid"/>
		<eoms:row name="机槽编号" value="sequence"/>
</eoms:excelExport>
--%>
<!--导出功能结束-->
...
<logic:present name="irmsBsSlotList" scope="request">
	<display:table id="irmsBsSlotList"
					name="irmsBsSlotList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsBsSlotList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="机槽名称">
						${irmsBsSlotList.labelCn}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="机槽状态">
						${irmsBsSlotList.status}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属机框">
						${irmsBsSlotList.relatedShelfCuid}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="机槽编号">
						${irmsBsSlotList.sequence}
				</display:column>
