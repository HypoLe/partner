...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsBsShelf"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>机框-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.IrmsBsShelf" method="post">
		<fieldset>
			<legend>快速查询</legend>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.IrmsBsShelf'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.bs.IrmsBsShelf" 
							 serviceBeanId="eomsService"   title="网络资源--基站设备及配套--机框" >
		<eoms:row name="机框名字" value="labelCn"/>
		<eoms:row name="所属机柜" value="relatedRackCuid"/>
		<eoms:row name="机框编号" value="sequence"/>
		<eoms:row name="机槽数" value="slotAmount"/>
</eoms:excelExport>
--%>
<!--导出功能结束-->
...
<logic:present name="irmsBsShelfList" scope="request">
	<display:table id="irmsBsShelfList"
					name="irmsBsShelfList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsBsShelfList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="机框名字">
						${irmsBsShelfList.labelCn}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属机柜">
						${irmsBsShelfList.relatedRackCuid}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="机框编号">
						${irmsBsShelfList.sequence}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="机槽数">
						${irmsBsShelfList.slotAmount}
				</display:column>
