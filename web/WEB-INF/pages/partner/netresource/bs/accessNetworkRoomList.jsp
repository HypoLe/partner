...
<div align="center"><b>接入网资源-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/bsbt/pnrbsbt.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.AccessNetworkRoom" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<input class="text" type="text" name="anrNameStringLike" value="${anrNameStringLike}" />
						</td>
						<td class="label">
						 	机房位置
						</td>
						<td class="content" >
								<input class="text" type="text" name="anrAddressStringLike" value="${anrAddressStringLike}" />
						</td>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/bsbt/pnrbsbt.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.AccessNetworkRoom'"/>
						</div>
					</td>
				</tr>
			</table>
...
<logic:present name="accessNetworkRoomList" scope="request">
	<display:table id="accessNetworkRoomList"
					name="accessNetworkRoomList" 
				...
				<display:column sortable="true" headerClass="sortable" title="机房名称">
						${accessNetworkRoomList.anrName}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所在地市">
						<eoms:id2nameDB id="${accessNetworkRoomList.city}" beanId="tawSystemAreaDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所在区域">
						<eoms:id2nameDB id="${accessNetworkRoomList.country}" beanId="tawSystemAreaDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所在局所">
						${accessNetworkRoomList.bureau}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="查看该机房设备">
	           		 <a href="${app}/netresource/bsbt/pnrbsbt.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.bs.AccessNetworkQuipment&anrIdStringEqual=${accessNetworkRoomList.id}" title="查看该机房设备">
