...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsTransDirbury"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>直埋-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransDirbury" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<input class="text" type="text" name="dirburyNameStringLike" value="${dirburyNameStringLike}" />
						</td>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransDirbury'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.trans.IrmsTransDirbury" 
							 serviceBeanId="eomsService"   title="网络资源--传输线路--直埋" >
		<eoms:row name="直埋名称" value="dirburyName"/>
...
<logic:present name="irmsTransDirburyList" scope="request">
	<display:table id="irmsTransDirburyList"
					name="irmsTransDirburyList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsTransDirburyList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="直埋名称">
						${irmsTransDirburyList.dirburyName}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属区域">
						${irmsTransDirburyList.relatedDistrict}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="设施状态">
						<eoms:id2nameDB id="${irmsTransDirburyList.status}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="工程名称">
						${irmsTransDirburyList.projectName}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="长度">
						${irmsTransDirburyList.length}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="竣工时间">
						${irmsTransDirburyList.completedDate}
				</display:column>
