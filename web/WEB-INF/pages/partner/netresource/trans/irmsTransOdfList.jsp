...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsTransOdf"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>光交接箱-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransOdf" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<input class="text" type="text" name="odfNameStringLike" value="${odfNameStringLike}" />
						</td>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransOdf'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.trans.IrmsTransOdf" 
							 serviceBeanId="eomsService"   title="网络资源--传输线路--光交接箱" >
		<eoms:row name="光交接箱名称" value="odfName"/>
		<eoms:row name="光交接箱编号" value="odfNo"/>
		<eoms:row name="所属区域" value="relatedDistrict"/>
		<eoms:row name="设备状态" value="status"/>
		<eoms:row name="设备厂家" value="vendor"/>
		<eoms:row name="具体位置" value="position"/>
		<eoms:row name="经度" value="longitude"/>
		<eoms:row name="纬度" value="latitude"/>
		<eoms:row name="型号" value="type"/>
		<eoms:row name="面数" value="faceNum"/>
		<eoms:row name="每面行数" value="eachFaceRowNum"/>
		<eoms:row name="每面列数" value="eachFaceColNum"/>
		<eoms:row name="产权" value="property"/>
		<eoms:row name="用途" value="use"/>
		<eoms:row name="使用单位" value="useUnit"/>
		<eoms:row name="所有权人" value="owner"/>
		<eoms:row name="容量" value="capacity"/>
</eoms:excelExport>
--%>
<!--导出功能结束-->
