...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsTransMarkstone"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>标石-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransMarkstone" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<input class="text" type="text" name="markStoneNameStringLike" value="${markStoneNameStringLike}" />
						</td>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransMarkstone'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.trans.IrmsTransMarkstone" 
							 serviceBeanId="eomsService"   title="网络资源--传输线路--标石" >
		<eoms:row name="标石名称" value="markStoneName"/>
		<eoms:row name="所属区域" value="relatedDistrict"/>
		<eoms:row name="经度" value="longitude"/>
		<eoms:row name="纬度" value="latitude"/>
		<eoms:row name="埋深" value="buryDeep"/>
...
<logic:present name="irmsTransMarkstoneList" scope="request">
	<display:table id="irmsTransMarkstoneList"
					name="irmsTransMarkstoneList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsTransMarkstoneList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="标石名称">
						${irmsTransMarkstoneList.markStoneName}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属区域">
						${irmsTransMarkstoneList.relatedDistrict}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="经度">
						${irmsTransMarkstoneList.longitude}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="纬度">
						${irmsTransMarkstoneList.latitude}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="埋深">
						${irmsTransMarkstoneList.buryDeep}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="产权">
						<eoms:id2nameDB id="${irmsTransMarkstoneList.property}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="使用单位">
						${irmsTransMarkstoneList.useUnit}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所有权人">
						${irmsTransMarkstoneList.owner}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="类型">
						<eoms:id2nameDB id="${irmsTransMarkstoneList.type}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
