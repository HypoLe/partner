...
function excelFileUpload() {
	...
	Ext.Ajax.request({
		...
		params :{resourceName:"IrmsTransOpconbox"},
		method : 'post',
		form:'excel2xmlForm',
		isUpload:true,
...
<div align="center"><b>光电接头盒-列表页面</b></div><br><br/>
...
<div id="listQueryObject" style="border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;display:none">
	<form action="${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransOpconbox" method="post">
		<fieldset>
			<legend>快速查询</legend>
			<table id="sheet" class="formTable">
						...
						<td class="content" >
								<input class="text" type="text" name="boxNameStringLike" value="${boxNameStringLike}" />
						</td>
						...
						<td class="content" >
								<input class="text" type="text" name="boxObjectStringEqual" value="${boxObjectStringEqual}" />
						</td>
						</tr>
				...
				<tr>
					<td colspan="4">
						<div align="left">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="button" class="btn" value="查询所有" onclick="window.location.href='${app}/netresource/pnrNetResourceAction.do?method=gotoListPage&model=com.boco.eoms.partner.netresource.model.trans.IrmsTransOpconbox'"/>
						</div>
					</td>
				</tr>
			</table>
...
<!--导出功能开始-->
<%--
<eoms:excelExport modelName="com.boco.eoms.partner.netresource.model.trans.IrmsTransOpconbox" 
							 serviceBeanId="eomsService"   title="网络资源--传输线路--光电接头盒" >
		<eoms:row name="光电接头盒名称" value="boxName"/>
		<eoms:row name="接头盒类型" value="boxType"/>
		<eoms:row name="所属区域" value="relatedDistrict"/>
		<eoms:row name="所属对象" value="boxObject"/>
		<eoms:row name="设备厂家" value="vendor"/>
		<eoms:row name="设备状态" value="status"/>
		<eoms:row name="接头方式" value="connectWay"/>
...
<logic:present name="irmsTransOpconboxList" scope="request">
	<display:table id="irmsTransOpconboxList"
					name="irmsTransOpconboxList" 
					pagesize="${pagesize}" size="${size}"
					requestURI="${app}/netresource/pnrNetResourceAction.do" export="false" 
					sort="list" cellspacing="0" cellpadding="0" class="table irmsTransOpconboxList" partialList="true" >
				<display:column sortable="true" headerClass="sortable" title="光电接头盒名称">
						${irmsTransOpconboxList.boxName}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="接头盒类型">
						<eoms:id2nameDB id="${irmsTransOpconboxList.boxType}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属区域">
						${irmsTransOpconboxList.relatedDistrict}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="所属对象">
						${irmsTransOpconboxList.boxObject}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="设备厂家">
						${irmsTransOpconboxList.vendor}
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="设备状态">
						<eoms:id2nameDB id="${irmsTransOpconboxList.status}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="接头方式">
						<eoms:id2nameDB id="${irmsTransOpconboxList.connectWay}" beanId="ItawSystemDictTypeDao"/>
				</display:column>
				<display:column sortable="true" headerClass="sortable" title="容量">
						${irmsTransOpconboxList.capacity}
				</display:column>
