...
jq(function(){
	jq("#addAndroidApp").click(function(){
		window.location='${app}/partner/inspect/androidAppManagerAction.do?method=goToEditAndroidApp';
	})
})
	...
	if(confirm("确定要删除吗?")){
		...
		<logic:present name="androidAppInfoList" scope="request">
			<display:table name="androidAppInfoList" cellspacing="0"
				...
				<a href="${app}/partner/inspect/androidAppManagerAction.do?method=goToEditAndroidApp&id=${androidAppInfoList.id }">${androidAppInfoList.appName}</a>
				</display:column>
		...
		<c:choose>
			<c:when test="${androidAppInfoList.downloadStatus == 0}">
			    <input type="radio" name="downloadStatus_${androidAppInfoList.id }" value="0" checked>发布
				<input type="radio" name="downloadStatus_${androidAppInfoList.id }" value="1" >暂停发布  
			</c:when>
			<c:when test="${androidAppInfoList.downloadStatus == 1}">
				<input type="radio" name="downloadStatus_${androidAppInfoList.id }" value="0" >发布  
				<input type="radio" name="downloadStatus_${androidAppInfoList.id }" value="1" checked>暂停发布  
			</c:when>
		</c:choose>
		&emsp;&emsp;<input align="right" type="button" onclick="changeDownloadStatus('${androidAppInfoList.id }')" value="修改" />
		</display:column>
		<display:column sortable="true" headerClass="sortable" title="操作">
		<input align="right" type="button" onclick="deleteApp('${androidAppInfoList.id }')" value="删除" />
		</display:column>
			</display:table>
		</logic:present>
