...
jq(function(){
	...
	jq("#up").click(function(){
		if(curPage == '' || curPage-0<=1){
			...
			jq("#showImg").html("<img  src='${app }/partner/inspect/inspectPlanExecute.do?method=showPicture&id="+pid+"&curPage="+curPage+"&idType=inspectPlan'>");
			Ext.Ajax.request({
...
//先获得屏幕的宽度和高度
		...
		jq("#shoupicture").show();
		//下一步操作时把这张图片显示出来
		jq("#showImg").html("<img  src='${app }/partner/inspect/inspectPlanExecute.do?method=showPicture&id="+id+"&curPage='>");
	...
	<display:table name="inspectPlanItemList" cellspacing="0" cellpadding="0"
		...
		<display:column title="正常值范围" >
			...
			<display:column title="巡检结果" >
				...
				<c:if test="${inspectPlanItemList.inputType eq 'text'}">${inspectPlanItemList.itemResult}</c:if>
...
<input type="button" onclick="javascript:history.back();" class="btn" value="返回"/>
	</br>
	</div>
</div>
<div id="shoupicture" style="background: gray;position: absolute;z-index: 15;">
