...
function relation1(){
	 ...
	 var objName= document.getElementsByName("checkbox11");
	        for (var i = 0; i<objName.length; i++){
	                ...
	                if(document.getElementById(objName[i].value).value!=''){
	                	alert("部分所选资源已选分配站点责任人，必须先取消再分配！");
	                	return;
	                }
	                }
	        } 
    ...
    var url ="${app}/partner/res/PnrResConfig.do?method=assignCheckPerWindow";
    ...
    Ext.Ajax.request({
    	method:"post",
		url: "${app}/partner/res/PnrResConfig.do?method=addAssignCheckPer",
		params:{
			seldelcar:string,
			prov:pro
		},
...
<div id="listQueryObject" 
	 style="display:none;border:1px solid #98c0f4;border-top:0;padding:5px;background-color:#eff6ff;">
		<html:form action="PnrResConfig.do?method=assignCheckPer" styleId="myform" method="post">
			...
			<table class="listTable">
				...
				<tr>
					...
					<td class="content">
						<eoms:comboBox name="resTypeStringEqual" defaultValue="${pnrResConfigForm.resType}" styleClass="select"
							id="resourceTeype" initDicId="${pnrResConfigForm.specialty}" alt="allowBlank:false"
							sub="resourceLevel" />
					</td>
					<td class="label">资源级别</td>
					<td class="content">
						<eoms:comboBox name="resLevelStringEqual" defaultValue="${pnrResConfigForm.resLevel}" styleClass="select"
				...
				<tr>
					...
					<input name="executeObjectStringEqual" id="executeOb" value="" type="hidden" />					
					<eoms:xbox id="provTree"
						...
						rootId="" rootText="公司树" valueField="executeOb" handler="provName"
	...
	<html:form action="PnrResConfig.do?method=calcleChargePerson" method="post" styleId="form2" >
	<br/>
	...
	<display:table name="list" cellspacing="0" cellpadding="0"
		...
		<display:column  sortable="true" title="站点负责人" 
				headerClass="sortable">
				<eoms:id2nameDB id="${list.chargePerson}" beanId="tawSystemUserDao" />
				<input name="chargePerson" type="hidden" id="${list.id}" value="${list.chargePerson}" />
		</display:column>
		
		<display:column  sortable="true"  title="巡检单位"
				headerClass="sortable"  paramId="id" paramProperty="id">
