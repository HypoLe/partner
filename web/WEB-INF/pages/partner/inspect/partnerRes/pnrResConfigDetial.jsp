...
function submitForm(){
  ...
  <body>
	...
	<table class="formTable" id="sheet">
		<c:if test="${existRes eq false}">
		<caption>
			<div class="header center">不存在该资源，可能已删除！</div>
		</caption>
		</c:if>
		<c:if test="${existRes eq true}">
		<caption>
		...
		<tr>
			...
			<td class="content">
				<eoms:id2nameDB id="${pnrResConfig.eographicalEnvironment}" beanId="ItawSystemDictTypeDao" />		
			</td>
			<td class="label">区域类型</td>
			<td class="content">
				<eoms:id2nameDB id="${pnrResConfig.regionType}" beanId="ItawSystemDictTypeDao" />		
			</td>
		</tr>
	...
	<!--<tr class="jizhan">
		...
		<tr class="family">
			...
			<td class="label">客户数</td>
			<td>
				${pnrResFamilyBroadband.clientNumber}
			</td>
			<td class="label">业务类型</td>
			<td>
			
			<c:choose>
				<c:when test="${pnrResFamilyBroadband.clientType eq '--请选择--' }">
				
				</c:when>
				<c:otherwise>
				${pnrResFamilyBroadband.clientType}
				</c:otherwise>
			</c:choose>
		...
		<tr class="family">
			...
			<td class="label">所属站点名称</td>
			<td>
				${pnrResFamilyBroadband.stationName}
			</td>
			<td class="label">所属站点编号</td>
			<td>
				${pnrResFamilyBroadband.stationNumber}
			</td>
		</tr>-->
		...
		<tr class="wlan">
			...
			<c:if test="${pnrResWlan.propertyType eq 1}">
			共享
			</c:if>
			<c:if test="${pnrResWlan.propertyType eq 2}">
			自建
			</c:if>
			<c:if test="${pnrResWlan.propertyType eq 3}">
			租用
			</c:if>
			</td>
		</tr>
		...
		<tr class="wlan">
			...
			<td class="label">TD开通年月  </td>
			<td>
				${pnrResWlan.TDOpenTime}
			</td>
		</tr>
		<tr class="wlan">
			<td class="label">搬迁与配置调整情况  </td>
			<td colspan="3">
				${pnrResWlan.adjustmentMessage}
			</td>
