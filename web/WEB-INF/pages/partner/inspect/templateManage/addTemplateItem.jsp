...
function changeInputType(obj){
	...
	if('1222406' == value){//自定义 2016-09-27添加
		jq(tr.getElementsByName("dictId")[i]).attr("disabled",true);
		jq(tr.getElementsByName("dictSpan")[i]).attr("disabled",true);
		jq(tr.getElementsByName("dictTypeName")[i]).attr("disabled",true);
		jq(tr.getElementsByName("hidenRadioValue")[i]).attr("disabled",true);
...
<form method="post" id="templateItemForm" name="templateItemForm" action="inspectTemplateManger.do?method=editTemplateItem">
	...
	<thead>
		<tr>
			<th>
				设备类别*<input type="hidden" name="templateId" value="${templateId }" />
			</th>
			<th>
				巡检项目*
			</th>
			...
			<th>
				正常值范围12*<div style="width: 140px;height: 1px;"></div>
			</th>
		...
		<tr id="templateItem">
			...
			<c:when test="${inspectTemplateItem.inputType eq 'custom'}"><!-- 自定义，用于巡检项的特殊情况 2016-09-27添加 --> 
			...
			<!-- 由于该周期没有用,暂时隐藏,默认为月,该模版中的字段对巡检而言没有用到 -->
			<td id="tdcycle" class="content" style="display: none;">
				<eoms:comboBox defaultValue="112200104" name="cycle" id="cycle" initDicId="1122001"
