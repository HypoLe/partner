...
jq(function(){
		...
		v.custom = function(){ 
			...
			for(var j=0;j<inputTypes.length;j++){
				...
				if(inputTypes[j].value=='1222404'){ 
					var tt = document.getElementById("defaultValue").value;
					var len = document.getElementsByName("defaultValuec")[muNum].options;
					var str = '';
					for(var m=0;m<len.length;m++){
						var op = document.getElementsByName("defaultValuec")[muNum].options[m];
						if(op.selected == true){
							str = str+op.value+'|';
						}
					}
					muNum = muNum+1;
					jq(document.getElementsByName("defaultValue")[j]).val(str);
				}
...
function changeInputType(obj){
	...
	if('1222401' == value){//文本
		...
		jq(tr.getElementsByName("tdDefaultValue")[i]).html("<td><input class=\"text\" type=\"hidden\" name=\"defaultValue\" id=\"defaultValue\"  /></td>");
		return;
	}
	if('1222402' == value){//数值
		jq(tr.getElementsByName("dictId")[i]).attr("disabled",true);
...
function getValues(num){
	...
	htmlStr3 = '<select id="defaultValuec" name="defaultValuec" multiple="multiple" class="select"  alt="allowBlank:false" >';
	...
	Ext.Ajax.request({
		...
		success: function ( result, request ) {
			...
			if(inputType == 'radio'){
				...
				htmlStr3 = htmlStr3 + '</select> <input type="hidden" id="defaultValue" name="defaultValue" value="aa">';
		...
		jq(document.getElementsByName("tdNormalRange")[num]).html('<div style="width: 140px;"></div>');
	}
}
...
function selectDevice(obj){
		...
		if(arr){
			...
			jq(tr).find("#bigitemName").val(arr[1]);
		}
	}
...
<form method="post" id="templateItemForm" name="templateItemForm" action="${app}/partner/inspect/inspectTemplateManger.do?method=editTemplateItem">
	<input type="hidden" name="templateId" value="${templateId }" />
	<input type="hidden" name="netDevice" value="netDevice"/>
	<table  id="tab" class="table list" cellspacing="0" cellpadding="0">
		...
		<tr id="templateItem">
			...
			<td class="content">
			<input class="text" type="text" name="bigitemName" readonly="readonly"
					id="bigitemName" value="${inspectTemplateItem.bigitemName }" onclick="selectDevice(this)" alt="allowBlank:false"/>
			...
			<td class="content">
				<input class="text" type="text" name="pictureNum" alt="re:/^((\d{1}?)|(10))?$/,re_vt:'请输入10以内的正整数'" />
			</td>
	...
	<input type="button" value="返回" class="btn" onclick="javascript:window.history.back();" />
</form>
</div>
