...
public class TawCommonsAccessoriesManagerCOSImpl extends BaseManager implements
	...
	public List saveFile(HttpServletRequest request, String appCode,
		...
		try {
			...
			if (request.getContentType() != null
				...
				//上传文件超过设置的文件大小
				int currentMaxSize=maxSize.intValue()*1024*1024;
				System.out.println("-----最大的大小="+currentMaxSize);
				int contentLength = request.getContentLength();//单位字节（B）
				System.out.println("---实际上传文件的大小="+contentLength);
				if(contentLength > currentMaxSize){
					TawCommonsAccessories tawCommonsAccessories=new TawCommonsAccessories();
					tawCommonsAccessories.setAccessoriesName("exceed");
					tawCommonsAccessories.setAccessoriesCnName("上传文件过大,请传"+maxSize+"M以下的文件");
				...
				String path = this.filePath.substring(this.filePath
						...
						+ config.getPath();
				//15-7-3添加当天日期文件夹-start
				  String fileName1 = StaticMethod
		    		.getCurrentDateTime("yyyyMMddHHmmss")+StaticMethod.getRandomCharAndNumr(4);
				String tPath = File.separatorChar + fileName1.substring(0, 6);
			    tPath = tPath.replace("\\", "/");
			    //15-7-3添加当天日期文件夹-end
			    String savePath = path+tPath;
			    String filePath = rootFilePath + path+tPath;
				...
				while ((part = mp.readNextPart()) != null) {
					...
					if (part.isFile()) {
						...
						if (fileCnName != null) {
							...
							accessories.setAccessoriesPath(savePath);
	...
	public TawCommonsAccessories getSystemToOther(String id, String uploadType) {
		...
		try {
			...
			if (uploadType.equals("ftp")) {
				contentPath = StaticMethod.nullObject2String(XmlManage.getFile(
						"/config/accessoriesServer.xml").getProperty(
						"rootpath.ftp"));
				...
				contentPath = StaticMethod.nullObject2String(XmlManage.getFile(
							"/config/accessoriesServer.xml").getProperty(
							"rootpath.http"));
					/*"ftp://ngsf:Eoms!118@10.25.2.114:21/upload/";*/
					/**/
	 ...
	 * 
	 */
	public List saveFiles(String fileCnName,String fileName,String savePath,long fileSize,
			String appCode) throws AccessoriesException {
		// TODO Auto-generated method stub
		List list = new ArrayList();
		System.out.println("进入-------saveFiles");
		try {
							...
							accessories.setAccessoriesName(fileName);
							accessories.setAccessoriesPath(savePath);
							accessories.setAccessoriesSize(fileSize);
							accessories.setAccessoriesUploadTime(StaticMethod
									.getLocalTime());

							accessories.setAppCode(appCode);
							this.dao.saveTawCommonsAccessories(accessories);
							
							
							if (!fileCnName.equals("")) {	
								list = dao.getAllFileByName("'"+fileCnName+"'");
							}
