			...
			if (pages != null) {
				...
				for (Iterator pageIt = pages.getPage().iterator(); pageIt
					...
					for (Iterator headersIt = page.getHeaders().iterator(); headersIt
						...
						//
						if (headers.getEndRow().intValue() < 0) {
							endRow = headers.getStartRow().intValue()
									+ list.size();
						} else if (list.size() > (headers.getEndRow()
								.intValue() - headers.getStartRow().intValue())) {
							endRow = headers.getEndRow().intValue();
						} else {
							endRow = headers.getStartRow().intValue()
									+ list.size();
						...
						/*
						 * if(list.size()) // 按行写入excel
						 */
						for (int i = headers.getStartRow().intValue(); i < endRow; i++) {
							Object obj = list.get(i
