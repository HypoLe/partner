...
public class TawSystemTreeBo {
	 ...
	 */
	/*  节点处没有传回参数hasRight
	public JSONArray getDeptUserTreeForPriv(String deptid,List roleList,String userid) {
		
		RoleIdList roleIdList = (RoleIdList) ApplicationContextHolder.getInstance().getBean("roleIdInGroup");
		ITawSystemAreaManager areaManager = (ITawSystemAreaManager) ApplicationContextHolder.getInstance().getBean("ItawSystemAreaManager");
		String hasRight = "0";
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String subDeptName = subDept.getDeptName();
				JSONObject jitem = new JSONObject();
				jitem.put("id", subDept.getDeptId());
				jitem.put("text", subDeptName);
				jitem.put(UIConstants.JSON_NODETYPE, "dept");
				...
				for(int k=0;k<roleList.size();k++){
					TawSystemSubRole tempRole = (TawSystemSubRole)roleList.get(k);
					if(tempRole.getRoleId() == roleIdList.getPrivAdminRoleId().intValue()){
					       if((tempRole.getDeptId()!=null&&!tempRole.getDeptId().equals(""))&&(tempRole.getArea()==null||tempRole.getArea().equals(""))){//子角色 只存了地市id，没存部门id
					    	   ...
					    	   if(tempRole.getDeptId().equals(subDept.getAreaid())){//属于这个地市的部门都有权限
						        ...
						        if(tempRole.getArea().equals(subDept.getDeptId())){//部门是子角色所选部门，或者部门是子角色所选部门的子部门，给其权限
					            	hasRight = "1";
							        break;
						        }
						        else { 
						            TawSystemDept tsd = null;
						            subDeptID = subDept.getParentDeptid();
						            while(!subDeptID.equals(DeptConstants.ROOT_PARENTDEPT_ID)) {
						               tsd = deptbo.getDeptinfobydeptid(subDeptID, "0");
						               if(tsd.getId()!=null){
						        ...
						        if(isArea.equals("1")){//当前部门在子角色规定的地域或子地域里，那么判断当前部门是否是子角色规定的部门或子部门
							        if(tempRole.getArea().equals(subDept.getDeptId())){
						            	hasRight = "1";
								        break;
							        }
							        else { 
							            ...
							            while(!subDeptID.equals(DeptConstants.ROOT_PARENTDEPT_ID)) {
							               ...
							               if(tsd.getId()!=null){
								               ...
								               subDeptID = DeptConstants.ROOT_PARENTDEPT_ID;
								               hasRight = "0";
							               }
							             }
							            if(hasRight.equals("1"))break;
						         ...
						         else hasRight = "0";//部门所属地市不在子角色地市里
						    }
					    else hasRight = "0";
					}
				} 
				if(hasRight.equals("1")){
					jitem.put("allowcm-deptpriv", true);
					jitem.put("allowcm-deptprivlook", true);
					jitem.put(UIConstants.JSON_ICONCLS, "dept");
				}
				else {
					jitem.put("allowcm-deptpriv", false);
					jitem.put("allowcm-deptprivlook", true);
					jitem.put(UIConstants.JSON_ICONCLS, "dept-readonly");
				}
			...
			if (userlist.size() > 0) {
				for (int j = 0; j < userlist.size(); j++) {
					...
					TawSystemDept dept = (TawSystemDept) deptbo.getDeptinfobydeptid(sysuser.getDeptid(),"0");
					...
					if(hasRight.equals("1")){
						jitem.put("allowcm-priv", true);
						jitem.put("allowcm-cptroom", true);
						jitem.put("allowcm-dept", true);
						jitem.put("allowcm-look", true);
					}
					else {
						jitem.put("allowcm-priv", false);
						jitem.put("allowcm-cptroom", false);
						jitem.put("allowcm-dept", false);
						jitem.put("allowcm-look", true);
					}
					json.put(jitem);
				}
	...
	//节点处传回了参数hasRight
	public JSONArray getDeptUserTreeForPriv(String deptid,List roleList,String userid,String hasRight) {
		
		RoleIdList roleIdList = (RoleIdList) ApplicationContextHolder.getInstance().getBean("roleIdInGroup");
		ITawSystemAreaManager areaManager = (ITawSystemAreaManager) ApplicationContextHolder.getInstance().getBean("ItawSystemAreaManager");
		String parentHasRight = hasRight;
	    if(userid.equals("admin")){
			hasRight = "1";
		}
					...
					if(tempRole.getRoleId() == roleIdList.getPrivAdminRoleId().intValue()){
					       if((tempRole.getDeptId()!=null&&!tempRole.getDeptId().equals(""))&&(tempRole.getArea()==null||tempRole.getArea().equals(""))){//子角色 只存了地市id，没存部门id
					    	   ...
					    	   if(tempRole.getDeptId().equals(subDept.getAreaid())){//属于这个地市的部门都有权限
						        ...
						        if(tempRole.getArea().equals(subDept.getDeptId())){//部门是子角色所选部门，给其权限;父部门没有权限，不用考虑子部门继承父部门权限的问题。
					            	hasRight = "1";
							        break;
						        }
						        else hasRight = "0";
