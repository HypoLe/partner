...
import com.boco.eoms.commons.statistic.base.dao.impl.DetailDAOOracleImpl;
...
//				+"," + "param is " + param +"," + "condSql size is " + condSql);
	...
	public List getDetailList(final int[] total,int pageIndex, int pageSize, Map actionMap ,String excelConfigURL,String statID) {
		...
		try {
			...
			//获取自定义数据源
			QueryDefine queryDefine = kpiDefine.getQueryDefineByFieldID(fieldId);
			com.boco.eoms.commons.statistic.base.config.model.DataSource dataSource = 
				queryDefine.getDataSource();
			Connection conn = null;

			if(dataSource !=null && dataSource.vilidataDataSource())
...
*/
			...
			if (total[0] < 0) {
				...
				if(conn != null)
				{
					total[0] = statJdbcDAO.queryForInt(totalSQL, conn);
				}
				else
				{
...
//			}
			...
			if(conn != null)
			{
				...
				IStatDetailDAO isdo=  null;
				if(driverClass.indexOf("oracle")!=0)
				{
					//oracle的分页代码
					isdo = new DetailDAOOracleImpl();
				}
				else if(driverClass.indexOf("informix")!=0)
				{
					//informix分页代码
					isdo = new DetailDAOInformixImpl();
				}
				list= isdo.getListDetail(conn,detailQueryStr, statDetailVO, pageIndex, pageSize);
			}
			else
			{
				list= statDetailDAO.getListDetail(detailQueryStr, statDetailVO, pageIndex, pageSize);
			}
