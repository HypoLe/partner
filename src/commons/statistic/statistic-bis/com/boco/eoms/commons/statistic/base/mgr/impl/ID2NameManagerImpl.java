...
public class ID2NameManagerImpl implements ID2NameManager {
	...
	public String id2Name(String id, String beanId) {
		...
		String name = null;
		try {
			// 添加一个特殊情况:痕迹管理报表中需要把code-name
			if("ItawSystemPrivOperationManager".equals(beanId)){
				ITawSystemPrivOperationManager bo = (ITawSystemPrivOperationManager)ApplicationContextHolder.getInstance().getBean(beanId);
				name=bo.getPrivOperationbyCode(id).getName();
			}else{
			// 通过beanid取bean
