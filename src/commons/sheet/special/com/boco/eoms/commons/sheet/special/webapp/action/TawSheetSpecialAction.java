 ...
 * 
 * @struts.action name="tawSheetSpecialForm" path="/tawSheetSpecials"
 *                scope="request" validate="false" parameter="method"
 *                input="mainMenu"
 * @struts.action name="tawSheetSpecialForm" path="/editTawSheetSpecial"
 *                scope="request" validate="false" parameter="method"
 *                input="list"
 * @struts.action name="tawSheetSpecialForm" path="/saveTawSheetSpecial"
 ...
 * @struts.action-forward name="search" path="/tawSheetSpecials.do"
 *                        redirect="true"
 */
public final class TawSheetSpecialAction extends BaseAction {
	...
	public ActionForward xdelete(ActionMapping mapping, ActionForm form,
		...
		TawSheetSpecial sepcial = (TawSheetSpecial) mgr
				.getSpecialByspecialId(speid);
		List sameLevelspe = (ArrayList) mgr.getSameLevelspecial(sepcial
				.getParspeid(), new Integer(sepcial.getOrdercode()));

		if ((sameLevelspe == null || sameLevelspe.size() <= 1)
				&& !sepcial.getParspeid().equals("-1")) {
		...
		if (sameLevelspe != null && sameLevelspe.size() > 0) {
			List sonSpeList = (ArrayList) mgr
					.getAllSonspecialByspecialid(speid);
			for (int i = 0; i < sonSpeList.size(); i++) {
				TawSheetSpecial sonspecial = (TawSheetSpecial) sonSpeList
						.get(i);
	...
	public ActionForward xedit(ActionMapping mapping, ActionForm form,
		...
		if (tawSheetSpecialForm.getId() != null) {
			...
			mgr.saveTawSheetSpecial(tawSheetSpecial, tawSheetSpecialForm
					.getNewuserid());
	...
	public ActionForward xsave(ActionMapping mapping, ActionForm form,
		...
		ActionMessages messages = new ActionMessages();
		TawSheetSpecialForm tawSheetSpecialForm = (TawSheetSpecialForm) form;
		boolean isNew = ("".equals(tawSheetSpecialForm.getId()) || tawSheetSpecialForm
				.getId() == null);
		...
		if (parSpeid == null
			...
			tawSheetSpecial.setParspeid(StaticVariable.AREA_DEFAULT_PARENTVAL);
			tawSheetSpecial.setLeaf(StaticVariable.AREA_DEFAULT_LEAFONE);
			tawSheetSpecial.setOrdercode(StaticVariable.AREA_DEFAULT_LEAFONE);
			...
			tawSheetSpecial.setLeaf(StaticVariable.AREA_DEFAULT_LEAFONE);
			tawSheetSpecial.setOrdercode(String.valueOf(new Integer(parSpe
					.getOrdercode()).intValue()
					+ StaticVariable.AREA_DEFAULT_ORDERCODEVAL.intValue()));
			// TawSheetSpecial parConspe =
			// (TawSheetSpecial)mgr.getSpecialByspecialId(parSpeid);
	...
	public ActionForward xget(ActionMapping mapping, ActionForm form,
		...
		String speid = request.getParameter("id");
		ITawSheetSpecialManager mgr = (ITawSheetSpecialManager) getBean("ItawSheetSpecialManager");
		TawSheetSpecial _objOneOpt = (TawSheetSpecial) mgr
				.getSpecialByspecialId(speid);
