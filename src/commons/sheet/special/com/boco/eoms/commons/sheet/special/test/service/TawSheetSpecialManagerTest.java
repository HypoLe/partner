...
public class TawSheetSpecialManagerTest extends BaseManagerTestCase {
	...
	protected void setUp() throws Exception {
		...
		tawSheetSpecialManager.setDao((TawSheetSpecialDao) tawSheetSpecialDao
				.proxy());
	}
	...
	public void testGetTawSheetSpecials() throws Exception {
		...
		tawSheetSpecialDao.expects(once()).method("getTawSheetSpecials").will(
				returnValue(results));

		List tawSheetSpecials = tawSheetSpecialManager
				.getTawSheetSpecials(null);
	...
	public void testGetTawSheetSpecial() throws Exception {
		// set expected behavior on dao
		tawSheetSpecialDao.expects(once()).method("getTawSheetSpecial").will(
				returnValue(new TawSheetSpecial()));
		TawSheetSpecial tawSheetSpecial = tawSheetSpecialManager
				.getTawSheetSpecial(tawSheetSpecialId);
	...
	public void testSaveTawSheetSpecial() throws Exception {
		...
		// set expected behavior on dao
		tawSheetSpecialDao.expects(once()).method("saveTawSheetSpecial").with(
				same(tawSheetSpecial)).isVoid();
	...
	public void testAddAndRemoveTawSheetSpecial() throws Exception {
		...
		tawSheetSpecialDao.expects(once()).method("removeTawSheetSpecial")
				.with(eq(tawSheetSpecialId));
		...
		Exception ex = new ObjectRetrievalFailureException(
				TawSheetSpecial.class, tawSheetSpecial.getId());
		tawSheetSpecialDao.expects(once()).method("removeTawSheetSpecial")
				.isVoid();
		tawSheetSpecialDao.expects(once()).method("getTawSheetSpecial").will(
				throwException(ex));
		...
		try {
			...
			fail("TawSheetSpecial with identifier '" + tawSheetSpecialId
					+ "' found in database");
	 ...
	 * 
	 */
	public void testListSpecialsByType() {
		List list = tawSheetSpecialManager.listSpecialsByType("1");
		assertNotNull(list);
