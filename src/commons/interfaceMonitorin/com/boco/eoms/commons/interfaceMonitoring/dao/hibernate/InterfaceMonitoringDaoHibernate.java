...
public class InterfaceMonitoringDaoHibernate extends BaseDaoHibernate implements
	...
	public void save(InterfaceMonitoring interfaceMonitoring) {
	 ...
	 public InterfaceMonitoring loadInterfaceMonitoring(String worksheetid) throws
     HibernateException {
   //Session s = HibernateUtil.currentSession();
		 InterfaceMonitoring interfaceMonitoring =new InterfaceMonitoring();
   String hql =
       "from InterfaceMonitoring as interfaceMonitoring where interfaceMonitoring.id = '" +
       worksheetid + "'";
   List list = getHibernateTemplate().find(hql);
   if(list.size()>0){
	   interfaceMonitoring=(InterfaceMonitoring) list.get(0);   
   }
   return interfaceMonitoring; 
 }
}
