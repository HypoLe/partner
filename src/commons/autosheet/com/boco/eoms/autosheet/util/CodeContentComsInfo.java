...
/*****************************************************************
 将指定的codename,record_id from taw_codecontent中取出来放入hashtable,
 并通过getCodeContentInfo让其驻留在内存里，并给出取数据的方法。
 *****************************************************************/
public class CodeContentComsInfo{
  ...
  private void getCodeContentInfo(String typeid) throws Exception{
    ...
    codenames=(ArrayList)(staticobj.getRecordFromObj("CodeContentInfo"+typeid,"codenames"));
    recordids=(ArrayList)(staticobj.getRecordFromObj("CodeContentInfo"+typeid,"recordids"));
  ...
  private void setCodeContentInfo(String typeid) throws Exception{
    ...
    rt = new RecordSet() ;
    try{
      rt.execute("select distinct dict_id,dict_name,dict_type from taw_ws_dict where deleted=0 and dict_type="+typeid) ;
      while(rt.next()){
      recordids.add(StaticMethod.null2String(rt.getString(1)));
        codenames.add(StaticMethod.fromBaseEncoding(rt.getString(2)));
