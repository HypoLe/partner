...
public class TawSystemSessionAction extends BaseAction {
	...
	private com.boco.eoms.db.util.ConnectionPool ds = com.boco.eoms.db.util.ConnectionPool
			.getInstance();
	private static int savetotalcount = 1;
	...
	public ActionForward performInit(ActionMapping mapping,
		...
		logSave log = logSave.getInstance(modeName, userid, "0001", request
				.getRemoteAddr(), userid + " 于:"
				+ StaticMethod.getCurrentDateTime() + " 登录系统.", "111");
		log.info();
		sessionform.setRomteaddr(request.getRemoteAddr());
		sessionform.setLoginTime(StaticMethod.getTimestamp());
		sessionform.setLoginTimeStr(StaticMethod.getLocalString());
		// 获取分公司信息
		// 由于目前系统暂时无法直接获取用户所属公司的信息，只能截取deptId的前3位，到boco.xml文件中查找相关的配置信息
		// 如果ID可以匹配，则在TawSystemSessionForm中增加所属公司的ID和名称，如果没有匹配的数据，那么就默认为省公司的ID和名称
		String tempCompany = "";
		String tempCompanyName = "";
		if (sessionform.getDeptid().length() >= 3) {
			tempCompany = sessionform.getDeptid().substring(0, 3);
			tempCompanyName = StaticMethod.nullObject2String(XmlManage
					.getBoco().getProperty("company.company" + tempCompany));
		}
		if (!tempCompanyName.equals("")) {
			sessionform.setCompanyId(tempCompany);
			sessionform.setCompanyName(tempCompanyName);
		} else {
			sessionform.setCompanyId("1");
			sessionform.setCompanyName("省公司");
		}
		...
		*/
		 ...
		 * .getInstance().getBean("WorkflowSecutiryService"); Subject subject =
		 * safeService.logIn(userid, password);
		...
		if ("admin".equals(sessionform.getUserid())) {
			if (menu_ip.equals("on")) {
				...
				request.getSession().setAttribute(
						"menu",
						Ip_Double_choice.ip2DoubleIpFromUI(usertree
								.getPrivAdminMenu(userid), ipconfig, type));
			} else {
			...
			for (Iterator it = wapMainList.iterator(); it.hasNext();) {
				...
				for (Iterator itNext = wapLeafList.iterator(); itNext.hasNext();) {
					TawSystemPrivOperation operationNext = (TawSystemPrivOperation) itNext
							.next();
					List wapLeafListNext = PrivMgrLocator
							.getPrivMgr()
							.listOpertionwap(
...
//		request.getSession().setMaxInactiveInterval(-1);
	 ...
	 * @param response
	 */
	public void performInit4A(ActionMapping mapping, ActionForm actionForm,
			HttpServletRequest request, HttpServletResponse response) {
	...
	public ActionForward saveWorkSerial(ActionMapping mapping,
		...
		// 外系统登录，例如门户，设置返回页面为填写值班记录页面
		String portalLogin = (String) request.getSession().getAttribute(
				"portalLogin");// 门户登录标志
		if (portalLogin != null && portalLogin.equals("1")) {
			response.reset();
			forward = "portalLoginSuccess";
		}
		...
		if (workserial > 0) {
			...
			try {
				...
				TawRmRecordDAO tawRmRecordDAO = new TawRmRecordDAO(ds);
				/**
				 * TawRmSysteminfoDAO tawRmSysteminfoDAO = new
				 * TawRmSysteminfoDAO( ds); tawRmSysteminfo =
				 * tawRmSysteminfoDAO.retrieve(roomid); // 得到错班时间 if
				 * (tawRmSysteminfo != null) { int_stagger =
				 * tawRmSysteminfo.getStaggertime(); } // 取上一个班次的班次号 int
				 * intWorkSerialTemp = tawRmRecordDAO.getExSerial(workserial, 1,
				 * int_stagger); TawRmRecordPerDAO tawRmRecordPerDAO = new
				 * TawRmRecordPerDAO(ds); // 取出上一个班次的未完成的事情遗留到这个班次来。 String
				 * whereSql = "Workserial = " + intWorkSerialTemp +
				 * " and complete_flag = 0"; boolean IsHangOver =
				 * tawRmRecordPerDAO .IsHangOver(intWorkSerialTemp + ""); if
				 * (!IsHangOver) { Vector vector =
				 * tawRmRecordPerDAO.getSearchResult(whereSql); if (vector !=
				 * null) { tawRmRecordPerDAO .saveTawRmRecordPer(vector,
				 * workserial); } }
				 */
				// 值班班长
		...
		try {
			...
			if ("admin".equals(sessionform.getUserid())) {
				...
				for (Iterator it = wapMainList.iterator(); it.hasNext();) {
					TawSystemPrivOperation operation = (TawSystemPrivOperation) it
							.next();
					List wapLeafList = PrivMgrLocator
							.getPrivMgr()
							.listOpertionwap(
									sessionform.getUserid(),
									sessionform.getDeptid(),
									...
									operation.getCode());
					// 逐级向下取.
					for (Iterator itNext = wapLeafList.iterator(); itNext
							.hasNext();) {
						TawSystemPrivOperation operationNext = (TawSystemPrivOperation) itNext
								.next();
						List wapLeafListNext = PrivMgrLocator
