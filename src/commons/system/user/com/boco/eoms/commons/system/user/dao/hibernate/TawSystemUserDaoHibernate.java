...
public class TawSystemUserDaoHibernate extends BaseDaoHibernate implements
	...
	public List getUserBydeptidsTask(String deptid,String rootId,String level) {
		...
		if (deptid.equals(rootId)&&deptid.substring(0, 3).equals("201")) {
			
			List<TawSystemDept> lists = new ArrayList<TawSystemDept>();
			String hql2 = " from TawSystemDept sysdept where sysdept.deptId=?";
			
			String[] values = {deptid};			
			lists = (ArrayList) getHibernateTemplate().find(hql2,values);
			String areaId="";
			
			for(int i =0 ;i< lists.size();i++){
				if(areaId.length()==0){
					areaId=lists.get(i).getAreaid();
				}else{
					areaId+=","+lists.get(i).getAreaid();
				}
			}
			int orderCode=3;
			int rootIdLength = rootId.length();
			if(rootIdLength==5){
				orderCode = 2;
			}else if(rootIdLength==7){
				orderCode = 3;
			}else if(rootIdLength==9){
				orderCode = 4;

			}else if(rootIdLength==11){
				orderCode = 5;

			}
			
			//找到部门id；
			String hqlAreaId = " from TawSystemDept sysdept where sysdept.ordercode="+orderCode+" and sysdept.areaid in("+areaId+")";			
			lists = (ArrayList) getHibernateTemplate().find(hqlAreaId);
			
			
			for(int i =0 ;i< lists.size();i++){
				if(deptids.length()==0){
					deptids=lists.get(i).getDeptId();
				}else{
					deptids+=","+lists.get(i).getDeptId();
				}
			}
		...
		String hql = "select systemuser from TawSystemUser systemuser ";
		
		if("".equals(level)){
				
			   hql += " where ";
		}else{
			   hql += ",PartnerUser partneruser where systemuser.userid = partneruser.userId and partneruser.personCardNo='"+level+"' and ";				
		}
		      
		hql += " systemuser.deptid in("
			+ deptids
			+ ") and systemuser.userid !='admin' and systemuser.deleted='0' order by systemuser.deptid asc,systemuser.username asc";
		// System.out.println(deptid+" "+rootId+" "+level+" "+"hql ===:"+hql);
	...
	public List getUserBydeptids(String deptid,String level) {
		...
		if("".equals(level)){
			 ...
			 hql += " ,PartnerUser p where systemuser.userid = p.userId and p.personCardNo = '"+level+"' and ";
						
		}
		
		hql += " systemuser.deptid='"
			+ deptid
			+ "' and systemuser.userid !='admin' and systemuser.deleted='0' order by systemuser.username";
...
//		System.out.println("hql:==="+hql);
	...
	public List getUserBydeptidsNoSelfTask(String deptid, String userid,String rootId,String level) {
		...
		if("".equals(level)){
		 ...
		 hql += " systemuser.deptid in("
			...
			+ userid + "'and systemuser.deleted='0' order by systemuser.deptid asc,systemuser.username asc";
	...
	public List getUserBydeptidsNoSelf(String deptid, String userid,String level) {
		...
		hql += " systemuser.deptid='"			
			+ deptid
			+ "' and systemuser.userid !='admin' and systemuser.userid !='"
			+ userid + "' order by systemuser.username";
	...
	public TawSystemUser getTawSystemUserByuserid(String userid) {
		TawSystemUser systemuser = new TawSystemUser();
		String hql = " from TawSystemUser systemuser where systemuser.userid='"
				+ userid + "' and systemuser.deleted = '0'";
		List list = new ArrayList();
		list = (List) getHibernateTemplate().find(hql);
		if (list != null) {
	 ...
	 * @return List
	 */
	public List getUsersByName(String userName,String userDeptname,String userId) {
		List list = new ArrayList();
		
		String hqlPre = "from TawSystemUser u where 1=1 ";
		
		
		if(!userDeptname.equals("")) {
			hqlPre += " and u.deptname like '%"+userDeptname+"%' ";
		}
		if(!userName.equals("")) {
			hqlPre += " and u.username like '%"+userName+"%' ";
		}
		if(!userId.equals("")) {
			hqlPre += " and u.userid like '%"+userId+"%' ";
		}
		
		hqlPre += " and deleted='0' order by username";
		hqlPre = hqlPre.replace("1=1  and", " ");
		list = (ArrayList) getHibernateTemplate().find(hqlPre);
//		if(userName.equals("")&&userId.equals("")&&!userDeptname.equals("")){
...
//			list = (ArrayList) getHibernateTemplate().find(hql);
//		}
//		if(!userName.equals("")&&!userDeptname.equals("")&&!userId.equals("")){
//			String hql = "from TawSystemUser u where u.username like '%" + userName
//			+ "%' or u.deptname like '%" + userDeptname + "%' or u.userid like '%" + userId + "%'  and deleted='0' order by username";
//			list = (ArrayList) getHibernateTemplate().find(hql);
//		}
//		if(userName.equals("")&&userDeptname.equals("")&&!userId.equals("")){
//			String hql = "from TawSystemUser u where u.userid like '%" + userId
//			+ "%' and deleted='0' order by username";
//			list = (ArrayList) getHibernateTemplate().find(hql);
//		}
		return list;
	...
	public List listByNameQuery(String q){
		String hql = "select u.username, u.userid from TawSystemUser u where u.username like '%" + q
				+ "%' and deleted='0'  order by username";
		List list = new ArrayList();
		list = (ArrayList) getHibernateTemplate().find(hql);
		return list;
	...
	/* (non-Javadoc)
	 * @see com.boco.eoms.commons.system.user.dao.TawSystemUserDao#getUserLike(java.lang.String)
	 */
	public List getUserLike(String deptid) {
		String hql = " select systemuser from TawSystemUser systemuser ,TawSystemDept dept where systemuser.deptid  = dept.deptid and dept.linkid like '"
				+ deptid
	 ...
	 * @param _deptId部门ID
	 * @param userid用户ID
	 * @param isPartners  是否是查询合作伙伴  1 或0
	 * @param containSelf  是否包含自己
	 * @return
	 * add by LEE 2013.4.18
	 */
	public List getSubUserBydeptid(String _deptId,String userid,String isPartners,boolean containSelf) {
		String isPartnersWhere = " ";
		if(null != isPartners){
			isPartnersWhere = "and systemuser.isPartners="+isPartners+"";
		}
		String containSelfWhere = "  ";
		if(!containSelf&&null != userid && !"".equals(userid)){
			containSelfWhere = "and systemuser.userid != '"+userid+"' ";
		}
		String hql = " from TawSystemUser systemuser"
				+ " where systemuser.deptid like '"
				+ _deptId
				+ "%' and systemuser.userid !='admin'  "+isPartnersWhere+"   "+containSelfWhere+"  order by username";
		List list = new ArrayList();
		list = (ArrayList) getHibernateTemplate().find(hql);
	...
	public String getAllSubAccount(String params) {
		...
		String queryString = "from TawSystemUser tawSystemUser where tawSystemUser.deleted=0";
		List list = new ArrayList();
		list = getHibernateTemplate().find(queryString);
		String allCount = Integer.valueOf(list.size()).toString();
		return allCount;
	}
	...
	public String romoveSubaccount(String userId) {
		...
		if (userId != null && !userId.equals("")) {
			try {
				TawSystemUser tawSystemUser = getSingleSubAccountInfo(userId);
				tawSystemUser.setDeleted("1");
				getHibernateTemplate().save(tawSystemUser);
				System.out.println(tawSystemUser.getUsername()
						+ "sssssssssssss");
				// getHibernateTemplate().delete(
				// this.getSingleSubAccountInfo(userId));
				returnString = "0";
			} catch (Exception e) {
				e.printStackTrace();
				returnString = "1";
	...
	public String saveSubAccount(final TawSystemUser tawSystemUser) {
		String returnString = "";
		if ((tawSystemUser.getId() == null)
				|| (tawSystemUser.getId().equals(""))) {
			try {
