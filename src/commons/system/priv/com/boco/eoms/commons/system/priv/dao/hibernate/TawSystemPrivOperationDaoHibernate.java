...
//			return (String) list.get(0);
		...
		// 获得可用Id顺序，版本发布降序，未发布升序 edit by chenyuanshu 20121201
		String _strHQL = "select distinct opt.code from TawSystemPrivOperation opt where opt.parentcode='"+fatherId+"' ";
		if ("true".equals(release)) {
			_strHQL = _strHQL + " order by opt.code desc ";
		} else if ("false".equals(release)) {
			_strHQL = _strHQL + " order by opt.code asc ";
		}else{
			
		}
		List _objResult = getHibernateTemplate().find(_strHQL);
		
		if (_objResult==null && _objResult.size() >= 99) {
    		return "99";
    	}
		HashMap<String,String> map = new HashMap<String,String>();
		for(int i = 0;_objResult!=null && i<_objResult.size();i++){
			String code = (String) _objResult.get(i);
			map.put(code, code);
		}
		if(map!=null && !map.isEmpty()){
			long codeVar = 99;
			if ("true".equals(release)) {
				codeVar = Long.valueOf(fatherId+"99").longValue();
				while(map.containsKey(String.valueOf(codeVar))){
					codeVar = codeVar -1;
				}
				
			} else if ("false".equals(release)) {
				 codeVar = Long.valueOf(fatherId+"01").longValue();
...
//		}
		...
		// 获得可用Id顺序，版本发布降序，未发布升序 edit by chenyuanshu 20121201
		String _strHQL = "select distinct opt.code from TawSystemPrivOperation opt where opt.parentcode='-1' ";
		if ("true".equals(release)) {
			_strHQL = _strHQL + " order by opt.code desc ";
		} else if ("false".equals(release)) {
			_strHQL = _strHQL + " order by opt.code asc ";
		...
		if(map!=null && !map.isEmpty()){
			long codeVar = 99;
			if ("true".equals(release)) {
				codeVar = Long.valueOf(99).longValue();
				while(map.containsKey(String.valueOf(codeVar))){
					codeVar = codeVar -1;
				}
				
			} else if ("false".equals(release)) {
				 codeVar = Long.valueOf(10).longValue();
				while(map.containsKey(String.valueOf(codeVar))){
					codeVar = codeVar +1;
				}
				if(codeVar>=99){
			...
			_strRtn = String.valueOf(codeVar);
		}
		return _strRtn;
	}
	...
	public List listOperation(String userId, String deptId, List roleIds,
		...
		hql += " and operation.hided<>'"
				+ PrivConstants.MENU_HIDDEN
				+ "' and operation.deleted<>'"
				+ Constants.DELETED_FLAG
				+ "' and (operation.loginType is null or operation.loginType = '"
				+ PrivConstants.PRIV_OPERATION_EOMS_LOING
	...
	public List listOperationwap(String userId, String deptId, List roleIds,
		...
		// menuitem.menuid in(select menuid from taw_system_priv_assign where
		// objectid='qujingbo') and operation.parentcode='-1'
		String inHql = getOperationInhsqls(userId, deptId, roleIds,
		...
		hql += " and operation.deleted<>'" + Constants.DELETED_FLAG
				+ "' and operation.loginType = '"
				+ PrivConstants.PRIV_OPERATION_WAP_LOING
				+ "' order by operation.orderby";
		// + "' order by cast(operation.orderby as integer)";
		/*
		 * List list = new ArrayList(); list =
	...
	private String getOperationInhsql(String userId, String deptId, List roleIds) {
		String inHql = "select distinct assign.privid from TawSystemPrivAssign assign,TawSystemPrivMenu menu where ((assign.objectid='"
				...
				+ StaticVariable.PRIV_ASSIGNTYPE_USER + "')"; // StaticVariable.NATURE_WAP
		// 该用户有所属部门
...
//					+ StaticVariable.PRIV_ASSIGNTYPE_DEPT + "'";
			inHql += " or assign.objectid ='" + deptId
					+ "' and assign.assigntype='"
					+ StaticVariable.PRIV_ASSIGNTYPE_DEPT + "'";
		}
		// 该用户有角色
		...
		inHql = inHql
				+ " ) and assign.privid = menu.privid and ( menu.nature is null or menu.nature = '"
	...
	private String getOperationInhsqls(String userId, String deptId,
		...
		inHql = inHql
				+ " ) and assign.privid = menu.privid and menu.nature = '"
				+ loginNature + "'";
		return inHql;
	}
	...
	public List listOperationAll(String userId, String deptId, List roleIds,
		...
		String hql = "select distinct operation from TawSystemPrivOperation operation ,TawSystemPrivMenuItem menuitem "
				+ "where operation.code=menuitem.code and menuitem.menuid in("
				+ inHql
				+ ") and (operation.parentcode in (select code from TawSystemPrivOperation where (parentcode like '"
				+ parentCode
				+ "%' or code='"
				+ parentCode
				+ "') and deleted<>'"
