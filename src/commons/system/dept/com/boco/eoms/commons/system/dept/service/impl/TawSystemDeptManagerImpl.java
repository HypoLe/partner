...
public class TawSystemDeptManagerImpl extends BaseManager implements
	...
	public List getNextLevecDeptsTask(String pardeptid, String delid,String rootId) {
		List list = new ArrayList();
		list = dao.getNextLevecDeptsTask(pardeptid, delid, rootId);
		return list;
	}
	 ...
	 * @return List
	 */
	public List searchByNamePartner(String word, String contacter,
			String deptphone) {
		List list = new ArrayList();
	...
	public TawSystemDept getSuperDept(String deptId) {
		TawSystemDept pDept = null;
		TawSystemDept dept = dao.getDeptinfobydeptid(deptId,
				DeptConstants.NO_DELETED);

		if (dept != null) {
			String pDeptId = dept.getParentDeptid();
			pDept = dao.getDeptinfobydeptid(pDeptId, DeptConstants.NO_DELETED);
		}
	 ...
	 * @return
	 */
	public List getFdeptInfobyLinkId(String deptid, String deleted) {
		List list = new ArrayList();
		int len = deptid.length();
		...
		list = dao.getFdeptInfobyLinkId(deptid, len, deleted);
		return list;
	}
	
	
	/**根据ParentDeptid查询所有符部门
	 * @param deptid
	 * @param deleted
	 * @return
	 */
	public List getFdeptInfobyParentDeptid(String deptid, String deleted) {
		List list = new ArrayList();
		//deptid = "1210201";
		boolean bool = true;
		while(bool){
			...
			if("1".equals(deptid)){
				TawSystemDept parentdept = dao.getDeptinfobydeptid(deptid,deleted);
				list.add(parentdept);
				bool = false;
			}else{
				TawSystemDept dept = dao.getDeptinfobydeptid(deptid,deleted);
				list.add(dept);
	...
	public List<TawSystemDept> getDeptidByDeptName(String deptName) {
		 return dao.getTawSystemByDeptName(deptName);
	}
