...
public class TawSystemDeptDaoHibernate extends BaseDaoHibernate implements
	...
	public List getNextLevecDepts(String pardeptid, String delid) {
		...
		String hql = " from TawSystemDept sysdept where sysdept.parentDeptid='"
				...
				+ "' order by linkid";
		//System.out.println("--TawSystemDeptDaoHibernate-getNextLevecDepts-302-"+hql);
		List list = getHibernateTemplate().find(hql);
		return list;		
	}
	/**
	 ...
	 * @return
	 */
	public List getNextLevecDeptsTask(String pardeptid, String delid,String rootId) {
		...
		if (!list.isEmpty()&&pardeptid.equals(rootId)&&pardeptid.substring(0, 3).equals("201")) {
			String areaId="";
			...
			//根据部门ID来找到组织结构 rootId 长度5，ordercode=3 ,长度7 ordercode=4 ,长度9 ordercode =5（即没有）
			
			int orderCode=3;
			int rootIdLength = rootId.length();
			if(rootIdLength==3){
				orderCode = 2;
			}else if(rootIdLength==5){
				orderCode = 3;
			...
			String hql2 = " from TawSystemDept sysdept where sysdept.ordercode="+orderCode+" and sysdept.areaid in("
				+areaId+ ")" + " and sysdept.deleted='" + delid
				+ "' order by linkid";

			list = (ArrayList) getHibernateTemplate().find(hql2);
		}
		return list;
	 ...
	 * @return
	 */
	public List getAreaid(String delid) {
		List list = new ArrayList();
		String hql = " select areaid from taw_system_dept where deptId ='"
				+ delid + "'";
	 ...
	 * @return
	 */ 
	public List getFdeptInfobyLinkId(String linkid, int len, String deleted) {

		List list = new ArrayList();
		String sql = "";
		if (deleted.equals("0") || deleted.equals("1")) {
			sql = " from TawSystemDept sysdept where sysdept.linkid like '"
					+ linkid + "%' and length(sysdept.linkid)<='" + len
					+ "' and sysdept.deleted='" + deleted + "'";
		} else {
			sql = " from TawSystemDept sysdept where sysdept.linkid like '"
					+ linkid + "%' and length(sysdept.linkid)<='" + len + "'";
	...
	/*
	 * provienceId2Name，即部门id转为部门名称 added by lee
	 * 
	 * @see com.boco.eoms.base.dao.ID2NameDAO#id2Name(java.lang.String)
	 */
	public String provienceId2Name(final String id) throws DictDAOException {
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
			throws HibernateException {
				// hql，以部门ID为条件查询
	 ...
	 * @return
	 */
	public List listDeptOfCity() {
		return getHibernateTemplate().find(
				"from TawSystemDept where (deptid= 1 or PARENTDEPTID =1)  and "
						+ StaticMethod.noDeletedCon(null)+ " order by deptid " );
	} 
	...
	public List<TawSystemDept> getTawSystemByDeptName(String deptName) {
		String hql = "from TawSystemDept d where d.deptName like '%" +deptName+ "%' and deleted='0' order by deptName";
		List list = new ArrayList();
		list = (ArrayList) getHibernateTemplate().find(hql);
		return list;
	}
