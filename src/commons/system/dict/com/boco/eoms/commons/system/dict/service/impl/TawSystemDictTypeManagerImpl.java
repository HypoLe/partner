...
public class TawSystemDictTypeManagerImpl extends BaseManager implements
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.boco.eoms.commons.system.dict.service.ITawSystemDictTypeManager#listDict(java.lang.String,
	...
	public List<TawSystemDictType> getTawSystemDictTypeByName(String dicName,String parentdictid,int length) {
		// TODO Auto-generated method stub
		List<TawSystemDictType> list =null;
		StringBuffer sql = new StringBuffer();
		sql.append("from TawSystemDictType p where p.dictName='").append(dicName).append("'");
		sql.append(" and p.parentDictId='");
		if(!"".equals(parentdictid)&&parentdictid.length()>=length){
			sql.append(parentdictid.substring(0, length)).append("'");
		}else{
			sql.append(parentdictid).append("'");
		}
//		System.out.println("---wozhogn-ddd------"+sql);
		list=dictTypeDao.find(sql.toString());
		return list;
	}
	 public PageModel searchPaginated(final String hql, final Object[] params, 
	  		  final int offset,final int pagesize){
		 PageModel p =dictTypeDao.searchPaginated(hql, params, offset, pagesize);
		 return p;
	 }
	 
	 public List<TawSystemDictType> getTawSystemDictTypeByParentdictId(String parentdictid) {
			// TODO Auto-generated method stub
			List<TawSystemDictType> list =null;
			StringBuffer sql = new StringBuffer();
			sql.append("from TawSystemDictType p where p.parentDictId='").append(parentdictid).append("'");
