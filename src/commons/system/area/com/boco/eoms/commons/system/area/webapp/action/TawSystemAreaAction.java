...
public final class TawSystemAreaAction extends BaseAction {
	...
	public ActionForward xdelete(ActionMapping mapping, ActionForm form,
		...
		JSONUtil.success(response, "删除成功！");
		return null;
	}
	...
	public ActionForward xedit(ActionMapping mapping, ActionForm form,
		...
		String id = tawSystemAreaForm.getId();
		// if an id is passed in, look up the user - otherwise
		// don't do anything - user is doing an add
		if (id != null) {
			ITawSystemAreaManager mgr = (ITawSystemAreaManager) getBean("ItawSystemAreaManager");
//			TawSystemArea tawSystemArea = (TawSystemArea) convert(tawSystemAreaForm);
			TawSystemArea tawSystemArea = mgr.getTawSystemArea(id);
			tawSystemArea = (TawSystemArea) convert(tawSystemAreaForm);
			if(!tawSystemArea.getAreacode().equals(tawSystemArea.getOldAreaName())){
				if(mgr.getAreaNameByName(tawSystemArea.getAreaname(),tawSystemArea.getParentAreaid(),id)){
	...
	/**
	 * 
	 *@Description:获取某区域的区县区域
	 *@date May 24, 2013 3:56:06 PM
	 ...
	 *@throws Exception
	 */
	public ActionForward getSonAreaByAreaId(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception{
		String areaid = request.getParameter("areaId");
		ITawSystemAreaManager mgr = (ITawSystemAreaManager) getBean("ItawSystemAreaManager");
		JsonArray jsonArray=new JsonArray();
		List<TawSystemArea> list=mgr.getSonAreaByAreaId(areaid);
		List itemList = new ArrayList();
		for (Iterator rowIt = list.iterator(); rowIt.hasNext();) {
			TawSystemArea tawSystemArea = (TawSystemArea) rowIt.next();
			TawCommonsUIListItem uiitem = new TawCommonsUIListItem();
			uiitem.setItemId(String.valueOf(tawSystemArea.getId()));
			uiitem.setText(tawSystemArea.getAreaname());
			uiitem.setValue(tawSystemArea.getAreaid());
			itemList.add(uiitem);
		}
		response.setContentType("text/xml;charset=UTF-8");
		// 返回JSON对象
