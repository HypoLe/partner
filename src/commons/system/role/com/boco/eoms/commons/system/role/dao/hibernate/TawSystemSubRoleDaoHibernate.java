...
public class TawSystemSubRoleDaoHibernate extends BaseDaoHibernate implements
	...
	/**
	 * 根据开始查询的记录序数start、每页最多记录数limit和查询条件whereStr查询子角色（分页）。 返回Map对象： {List}
	 * result 查询子角色结果集的当前分页的list {Integer} total 子角色结果集的总数
	 * 
	 * @param start
	 *            开始查询序数
	 * @param limit
	 ...
	 * @return Map
	 */
	public Map mapTawSystemSubRoles(final Integer start, final Integer limit,
	...
	public List getSubRoles(String roleId, Hashtable filterHash)
		...
		if (filterHash != null) {
			...
			while (eunm.hasMoreElements()) {
				...
				whereString += " and (t."
						+ RoleXmlSchema.getInstance().getColumnName(key) + "='"
						+ value + "' or t."
						+ RoleXmlSchema.getInstance().getColumnName(key)
						+ " is null or t."
						+ RoleXmlSchema.getInstance().getColumnName(key)
