...
public class TawSystemRoleDaoHibernate extends BaseDaoHibernate implements
	...
	public List listFirstLevelAreaByRoleId(final long roleId) {
		String hql = "from TawSystemArea ar where ar.areaid in " +
				"(select case when mod(length(a.areaid),2)=0 then substr(a.areaid,0,2) else substr(a.areaid,0,1) end  " + 
				" from TawSystemArea a ,TawSystemSubRole r where r.roleId=" + roleId + " and a.areaid=r.deptId and r.deleted='0' )";
		List li = getHibernateTemplate().find(hql);
		return li;
	}
	 ...
	 * @return <TawSystemArea>
	 */
	public List listSubLevelAreaByRoleId(final long roleId, final String areaId) {

		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				SQLQuery query = session.createSQLQuery(" select ar.* from Taw_System_Area ar where ar.areaid in " +
						"(select substr(a.areaid,0,length(:areaId)+2)" + 
						" from Taw_System_Area a ,Taw_System_Sub_Role r where r.roleId=:roleId" +
						" and a.areaid=r.deptId and r.deleted='0' and a.areaid like :areaIdlike) order by ar.ordercode");

				return query.addEntity("areaid", com.boco.eoms.commons.system.area.model.TawSystemArea.class)
							.setLong("roleId", roleId)
							.setString("areaId", areaId)
							.setString("areaIdlike", areaId + "__%")
							.list();
			}
		};
		return (List) getHibernateTemplate().execute(callback);
