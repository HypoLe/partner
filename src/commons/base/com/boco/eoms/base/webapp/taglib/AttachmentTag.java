	...
	public int doStartTag() {
		...
		try {
			...
			if (name != null && property != null && scope != null) {
				this.attachmentIds = (String) TagUtils.getInstance().lookup(
	...
	public int doEndTag() throws JspTagException {
		...
		try {
			...
			if (!viewFlag.equals("Y")) {
				html = "<iframe id=\"UIFrame1-"
						+ idField
						+ "\" name=\"UIFrame1-"
						+ idField
						+ "\" class=\"uploadframe\" frameborder=\"0\" "
						+ "scrolling=\"auto\" src=\""
						+ path
						+ "/accessories/pages/upload.jsp?appId="
						+ appCode
						+ "&filelist="
						+ attachmentIds
						+ "&idField="
						...
						+ "&startsWith="
						+ startsWith
						+ "\" style=\"height:80%;width:100%\"></iframe><input type=\"hidden\" "
						+ "name=\"" + idField + "\" id=\"" + idField
						+ "\" value=\"" + attachmentIds + "\"";
				if (alt != null && !alt.equals("")) {
					html = html
							+ " alt=\"blankText:'请上传附件。',iframeId:'IFrame1-"
							+ idField + "'," + alt + "\"/>";
				} else {
					html = html + " />";
				}
				...
				// "\" name=\"VIFrame1-"+idField+"\" class=\"uploadframe\"
				// frameborder=\"0\" "
				// + "scrolling=\"auto\" src=\"" + path +
				// "/accessories/pages/view.jsp?appId=" + appCode
				// + "&filelist=" + attachmentIds + "&idField="+idField+"\"
				// style=\"height:100%;width:100%\"></iframe><input
				// type=\"hidden\" "
				// + "name=\"" + idField +"\" id=\""+idField+ "\"
				// value=\""+attachmentIds+"\"/>";

				html = "<iframe id=\"VIFrame1-" + idField
						+ "\" name=\"VIFrame1-" + idField
						+ "\" class=\"uploadframe\" frameborder=\"0\" "
						+ "scrolling=\"auto\" src=\"" + path
						+ "/accessories/pages/view.jsp?appId=" + appCode
						+ "&filelist=" + attachmentIds + "&idField=" + idField
