...
public class SSOFilter extends CASFilter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		// 过滤掉登出方法拦截，避免出现连续两次登录才能进入系统的情况
		String method = StaticMethod.null2String(request.getParameter("method"));
		if (null != requestURI && method.equalsIgnoreCase("saveUserLogOut")) {
			fc.doFilter(request, response);
			return;
		}
		...
		SSOConfig ssoconfig = ((SSOConfig) attributes.getRegister().get(
				serverName));
		String casAddr = ssoconfig.getCasLogin().trim();
		casLogin = casAddr + "/login";
		casServerName = ssoconfig.getEomsServerName().trim() + ":" + request.getServerPort();
		...
		if (session != null) // probably unncessary
		{
			...
			// 加载eoms用户,by leo
			if (session.getAttribute("sessionform") == null) {
				// 获取用户session信息
				...
				if (userId != null) {
					...
					/**
					 * 作业计划获取代理用户逻辑，暂时去掉 modify by 陈元蜀 2012-08-29 begin
					 * ITawwpStubUserMgr tawwpStubUserMgr = (ITawwpStubUserMgr)
					 * ApplicationContextHolder
					 * .getInstance().getBean("tawwpStubUserMgr"); List
					 * stubUserVOList = new ArrayList();
					 * 
					 * try { // 获取代理人信息VO对象集合
					 * 
					 * stubUserVOList = tawwpStubUserMgr
					 * .listStubUserByStubuser(userId); } catch (Exception e) {
					 * System.out.print(e); }
