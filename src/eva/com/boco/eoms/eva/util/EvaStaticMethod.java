...
public class EvaStaticMethod {
	...
	public static String getStartTimeByCycle(String cycle,
		...
		}else if(EvaConstants.CYCLE_HALFYEAR.equals(cycle)){
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			int month = Integer.parseInt(currentDateTime.substring(5, 7));
			Date date = null;
			if (month <= 6) {
				date = DateTimeUtil.getFirstDayOfMonth(year, 1);
			} else if (month <= 12) {
				date = DateTimeUtil.getFirstDayOfMonth(year, 7);
			} 
			startTime = DateTimeUtil.dateToString(date);
	...
	public static String getEndTimeByCycle(String cycle, String currentDateTime) {
		...
		if (EvaConstants.CYCLE_YEAR.equals(cycle)) {
			...
			Date date = null;
			if (month <= 6) {
				date = DateTimeUtil.getLastDayOfMonth(year, 6);
			} else if (month <= 12) {
				date = DateTimeUtil.getLastDayOfMonth(year, 12);
			} 
			endTime = DateTimeUtil.dateToString(date);
		} else if (EvaConstants.CYCLE_MONTH.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
	...
	//接口调用
	public static String telService(String serviceCode, String userStr, String startTime, String endTime){
		NopTradeServiceDelegate nopTradeServiceDelegate = new NopTradeServiceDelegate();
		String[] users = userStr.split(",");
		StringBuffer dateStr = new StringBuffer();;
		dateStr.append("<root><systemid>partner</systemid><systemname>合作伙伴</systemname>");
		try {
			dateStr.append("<starttime>"+startTime+" 00:00:00</starttime>");
			dateStr.append("<endtime>"+endTime+" 00:00:00</endtime>");
			dateStr.append("<userinfo>");
			for(int i=0;i<users.length;i++){
				dateStr.append("<userid>"+users[i]+"</userid>");
			}
			dateStr.append("</userinfo></root>");
			return nopTradeServiceDelegate.service(
					serviceCode, "1", "2", "3", "127.0.0.1", "4", dateStr.toString());
		} catch (Exception e) {
			e.printStackTrace();
			return "errors";
		}
	...
	public static String getDataFromXml(String xmlStr){
		...
		Element dataElm=root.element("resultData");// 
		dataStr = dataElm.element("intimerate").getText();
		}catch(Exception e){
			e.printStackTrace();
			return "100.00";
		}
