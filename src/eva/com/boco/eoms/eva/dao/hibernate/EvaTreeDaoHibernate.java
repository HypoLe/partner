...
public class EvaTreeDaoHibernate extends BaseDaoHibernate implements
	...
	public EvaTree getNodeByRemark(String remark){
			EvaTree node = new EvaTree();
			String hql = " from EvaTree tree where tree.nodeRemark = '"+remark+"'";
			List list = getHibernateTemplate().find(hql);
			if (null != list && 0 < list.size()) {
				node = (EvaTree) list.get(0);
			}
	...
	public List getEvaKpiByNodeId(final String parentNodeId) {
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				String queryStr = " select kpi.cycle from EvaTree tree,EvaKpi kpi where tree.objectId=kpi.id and tree.parentNodeId='"+ parentNodeId + "'";
	...
	public List getEvaKpiByParentNodeId(final String parentNodeId) {
		
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				String queryStr =  " from EvaTree tree where tree.parentNodeId= '"+parentNodeId+"'";
				Query query = session.createQuery(queryStr);
				return query.list();
			}
		};
