...
public class TreeGridUtil {
	...
	public static org.w3c.dom.Document parseXML(String XML) {
		if (XML == null)
			XML = "";
		if (XML.equals(""))
			return null;
	...
	public static org.w3c.dom.Element[] getChanges(String XML) {
		return getChanges(parseXML(XML));
	}

	public static org.w3c.dom.Element[] getChanges(org.w3c.dom.Document XML) {
		if (XML == null)
			return null;
	...
	int getPagePos(org.w3c.dom.Document XML) {
		...
		return Integer.valueOf(
				((org.w3c.dom.Element) (XML.getElementsByTagName("B").item(0)))
						.getAttribute("Pos")).intValue();
	}
	...
	String getPageId(org.w3c.dom.Document XML) {
		if (XML == null)
			return null;
		return ((org.w3c.dom.Element) (XML.getElementsByTagName("B").item(0)))
				.getAttribute("id");
	}
	...
	String[] getSortCols(org.w3c.dom.Document XML) {
		if (XML == null)
			return null;
		org.w3c.dom.Element Cfg = (org.w3c.dom.Element) XML
				.getElementsByTagName("Cfg").item(0);
	...
	public static String getId(org.w3c.dom.Element I) {
		return I.getAttribute("id");
	}
	public static String getTaskId(org.w3c.dom.Element I) {
		return I.getAttribute("taskId");
	}
	public static String getParent(org.w3c.dom.Element I) {
		return I.getAttribute("Parent");
	}
	...
	String toSQLUpdateString(String name, org.w3c.dom.Element I, String attname) {
		return I.hasAttribute(attname) ? name + "='"
				+ I.getAttribute(attname).replaceAll("'", "''") + "'," : "";
	}
	...
	// Useful when building comma separated list in loop, call it after list is
	// built to strip ending comma(s)
	String trimSQL(String value) {
		return value == null ? "" : value.replaceAll("[\\,\\s]*$", "");
	}

	// ------------------------------------------------------------------------------------------------------------------
	// Returns attribute values in format
	// "outName[0]=value_of_attrNames[0],outName[1]=value_of_attrNames[1], ..."
	...
	String toSQLUpdate(org.w3c.dom.Element I, String[] attrNames,
			String[] outNames, boolean attrIsString[]) {
	...
	// If the attribute does not exists, is not included
	// If attrIsString[x] is true, it encloses attribute value by ' '
	String toSQLInsert(org.w3c.dom.Element I, String[] attrNames,
	...
	// If name is found in names array, it checks types for string type and if
	// found returns in ' '
	public static String getValue(String[] names, int[] types, String name, String value) {
		for (int i = 0; i < names.length; i++) {
			if (names[i].equalsIgnoreCase(name)) {
				int t = types[i];
				if (value.equals("")
						&& (t == java.sql.Types.DATE || t == java.sql.Types.TIME))
					return "NULL";
	...
	// In Parent column the row has value of id column of parent row => All
	// parent's children have its id in their Parent column
	// bodyParent is value of Parent for new added root rows
	boolean saveTree(String XML, java.sql.Statement Cmd, String table,
			String idCol, String parentCol, String bodyParent)
		...
		for (int i = 0; i < Ch.length; i++) {
			...
			if (isDeleted(I)) { // Deleting
				Cmd.executeUpdate("DELETE FROM " + table + " WHERE " + idCol
						+ "=" + getValue(names, types, idCol, id));
				...
				for (int a = 0; a < A.getLength(); a++) {
					...
					if (!name.equals("Added") && !name.equals("Changed")
						...
						Cols.append("" + name + ",");
						Vals.append(getValue(names, types, name, N
								.getNodeValue())
								+ ",");
					}
				}
				...
				Vals.append("'"
						+ (!I.getAttribute("Parent").equals("") ? I
								.getAttribute("Parent") : bodyParent) + "'");
				...
				for (int a = 0; a < A.getLength(); a++) {
					...
					if (!name.equals("Added") && !name.equals("Changed")
						...
						S
								.append(name
										+ "="
										+ getValue(names, types, name, N
												.getNodeValue()) + ",");
					}
				}
				S.setLength(S.length() - 1);
				S.append(" WHERE " + idCol + "="
						+ getValue(names, types, idCol, id));
	...
	// Returns value as XML string quoted by single quote and replaced <,&,' by
	// XML entities
	...
	String toXML(String name, java.sql.ResultSet R, String colName)
			throws java.sql.SQLException {
		return toXML(name, R.getString(colName));
	}

	void toXML(StringBuffer S, String name, java.sql.ResultSet R, String colName)
			throws java.sql.SQLException {
	...
	// ------------------------------------------------------------------------------------------------------------------
	// XML representation of one row with given attribute names and their values
	// from ResultSet in columns colNames
	// end is the string appended after row, for rows with children use ">",
	// without children "/>", or you can set own attributes
	// Does not move cursor in ResultSet
	String getRowXML(java.sql.ResultSet R, String[] names, String[] colNames)
			throws java.sql.SQLException {
		return getRowXML(R, names, colNames, "/>");
	}

	String getRowXML(java.sql.ResultSet R, String[] names, String[] colNames,
			String end) throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		S.append("<I");
		for (int i = 0; i < names.length; i++)
	...
	// without children "/>", or you can set own attributes
	// Does not move cursor in ResultSet
	String getRowXML(java.sql.ResultSet R, String[] names, int[] colIndexes)
			throws java.sql.SQLException {
	...
	// ------------------------------------------------------------------------------------------------------------------
	// XML representation of one row with given attribute names and their values
	// from ResultSet in columns indexed as attribute names
	// end is the string appended after row, for rows with children use ">",
	// without children "/>", or you can set own attributes
	// Does not move cursor in ResultSet
	String getRowXML(java.sql.ResultSet R, String[] names)
			throws java.sql.SQLException {
		return getRowXML(R, names, "/>");
	}

	String getRowXML(java.sql.ResultSet R, String[] names, String end)
			throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		S.append("<I");
		for (int i = 0; i < names.length; i++)
	...
	// names
	String getTableXML(java.sql.ResultSet R, String[] names, String[] colNames)
			throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
	...
	// names and their values from ResultSet in columns indexed as attribute
	// names
	String getTableXML(java.sql.ResultSet R, String[] names, int[] colIndexes)
			throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		S.append("<Grid><Body><B>");
		while (R.next()) {
	...
	// names and their values from ResultSet in columns indexed as attribute
	// names
	String getTableXML(java.sql.ResultSet R, String[] names)
			throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		S.append("<Grid><Body><B>");
		while (R.next()) {
	...
	// ------------------------------------------------------------------------------------------------------------------
	// Returns complete XML representation from database resultset. Uses the
	// same TreeGrid column names as in resultset
	...
	String getTableXML(java.sql.ResultSet R, String idCol)
			throws java.sql.SQLException {
		java.sql.ResultSetMetaData M = R.getMetaData();
		int cnt = M.getColumnCount();
	...
	// names are TreeGrid attribute names in order in that are filled from
	// columns from database table
	// names must contain name "id", this is identify attribute
	// names must also contain "Parent", this is column related to id in
	// Parent<-Child.
	// In Parent column the row has value of id column of parent row => All
	// parent's children have its id in their Parent column
	// For deep==false reads only one level of tree, for server side child
	// paging
	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String[] colNames, String bodyParent) throws java.sql.SQLException {
		return getTreeXML(Cmd, table, orderNames(Cmd, table, names, colNames),
				bodyParent);
	}

	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String[] colNames, String bodyParent, String headParent,
	...
	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String bodyParent) throws java.sql.SQLException {
		return getTreeXML(Cmd, table, names, bodyParent, null, null, true);
	}

	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String bodyParent, String headParent, String footParent,
			boolean deep) throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		S.append("<Grid>");
		if (headParent != null)
			S.append("<Head>" + getTreeXML(Cmd, table, names, headParent, deep)
					+ "</Head>");
		if (bodyParent != null)
			S.append("<Foot>" + getTreeXML(Cmd, table, names, footParent, deep)
					+ "</Foot>");
		S.append("<Body><B>" + getTreeXML(Cmd, table, names, bodyParent, deep)
				+ "</B></Body>");
		S.append("</Grid>");
	...
	// orders names (attribute names) to match indexes in table according to
	// colNames
	String[] orderNames(java.sql.Statement Cmd, String table, String[] names,
			String[] colNames) throws java.sql.SQLException {
		java.sql.ResultSet R = Cmd.executeQuery("SELECT TOP 1 * FROM " + table);
		java.sql.ResultSetMetaData M = R.getMetaData();
		int cnt = M.getColumnCount();
	...
	// ------------------------------------------------------------------------------------------------------------------
	// Returns children of row with id parentVal => returns all rows that have
	// in their Parent column value parentVal
	// Other parameters are the same as in previous function getTreeXML
	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String[] colNames, String parentVal, boolean deep)
			throws java.sql.SQLException {
		return getTreeXML(Cmd, table, orderNames(Cmd, table, names, colNames),
				parentVal, deep);
	}
	...
	String getTreeXML(java.sql.Statement Cmd, String table, String[] names,
			String parentVal, boolean deep) throws java.sql.SQLException {
		StringBuffer S = new StringBuffer();
		...
		java.sql.ResultSet R = Cmd.executeQuery("SELECT * FROM "
				+ table
				+ (parent > 0 ? " WHERE " + names[parent - 1] + "='"
						+ parentVal + "'" : ""));
		if (R == null)
			return "";
		while (R.next()) {
	...
	// ------------------------------------------------------------------------------------------------------------------
	// Returns statemenet to HSQLDB database. This database provider is used for
	// examples
	java.sql.Statement getHsqlStatement(HttpServletRequest request,
			JspWriter out, String dbPath, String user, String pass)
			throws Exception {
	...
	java.sql.Statement getHsqlStatement(HttpServletRequest request,
			JspWriter out, String dbPath, String user, String pass,
			boolean fromHTML) throws Exception {
		String Path = request.getServletPath().replaceAll("[^\\/\\\\]*$", ""); // Relative
																				// path
																				// to
																				// script
																				// directory
		...
		try {
			...
			Conn = java.sql.DriverManager.getConnection("jdbc:hsqldb:file:"
					+ request.getRealPath(Path + dbPath), "sa", "");
			return Conn.createStatement();
		} catch (Exception e) {
			// String Err = "! Failed to load HSQLDB JDBC driver.<br>You need to
			// copy <b>hsqldb.jar</b> file to your shared lib directory and
			// <b>restart</b> your http server.";
			...
			try {
				out.print(fromHTML ? "<font color=red>" + Err + "</font>"
						: "<Grid><IO Result='-1' Message='" + Err
								+ "'/></Grid>");
				out.close();
			} catch (Exception e2) {
			}
