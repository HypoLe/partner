...
public class MappingJdbcDaoHibernate extends JdbcDaoSupport implements
		MappingJdbcDao {
	...
	public String dictToName(String appcode, String sheetkey) {
		...
		String sql = "select new_table from mappingstorage where app_code= '"
				+ appcode + "'";
		...
		if (tablenamelist != null && tablenamelist.size() > 0) {
			...
			while (_objIterator.hasNext()) {
				...
				tablename = StaticMethod.nullObject2String(_objMap
						.get("new_table"));
				if (!tablename.equals("")) {
					break;
				}
			...
			String sql1 = "select dictid from " + tablename
					+ " where sheetkey= '" + sheetkey + "'";
			...
			if (dictlist != null && dictlist.size() > 0) {
				...
				while (_objIteratordict.hasNext()) {
					...
					userDutydict.add(StaticMethod.nullObject2String(_objMap
							.get("dictid")));
				}
				List rootidlist = new ArrayList();
				List userDutyrootid = new ArrayList();
				String sql2 = "select rootid from " + tablename
						+ " where sheetkey= '" + sheetkey + "'";
				...
				if (rootidlist != null && rootidlist.size() > 0) {
					...
					while (_objIteratorrootid.hasNext()) {
						...
						rootid = StaticMethod.nullObject2String(_objMap
								.get("rootid"));
						if (!rootid.equals("")) {
							break;
						}
					...
					String newStr = "";
					for (int i = 0, j = userDutydict.size(); userDutydict != null
							&& i < j; i++) {
	...
	public String dictIdToNamee(String id) {
		...
		if (length == len) {
			...
			// return "";
			String newStr = id.substring(0, id.lastIndexOf(id
					.substring(length - 2)));
			return dictIdToNamee(newStr) + id2nameservice.id2Name(id, beanid)
					+ "-";
			// }else {
			// String newStr = id.substring(0,
			// id.lastIndexOf(id.substring(length - 2)));
			// return dictIdToNamee(newStr) + id2nameservice.id2Name(id, beanid)
			// + "-";
		}
	}
	...
	public void genTable(String tablename, String dbType) {
		...
		sql1.append("create index " + tablename + " on " + tablename);
		sql1.append(" (app_code)");
		if (dbType.equals("org.hibernate.dialect.OracleDialect")) {
			sql
					.append("(app_code varchar2(32),"
							+ "sheetKey varchar2(32),rootId varchar2(32),dictId varchar2(200),context varchar2(200) )");

		} else {

			sql
					.append("(app_code varchar(32),sheetkey varchar(32),rootId varchar(20),dictId varchar(100) )");
		}
	...
	public void insertValue(String appcode, String sheetKey, String rootId,
		...
		while (_objIterator.hasNext()) {
			...
			Map _objMap = (Map) _objIterator.next();
			namelist.add(StaticMethod.nullObject2String(_objMap
					.get("new_table")));
		}
		...
		for (int i = 0, j = strArray.length; i < j; i++) {
			...
			sql1.append("insert into " + tablename
					+ "(app_code, sheetKey,rootId,dictId)");
			sql1.append(" values('" + appcode + "', '" + sheetKey + "','"
					+ rootId + "','" + strArray[i] + "')");
			getJdbcTemplate().execute(sql1.toString());
		}
