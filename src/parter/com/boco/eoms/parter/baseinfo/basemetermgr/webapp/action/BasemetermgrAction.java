...
public final class BasemetermgrAction extends BaseAction {
    ...
    public ActionForward xsave(ActionMapping mapping, ActionForm form,
		...
		basemetermgr.setMaintenUnitIdVal(tempid);
	...
	public ActionForward toUpdateEdit(ActionMapping mapping, ActionForm form,
		...
		if(id!=null){basemetermgrForm.setId(id);}
		if (basemetermgrForm.getId() != null) {
			IBasemetermgrManager mgr = (IBasemetermgrManager) getBean("IbasemetermgrManager");
			Basemetermgr forums = mgr.getBasemetermgr(basemetermgrForm.getId());
			basemetermgrForm = (BasemetermgrForm) convert(forums);
			...
			String userJosn = this.getDeptJson(basemetermgrForm.getMaintenUnitIdVal());
			request.setAttribute("userJosn", userJosn);
		}
...
//	 得到由userId返回JSON
	public String getDeptJson(String userId) {
		//ITawSystemDeptManager mgr = (ITawSystemDeptManager) getBean("ItawSystemDeptSaveManagerFlush");
		ITawSystemDeptManager mgr = (ITawSystemDeptManager) getBean("ItawSystemDeptManager");
		
		String userJSON = "";
		String userName = "";
		try {
			userName = mgr.id2Name(userId);
		...
		if (userId != null && !"".equals(userId)) {
			...
			for (int i = 0; i < name.length; i++) {
				userJSON = userJSON + ",{\"nodeType\":\"dept\",\"name\":\""
						+ name[i] + "\",\"id\":\"" + id[i] + "\"}";
			}
			userJSON = userJSON.substring(1, userJSON.length());
			userJSON = "[" + userJSON + "]";
			return userJSON;
		} else {
			return "[]";
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String deptStr = "";
		String tempid = request.getParameter("tempid");
		basemetermgrForm.setMaintenUnitIdVal(tempid);
