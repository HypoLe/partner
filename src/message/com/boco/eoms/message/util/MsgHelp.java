...
public class MsgHelp {
	...
	public static String getSingleProperty(String path) {
		...
		try {
			filePath = StaticMethod
					.getFilePathForUrl("classpath:com/boco/eoms/message/config/messageConfig.xml");
	 ...
	 * @return 下一条消息的发送时间
	 */
	public static Date genNextSendTime(Date dispatchTime, SmsService service) {
		Date date = null;
		Calendar cd = Calendar.getInstance();
		cd.setTime(dispatchTime);
		if ("3".equals(service.getCycleStatus())) { // 按天循环，时间推迟到一天后的某时
			int hour = Integer.parseInt(service.getCycleHour());
			cd.add(Calendar.DAY_OF_MONTH, 1); // 第二天
			cd.set(Calendar.HOUR_OF_DAY, hour);
			date = cd.getTime();
		} else if ("2".equals(service.getCycleStatus())) { // 按月循环
			int hour = Integer.parseInt(service.getCycleHour());
			int day = Integer.parseInt(service.getCycleDay());
			cd.add(Calendar.MONTH, 1); // 
			cd.set(Calendar.HOUR_OF_DAY, hour);
			cd.set(Calendar.DAY_OF_MONTH, day);
		} else if ("1".equals(service.getCycleStatus())) { // 按年循环
			int hour = Integer.parseInt(service.getCycleHour());
			int day = Integer.parseInt(service.getCycleDay());
			int month = Integer.parseInt(service.getCycleMonth());
