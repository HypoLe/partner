 ...
 * 
 */
public class SchedulerDaoHibernate_SiChuan extends BaseDaoHibernate implements
		ISchedulerDao {
	...
	public boolean smsMonitorScheduler(String mobiles, String content) {
		System.out.println("=======---------------i'm in smsOuterConfigImpl's sendSms method--------------------=======");
		...
		try {			
			...
			String sql = "insert into sp_submit (desttermid,msgcontent,serviceid,feecode) values ('"+ mobiles + "','" + content + "','help','3')";
		...
		System.out.println("=======---------------when i quit,i want to know de return value,its value is --------------------======="+returnStr);
		return returnStr;
	}
	...
	public boolean voiceMonitorScheduler(String t_no, String t_alloc_time, String t_finish_time, String t_content, String t_tel_num, String t_tel_num2, String dispatch_tel) {
		BocoLog.info(this, "i'm in SiChuan's method!!!!!");
		...
		try {			
			...
			BocoLog.info(this, sql);
			stmt.execute(sql);
			
		} catch (Exception e) {
			BocoLog.debug(this, "i got a exception!!!!!!!!");
			status = false;			
			e.printStackTrace();
		} finally {
			try {
