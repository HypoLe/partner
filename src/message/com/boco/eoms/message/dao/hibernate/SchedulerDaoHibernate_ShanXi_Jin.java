...
import com.boco.eoms.message.sxjin.util.shortMsg;
 ...
 * 
 */
public class SchedulerDaoHibernate_ShanXi_Jin extends BaseDaoHibernate implements
		ISchedulerDao {
	...
	public boolean smsMonitorScheduler(String mobiles, String content) {
		BocoLog.info(this, "i'm in ShanXi_Jin's smsOuterConfigImpl's sendSms method");
		content = content.trim();
        System.out.println("短信调用，手记号码：" + mobiles);
        boolean returnValue = true;
        try{
        	shortMsg.sendLongSms(1,mobiles,content);
        } catch(Exception e){
        	returnValue = false;
        	e.printStackTrace();
        }        
    ...
    {
        java.io.FileInputStream conf = null;
