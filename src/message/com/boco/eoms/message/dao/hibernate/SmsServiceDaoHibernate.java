...
public class SmsServiceDaoHibernate extends BaseDaoHibernate implements SmsServiceDao {
	...
	public List getCustomServices(String receiverId) {
		String shql = "from SmsService service where service.status='"+MsgConstants.SERVICE+"' and service.selStatus='"+MsgConstants.UNPOSITIVE+"' and service.id not in(select apply.serviceId from SmsApply apply where apply.receiverId='"+receiverId+"' and apply.selStatus='"+MsgConstants.UNPOSITIVE+"')" ;
		return getHibernateTemplate().find(shql);
	}
	
	public List getServicesBySelStatus(String selstatus) {
		String hql = "from SmsService service where service.status='"+MsgConstants.SERVICE+"' and service.selStatus='"+selstatus+"'";
		return getHibernateTemplate().find(hql);
	}
}
