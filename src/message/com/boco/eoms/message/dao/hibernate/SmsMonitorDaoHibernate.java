...
import com.boco.eoms.message.interfacesclient.UserServiceLocator;
...
public class SmsMonitorDaoHibernate extends BaseDaoHibernate implements SmsMonitorDao {
    ...
    public void saveSmsMonitor(final SmsMonitor smsMonitor) {
    	smsMonitor.setContent(smsMonitor.getContent().trim());
...
//					returnMsg = (String)m.invoke(callback, new Object[]{smsApply.getServiceId()});
	...
	public String sendMsg(String serviceId, String msg, String buizId, String orgIds, String dispatchTime) {
		BocoLog.info(this, "enter sendMsg method,serviceId=="+serviceId+"orgIds=="+orgIds+"dispatchTime is "+dispatchTime);
		...
		try {
			if(orgIds != null && orgIds.length() != 0) {
				BocoLog.info(this, "orgId is not null");
				List userList = MsgHelp.getUserList(orgIds);
				BocoLog.info(this, "userList's size "+userList.size());
				Iterator it = userList.iterator();
				while(it.hasNext()) {
					BocoLog.info(this, "======-------===========");
					user = (TawSystemUser)it.next();
					userId = user.getUserid();
					BocoLog.info(this, "============================================="+userId);
					String mobile = StaticMethod.null2String(user.getMobile());
					if(null == mobile || mobile.equals("") || dispatchTime == null || dispatchTime.equals("")) {
						BocoLog.info(this, userId+"didn't config mobile,mobile is "+mobile+" or dispatchTime is ("+dispatchTime +")");
						continue;
					}
					...
					if(apply != null) {
						BocoLog.info(this, userId+"have this apply");
						//当apply表中有DIY状态的数据时
						String isSendUnDuty = apply.getIsSendUnDuty();//非值班状态是否发送
						BocoLog.info(this,"isSendUnDuty=="+isSendUnDuty);
						if(apply.getDeleted() == null || apply.getDeleted().equals(MsgConstants.DIYED)) {
							...
							BocoLog.info(this, userId+"has no data in smsApply,means no apply service");
							continue;
						//end
						}
					} else {
						BocoLog.info(this, userId+"has not service what serviceId is "+serviceId);
						continue;
					}
					
				}
			} else {
				BocoLog.info(this, "orgIds is null =="+orgIds);
			}
		} catch (Exception e){
			e.printStackTrace();
			status = "false";
...
//		apply = applyMgr.getSimpleApply(serviceId, userId, MsgConstants.DIYED);
	...
	public void closeMsg(String serviceId, String buizId) {
		BocoLog.info(this, "serviceId is  ==="+serviceId+" ,buizId is ==="+buizId);
		...
		BocoLog.info(this, "toDelList'size is " + toDelList.size());
		Iterator it = toDelList.iterator();
		while(it.hasNext()) {
			if(MsgConstants.MSGTYPE_EMAIL.equals(msgType)) {
	...
	public String sendSms4OutSystem(String serviceId, String msg, String buzId,
		...
		UserServiceLocator userServiceLocator=new UserServiceLocator();
		UserServicePortType userServicePortType;
		String str = "";
		try {
			userServicePortType = userServiceLocator.getUserServiceHttpPort();
			str = userServicePortType.getMobilePair(users, selStatus);
