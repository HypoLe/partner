 ...
 * 
 */
public class SchedulerDaoHibernate_FuJian extends BaseDaoHibernate implements
		ISchedulerDao {
	...
	public boolean smsMonitorScheduler(String mobiles, String content) {
		System.out.println("i'm in FuJian's smsOuterConfigImpl's sendSms method");
		if(mobiles == null || "".equals(mobiles)){
			return false;
		}	
		
		String imIP = MsgHelp.getSingleProperty("//message/msg/imIp");
		String loginName = MsgHelp.getSingleProperty("//message/msg/loginName");
		String loginPWD = MsgHelp.getSingleProperty("//message/msg/loginPWD");
		String apiCode = MsgHelp.getSingleProperty("//message/msg/apiCode");
		String dbName = MsgHelp.getSingleProperty("//message/msg/dbName");
		APIClient apiClient = new APIClient();
		
		try {
			apiClient.init(imIP, loginName, loginPWD, apiCode, dbName);
			String[] newMobiles = splitMobiles(mobiles);
			int result = apiClient.sendSM(newMobiles, content, 0L);
		//	apiClient.sendSM(mobiles, content, result);
			if (result == 0) {
	...
	public String[] splitMobiles(String mobile){
