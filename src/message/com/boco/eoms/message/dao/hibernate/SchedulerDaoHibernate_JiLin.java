 ...
 * 
 */
public class SchedulerDaoHibernate_JiLin extends BaseDaoHibernate implements
		ISchedulerDao {
	...
	public boolean smsMonitorScheduler(String mobiles, String content) {
		...
		BocoLog.info(SmsMonitorManagerImpl.class, "发送手机号:"+mobiles+";发送内容:"+content);
		if("13800000000".equals(mobiles)) {
			...
			try {			
				...
				String sql = "insert into sendsms (mobile,content) values (?,?)";
				BocoLog.info(SmsMonitorManagerImpl.class, sql);
				...
				try {
					...
					BocoLog.error(SmsMonitorManagerImpl.class, ex.getMessage());
				}
			}
		}
		return returnStr;
