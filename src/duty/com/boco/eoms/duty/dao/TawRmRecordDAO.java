...
public class TawRmRecordDAO extends DAO {

	public TawRmRecordDAO(com.boco.eoms.db.util.ConnectionPool ds) {
		super(ds);
	}

	public TawRmRecordDAO() {
		super();
	}
	...
	public void getNewManiRecord(int workserial,String roomId) throws SQLException {
		...
		try {
			...
			// 先删除
			/*
			 * conndel = ds.getConnection(); sqldel = "Delete from taw_rm_record
			 * where id = " + workserial + ""; pstmtdel =
			 * conndel.prepareStatement(sqldel); pstmtdel.executeUpdate();
			 * conndel.commit(); close(pstmtdel); close(conndel);
			 */
			// 交班人，
			...
			Sql_sel = "Select id, room_id, region_id, StartTime_defined,EndTime_defined,Dutydate,Dutymaster from taw_rm_assignwork where id = "
					+ workserial + "";
			pstmt = conn.prepareStatement(Sql_sel);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				// 在创建
				temp_sql = "INSERT INTO taw_rm_record (Id,Region_id,Room_id,Flag,Starttime,"
	...
	public void updateSub_setEnd(int workserial) throws SQLException {
		String sql = null;
		sql = "Update TAW_RM_RECORD_SUB SET STATUS = 1 where workserial =?";
		com.boco.eoms.db.util.BocoConnection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
	 ...
	 * @throws SQLException
	 */
	public void saveRecord(TawRmRecord tawRmRecord,
			TawRmRecordSub tawRmRecordSub, boolean isDutymaster,
			int workserial, String userId) throws SQLException {
		String sql;
    //ֵ��ס��¼
    sql = "Update TAW_RM_RECORD SET Weather = ?,Temperature = ?,WET = ?,Clean = ?,Clean1 = ?,Conditioner = ?,dutyrecord = ?,notes = ?, netfault=?, importantaffair=?, harmony=?, otheraffair=? WHERE ID = " +
        workserial + "";
    //ֵ���Ӽ�¼
    String sqlsub =
        "Update TAW_RM_RECORD_SUB SET Statement = ? WHERE WorkSerial = " +
		...
		try {
			...
			String dutycheck = "";
			pstmt1 = connsub
					.prepareStatement("select dutycheck from taw_rm_record_sub where workserial="
							+ workserial);
			...
			pstmt2 = connsub
					.prepareStatement("update taw_rm_record set dutycheck = '"
	 ...
	 * @throws SQLException
	 */
	public void insert(TawRmRecord tawRmRecord, TawRmRecordSub tawRmRecordSub)
			throws SQLException {
		String sql;

		sql = "INSERT INTO taw_rm_record "
		...
		String sqlsub = "insert into taw_rm_record_sub "
	...
	public void update(TawRmRecord tawRmRecord) throws SQLException {
		...
		try {
			...
			sql = "UPDATE taw_rm_record SET region_id=?, room_id=?, flag=?, "
	...
	public void delete(int id) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "DELETE FROM taw_rm_record WHERE id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, id);
			pstmt.executeUpdate();
			close(pstmt);
	 ...
	 * @throws SQLException
	 */
	public TawRmRecord retrieve(int id) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT * FROM taw_rm_record WHERE id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, id);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	 ...
	 * @throws SQLException
	 */
	public TawRmRecord retrieve1(int id) throws SQLException {
		TawRmRecord tawRmRecord = null;
		com.boco.eoms.db.util.BocoConnection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public List list() throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT region_id, room_id, flag, starttime, endtime, weather, temperature, wet, dutydate, clean, clean1, conditioner, hander, dutyman, receiver, dutyrecord, notes, id FROM taw_rm_record";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public List listDutyman(int workserial) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "select t.dutyman,u.username from taw_rm_record_sub t,taw_System_user u where t.workserial = "
	...
	public List search(int offset, int limit, String condition)
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT * FROM taw_rm_record " + condition;
	...
	public List search7Days(int deptid) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT a.* FROM taw_rm_record a,taw_rm_systeminfo b where a.room_id = b.room_id and b.dept_id="
					+ deptid
					+ " and a.starttime >= '"
					+ starttime
					+ "' and a.starttime <= '"
			...
			while (rs.next()) {
				...
				sql1 = " select dutyman, statement,typemode,trans,switch,data,system,other,dutySummary,importRecord, importFault,"
						+ " netCut, netKPI from taw_rm_record_sub where workserial="
						+ tawRmRecord.getId();
	...
	public List listReceiverMaster(int offset, int limit, String name,
		...
		try {
			conn = ds.getConnection();
			sql = "select taw_rm_assignwork.dutymaster "
					+ "from taw_rm_assignwork "
					+ "where taw_rm_assignwork.starttime_defined in "
					+ "(select distinct taw_rm_assignwork.endtime_defined "
					+ "from taw_rm_assign_sub, taw_rm_assignwork "
					+ "where taw_rm_assignwork.id in "
					+ "(select taw_rm_assign_sub.workserial "
					+ "from taw_rm_assign_sub where taw_rm_assign_sub.dutyman = "
					+ "'" + name + "'" + ")"
					+ " and taw_rm_assignwork.endtime_defined >=" + "'" + date
					+ "'" + " and taw_rm_assignwork.starttime_defined <= "
					+ "'" + date + "'" + " and taw_rm_assignwork.room_id = "
					+ roomid + ")";
			System.out.println("sql=" + sql);
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public Vector getOneAssignMember(int workserial) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "Select a.id,a.Dutyman,u.username from taw_rm_assign_sub a,taw_System_user u where a.workserial = "
	...
	public Vector getOneAssignMemberName(int workserial) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			String sql = "Select a.id,a.Dutyman,u.username from taw_rm_assign_sub a,taw_System_user u where a.workserial = "
					+ workserial + " and a.dutyman = u.userid order by a.id";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public int getExSerial(int workserial, int serialtype,int stagger) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			Sql = "select * from taw_rm_assignwork where id = " + workserial
					+ "";
			sqlw = "";
			pstmt = conn.prepareStatement(Sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				...
				if (serialtype == 1) { // 前一班次
					sqlw = "Select id from taw_rm_assignwork where room_id = "
	...
	public int receiverWorkSerial(String roomId, String time,String userid)
		...
		try {
			...
			try {
				...
				sql = "select a.id,a.workid,a.starttime_defined from taw_rm_assignwork a ,taw_rm_assign_sub b "
	...
	public int receivedrWorkSerial(int roomId) throws SQLException {
		...
		try {
			...
			conn = ds.getConnection();
			sql = "select id,workid,starttime_defined from taw_rm_assignwork "
	...
	public String receiverDutyMaster(int workserial) throws SQLException {
		...
		try {
			...
			conn = ds.getConnection();
			sql = "Select DutyMaster from taw_rm_assignwork where ID =  "
					+ workserial + "";
	...
	public boolean receiverCheckMainRecordExist(int workserial)
		...
		try {
			conn = ds.getConnection();
			sql = "select id from taw_rm_record where id =  " + workserial + "";
			// System.out.println("sql=" + sql);
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public boolean receiverCheckSubRecordExist(int workserial, String userid)
		...
		try {
			conn = ds.getConnection();
			sqls = "Select id from taw_rm_record_sub where WorkSerial = "
					+ workserial + " and DutyMan = '" + userid + "'";
			// System.out.println("sql=" + sql);
	...
	public void getSaveSubRecord(int workserial, String userId)
			throws SQLException {
		String sql = "INSERT INTO taw_rm_record_sub(room_id,workserial,"
		...
		try {
			...
			// 取基本信息
			conn = ds.getConnection();
			Sql = "Select id, room_id, region_id, StartTime_defined,EndTime_defined,Dutydate,Dutymaster from taw_rm_assignwork where id = "
					+ workserial + "";
			pstmt = conn.prepareStatement(Sql);
			rs = pstmt.executeQuery();
			conncreate = ds.getConnection();
		...
		try {
			...
			while (rs.next()) { // 判断主记录是否存在
				...
				if (dutyman.equals("")) {
					...
					sqls = "Select id, DutyMan, Statement from taw_rm_record_sub where WorkSerial = "
	...
	public Vector getStatisticsVector(int roomId, String starttime,
		...
		try {
			conn = ds.getConnection();
			sql = " Select id From TAW_RM_ASSIGNWORK WHERE room_id = ? and starttime_defined >= ? and starttime_defined <= ? ";
	...
	public Vector getStatisticsVector(String WorkserialString)
		...
		try {
			conn = ds.getConnection();
			sql = " Select distinct s.dutyman ,u.username From TAW_RM_ASSIGN_SUB s , TAW_SYSTEM_USER u WHERE s.dutyman=u.userid and s.workserial in ("
					+ WorkserialString + ")";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public Vector getStatisticVector(String dutyman, Vector getStatistic,
		...
		try {
			conn = ds.getConnection();
			sql = " select workflag,count(workflag) From TAW_RM_RECORD_SUB where dutyman=? and workserial in ("
					+ WorkserialString
			...
			close(rs);
			close(pstmt);
			sql = "select count(*) from TAW_RM_ASSIGN_SUB where dutyman=? and workserial in ("
					+ WorkserialString
					+ ") and workserial not in (select workserial From TAW_RM_RECORD_SUB where dutyman=? )";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, dutyman);
	...
	public List getDutyStatistics(int roomId, String starttime, String endtime)
		...
		try {
			conn = ds.getConnection();
			// 将数据插入临时表
			sql = " select extend(starttime_defined,hour to second)||'-'||extend(endtime_defined,hour to second) defined_time , dutyman , username from taw_rm_record_sub , taw_System_user where room_id=? and starttime_defined >= ? and starttime_defined <= ? and dutyman=userid order by dutyman,defined_time "
	...
	public void auditBacth(TawRmRecord tawRmRecord, String chkSel)
			throws SQLException {
		com.boco.eoms.db.util.BocoConnection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			...
			sql = "UPDATE taw_rm_record SET au_content =?,"
	...
	public List listAutoMerge() throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT distinct id FROM taw_rm_assignwork where starttime_defined='"
	...
	public String getCheckman(String workserial, String dutycheckId)
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT dutycheck,dutyman FROM taw_rm_record_sub where workserial='"
					+ workserial + "'";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	 ...
	 * @throws SQLException
	 */
	public void saveRecord1(TawRmRecord tawRmRecord,
			TawRmRecordSub tawRmRecordSub, boolean isDutymaster,
		...
		// 值班住纪录
		sql = "Update TAW_RM_RECORD SET Weather = ?,Temperature = ?,WET = ?,Clean = ?,Clean1 = ?,Conditioner = ?,dutyrecord = ?,notes = ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE ID = "
				+ workserial + "";
		// 值班子记录

		String sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,trans= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
				+ workserial + "  AND Dutyman = '" + userId + "'";
		...
		try {
			...
			if (type == 1) {
				...
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,trans= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 2) {
				switch1 = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,switch= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 3) {
				data = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,data= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 4) {
				system = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,system= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 5) {
				other = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,other= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 6) {
				dutySummary = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,dutySummary= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 7) {
				importRecord = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,importRecord= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 8) {
				importFault = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,importFault= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 9) {
				netCut = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,netCut= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else if (type == 0) {
				netKPI = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,netKPI= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
						+ workserial + "  AND Dutyman = '" + userId + "'";
			} else {
				data = tawRmRecord.getStatement();
				sqlsub = "Update TAW_RM_RECORD_SUB SET Statement = ?,dutycheck = ?,typemode= ?,data= ?, importantnetfault=?, importantsocietydisaster=?, needcorrespond=?, handoverproceeding=? WHERE WorkSerial = "
	...
	public String coalitionSub1(int workserial) throws SQLException {
		...
		try {
			for (int i = 1; i < 6; i++) {
				conn = ds.getConnection();
				sql = " select  a.dutyman,b.typemode from taw_rm_assign_sub a , taw_rm_record_sub b WHERE  a.workserial=b.workserial";
				...
				while (rs.next()) { // 判断主记录是否存在
					...
					if (dutyman.equals("")) {
						...
						conns = ds.getConnection();
						if (i == 1) {
							sqls = "Select id, DutyMan, trans from taw_rm_record_sub where WorkSerial = "
									+ workserial
									+ " And Dutyman = '"
									+ dutyman
									+ "'";
						...
						if (i == 2) {
							sqls = "Select id, DutyMan, switch from taw_rm_record_sub where WorkSerial = "
									+ workserial
									+ " And Dutyman = '"
									+ dutyman
									+ "'";
						...
						if (i == 3) {
							sqls = "Select id, DutyMan, data from taw_rm_record_sub where WorkSerial = "
									+ workserial
									+ " And Dutyman = '"
									+ dutyman
									+ "'";
						...
						if (i == 4) {
							sqls = "Select id, DutyMan, system from taw_rm_record_sub where WorkSerial = "
									+ workserial
									+ " And Dutyman = '"
									+ dutyman
									+ "'";
						...
						if (i == 5) {
							sqls = "Select id, DutyMan, other from taw_rm_record_sub where WorkSerial = "
									+ workserial
									+ " And Dutyman = '"
									+ dutyman
									+ "'";
		...
		try {
			...
			while (rs.next()) { // 判断主记录是否存在
				...
				if (dutyman.equals("")) {
					...
					sqls = "Select id, DutyMan,trans,switch,data,system,other,dutySummary,importRecord, importFault, netCut, netKPI,importantnetfault, importantsocietydisaster, needcorrespond, handoverproceeding from taw_rm_record_sub where WorkSerial = "
							+ workserial + " And Dutyman = '" + dutyman + "'";
	...
	public List getAddons(String roomid) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "select id ,name, remark, model, url, roomId, creatUser, creatTime, deleted from taw_rm_addons where roomId = '"
	...
	public void deleteAddons(String id) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "update taw_rm_addons set deleted = 1 where id = '" + id
					+ "'";
			pstmt = conn.prepareStatement(sql);
			pstmt.executeUpdate();
			conn.commit();
	...
	public String getAddonsUrl(String id) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "select url from taw_rm_addons where id=" + id
					+ " and deleted = 0";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public int retrieveTmp(int workserial, int roomId) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT count(*) FROM TAW_RM_RECORD_Additional WHERE workserial=? and ROOM_ID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, workserial);
	...
	public void deleteAdditionallist(int roomId, int workserial)
		...
		try {
			...
			conn = ds.getConnection();
			sql = "delete TAW_RM_RECORD_Additional where ROOM_ID=? and WORKSERIAL=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, roomId);
	...
	public boolean if_tempManager(int roomId, String userId)
		...
		ResultSet rs = null;
		try {
			sql = "select * from  taw_rm_user  where duty_flag=1 and user_id='"
					+ userId + "'";
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public List retrieveRECORDExcelFile(String time) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			sql = "select t.room_id,t.starttime,t.endtime,t.weather ,t.temperature,t.wet,t.dutydate,t.dutyrecord,t.dutyman from taw_rm_record t where t.dutydate=?";
			pstmt = conn.prepareStatement(sql);
	...
	public void insetAdditional(int roomId, int workserial, String user_id,
		...
		try {
			...
			conn = ds.getConnection();
			sql = "INSERT INTO TAW_RM_RECORD_Additional "
	...
	public List retrieveAdditionalList(int workserial, int roomId,
		...
		try {
			conn = ds.getConnection();
			sql = "SELECT url FROM TAW_RM_RECORD_Additional WHERE workserial=? and ROOM_ID=? and SHEET_NAME=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, workserial);
			pstmt.setInt(2, roomId);
	...
	public List getTawRmExchangeTime(String roomid, String startTime, String endTime, String flag) {
	    ...
	    try {
	    	conn = ds.getConnection();
		String sql = "select workserial,starttime_defined,endtime_defined,receiver from taw_rm_assignwork t,taw_Rm_exchange_per ps WHERE t.id=ps.workserial and t.room_id=? and t.starttime_defined<=? and t.endtime_defined>=? and ps.flag=?";
		pstmt = conn.prepareStatement(sql);
