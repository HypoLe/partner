...
public class TawRmAssignworkDAO extends DAO {

  public TawRmAssignworkDAO(com.boco.eoms.db.util.ConnectionPool ds) {
    super(ds);
  }
  ...
  public void update_dutymaster(int workserial,String dutyman_from,String dutyman_to) throws SQLException {
    ...
    try {
      ...
      String sql = "UPDATE taw_rm_assignwork SET dutymaster=? WHERE id=? and dutymaster=?";
  ...
  public String getStartTime(int id) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT starttime_defined FROM taw_rm_assignwork WHERE id=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, id);
  ...
  public int get_id(int room_id,String dutydate,int workid){
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT id FROM taw_rm_assignwork where room_id=? and dutydate=? and workid=?";
      pstmt = conn.prepareStatement(sql);
  ...
  public Vector getvecAssignAuto(int room_id,String str_start,String str_end){
    ...
    try {
      ...
      conn = ds.getConnection();
      String sql = "SELECT ID,ROOM_ID,starttime_defined,DUTYDATE,WORKID  FROM TAW_RM_ASSIGNWORK where ROOM_ID=? and DUTYDATE >= ? and DUTYDATE <=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, room_id);
  ...
  public void insert(int room_id,String dutydate,int workid,String dutymaster,String starttime_defined,String endtime_defined) throws SQLException {
    String sql;
    sql = "INSERT INTO taw_rm_assignwork ( room_id, dutydate, workid, dutymaster, starttime_defined, endtime_defined) VALUES (?, ?, ?, ?, ?, ?)";
    com.boco.eoms.db.util.BocoConnection conn = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try {   
  ...
  public void delete_room_date(int roomId,String dutydate) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "DELETE FROM taw_rm_assignwork WHERE room_id=? and dutydate=?";
      pstmt = conn.prepareStatement(sql);
  ...
  public void insert(TawRmAssignwork tawRmAssignwork) throws SQLException {
    String sql;
    sql = "INSERT INTO taw_rm_assignwork (id, region_id, room_id, dutydate, workid, flag, dutymaster, starttime_defined, endtime_defined, smsflag, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    com.boco.eoms.db.util.BocoConnection conn = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try {
   ...
   * @throws SQLException
   */
  public void update(TawRmAssignwork tawRmAssignwork) throws SQLException {
    com.boco.eoms.db.util.BocoConnection conn = null;
    PreparedStatement pstmt = null;
    try {
      conn = ds.getConnection();
      String sql = "UPDATE taw_rm_assignwork SET region_id=?, room_id=?, dutydate=?, workid=?, flag=?, dutymaster=?, starttime_defined=?, endtime_defined=?, smsflag=?, notes=? WHERE id=?";
      pstmt = conn.prepareStatement(sql);
  ...
  public void delete(int id) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "DELETE FROM taw_rm_assignwork WHERE id=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, id);
      pstmt.executeUpdate();
      close(pstmt);
   ...
   * @throws SQLException
   */
  public TawRmAssignwork retrieve(int id) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT * FROM taw_rm_assignwork WHERE id=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, id);
      rs = pstmt.executeQuery();
      if (rs.next()) {
  ...
  public Vector getWorkNumofDayByMonth(String YearMonth,int roomId) throws SQLException {
    ...
    try {
      ...
      String sql = "SELECT dutydate,count(dutydate) FROM taw_rm_assignwork WHERE "+conf+"  group by dutydate order by dutydate ";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      int i=0;
      while (rs.next()) {
  ...
  public Vector getWorkSerialofMonth(String YearMonth,int roomId) throws SQLException {
    ...
    try {
      ...
      String sql = "SELECT id,Workid,StartTime_defined,Endtime_defined FROM taw_rm_assignwork WHERE "+conf+" and room_id='"+roomId+"' order by StartTime_defined";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      TawRmAssignwork  tawRmAssignwork=null;
      while (rs.next()) {
  ...
  public String  getMaxDutydate(int roomId) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT max(dutydate) FROM taw_rm_assignwork WHERE  room_id= "+roomId+" ";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      //TawRmAssignwork  tawRmAssignwork=null;
      if (rs.next()) {
  ...
  public boolean  IsAssignWork(int roomId,String dutydate) throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT dutydate FROM taw_rm_assignwork WHERE  room_id= "+roomId+" and dutydate='"+dutydate+"'";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      if (rs.next()) {
        have=true;
  ...
  public List list() throws SQLException {
    ...
    try {
      conn = ds.getConnection();
      String sql = "SELECT region_id, room_id, dutydate, workid, flag, dutymaster, starttime_defined, endtime_defined, smsflag, notes, id FROM taw_rm_assignwork";
      pstmt = conn.prepareStatement(sql);
      rs = pstmt.executeQuery();
      while(rs.next()) {
  ...
  public String getAssignWork(String roomid,String userid,String date) throws SQLException {
	  ...
	  String workid = "";
	    ...
	    try {
	      conn = ds.getConnection();
	      String sql = "SELECT id FROM taw_rm_assignwork where room_id = '"+roomid+"' and  dutymaster = '"+userid+"' and starttime_defined<='"+date+"' and endtime_defined >= '"+date+"'";
	      pstmt = conn.prepareStatement(sql);
	      rs = pstmt.executeQuery();	      
	...
	public String getTimeByWorkSerial(String workSerial)
		...
		try {
			conn = ds.getConnection();
			String sql = "select starttime_defined,endtime_defined from taw_rm_assignwork where id = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public List getDutyAssignwork(String roomId, String dataTime) {
		...
		try {
			conn = ds.getConnection();
			String sql = "SELECT a.* FROM taw_rm_assignwork a "
