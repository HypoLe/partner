...
public class TawRmAssignworkBO extends BO {
	public TawRmAssignworkBO(com.boco.eoms.db.util.ConnectionPool ds) {
		super(ds);
	}

	public TawRmAssignworkBO() {
		super();
	}
	...
	public Vector getQueryResultVector(int roomId, String time)
		...
		try {
			...
			conn = ds.getConnection();
			String sql = "";
			sql = "select a.id,a.dutydate,a.starttime_defined,a.endtime_defined,b.username from taw_rm_assignwork a,taw_system_user b WHERE a.room_id = ? and a.dutydate = ?   and b.userid = a.dutymaster order by a.starttime_defined";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, roomId);
	...
	public void updateAssignworkAddExpert(int roomId, int date_num,
			int team_num, int user_num, Vector vector_date, Vector vector_team,
			Vector vector_user, Vector vector_expert) throws SQLException {
		TawRmAssignworkDAO tawRmAssignworkDAO = new TawRmAssignworkDAO(ds);
		TawRmAssignSubDAO tawRmAssignSubDAO = new TawRmAssignSubDAO(ds);
			...
			for (team_num_i = 0; team_num_i < team_num; team_num_i++) {
				...
				dutymaster = StaticMethod.dbNull2String(String
						.valueOf(vector_user
								.elementAt((date_num_i * team_num + team_num_i)
										* user_num)));
				starttime_defined = StaticMethod.getLocalString(String
						.valueOf(vector_team.elementAt(date_num_i * team_num
								+ team_num_i)));
				if (team_num_i != 0) {
					starttime_defined = StaticMethod
							.getLocalString(StaticMethod.getDateForMinute(
				...
				endtime_defined = StaticMethod.getLocalString(String
						.valueOf(vector_team.elementAt(date_num_i * team_num
								+ team_num_i + 1)));
				tawRmAssignworkDAO.insert(roomId, dutydate, workid, dutymaster,
						starttime_defined, endtime_defined);
				...
				expert = (String) vector_expert.get(team_num_i);
				if (!"".equals(expert))
	...
	public void updateAssignwork2AddExpert(int roomId, int date_num,
			int team_num, int user_num, String dutydate, Vector vector_team,
			Vector vector_user, int cycle, Vector vector_expert)
			throws SQLException {
		TawRmAssignworkDAO tawRmAssignworkDAO = new TawRmAssignworkDAO(ds);
		TawRmAssignSubDAO tawRmAssignSubDAO = new TawRmAssignSubDAO(ds);
		TawRmAssignExpertDAO tawRmAssignExpertDAO = new TawRmAssignExpertDAO(ds);
		...
		for (team_num_i = 0; team_num_i < vector_team.size() - 1; team_num_i++) {
			...
			if (cycle == 0) {
				dutymaster = StaticMethod.dbNull2String(String
						.valueOf(vector_user.elementAt(team_num_i * user_num
								+ cycle * team_num * user_num)));
				expert = (String) vector_expert.get(cycle);
			} else {
				dutymaster = StaticMethod.dbNull2String(String
						.valueOf(vector_user.elementAt(team_num_i * user_num
								+ cycle * team_num * user_num)));
	...
	public void updateAssignwork1AddExpert(int roomId, int date_num,
			int team_num, int user_num, Vector vector_date, Vector vector_team,
			Vector vector_user, Vector vector_expert) throws SQLException {
		TawRmAssignworkDAO tawRmAssignworkDAO = new TawRmAssignworkDAO(ds);
		TawRmAssignSubDAO tawRmAssignSubDAO = new TawRmAssignSubDAO(ds);
	...
	public boolean checkDutyRecordExist(int room_id, String starttime_defined)
		...
		try {
			conn = ds.getConnection();
			String sql = "select count(*) from taw_rm_record where id in (select id from taw_rm_assignwork where starttime_defined = ? and room_id = ?)";
			pstmt = conn.prepareStatement(sql);
	...
	public void delete_room_starttime_main(int roomId, String starttime_defined)
		...
		try {
			conn = ds.getConnection();
			String sql = "DELETE FROM taw_rm_assignwork WHERE room_id=? and starttime_defined=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, roomId);
	...
	public void delete_room_starttime_sub(int roomId, String starttime_defined)
		...
		try {
			conn = ds.getConnection();
			String sql = "DELETE FROM taw_rm_assign_sub WHERE workserial in (select id from taw_rm_assignwork where room_id=? and starttime_defined=?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, roomId);
			pstmt.setString(2, starttime_defined);
			pstmt.executeUpdate();
	...
	public Vector getRoomSelect() throws SQLException {
		...
		try {
			conn = ds.getConnection();
			String sql = "SELECT id,roomname FROM taw_system_cptroom WHERE deleted=0 order by roomname";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			// null
			sql = null;
	...
	public void RemindAssignwork(int roomId, String userid, String result)
		...
		TawRmAssignworkDAO tawRmAssignworkDAO = new TawRmAssignworkDAO(ds);
		TawRmSysteminfoDAO tawRmSysteminfoDAO = new TawRmSysteminfoDAO(ds);
		TawRmSysteminfo tawRmSysteminfo = tawRmSysteminfoDAO.retrieve(String
				.valueOf(roomId));
	...
	public String retrieve(int id) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			String sql = "SELECT roomname FROM taw_system_cptroom WHERE deleted=0 and id=? order by roomname";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, id);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public List getSubDutyMan(String id) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select a.dutyman from taw_rm_assign_sub a where  a.workserial = ? order by a.id";
			pstmt = conn.prepareStatement(sql);
