...
/** 
 * Description: 线路巡检计划轮询(自动生成计划、资源、巡检项)-改造，生成设备巡检项
 * Copyright:   Copyright (c)2012
 * Company:     BOCO 
 * @author:     zhangkeqi
 * @version:    1.0 
 * Create at:   Fri 5, 2013 3:06:23 PM 
 */
public class TransLineInspectScheduler implements Job{
...
//				//将本月必须执行的线路巡检资源关联到合适的计划上(跨月的并且本月必须执行的所有元任务关联到本月计划中，依据是元任务的周期结束时间小于当前月的结束时间)
...
//			}
	...
	public void generateInspectPlanRes(LocalDate now){
		...
		int[] doubleMonthScope = {1,3,5,7,9};
		...
		System.out.println(month+"月份所有地市线路巡检资源生成共耗时:"+(endTime-startTime)+"ms，" +
				"如果未生成成功请检查资源是否已经分配周期和巡检小组，并且该资源所属资源类别的巡检模板是否已经创建");
	}
	
	/**
	 * 生成巡检计划资源
	 * 规则：按周期为所有地市分别生成所有线路巡检资源
	 * @param cycle
...
//			List<Map<String,Object>> cityList = inspectPlanMgr.findResConfigAllCity();
			List<Map<String,Object>> cityList = transLineService.getTransLineDao().getJdbcTemplate().queryForList("select tl_dis as city from pnr_res_config where tl_inspect_flag='1' group by tl_dis");
...
//					deviceInspectService.generateInspectPlanRes(cycle, city, cycleStartTime, cycleEndTime,today);
					//线路巡检
					transLineService.generateInspectPlanRes(cycle, city, cycleStartTime, cycleEndTime,today);
				} catch (Exception e) {
					e.printStackTrace();
					System.out.println("地市代码为"+city+"周期为"+cycleName+"的线路巡检资源生成失败");
				}
			}
			System.out.println("周期为"+cycleName+"的线路巡检资源生成完毕");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("所有地市周期为"+cycleName+"的线路巡检资源生成失败");
		}
	}
	
	/**
	 * 生成巡检项
	 * 规则：查询当天生成的线路巡检资源并按所有地市分别生成巡检项
	 */
...
//					inspectPlanMgr.generateInspectPlanItem(city,today);
					...
					e.printStackTrace();
					System.out.println("地市代码为"+city+"的线路巡检资源巡检项生成失败");
				}
				System.out.println("地市代码为"+city+"的线路巡检资源巡检项生成成功");
			}
			System.out.println("线路巡检资源巡检项生成完毕，如果未生成成功请检查资源所属资源类别的巡检模板项是否已经创建");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("所有地市的线路巡检资源巡检项生成失败");
		}
	}
...
//		} catch (Exception e) {
//			e.printStackTrace();
//			System.out.println("将本月必须执行的线路巡检资源关联到合适的计划上失败");
//		}
//		
//	}
}
