...
import com.boco.eoms.partner.siteEva.webapp.form.SiteEvaReportMultiDeptForm;
...
public final class SiteEvaReportInfoAction extends BaseAction {
	/**
	 * 单一月份、单一厂商查询初始
	 * 
	 * @param mapping
	...
	public ActionForward reportSingle(ActionMapping mapping, ActionForm form,
				...
				if (SiteEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					...
					if (ekis.getIsPublish() != null
							...
							&& ekis.getIsPublish().equals(
									SiteEvaConstants.TASK_PUBLISHED)) {
		...
		// 找到任务权重总分和实际总得分：
		ISiteEvaReportInfoMgr reportInfoMgr = (ISiteEvaReportInfoMgr) getBean("IsiteEvaReportInfoMgr");
		...
		if (!reportInfoList.isEmpty()) {
			SiteEvaReportInfo eri = (SiteEvaReportInfo) reportInfoList.get(0);
	...
	public ActionForward reportMultiDept(ActionMapping mapping,
		...
		for (int m = 0; m < reportInfoList.size(); m++) {
			SiteEvaReportInfo eri = (SiteEvaReportInfo) reportInfoList.get(m);
			if (totalWeight == null || "".equals(totalWeight)) {
				totalWeight = eri.getTotalWeight();
			}
			SiteEvaReportMultiDeptForm ermdf = new SiteEvaReportMultiDeptForm();
			...
			for (int i = 1; i <= maxListNo; i++) {
				...
				for (int j = 0; j < rowList.size(); j++) {
					...
					if (SiteEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
						...
						ISiteEvaKpiMgr siteEvaKpiMgr = (ISiteEvaKpiMgr) getBean("IsiteEvaKpiMgr");
						kpiWeights.put(siteEvaKpiMgr.id2Name(etd.getKpiId()), etd
								.getWeight());
					}
				}
			}
