 ...
 * 
 */
public class SiteEvaStaticMethod {
	...
	public static String getStartTimeByCycle(String cycle,
		...
		if (SiteEvaConstants.CYCLE_YEAR.equals(cycle)) {
		...
		} else if (SiteEvaConstants.CYCLE_QUARTER.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			...
			startTime = DateTimeUtil.dateToString(date);
		} else if (SiteEvaConstants.CYCLE_MONTH.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			int month = Integer.parseInt(currentDateTime.substring(5, 7));
			Date date = DateTimeUtil.getFirstDayOfMonth(year, month);
			startTime = DateTimeUtil.dateToString(date);
		} else if (SiteEvaConstants.CYCLE_WEEK.equals(cycle)) {
	...
	public static String orgId2Name(String orgId, String orgType)
		...
		String orgName = "";
		// 若为角色则显示角色名称
		if (SiteEvaConstants.ORG_SUBROLE.equals(orgType)) {
		...
		// 若为用户则显示用户名称
		else if (SiteEvaConstants.ORG_USER.equals(orgType)) {
		...
		// 若为部门则显示部门名称
		else if (SiteEvaConstants.ORG_DEPT.equals(orgType)) {
	...
	public static List jsonOrg2Orgs(String orgs) {
		...
		for (Iterator it = jsonOrgs.iterator(); it.hasNext();) {
			...
			// 限制发布范围
			if (SiteEvaConstants.ORG_DEPT.equals(nodeType)
				...
				for (int i = 0; i < depts.size(); i++) {
					...
					orgList.add(new SiteEvaOrg(dept.getDeptId(),
							StaticVariable.PRIV_ASSIGNTYPE_DEPT));
				}
			} else if (SiteEvaConstants.ORG_SUBROLE.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_ROLE.equals(nodeType)) {
				orgList
						.add(new SiteEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_ROLE));
			} else if (SiteEvaConstants.ORG_USER.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_USER.equals(nodeType)) {
				orgList
						.add(new SiteEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_USER));
			}
		}
		return orgList;
	}
	...
	public static String getOrgList(List orgs) {
		...
		if (null != orgs && !orgs.isEmpty()) {
			...
			for (Iterator it = orgs.iterator(); it.hasNext();) {
				SiteEvaOrg org = (SiteEvaOrg) it.next();
				int orgtype = StaticMethod.null2int(org.getOrgType());
				JSONObject item = new JSONObject();
				// 构造json对象
				item.put(UIConstants.JSON_ID, org.getOrgId());
				item.put(UIConstants.JSON_NAME, SiteEvaStaticMethod.getOrgName(org
	...
	public static void main(String[] args) {
		System.out.println("本年第一天："
				+ getStartTimeByCycle(SiteEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本季度第一天："
				+ getStartTimeByCycle(SiteEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本月第一天："
				+ getStartTimeByCycle(SiteEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本周第一天："
				+ getStartTimeByCycle(SiteEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本年最后一天："
				+ getEndTimeByCycle(SiteEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本季度最后一天："
				+ getEndTimeByCycle(SiteEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本月最后一天："
				+ getEndTimeByCycle(SiteEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
		System.out.println("本周最后一天："
				+ getEndTimeByCycle(SiteEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(SiteEvaConstants.DATE_FORMAT)));
	}
}
