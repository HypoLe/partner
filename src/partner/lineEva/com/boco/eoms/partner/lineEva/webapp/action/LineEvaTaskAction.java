...
import com.boco.eoms.partner.lineEva.util.RoleIdList;
import com.boco.eoms.partner.lineEva.webapp.form.LineEvaKpiInstanceForm;

public final class LineEvaTaskAction extends BaseAction {
	...
	public ActionForward taskView(ActionMapping mapping, ActionForm form,
		...
		for (int i = 0; i < list.size(); i++) {
			LineEvaTree et = (LineEvaTree) list.get(i);
			templateType.add(et);
		}
...
//		}
	...
	public ActionForward changeTemplateType(ActionMapping mapping,
		...
		TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
		ILineEvaTaskMgr taskMgr = (ILineEvaTaskMgr) getBean("IlineEvaTaskMgr");
		ILineEvaTemplateMgr tempMgr = (ILineEvaTemplateMgr) getBean("IlineEvaTemplateMgr");
		ILineEvaTreeMgr treeMgr = (ILineEvaTreeMgr) getBean("IlineEvaTreeMgr");

		//2009-8-4 模版显示加角色控制，考核模版区（省）负责人能看到所有模版
		List list = new ArrayList();
		TawSystemSessionForm sessionForm = (TawSystemSessionForm) request.getSession().getAttribute("sessionform");
		RoleIdList roleIdList = (RoleIdList) getBean("lineEvaRoleIdList");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdmin(LineEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrg(sessionform.getDeptid(),LineEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			LineEvaTask et = (LineEvaTask) list.get(i);
			LineEvaTemplate template = templateMgr.getTemplate(et.getTemplateId());
			String starttime = "";
	...
	public ActionForward changeTemplateTypeAndTime(ActionMapping mapping,
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdminInTime(LineEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrgInTime(sessionform.getDeptid(),LineEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
	...
	public ActionForward taskDetailList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		ILineEvaTaskDetailMgr taskDetailMgr = (ILineEvaTaskDetailMgr) getBean("IlineEvaTaskDetailMgr");
		ILineEvaKpiInstanceMgr lineEvaKpiInstanceMgr = (ILineEvaKpiInstanceMgr) getBean("IlineEvaKpiInstanceMgr");
		ILineEvaKpiMgr lineEvaKpiMgr = (ILineEvaKpiMgr) getBean("IlineEvaKpiMgr");
		...
		ILineEvaTaskMgr taskMgr = (ILineEvaTaskMgr) getBean("IlineEvaTaskMgr");
		ILineEvaTemplateMgr templateMgr = (ILineEvaTemplateMgr) getBean("IlineEvaTemplateMgr");
		LineEvaTask et = taskMgr.getTaskById(taskId);
		String taskName = templateMgr.id2Name(et.getTemplateId());
		
		LineEvaTemplate lineEvaTemplate = templateMgr.getTemplate(et.getTemplateId());
		String parIds = lineEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
...
//		String timeType = "月度";// 后续开发
		...
		for (int i = 1; i <= maxListNo; i++) {
			...
			for (int j = 0; j < rowList.size(); j++) {
				LineEvaTaskDetail etd = (LineEvaTaskDetail) rowList.get(j);
				LineEvaKpiInstanceForm ekif = new LineEvaKpiInstanceForm();
				...
				if (LineEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					LineEvaKpiInstance ekis = new LineEvaKpiInstance();
					for(int k =0 ; parIdssList.size()>k;k++){
						ekis = lineEvaKpiInstanceMgr
...
//						}
					...
					if (ekis.getIsPublish() != null
						...
						ekif.setIsPublish(LineEvaConstants.TASK_NOT_PUBLISHED);
					}
				...
				// 算法加入
				LineEvaKpi ek = lineEvaKpiMgr.getKpi(ekif.getKpiId());
	...
	public ActionForward saveTaskDetail(ActionMapping mapping, ActionForm form,
		...
		if (xsave(mapping, form, request, response,
				LineEvaConstants.TASK_NOT_PUBLISHED))
	...
	public ActionForward commitTaskDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (xsave(mapping, form, request, response, LineEvaConstants.TASK_PUBLISHED))
			return mapping.findForward("success");
		else
			return mapping.findForward("fail");
	}
	...
	public boolean xsave(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			ILineEvaTaskDetailMgr taskDetailMgr = (ILineEvaTaskDetailMgr) getBean("IlineEvaTaskDetailMgr");
			ILineEvaKpiInstanceMgr lineEvaKpiInstanceMgr = (ILineEvaKpiInstanceMgr) getBean("IlineEvaKpiInstanceMgr");
			ILineEvaReportInfoMgr lineEvaReportInfoMgr = (ILineEvaReportInfoMgr) getBean("IlineEvaReportInfoMgr");
			AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
//			有几个合作伙伴保存几次
			for(int f = 0 ; parIdss.length>f;f++){
				...
				for (int i = 1; i <= maxListNo; i++) {
					...
					for (int j = 0; j < rowList.size(); j++) {
						...
						if (LineEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
							...
							LineEvaKpiInstance ekis = lineEvaKpiInstanceMgr
									.getKpiInstanceByTimeAndPartner(etd.getId(),
							...
							lineEvaKpiInstanceMgr.saveKpiInstance(ekis);
						}
					}
					if (rowList.size() > maxLevel) {
						maxLevel = rowList.size();
				...
				if (saveType.equals(LineEvaConstants.TASK_PUBLISHED)) {
					LineEvaReportInfo eri = new LineEvaReportInfo();
					...
					lineEvaReportInfoMgr.saveLineEvaReportInfo(eri);
				}
			}
			return true;
		} catch (Exception ex) {
...
//		String deptName = sessionform.getDeptname();
	...
	public ActionForward xquery(ActionMapping mapping, ActionForm form,
		...
		if (searchType == null || "".equals(searchType)) {
			searchType = LineEvaConstants.TASK_PUBLISHED;
		}
		...
		for (int i = 0; i < rowList.size(); i++) {
			LineEvaTaskDetail etd = (LineEvaTaskDetail) rowList.get(i);
			if (LineEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
				taskList = lineEvaKpiInstanceMgr
						.getKpiInstanceListByTimeAndPartner(etd.getId(),
								partner, timeType, startTime, endTime,
								searchType);
				for (int j = 0; j < taskList.size(); j++) {
					LineEvaKpiInstance edi = (LineEvaKpiInstance) taskList.get(j);
		...
		for (int i = 0; i < taskDetailList.size(); i++) {
			LineEvaKpiInstance eki = (LineEvaKpiInstance) taskDetailList.get(i);
			LineEvaKpiInstanceForm ekif = (LineEvaKpiInstanceForm)convert(eki);
			LineEvaTaskDetail etd = taskDetailMgr.getTaskDetailById(eki.getTaskDetailId());
			LineEvaTask et = taskMgr.getTaskById(etd.getTaskId());
