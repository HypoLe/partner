...
import com.boco.eoms.partner.lineEva.webapp.form.LineEvaReportMultiDeptForm;
...
public final class LineEvaReportInfoAction extends BaseAction {
	/**
	 * 单一月份、单一厂商查询初始
	 * 
	 * @param mapping
	...
	public ActionForward reportSingle(ActionMapping mapping, ActionForm form,
				...
				if (LineEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					...
					if (ekis.getIsPublish() != null
							...
							&& ekis.getIsPublish().equals(
									LineEvaConstants.TASK_PUBLISHED)) {
		...
		// 找到任务权重总分和实际总得分：
		ILineEvaReportInfoMgr reportInfoMgr = (ILineEvaReportInfoMgr) getBean("IlineEvaReportInfoMgr");
		...
		if (!reportInfoList.isEmpty()) {
			LineEvaReportInfo eri = (LineEvaReportInfo) reportInfoList.get(0);
	...
	public ActionForward reportMultiDept(ActionMapping mapping,
		...
		for (int m = 0; m < reportInfoList.size(); m++) {
			LineEvaReportInfo eri = (LineEvaReportInfo) reportInfoList.get(m);
			if (totalWeight == null || "".equals(totalWeight)) {
				totalWeight = eri.getTotalWeight();
			}
			LineEvaReportMultiDeptForm ermdf = new LineEvaReportMultiDeptForm();
			...
			for (int i = 1; i <= maxListNo; i++) {
				...
				for (int j = 0; j < rowList.size(); j++) {
					...
					if (LineEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
						...
						ILineEvaKpiMgr lineEvaKpiMgr = (ILineEvaKpiMgr) getBean("IlineEvaKpiMgr");
						kpiWeights.put(lineEvaKpiMgr.id2Name(etd.getKpiId()), etd
								.getWeight());
					}
				}
			}
