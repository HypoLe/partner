...
public class LineEvaTaskDetailDaoHibernate extends BaseDaoHibernate implements
		ILineEvaTaskDetailDao {

	public int getMaxListNoOfTask(final String taskId) {
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				String hql = "select max(cast(detail.listNo as integer)) from LineEvaTaskDetail detail where detail.taskId='"
						+ taskId + "'";
	...
	public List listDetailOfTaskByListNo(String taskId, String listNo) {
		String sql = "from LineEvaTaskDetail detail where 1=1 ";
	...
	public LineEvaTaskDetail getTaskDetailById(String id) {	
		LineEvaTaskDetail task = new LineEvaTaskDetail();		
		List list = getHibernateTemplate().find(
				"from LineEvaTaskDetail detail where detail.id='" + id + "'");
		if(!list.isEmpty()){
			task = (LineEvaTaskDetail)list.get(0);
		}
		return task;
	}
