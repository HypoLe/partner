...
import clover.org.apache.velocity.runtime.parser.node.w;
...
import com.boco.eoms.partner.assiEva.util.RoleIdList;
...
public final class AssiEvaTaskAction extends BaseAction {
...
//		得到专业类型		
		...
		for (int i = 0; i < list.size(); i++) {
			AssiEvaTree et = (AssiEvaTree) list.get(i);
			templateType.add(et);
		}
...
//		}
	...
	public ActionForward changeTemplateType(ActionMapping mapping,
		...
		TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
		IAssiEvaTaskMgr taskMgr = (IAssiEvaTaskMgr) getBean("IassiEvaTaskMgr");
		IAssiEvaTemplateMgr tempMgr = (IAssiEvaTemplateMgr) getBean("IassiEvaTemplateMgr");
		IAssiEvaTreeMgr treeMgr = (IAssiEvaTreeMgr) getBean("IassiEvaTreeMgr");

		//2009-8-4 模版显示加角色控制，考核模版区（省）负责人能看到所有模版
		List list = new ArrayList();
		TawSystemSessionForm sessionForm = (TawSystemSessionForm) request.getSession().getAttribute("sessionform");
		RoleIdList roleIdList = (RoleIdList) getBean("assiEvaRoleIdList");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdmin(AssiEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrg(sessionform.getDeptid(),AssiEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			AssiEvaTask et = (AssiEvaTask) list.get(i);
			AssiEvaTemplate template = templateMgr.getTemplate(et.getTemplateId());
			String starttime = "";
	...
	public ActionForward changeTemplateTypeAndTime(ActionMapping mapping,
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdminInTime(AssiEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrgInTime(sessionform.getDeptid(),AssiEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
	...
	public ActionForward taskDetailList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		IAssiEvaTaskDetailMgr taskDetailMgr = (IAssiEvaTaskDetailMgr) getBean("IassiEvaTaskDetailMgr");
		IAssiEvaKpiInstanceMgr assiEvaKpiInstanceMgr = (IAssiEvaKpiInstanceMgr) getBean("IassiEvaKpiInstanceMgr");
		IAssiEvaKpiMgr assiEvaKpiMgr = (IAssiEvaKpiMgr) getBean("IassiEvaKpiMgr");
		...
		IAssiEvaTaskMgr taskMgr = (IAssiEvaTaskMgr) getBean("IassiEvaTaskMgr");
		IAssiEvaTemplateMgr templateMgr = (IAssiEvaTemplateMgr) getBean("IassiEvaTemplateMgr");
		AssiEvaTask et = taskMgr.getTaskById(taskId);
		String taskName = templateMgr.id2Name(et.getTemplateId());
		
		AssiEvaTemplate assiEvaTemplate = templateMgr.getTemplate(et.getTemplateId());
		String parIds = assiEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
...
//		String timeType = "月度";// 后续开发
		...
		for (int i = 1; i <= maxListNo; i++) {
			...
			for (int j = 0; j < rowList.size(); j++) {
				AssiEvaTaskDetail etd = (AssiEvaTaskDetail) rowList.get(j);
				AssiEvaKpiInstanceForm ekif = new AssiEvaKpiInstanceForm();
				...
				if (AssiEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					AssiEvaKpiInstance ekis = new AssiEvaKpiInstance();
					for(int k =0 ; parIdssList.size()>k;k++){
						ekis = assiEvaKpiInstanceMgr
...
//						}
					...
					if (ekis.getIsPublish() != null
						...
						ekif.setIsPublish(AssiEvaConstants.TASK_NOT_PUBLISHED);
					}
				...
				// 算法加入
				AssiEvaKpi ek = assiEvaKpiMgr.getKpi(ekif.getKpiId());
		...
		request.setAttribute("specialty", specialty); // 被考核合作伙伴
	...
	public ActionForward saveTaskDetail(ActionMapping mapping, ActionForm form,
		...
		if (xsave(mapping, form, request, response,
				AssiEvaConstants.TASK_NOT_PUBLISHED))
	...
	public ActionForward commitTaskDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (xsave(mapping, form, request, response, AssiEvaConstants.TASK_PUBLISHED))
			return mapping.findForward("success");
		else
			return mapping.findForward("fail");
	}
	...
	public boolean xsave(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			IAssiEvaTaskDetailMgr taskDetailMgr = (IAssiEvaTaskDetailMgr) getBean("IassiEvaTaskDetailMgr");
			IAssiEvaKpiInstanceMgr assiEvaKpiInstanceMgr = (IAssiEvaKpiInstanceMgr) getBean("IassiEvaKpiInstanceMgr");
			IAssiEvaReportInfoMgr assiEvaReportInfoMgr = (IAssiEvaReportInfoMgr) getBean("IassiEvaReportInfoMgr");
			AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
//			有几个合作伙伴保存几次
			for(int f = 0 ; parIdss.length>f;f++){
				...
				for (int i = 1; i <= maxListNo; i++) {
					...
					for (int j = 0; j < rowList.size(); j++) {
						...
						if (AssiEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
							...
							AssiEvaKpiInstance ekis = assiEvaKpiInstanceMgr
									.getKpiInstanceByTimeAndPartner(etd.getId(),
							...
							assiEvaKpiInstanceMgr.saveKpiInstance(ekis);
						}
					}
					if (rowList.size() > maxLevel) {
						maxLevel = rowList.size();
				...
				if (saveType.equals(AssiEvaConstants.TASK_PUBLISHED)) {
					AssiEvaReportInfo eri = new AssiEvaReportInfo();
					...
					assiEvaReportInfoMgr.saveAssiEvaReportInfo(eri);
				}
			}
			return true;
		} catch (Exception ex) {
...
//		AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
		...
		if (searchType == null || "".equals(searchType)) {
			searchType = AssiEvaConstants.TASK_PUBLISHED;
		}
//		得到合作伙伴id,考核几个合作伙伴就循环几次
		AssiEvaTask aet = taskMgr.getTaskById(taskId);
		AssiEvaTemplate assiEvaTemplate = templateMgr.getTemplate(aet.getTemplateId());
		String parIds = assiEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
			parIds = parIds.substring(1);
		}
		...
		List rowList = taskDetailMgr.listDetailOfTaskByListNo(taskId, String.valueOf("1"));
		List taskDetailList = new ArrayList();
		List taskList = null;
		for(int k = 0; k < parIdssTemp.length; k++){
			for (int i = 0; i < rowList.size(); i++) {
				AssiEvaTaskDetail etd = (AssiEvaTaskDetail) rowList.get(i);
				if (AssiEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					taskList = assiEvaKpiInstanceMgr
							.getKpiInstanceListByTimeAndPartner(etd.getId(),
									parIdssTemp[k], timeType, startTime, endTime,
									searchType);
					for (int j = 0; j < taskList.size(); j++) {
						AssiEvaKpiInstance edi = (AssiEvaKpiInstance) taskList.get(j);
		...
		for (int i = 0; i < taskDetailList.size(); i++) {
			AssiEvaKpiInstance eki = (AssiEvaKpiInstance) taskDetailList.get(i);
			AssiEvaKpiInstanceForm ekif = (AssiEvaKpiInstanceForm)convert(eki);
			AssiEvaTaskDetail etd = taskDetailMgr.getTaskDetailById(eki.getTaskDetailId());
			AssiEvaTask et = taskMgr.getTaskById(etd.getTaskId());
