...
import com.boco.eoms.partner.assiEva.webapp.form.AssiEvaReportMultiDeptForm;
...
public final class AssiEvaReportInfoAction extends BaseAction {
	/**
	 * 单一月份、单一厂商查询初始
	 * 
	 * @param mapping
	...
	public ActionForward reportSingle(ActionMapping mapping, ActionForm form,
				...
				if (AssiEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					...
					if (ekis.getIsPublish() != null
							...
							&& ekis.getIsPublish().equals(
									AssiEvaConstants.TASK_PUBLISHED)) {
		...
		// 找到任务权重总分和实际总得分：
		IAssiEvaReportInfoMgr reportInfoMgr = (IAssiEvaReportInfoMgr) getBean("IassiEvaReportInfoMgr");
		...
		if (!reportInfoList.isEmpty()) {
			AssiEvaReportInfo eri = (AssiEvaReportInfo) reportInfoList.get(0);
	...
	public ActionForward reportMultiDept(ActionMapping mapping,
		...
		for (int m = 0; m < reportInfoList.size(); m++) {
			AssiEvaReportInfo eri = (AssiEvaReportInfo) reportInfoList.get(m);
			if (totalWeight == null || "".equals(totalWeight)) {
				totalWeight = eri.getTotalWeight();
			}
			AssiEvaReportMultiDeptForm ermdf = new AssiEvaReportMultiDeptForm();
			...
			for (int i = 1; i <= maxListNo; i++) {
				...
				for (int j = 0; j < rowList.size(); j++) {
					...
					if (AssiEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
						...
						IAssiEvaKpiMgr assiEvaKpiMgr = (IAssiEvaKpiMgr) getBean("IassiEvaKpiMgr");
						kpiWeights.put(assiEvaKpiMgr.id2Name(etd.getKpiId()), etd
								.getWeight());
					}
				}
			}
