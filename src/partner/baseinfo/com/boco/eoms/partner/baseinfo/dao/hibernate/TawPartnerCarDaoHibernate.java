 ...
 * 
 */
public class TawPartnerCarDaoHibernate extends BaseDaoHibernate implements
		TawPartnerCarDao, ID2NameDAO {
	...
	public List getTawPartnerCars() {
		...
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				String queryStr = "from TawPartnerCar tawPartnerCar where tawPartnerCar.deleted <> '1'";
	...
	public void saveTawPartnerCar(final TawPartnerCar tawPartnerCar) {
		if ((tawPartnerCar.getId() == null)
				|| (tawPartnerCar.getId().equals("")))
	...
	public Map getTawPartnerCars(final Integer curPage, final Integer pageSize,
		...
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				String queryStr = "from TawPartnerCar tawPartnerCar where deleted = '0'";
				...
				queryStr+= " order by addTime desc";
				Query query = session.createQuery(queryStr);
	...
	public String name2Id(final String dictName, final String parentDictId) {
		...
		dictType = (TawSystemDictType) getHibernateTemplate().execute(callback);
	...
	public String[] getDictIdByParentId(final String parentDictId) {
		...
		arrayStr = (String[]) getHibernateTemplate().execute(callback);

		return arrayStr;
	}

	/**
	 * id:EOMS-liujinlong-20090921105914 开发人邮箱：liujinlong@boco.com.cn
	 * 时间：2009-09-21 开发目的：将删除改为设置删除字段置deleted为1 参数deptID：地域部门树中的合作伙伴节点ID
	 */
	public void removeTawPartnerCarByDeptID(final String deptID) {
		final String hql = "update TawPartnerCar tawPartnerCar set deleted = '1' where tawPartnerCar.dept_id = '"
				+ deptID + "'";
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
	...
	public String getIdByName(String name) {
		String hql="from TawSystemDictType where dictName='"+name+"'";
		List<TawSystemDictType> list=getHibernateTemplate().find(hql);
		if(!list.isEmpty()){
		String idtoname =list.get(0).getDictId();
	...
	public String getDictIdByPartnerId(String id) {
		String hql="from PartnerDept where id="+id;
		PartnerDept dept=(PartnerDept)getHibernateTemplate().get(PartnerDept.class, id);
		if(dept!=null){
			return dept.getDeptMagId();
			}
		return "";
		}
