...
import com.boco.eoms.partner.tranEva.util.RoleIdList;
...
public final class TranEvaTaskAction extends BaseAction {
	...
	public ActionForward taskView(ActionMapping mapping, ActionForm form,
		...
		for (int i = 0; i < list.size(); i++) {
			TranEvaTree et = (TranEvaTree) list.get(i);
			templateType.add(et);
		}
...
//		}
	...
	public ActionForward changeTemplateType(ActionMapping mapping,
		...
		TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
		ITranEvaTaskMgr taskMgr = (ITranEvaTaskMgr) getBean("ItranEvaTaskMgr");
		ITranEvaTemplateMgr tempMgr = (ITranEvaTemplateMgr) getBean("ItranEvaTemplateMgr");
		ITranEvaTreeMgr treeMgr = (ITranEvaTreeMgr) getBean("ItranEvaTreeMgr");

		//2009-8-4 模版显示加角色控制，考核模版区（省）负责人能看到所有模版
		List list = new ArrayList();
		TawSystemSessionForm sessionForm = (TawSystemSessionForm) request.getSession().getAttribute("sessionform");
		RoleIdList roleIdList = (RoleIdList) getBean("tranEvaRoleIdList");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdmin(TranEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrg(sessionform.getDeptid(),TranEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			TranEvaTask et = (TranEvaTask) list.get(i);
			TranEvaTemplate template = templateMgr.getTemplate(et.getTemplateId());
			String starttime = "";
	...
	public ActionForward changeTemplateTypeAndTime(ActionMapping mapping,
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdminInTime(TranEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrgInTime(sessionform.getDeptid(),TranEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
	...
	public ActionForward taskDetailList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		ITranEvaTaskDetailMgr taskDetailMgr = (ITranEvaTaskDetailMgr) getBean("ItranEvaTaskDetailMgr");
		ITranEvaKpiInstanceMgr tranEvaKpiInstanceMgr = (ITranEvaKpiInstanceMgr) getBean("ItranEvaKpiInstanceMgr");
		ITranEvaKpiMgr tranEvaKpiMgr = (ITranEvaKpiMgr) getBean("ItranEvaKpiMgr");
		...
		ITranEvaTaskMgr taskMgr = (ITranEvaTaskMgr) getBean("ItranEvaTaskMgr");
		ITranEvaTemplateMgr templateMgr = (ITranEvaTemplateMgr) getBean("ItranEvaTemplateMgr");
		TranEvaTask et = taskMgr.getTaskById(taskId);
		String taskName = templateMgr.id2Name(et.getTemplateId());
		
		TranEvaTemplate tranEvaTemplate = templateMgr.getTemplate(et.getTemplateId());
		String parIds = tranEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
...
//		String timeType = "月度";// 后续开发
		...
		for (int i = 1; i <= maxListNo; i++) {
			...
			for (int j = 0; j < rowList.size(); j++) {
				TranEvaTaskDetail etd = (TranEvaTaskDetail) rowList.get(j);
				TranEvaKpiInstanceForm ekif = new TranEvaKpiInstanceForm();
				...
				if (TranEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					TranEvaKpiInstance ekis = new TranEvaKpiInstance();
					for(int k =0 ; parIdssList.size()>k;k++){
						ekis = tranEvaKpiInstanceMgr
...
//						}
					...
					if (ekis.getIsPublish() != null
						...
						ekif.setIsPublish(TranEvaConstants.TASK_NOT_PUBLISHED);
					}
				...
				// 算法加入
				TranEvaKpi ek = tranEvaKpiMgr.getKpi(ekif.getKpiId());
	...
	public ActionForward saveTaskDetail(ActionMapping mapping, ActionForm form,
		...
		if (xsave(mapping, form, request, response,
				TranEvaConstants.TASK_NOT_PUBLISHED))
	...
	public ActionForward commitTaskDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (xsave(mapping, form, request, response, TranEvaConstants.TASK_PUBLISHED))
			return mapping.findForward("success");
		else
			return mapping.findForward("fail");
	}
	...
	public boolean xsave(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			ITranEvaTaskDetailMgr taskDetailMgr = (ITranEvaTaskDetailMgr) getBean("ItranEvaTaskDetailMgr");
			ITranEvaKpiInstanceMgr tranEvaKpiInstanceMgr = (ITranEvaKpiInstanceMgr) getBean("ItranEvaKpiInstanceMgr");
			ITranEvaReportInfoMgr tranEvaReportInfoMgr = (ITranEvaReportInfoMgr) getBean("ItranEvaReportInfoMgr");
			AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
//			有几个合作伙伴保存几次
			for(int f = 0 ; parIdss.length>f;f++){
				...
				for (int i = 1; i <= maxListNo; i++) {
					...
					for (int j = 0; j < rowList.size(); j++) {
						...
						if (TranEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
							...
							TranEvaKpiInstance ekis = tranEvaKpiInstanceMgr
									.getKpiInstanceByTimeAndPartner(etd.getId(),
							...
							tranEvaKpiInstanceMgr.saveKpiInstance(ekis);
						}
					}
					if (rowList.size() > maxLevel) {
						maxLevel = rowList.size();
				...
				if (saveType.equals(TranEvaConstants.TASK_PUBLISHED)) {
					TranEvaReportInfo eri = new TranEvaReportInfo();
					...
					tranEvaReportInfoMgr.saveTranEvaReportInfo(eri);
				}
			}
			return true;
		} catch (Exception ex) {
...
//		String deptName = sessionform.getDeptname();
	...
	public ActionForward xquery(ActionMapping mapping, ActionForm form,
		...
		if (searchType == null || "".equals(searchType)) {
			searchType = TranEvaConstants.TASK_PUBLISHED;
		}
		...
		for (int i = 0; i < rowList.size(); i++) {
			TranEvaTaskDetail etd = (TranEvaTaskDetail) rowList.get(i);
			if (TranEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
				taskList = tranEvaKpiInstanceMgr
						.getKpiInstanceListByTimeAndPartner(etd.getId(),
								partner, timeType, startTime, endTime,
								searchType);
				for (int j = 0; j < taskList.size(); j++) {
					TranEvaKpiInstance edi = (TranEvaKpiInstance) taskList.get(j);
		...
		for (int i = 0; i < taskDetailList.size(); i++) {
			TranEvaKpiInstance eki = (TranEvaKpiInstance) taskDetailList.get(i);
			TranEvaKpiInstanceForm ekif = (TranEvaKpiInstanceForm)convert(eki);
			TranEvaTaskDetail etd = taskDetailMgr.getTaskDetailById(eki.getTaskDetailId());
			TranEvaTask et = taskMgr.getTaskById(etd.getTaskId());
