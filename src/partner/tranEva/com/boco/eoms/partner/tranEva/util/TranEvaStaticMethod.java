 ...
 * 
 */
public class TranEvaStaticMethod {
	...
	public static String getStartTimeByCycle(String cycle,
		...
		if (TranEvaConstants.CYCLE_YEAR.equals(cycle)) {
		...
		} else if (TranEvaConstants.CYCLE_QUARTER.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			...
			startTime = DateTimeUtil.dateToString(date);
		} else if (TranEvaConstants.CYCLE_MONTH.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			int month = Integer.parseInt(currentDateTime.substring(5, 7));
			Date date = DateTimeUtil.getFirstDayOfMonth(year, month);
			startTime = DateTimeUtil.dateToString(date);
		} else if (TranEvaConstants.CYCLE_WEEK.equals(cycle)) {
	...
	public static String orgId2Name(String orgId, String orgType)
		...
		String orgName = "";
		// 若为角色则显示角色名称
		if (TranEvaConstants.ORG_SUBROLE.equals(orgType)) {
		...
		// 若为用户则显示用户名称
		else if (TranEvaConstants.ORG_USER.equals(orgType)) {
		...
		// 若为部门则显示部门名称
		else if (TranEvaConstants.ORG_DEPT.equals(orgType)) {
	...
	public static List jsonOrg2Orgs(String orgs) {
		...
		for (Iterator it = jsonOrgs.iterator(); it.hasNext();) {
			...
			// 限制发布范围
			if (TranEvaConstants.ORG_DEPT.equals(nodeType)
				...
				for (int i = 0; i < depts.size(); i++) {
					...
					orgList.add(new TranEvaOrg(dept.getDeptId(),
							StaticVariable.PRIV_ASSIGNTYPE_DEPT));
				}
			} else if (TranEvaConstants.ORG_SUBROLE.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_ROLE.equals(nodeType)) {
				orgList
						.add(new TranEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_ROLE));
			} else if (TranEvaConstants.ORG_USER.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_USER.equals(nodeType)) {
				orgList
						.add(new TranEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_USER));
			}
		}
		return orgList;
	}
	...
	public static String getOrgList(List orgs) {
		...
		if (null != orgs && !orgs.isEmpty()) {
			...
			for (Iterator it = orgs.iterator(); it.hasNext();) {
				TranEvaOrg org = (TranEvaOrg) it.next();
				int orgtype = StaticMethod.null2int(org.getOrgType());
				JSONObject item = new JSONObject();
				// 构造json对象
				item.put(UIConstants.JSON_ID, org.getOrgId());
				item.put(UIConstants.JSON_NAME, TranEvaStaticMethod.getOrgName(org
	...
	public static void main(String[] args) {
		System.out.println("本年第一天："
				+ getStartTimeByCycle(TranEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本季度第一天："
				+ getStartTimeByCycle(TranEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本月第一天："
				+ getStartTimeByCycle(TranEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本周第一天："
				+ getStartTimeByCycle(TranEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本年最后一天："
				+ getEndTimeByCycle(TranEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本季度最后一天："
				+ getEndTimeByCycle(TranEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本月最后一天："
				+ getEndTimeByCycle(TranEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
		System.out.println("本周最后一天："
				+ getEndTimeByCycle(TranEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(TranEvaConstants.DATE_FORMAT)));
	}
}
