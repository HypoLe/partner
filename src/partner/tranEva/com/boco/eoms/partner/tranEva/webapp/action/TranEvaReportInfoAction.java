...
import com.boco.eoms.partner.tranEva.webapp.form.TranEvaReportMultiDeptForm;
...
public final class TranEvaReportInfoAction extends BaseAction {
	/**
	 * 单一月份、单一厂商查询初始
	 * 
	 * @param mapping
	...
	public ActionForward reportSingle(ActionMapping mapping, ActionForm form,
				...
				if (TranEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					...
					if (ekis.getIsPublish() != null
							...
							&& ekis.getIsPublish().equals(
									TranEvaConstants.TASK_PUBLISHED)) {
		...
		// 找到任务权重总分和实际总得分：
		ITranEvaReportInfoMgr reportInfoMgr = (ITranEvaReportInfoMgr) getBean("ItranEvaReportInfoMgr");
		...
		if (!reportInfoList.isEmpty()) {
			TranEvaReportInfo eri = (TranEvaReportInfo) reportInfoList.get(0);
	...
	public ActionForward reportMultiDept(ActionMapping mapping,
		...
		for (int m = 0; m < reportInfoList.size(); m++) {
			TranEvaReportInfo eri = (TranEvaReportInfo) reportInfoList.get(m);
			if (totalWeight == null || "".equals(totalWeight)) {
				totalWeight = eri.getTotalWeight();
			}
			TranEvaReportMultiDeptForm ermdf = new TranEvaReportMultiDeptForm();
			...
			for (int i = 1; i <= maxListNo; i++) {
				...
				for (int j = 0; j < rowList.size(); j++) {
					...
					if (TranEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
						...
						ITranEvaKpiMgr tranEvaKpiMgr = (ITranEvaKpiMgr) getBean("ItranEvaKpiMgr");
						kpiWeights.put(tranEvaKpiMgr.id2Name(etd.getKpiId()), etd
								.getWeight());
					}
				}
			}
