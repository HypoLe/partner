 ...
 * 
 */
public abstract class AssKpiConfigMgrImpl implements IAssKpiConfigMgr {
	...
	public List getConfigsByKpiId( final String kpiId , final String nodeType) {
		return assKpiConfigDao.getConfigsByKpiId(kpiId,nodeType);
	}	
	
	public float getLastWeightByRange( final String parentNodeId , final float value) {
		List list = assKpiConfigDao.getNextLevelAssKpiConfigs(parentNodeId);
		AssKpiConfig assKpiConfig = null;
		for(int i=0;i<list.size();i++){
			assKpiConfig = (AssKpiConfig)list.get(i);
			//下边界
			if("10".equals(assKpiConfig.getValueConfig())&&
					value>=assKpiConfig.getMinValue().floatValue()
					&&value<assKpiConfig.getMaxValue().floatValue()){
				return assKpiConfig.getWeight();

				//上边界
			}else if("01".equals(assKpiConfig.getValueConfig())&&
					value>assKpiConfig.getMinValue().floatValue()
					&&value<=assKpiConfig.getMaxValue().floatValue()){
				return assKpiConfig.getWeight();
			}
