...
/**
 * 类说明：网络资源--基站设备及配套--动环监控设备:动环监控设备DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsMmeDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_mme("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*枚举值：监控主机、视频服务器、主服务器、交换机、业务台、防火墙、路由器*/
			...
			/*格式为：YYYY-MM-DD*/
			ps.setString(12,StaticMethod.nullObject2String(data.get("start_time")));
			/*格式为：YYYY-MM-DD*/
			ps.setString(13,StaticMethod.nullObject2String(data.get("end_time")));
			/*枚举值：工程，现网，空载，退网*/
			ps.setString(14,StaticMethod.nullObject2String(data.get("status")));
			/*动环监控厂家*/
			ps.setString(15,StaticMethod.nullObject2String(data.get("rating_power")));
			/*设备的IP地市，若无地址则不填写*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("device_addr")));
			/*移动公司该设备的负责人*/
