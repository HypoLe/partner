...
/**
 * 类说明：网络资源--基站设备及配套--UPS主机:UPS主机DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsUpsDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_ups("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*符合系统编号规范*/
			ps.setString(8,StaticMethod.nullObject2String(data.get("systemcode")));
			/*不带品牌说明的设备产品型号规格*/
			...
			/*单台UPS额定输出功率，单位：KVA*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("rating_power")));
			/*额定输出电压，单位V，下拉选择:380,220*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("rating_voltage")));
			/*枚举值：N+1（N>1）,1+1主从,单机,1+1均分,双母线,
其他
N+1（N>1）
1+1主从
单机
...
双母线
其他*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("work_mode")));
			/*移动公司该设备的负责人*/
			ps.setString(20,StaticMethod.nullObject2String(data.get("preserver")));
			/*备注*/
			ps.setString(21,StaticMethod.nullObject2String(data.get("remark")));
			/*创建时间*/
			ps.setString(22,StaticMethod.nullObject2String(data.get("create_time")));
			/*机房（基站）名称ID*/
			ps.setString(23,StaticMethod.nullObject2String(data.get("related_room_id")));
			/*所属巡检点主键id*/
