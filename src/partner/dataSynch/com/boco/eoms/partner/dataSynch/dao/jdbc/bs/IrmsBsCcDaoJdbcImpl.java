...
/**
 * 类说明：网络资源--基站设备及配套--风光互补控制器:风光互补控制器DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsCcDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_cc("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*单位：块*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("solar_panels_count")));
			/*单位：个*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("fan_count")));
			/*单位：KW*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("exp_rating_capacit")));
			/*移动公司该设备的负责人*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("preserver")));
			/*创建时间*/
