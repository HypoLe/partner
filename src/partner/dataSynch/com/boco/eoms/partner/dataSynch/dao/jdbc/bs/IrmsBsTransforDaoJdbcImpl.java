...
/**
 * 类说明：网络资源--基站设备及配套--变压器或调压器:变压器或调压器DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsTransforDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_transfor("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*枚举值：干式,油浸式*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("type")));
			/*额定视在功率，单位：KVA*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("rating_power")));
			/*输入额定电压，单位：KV*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("imp_rating_voltage")));
			/*输出额定电压，单位：V*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("ext_rating_voltage")));
			/*移动公司该设备的负责人*/
			ps.setString(20,StaticMethod.nullObject2String(data.get("preserver")));
			/*创建时间*/
			ps.setString(21,StaticMethod.nullObject2String(data.get("create_time")));
			/*备注*/
