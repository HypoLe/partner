...
/**
 * 类说明：网络资源--基站设备及配套--发电机组:发电机组DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsGrneratDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_grnerat("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
...
符合设备命名规范*/
			...
			/*单位：KVA*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("rating_power")));
			/*单位：V （220或380。只填数值不带单位。）*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("rating_voltage")));
			/*枚举值：自动启动，手动启动*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("start_mode")));
			/*枚举值：风冷，水冷*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("cool_mode")));
			/*包括品牌、型号*/
			ps.setString(20,StaticMethod.nullObject2String(data.get("start_battery_mode")));
			/*单位：组*/
			ps.setString(21,StaticMethod.nullObject2String(data.get("start_battery_coun")));
			/*单位：AH*/
			ps.setString(22,StaticMethod.nullObject2String(data.get("start_battery_capa")));
			/*单位：升*/
			ps.setString(23,StaticMethod.nullObject2String(data.get("tank_capacity")));
			/*移动公司该设备的负责人*/
			ps.setString(24,StaticMethod.nullObject2String(data.get("preserver")));
			/*创建时间*/
			ps.setString(25,StaticMethod.nullObject2String(data.get("create_time")));
			/*备注*/
			ps.setString(26,StaticMethod.nullObject2String(data.get("remark")));
			/*机房（基站）名称ID*/
			ps.setString(27,StaticMethod.nullObject2String(data.get("related_room_id")));
			/*所属巡检点主键id*/
			ps.setString(28,StaticMethod.nullObject2String(data.get("inspect_id")));
			/*所属巡检点名称*/
			ps.setString(29,StaticMethod.nullObject2String(data.get("inspect_name")));
