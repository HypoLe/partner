...
/**
 * 类说明：网络资源--基站设备及配套--太阳能设备:太阳能设备DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsSeeDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_see("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*枚举值：高压配电、低压配电、发电机组、开关电源系统、UPS系统 、列头柜、专用空调、中央空调、普通*/
			...
			/*额定输出功率，单位：KVA*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("solar_panels_ratin")));
			/*枚举值：48,24*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("exp_voltage")));
			/*移动公司该设备的负责人*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("preserver")));
			/*创建时间*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("create_time")));
			/*备注*/
			ps.setString(20,StaticMethod.nullObject2String(data.get("remark")));
			/*机房（基站）名称ID*/
