...
/**
 * 类说明：网络资源--基站设备及配套--专用空调:专用空调DaoJdbcImpl实现类
 * 创建人： zhangkeqi
 * 创建时间：2012-11-16 23:11:36
 */
public class IrmsBsSpeairconDaoJdbcImpl extends AbstractDataSynchDaoJdbc {
		/**
		 * 批量插入语句
		 */
		public String getBatchInsertSql(String table){
			return "insert into "+table+"_irms_bs_speaircon("+
						"id,"+
						"related_room,"+
						"device_type,"+
						"device_subclass,"+
		...
		public PreparedStatement addPsBatch(PreparedStatement ps,Map<String,Object> data) throws Exception{
			...
			/*单位：KW*/
			ps.setString(16,StaticMethod.nullObject2String(data.get("cooling_val")));
			/*单位：KW*/
			ps.setString(17,StaticMethod.nullObject2String(data.get("hoting_val")));
			/*额定用电功率，单位KW*/
			ps.setString(18,StaticMethod.nullObject2String(data.get("imp_rating_power")));
			/*枚举值：380,220*/
			ps.setString(19,StaticMethod.nullObject2String(data.get("rating_voltage")));
			/*空调室外机型号*/
			ps.setString(20,StaticMethod.nullObject2String(data.get("outdoor_model")));
			/*移动公司该设备的负责人*/
