...
import com.boco.eoms.partner.netresource.dao.KeyCustomerRoomDao;
...
public class KeyCustomerRoomMgrImpl extends CommonGenericServiceImpl<KeyCustomerRoom> implements KeyCustomerRoomMgr {

	private KeyCustomerRoomDao  kcroomDao;
	private IKeyCustomerRoomDaoJdbc kcroomJdbc;
	...
	public List<KeyCustomerRoom> getKeyCustomerRoomName(String keyCustomerRoomName) {
		...
		hql.append(" select new KeyCustomerRoom(p.id,p.kcroomName) from KeyCustomerRoom p where p.kcroomName='").append(keyCustomerRoomName).append("'");
		
		List<KeyCustomerRoom> list =kcroomDao.findByHql(hql.toString());
	 ...
	 * @return
	 */
	public ImportResult importKeyCustomerRoomFromFile( FormFile formFile,String province,String specialty,String creatorId,String osPath) throws Exception{
		...
		ImportResult result=ei.importFromFile(formFile,1,new DataSaveCallback(){
			...
			public List<Object> fromRow2Object(HSSFRow row) throws Exception {
				...
				KeyCustomerRoom  bsBtAp = null;
				errorDatas = new String[49];
				errMsg=new StringBuilder();
				
				int colNum=0;
				try {
					bsBtAp = new KeyCustomerRoom();
					...
					if("".equals(apName)){
						...
						List<KeyCustomerRoom> sbsbtlist=getKeyCustomerRoomName(apName);
						if(sbsbtlist.size()>0){
							errMsg.append("该位置名称在系统中已存在;");
							//throw new Exception("该机房位置在系统中已存在");
						}					
					}
					bsBtAp.setKcroomName(apName);
					colNum++;
					...
					bsBtAp.setKcroomAddress(apAddress);
					colNum++;
					//资产归属
					...
					bsBtAp.setKcroomVested(apVested);
					colNum++;
...
//					保存错误信息的集合
		...
		try {
			...
			if(mainList.size()>0){
				...
				StringBuffer insertMainSql = new StringBuffer();
				 insertMainSql.append(" insert into PNR_KEY_CUSTOMER_ROOM (ID, KCROOM_NAME,CITY , country,BUREAU ,");//5
				 insertMainSql.append(" ACCESS_TYPE ,KCROOM_ADDRESS,");//2
				 ...
				 insertMainSql.append(" KCROOM_VESTED,IS_SHARE,SHARE_OPERATORS,KCROOM_CATEGORIES,KCROOM_SMALL_CLASS,");//5
				 insertMainSql.append(" GROUP_KCROOM_TYPES,GROUP_ROOM_BEAR,ALIGNMENT_METHOD,CHUTES,FIRE_SYSTEM,");//5
				...
				for(int i = 0;i<mainlist1.size();i++){
					kcroomJdbc.importData(mainlist1.get(i),stmt1,1);
							stmt1.executeBatch();
							stmt1.clearBatch();
				...
				for(int k=0;k<mainList.size();k++){
					...
					PnrResConfig pnrResConfig=null;
					KeyCustomerRoom sr = (KeyCustomerRoom) mainList.get(k);
					//资源信息
					pnrResConfig = new PnrResConfig();
					pnrResConfig.setId(UUIDHexGenerator.getInstance().getID());
					pnrResConfig.setResName(sr.getKcroomName());
					//专业
					...
					pnrResConfig.setSubResTable("pnr_key_customer_room");
			...
			if(errorSize>0){
				//开始将错误的信息导入到excel中 
				String outfilepath ="zhongdiankehujifang"+creatorId+".xls";
				...
				String sheetName="keyCustomerRoom";
	...
	public KeyCustomerRoomDao getKcroomDao() {
		return kcroomDao;
	}

	public void setKcroomDao(KeyCustomerRoomDao kcroomDao) {
		this.kcroomDao = kcroomDao;
		this.setCommonGenericDao(kcroomDao);
	}

	public IKeyCustomerRoomDaoJdbc getKcroomJdbc() {
		return kcroomJdbc;
	}

	public void setKcroomJdbc(IKeyCustomerRoomDaoJdbc kcroomJdbc) {
		this.kcroomJdbc = kcroomJdbc;
	}
