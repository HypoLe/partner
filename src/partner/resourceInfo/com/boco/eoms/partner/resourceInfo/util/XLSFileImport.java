...
public  abstract class XLSFileImport {
	public ImportResult xlsFileValidate(File file) throws Exception{
		HSSFSheet sheet = this.fileCheck(file);
		XLSModel xlsModel=this.getXLSModel();
		int count = 0;
		ImportResult result = new ImportResult();
		try {
			for (int i = xlsModel.getAddStartRowNum(); i <= sheet.getLastRowNum(); i++) { // 获取工作表中每行数据
				HSSFRow row = sheet.getRow(i);
				if(!this.blankRowCheck(row,xlsModel)) {
					if (this.doSaveRow2Data(row)) {
						count++;
					}
				} else {
					break;
	...
	public ImportResult xlsFileValidate(FormFile formFile) throws Exception{
	 ...
	 * @throws Exception
	 */
	public abstract  boolean doSaveRow2Data(HSSFRow row) throws Exception;
}
