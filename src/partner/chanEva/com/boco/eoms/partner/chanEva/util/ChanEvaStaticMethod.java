 ...
 * 
 */
public class ChanEvaStaticMethod {
	...
	public static String getStartTimeByCycle(String cycle,
		...
		if (ChanEvaConstants.CYCLE_YEAR.equals(cycle)) {
		...
		} else if (ChanEvaConstants.CYCLE_QUARTER.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			...
			startTime = DateTimeUtil.dateToString(date);
		} else if (ChanEvaConstants.CYCLE_MONTH.equals(cycle)) {
			int year = Integer.parseInt(currentDateTime.substring(0, 4));
			int month = Integer.parseInt(currentDateTime.substring(5, 7));
			Date date = DateTimeUtil.getFirstDayOfMonth(year, month);
			startTime = DateTimeUtil.dateToString(date);
		} else if (ChanEvaConstants.CYCLE_WEEK.equals(cycle)) {
	...
	public static String orgId2Name(String orgId, String orgType)
		...
		String orgName = "";
		// 若为角色则显示角色名称
		if (ChanEvaConstants.ORG_SUBROLE.equals(orgType)) {
		...
		// 若为用户则显示用户名称
		else if (ChanEvaConstants.ORG_USER.equals(orgType)) {
		...
		// 若为部门则显示部门名称
		else if (ChanEvaConstants.ORG_DEPT.equals(orgType)) {
	...
	public static List jsonOrg2Orgs(String orgs) {
		...
		for (Iterator it = jsonOrgs.iterator(); it.hasNext();) {
			...
			// 限制发布范围
			if (ChanEvaConstants.ORG_DEPT.equals(nodeType)
				...
				for (int i = 0; i < depts.size(); i++) {
					...
					orgList.add(new ChanEvaOrg(dept.getDeptId(),
							StaticVariable.PRIV_ASSIGNTYPE_DEPT));
				}
			} else if (ChanEvaConstants.ORG_SUBROLE.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_ROLE.equals(nodeType)) {
				orgList
						.add(new ChanEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_ROLE));
			} else if (ChanEvaConstants.ORG_USER.equals(nodeType)
					|| StaticVariable.PRIV_ASSIGNTYPE_USER.equals(nodeType)) {
				orgList
						.add(new ChanEvaOrg(id, StaticVariable.PRIV_ASSIGNTYPE_USER));
			}
		}
		return orgList;
	}
	...
	public static String getOrgList(List orgs) {
		...
		if (null != orgs && !orgs.isEmpty()) {
			...
			for (Iterator it = orgs.iterator(); it.hasNext();) {
				ChanEvaOrg org = (ChanEvaOrg) it.next();
				int orgtype = StaticMethod.null2int(org.getOrgType());
				JSONObject item = new JSONObject();
				// 构造json对象
				item.put(UIConstants.JSON_ID, org.getOrgId());
				item.put(UIConstants.JSON_NAME, ChanEvaStaticMethod.getOrgName(org
	...
	public static void main(String[] args) {
		System.out.println("本年第一天："
				+ getStartTimeByCycle(ChanEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本季度第一天："
				+ getStartTimeByCycle(ChanEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本月第一天："
				+ getStartTimeByCycle(ChanEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本周第一天："
				+ getStartTimeByCycle(ChanEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本年最后一天："
				+ getEndTimeByCycle(ChanEvaConstants.CYCLE_YEAR, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本季度最后一天："
				+ getEndTimeByCycle(ChanEvaConstants.CYCLE_QUARTER, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本月最后一天："
				+ getEndTimeByCycle(ChanEvaConstants.CYCLE_MONTH, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
		System.out.println("本周最后一天："
				+ getEndTimeByCycle(ChanEvaConstants.CYCLE_WEEK, DateTimeUtil
						.getCurrentDateTime(ChanEvaConstants.DATE_FORMAT)));
	}
}
