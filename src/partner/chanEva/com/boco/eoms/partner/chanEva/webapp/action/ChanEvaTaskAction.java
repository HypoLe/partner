...
import com.boco.eoms.partner.chanEva.util.RoleIdList;
import com.boco.eoms.partner.chanEva.webapp.form.ChanEvaKpiInstanceForm;

public final class ChanEvaTaskAction extends BaseAction {
...
//		}
	...
	public ActionForward changeTemplateType(ActionMapping mapping,
		...
		TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
		IChanEvaTaskMgr taskMgr = (IChanEvaTaskMgr) getBean("IchanEvaTaskMgr");
		IChanEvaTemplateMgr tempMgr = (IChanEvaTemplateMgr) getBean("IchanEvaTemplateMgr");
		IChanEvaTreeMgr treeMgr = (IChanEvaTreeMgr) getBean("IchanEvaTreeMgr");

		//2009-8-4 模版显示加角色控制，考核模版区（省）负责人能看到所有模版
		List list = new ArrayList();
		TawSystemSessionForm sessionForm = (TawSystemSessionForm) request.getSession().getAttribute("sessionform");
		RoleIdList roleIdList = (RoleIdList) getBean("chanEvaRoleIdList");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdmin(ChanEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrg(sessionform.getDeptid(),ChanEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			ChanEvaTask et = (ChanEvaTask) list.get(i);
			ChanEvaTemplate template = templateMgr.getTemplate(et.getTemplateId());
			String starttime = "";
	...
	public ActionForward changeTemplateTypeAndTime(ActionMapping mapping,
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdminInTime(ChanEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrgInTime(sessionform.getDeptid(),ChanEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
	...
	public ActionForward taskDetailList(ActionMapping mapping, ActionForm form,
		...
		ChanEvaTemplate chanEvaTemplate = templateMgr.getTemplate(et.getTemplateId());
		String parIds = chanEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
		...
		for (int i = 1; i <= maxListNo; i++) {
			...
			for (int j = 0; j < rowList.size(); j++) {
				...
				if (ChanEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					ChanEvaKpiInstance ekis = new ChanEvaKpiInstance();
					for(int k =0 ; parIdssList.size()>k;k++){
						ekis = chanEvaKpiInstanceMgr
...
//						}
					...
					if (ekis.getIsPublish() != null
						...
						ekif.setIsPublish(ChanEvaConstants.TASK_NOT_PUBLISHED);
					}
	...
	public ActionForward saveTaskDetail(ActionMapping mapping, ActionForm form,
		...
		if (xsave(mapping, form, request, response,
				ChanEvaConstants.TASK_NOT_PUBLISHED))
	...
	public ActionForward commitTaskDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (xsave(mapping, form, request, response, ChanEvaConstants.TASK_PUBLISHED))
			return mapping.findForward("success");
		else
			return mapping.findForward("fail");
	}
	...
	public boolean xsave(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			IChanEvaTaskDetailMgr taskDetailMgr = (IChanEvaTaskDetailMgr) getBean("IchanEvaTaskDetailMgr");
			IChanEvaKpiInstanceMgr chanEvaKpiInstanceMgr = (IChanEvaKpiInstanceMgr) getBean("IchanEvaKpiInstanceMgr");
			IChanEvaReportInfoMgr chanEvaReportInfoMgr = (IChanEvaReportInfoMgr) getBean("IchanEvaReportInfoMgr");
			AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
//			有几个合作伙伴保存几次
			for(int f = 0 ; parIdss.length>f;f++){
				...
				for (int i = 1; i <= maxListNo; i++) {
					...
					for (int j = 0; j < rowList.size(); j++) {
						...
						if (ChanEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
							...
							chanEvaKpiInstanceMgr.saveKpiInstance(ekis);
						}
					}
					if (rowList.size() > maxLevel) {
						maxLevel = rowList.size();
				...
				if (saveType.equals(ChanEvaConstants.TASK_PUBLISHED)) {
					ChanEvaReportInfo eri = new ChanEvaReportInfo();
					...
					chanEvaReportInfoMgr.saveChanEvaReportInfo(eri);
				}
			}
			return true;
		} catch (Exception ex) {
...
//		String deptName = sessionform.getDeptname();
	...
	public ActionForward xquery(ActionMapping mapping, ActionForm form,
		...
		if (searchType == null || "".equals(searchType)) {
			searchType = ChanEvaConstants.TASK_PUBLISHED;
		}
		...
		for (int i = 0; i < rowList.size(); i++) {
			ChanEvaTaskDetail etd = (ChanEvaTaskDetail) rowList.get(i);
			if (ChanEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
				taskList = chanEvaKpiInstanceMgr
						.getKpiInstanceListByTimeAndPartner(etd.getId(),
								partner, timeType, startTime, endTime,
								searchType);
				for (int j = 0; j < taskList.size(); j++) {
					ChanEvaKpiInstance edi = (ChanEvaKpiInstance) taskList.get(j);
		...
		for (int i = 0; i < taskDetailList.size(); i++) {
			ChanEvaKpiInstance eki = (ChanEvaKpiInstance) taskDetailList.get(i);
			ChanEvaKpiInstanceForm ekif = (ChanEvaKpiInstanceForm)convert(eki);
			ChanEvaTaskDetail etd = taskDetailMgr.getTaskDetailById(eki.getTaskDetailId());
			ChanEvaTask et = taskMgr.getTaskById(etd.getTaskId());
