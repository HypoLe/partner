...
public class SheetDealLimitDAOImpl extends BaseDaoHibernate implements ISheetDealLimitDAO {
	 ...
	 * 
	 */
	public StepLimit getStepLimitByCondition(String flowName, String specialty1, String specialty2, String taskName) {
		String hql = "from StepLimit stepLimit where stepLimit.taskName='"+taskName+"' and stepLimit.levelId = (select id from LevelLimit levelLimit where flowName='"+flowName+"' and specialty1='"+specialty1+"' and specialty2='"+specialty2+"')";
		StepLimit steplimit = new StepLimit();
		List list = getHibernateTemplate().find(hql);
		if (list.size() == 0) {
			steplimit = null;
		} else {
			steplimit = (StepLimit) list.get(0);
		}
		return steplimit;
	}
