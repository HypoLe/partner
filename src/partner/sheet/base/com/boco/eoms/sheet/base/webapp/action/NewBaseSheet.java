...
import com.boco.eoms.sheet.base.adapter.service.partner.wps.WPSEngineServiceMethod;
...
public abstract class NewBaseSheet implements NewIBaseSheet {
	...
	public void newSaveDraft(ActionMapping mapping, ActionForm form,
		...
		mainObject.setSendContact(sessionform.getContactMobile());
		mainObject.setSendTime(StaticMethod.getLocalTime());
		mainObject.setSheetId(this.mainService.getSheetId(null));
	...
	public void newPerformNonFlow(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			/** 
			 * modify by 陈元蜀 2012-09-11 begin
			 * eoms才有的逻辑，合作伙伴中不需要
			...
			if (!pbcTaskId.equals("")) {
				...
				java.security.PrivilegedAction getActivite = new java.security.PrivilegedAction() {
					public Object run() {
						...
						try {
							...
							if (task != null
									&& (taskName.equals("cc")
											|| taskName.equals("advice") || taskName
											.equals("reply"))) {
								ishaveActivite = "true";
							}
						} catch (Exception e) {
				...
				HashMap linkMap = (HashMap) this.getFlowEngineMap().get("link");
				int operateType = StaticMethod.nullObject2int(linkMap
						.get("operateType"));
				// 说明是老流程
				if (respone != null) {
...
//			businessFlowService.createSubTaskByMap(TKID, parentTaskId, subTaskName, taskNameSpace, parentTaskName, map, sessionMap);
	...
	public String getAttach(List attachList) {
		...
		return wm.getAttach(attachList, this.getSheetAttachCode());
	}
