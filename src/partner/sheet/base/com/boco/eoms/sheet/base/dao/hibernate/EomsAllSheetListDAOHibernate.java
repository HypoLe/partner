...
public class EomsAllSheetListDAOHibernate extends BaseSheetDaoHibernate
		...
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				try {
					...
					String queryGruopCount = " select t.sheetType as sheetType,t.sheetTypeName as sheetTypeName,count(t.id) as c ";
					if (sql_orderby > 0)
	...
	public HashMap getAllMainsByHql(final String hql, final Integer curPage,
			final Integer pageSize) throws HibernateException {
		...
		HibernateCallback callback = new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				try {
					...
					SQLQuery query = session.createSQLQuery(hql);
					query.addEntity(EomsStartedByMeView.class);
