...
public class SheetAction extends NewSheetAction {
	...
	public ActionForward performQuery(ActionMapping mapping, ActionForm form,
			...
			throws Exception {
		String beanName = mapping.getAttribute();
		beanName = beanName + "List";
		ISheetListService baseSheet = (ISheetListService) getBean(beanName);
	...
	public ActionForward showSheetDealList(ActionMapping mapping,
		...
		//手机获取工单历史add by lee 2013.5.3 17:59
		if(com.boco.eoms.mobile.util.MobileCommonUtils.isAndroidRequest(request)){
			request.setAttribute("beanName", beanName);
			com.boco.eoms.mobile.util.SheetLinkHandler.handlerLinkData(request, response);
			return null;
		}
...
//				.getParameter("appCode"));
		...
		if (!sql.equals("")) {
			...
			String linkTable = HibernateConfigurationHelper
					.getTableName(baseSheet.getLinkService().getLinkObject().getClass());
			// 替换poMain为main表名
...
//							}
						...
						if (!attidCorrlection.equals("")) {
							...
							for (int i = 0; attTemList != null
									&& i < attTemList.size(); i++) {
								TawCommonsAccessories tawCommonsAccessories = (TawCommonsAccessories) attTemList.get(i);
								/**
								 * 获取原始文件路径以及目录
								 */
								if (uploadPath.equals("")) {
									String configPath = AccessoriesMgrLocator
											...
											.getFilePath(tawCommonsAccessories.getAppCode());
									...
									if (configPath.indexOf(separator) > 0) {
										...
										if (configPath.lastIndexOf(separator) == configPath
											...
											configPath = configPath
													.substring(configPath.lastIndexOf(separator) - 1,configPath.lastIndexOf(separator));
										}
									}
									newFilePath = uploadPath + newForder + separator;
									com.boco.eoms.common.util.StaticMethod.newFolder(newFilePath);
									BocoLog.info(this, "新建打包目录" + newFilePath + "成功");
									newFileFullPath = uploadPath + newForder + separator + sheetId + separator;
								}
								...
								try {
									...
									BocoLog.info(this, "拷贝到"+ newfileFullPathWithCNName);
									...
									if (file.exists()) {
										...
										BocoLog.info(this, "新建工单目录"+ newFileFullPath + "成功");
										com.boco.eoms.common.util.StaticMethod.copyFile(fileFullPath,newfileFullPathWithCNName);
										BocoLog.info(this, "已经将"+ fileFullPath);
										BocoLog.info(this, "拷贝到"+ newfileFullPathWithCNName);
									} else {
										if (errString.equals("")) {
											errString = "以下文件:\\n";
									...
									BocoLog.info(this, ex.getMessage() + "文件"+ newfileFullPathWithCNName+ "不存在");
									continue;
								}
							}
						}
	 ...
	 * 通过EOMS工单流水号查询工单列表
	 */
	public ActionForward showSheetListByEomsSheetId(ActionMapping mapping, ActionForm form,
			HttpServletRequest request,HttpServletResponse response) throws Exception {
		String beanName = mapping.getAttribute();
		beanName = beanName + "ListShow";
		ISheetListShowService baseSheet = (ISheetListShowService) getBean(beanName);
		baseSheet.showSheetListByEomsSheetId(mapping, form, request, response);
		return mapping.findForward("mainlist");
	}
	...
	public ActionForward showStatisticInfo(ActionMapping mapping, ActionForm form,
		...
		beanName = beanName + "InfoShow";
		ISheetInfoShowService baseSheet = (ISheetInfoShowService) getBean(beanName);
		baseSheet.showStatisticInfo(mapping, form, request, response);
		return mapping.findForward("sheetStatistic");
	}
}
