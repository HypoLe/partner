...
public abstract class BaseSheet extends NewBaseSheet implements IBaseSheet {
	...
	public void performAdd(ActionMapping mapping, ActionForm form,
		...
		BaseMain main = this.getMainService().getMainObject().getClass().newInstance();
		BaseLink link = this.getLinkService().getLinkObject().getClass().newInstance();
...
//				response);// 获取main link operate对象
		...
		HashMap WpsMap = SheetUtils.actionMapToEngineMap(map,columnMap);
		...
		/** add by qinmin* */
		businessFlowService.triggerProcessByMap(processTemplateName, operateName,
	...
	public void performDeal(ActionMapping mapping, ActionForm form,
		...
		// finish task
		businessFlowService.finishTaskByMap(taskId, getFlowEngineMap(),
				sessionMap);
	}
...
//		businessFlowService.cancel(processId, sessionMap);
	...
	public void performClaim(ActionMapping mapping, ActionForm form,
		...
		sessionMap.put("userId", sessionform.getUserid());
		sessionMap.put("password", sessionform.getPassword());
		businessFlowService.claimTaskByMap(taskId, getFlowEngineMap(), sessionMap);
	...
	public void performCreateSubTask(ActionMapping mapping, ActionForm form,
		...
		for (int i = 0; i < dealPerformers.length; i++) {
			...
			map.put("dealPerformer", dealPerformers[i]);
//			businessFlowService.createSubTaskByMap(TKID, parentTaskId, subTaskName, taskNameSpace, parentTaskName, map, sessionMap);
		}
	}
	...
	public void performTransferWorkItem(ActionMapping mapping, ActionForm form,
		...
		if (sessionform != null) {
			...
			// 工单转移
			businessFlowService.transferWorkItemByMap(taskId, fromOwnerUserId,
	...
	public void performBatchDeal(ActionMapping mapping, ActionForm form,
		...
		while(iterator.hasNext()){
		  ...
		  try {			   
		   ...
		   Map map = request.getParameterMap();
		   map.put("piid", task.getProcessId());
		   ...
		   map.put("mainId", task.getSheetKey());
				...
				// finish task
				businessFlowService.finishTaskByMap(taskId,
						getFlowEngineMap(), sessionMap);
	...
	public void getDoneAllListsForPortal(ActionMapping mapping,
		...
		HashMap taskListMap = null;
		if (pageSize != null && pageSize.intValue() != 0) {
			taskListMap = this.getTaskService().getDoneAllSheetTask(condition,
					userId, deptId, "", pageIndex, pageSize);
		...
		Feed feed = AtomUtilForTask.showDoneAllSheetListForPortal(taskListMap,
				request);
		OutputStream os = response.getOutputStream();
		PrintStream ps = new PrintStream(os);
		feed.getDocument().writeTo(ps);
