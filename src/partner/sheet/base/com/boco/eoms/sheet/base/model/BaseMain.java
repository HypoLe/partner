...
public class BaseMain implements Serializable{
     ...
     * 
     */
    private String sendObjectTotalJSON;
    
    private String feeType;        //计费方式（归档）
    private String feeReason;      //原因（归档时计费方式选择半次或不计费时必填）
    private Float scoreInTime;     //相应及时率（归档）
    private Float scoreStandard;   //通报规范性（归档）
    private Float scoreFinish;     //任务完成情况（归档）
    private Float scoreStatisfied; //客户满意度（归档）
    private Float scoreTotal;      //总分
    
	public String getSendObjectTotalJSON() {
		return sendObjectTotalJSON;
	}

	public void setSendObjectTotalJSON(String sendObjectTotalJSON) {
		this.sendObjectTotalJSON = sendObjectTotalJSON;
	}
