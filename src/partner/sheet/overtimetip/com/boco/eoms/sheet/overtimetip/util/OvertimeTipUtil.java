...
public class OvertimeTipUtil {
	...
	/**
 	 * 根据完成时限得到超时类型：0未超时，1已超时，2将要超时
	 * @param date 完成时限
	 * @param minute 即将超时的分钟数
	 * @return
	 */
	public static String setOvertimeTipFlag(Date sheetCompleteLimitDate,int minute){
		String overtimeType = "";
		DateTime completeLimitDate = new DateTime(sheetCompleteLimitDate);
		DateTime now = new DateTime();
		if(now.isAfter(completeLimitDate)){//超时
			overtimeType = "1";
		}else{
			if(now.plusMinutes(minute).isAfter(completeLimitDate)){//在minute分钟后就会超时
				overtimeType = "2"; //将要超时
			}else{
				overtimeType = "0"; //未超时
			}
