...
import com.boco.eoms.assEva.webapp.form.AssEvaReportMultiDeptForm;
...
public final class AssEvaReportInfoAction extends BaseAction {
	/**
	 * 单一月份、单一厂商查询初始
	 * 
	 * @param mapping
	...
	public ActionForward reportSingle(ActionMapping mapping, ActionForm form,
				...
				if (AssEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					...
					if (ekis.getIsPublish() != null
							...
							&& ekis.getIsPublish().equals(
									AssEvaConstants.TASK_PUBLISHED)) {
		...
		// 找到任务权重总分和实际总得分：
		IAssEvaReportInfoMgr reportInfoMgr = (IAssEvaReportInfoMgr) getBean("IassEvaReportInfoMgr");
		...
		if (!reportInfoList.isEmpty()) {
			AssEvaReportInfo eri = (AssEvaReportInfo) reportInfoList.get(0);
	...
	public ActionForward reportMultiDept(ActionMapping mapping,
		...
		for (int m = 0; m < reportInfoList.size(); m++) {
			AssEvaReportInfo eri = (AssEvaReportInfo) reportInfoList.get(m);
			if (totalWeight == null || "".equals(totalWeight)) {
				totalWeight = eri.getTotalWeight();
			}
			AssEvaReportMultiDeptForm ermdf = new AssEvaReportMultiDeptForm();
			...
			for (int i = 1; i <= maxListNo; i++) {
				...
				for (int j = 0; j < rowList.size(); j++) {
					...
					if (AssEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
						...
						IAssEvaKpiMgr assEvaKpiMgr = (IAssEvaKpiMgr) getBean("IassEvaKpiMgr");
						kpiWeights.put(assEvaKpiMgr.id2Name(etd.getKpiId()), etd
								.getWeight());
					}
				}
			}
