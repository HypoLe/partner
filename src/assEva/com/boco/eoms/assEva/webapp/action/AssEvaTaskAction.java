...
import com.boco.eoms.assEva.util.RoleIdList;
...
public final class AssEvaTaskAction extends BaseAction {
...
//		得到专业类型		
		...
		for (int i = 0; i < list.size(); i++) {
			AssEvaTree et = (AssEvaTree) list.get(i);
			templateType.add(et);
		}
...
//		}
	...
	public ActionForward changeTemplateType(ActionMapping mapping,
		...
		TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
		IAssEvaTaskMgr taskMgr = (IAssEvaTaskMgr) getBean("IassEvaTaskMgr");
		IAssEvaTemplateMgr tempMgr = (IAssEvaTemplateMgr) getBean("IassEvaTemplateMgr");
		IAssEvaTreeMgr treeMgr = (IAssEvaTreeMgr) getBean("IassEvaTreeMgr");

		//2009-8-4 模版显示加角色控制，考核模版区（省）负责人能看到所有模版
		List list = new ArrayList();
		TawSystemSessionForm sessionForm = (TawSystemSessionForm) request.getSession().getAttribute("sessionform");
		RoleIdList roleIdList = (RoleIdList) getBean("assEvaRoleIdList");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdmin(AssEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrg(sessionform.getDeptid(),AssEvaConstants.TEMPLATE_ACTIVATED, templateTypeId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			AssEvaTask et = (AssEvaTask) list.get(i);
			AssEvaTemplate template = templateMgr.getTemplate(et.getTemplateId());
			String starttime = "";
...
//		IAssEvaTreeMgr treeMgr = (IAssEvaTreeMgr) getBean("IassEvaTreeMgr");
		...
		if(isProvinceAdmin.equals("Y")){
			list = taskMgr.listTaskOfProvinceAdminInTime(AssEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
		else if(isProvinceAdmin.equals("N")){
			list = taskMgr.listTaskOfOrgInTime(sessionform.getDeptid(),AssEvaConstants.TEMPLATE_ACTIVATED, templateTypeId, start, end);
		}
	...
	public ActionForward taskDetailList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		IAssEvaTaskDetailMgr taskDetailMgr = (IAssEvaTaskDetailMgr) getBean("IassEvaTaskDetailMgr");
		IAssEvaKpiInstanceMgr assEvaKpiInstanceMgr = (IAssEvaKpiInstanceMgr) getBean("IassEvaKpiInstanceMgr");
		IAssEvaKpiMgr assEvaKpiMgr = (IAssEvaKpiMgr) getBean("IassEvaKpiMgr");
		...
		IAssEvaTaskMgr taskMgr = (IAssEvaTaskMgr) getBean("IassEvaTaskMgr");
		IAssEvaTemplateMgr templateMgr = (IAssEvaTemplateMgr) getBean("IassEvaTemplateMgr");
		AssEvaTask et = taskMgr.getTaskById(taskId);
		String taskName = templateMgr.id2Name(et.getTemplateId());
		
		AssEvaTemplate assEvaTemplate = templateMgr.getTemplate(et.getTemplateId());
		String parIds = assEvaTemplate.getParIds();
		if(parIds.indexOf(",")==0){
...
//		String timeType = "月度";// 后续开发
		...
		for (int i = 1; i <= maxListNo; i++) {
			...
			for (int j = 0; j < rowList.size(); j++) {
				AssEvaTaskDetail etd = (AssEvaTaskDetail) rowList.get(j);
				AssEvaKpiInstanceForm ekif = new AssEvaKpiInstanceForm();
				...
				if (AssEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
					AssEvaKpiInstance ekis = new AssEvaKpiInstance();
					for(int k =0 ; parIdssList.size()>k;k++){
						ekis = assEvaKpiInstanceMgr
...
//						}
					...
					if (ekis.getIsPublish() != null
						...
						ekif.setIsPublish(AssEvaConstants.TASK_NOT_PUBLISHED);
					}
				...
				// 算法加入
				AssEvaKpi ek = assEvaKpiMgr.getKpi(ekif.getKpiId());
	...
	public ActionForward saveTaskDetail(ActionMapping mapping, ActionForm form,
		...
		if (xsave(mapping, form, request, response,
				AssEvaConstants.TASK_NOT_PUBLISHED))
	...
	public ActionForward commitTaskDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (xsave(mapping, form, request, response, AssEvaConstants.TASK_PUBLISHED))
			return mapping.findForward("success");
		else
			return mapping.findForward("fail");
	}
	...
	public boolean xsave(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			IAssEvaTaskDetailMgr taskDetailMgr = (IAssEvaTaskDetailMgr) getBean("IassEvaTaskDetailMgr");
			IAssEvaKpiInstanceMgr assEvaKpiInstanceMgr = (IAssEvaKpiInstanceMgr) getBean("IassEvaKpiInstanceMgr");
			IAssEvaReportInfoMgr assEvaReportInfoMgr = (IAssEvaReportInfoMgr) getBean("IassEvaReportInfoMgr");
			AreaDeptTreeMgr areaDeptTreeMgr = (AreaDeptTreeMgr) getBean("areaDeptTreeMgr");
//			有几个合作伙伴保存几次
			for(int f = 0 ; parIdss.length>f;f++){
				...
				for (int i = 1; i <= maxListNo; i++) {
					...
					for (int j = 0; j < rowList.size(); j++) {
						...
						if (AssEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
							...
							AssEvaKpiInstance ekis = assEvaKpiInstanceMgr
									.getKpiInstanceByTimeAndPartner(etd.getId(),
							...
							assEvaKpiInstanceMgr.saveKpiInstance(ekis);
						}
					}
					if (rowList.size() > maxLevel) {
						maxLevel = rowList.size();
				...
				if (saveType.equals(AssEvaConstants.TASK_PUBLISHED)) {
					AssEvaReportInfo eri = new AssEvaReportInfo();
					...
					assEvaReportInfoMgr.saveAssEvaReportInfo(eri);
				}
			}
			return true;
		} catch (Exception ex) {
...
//		String deptName = sessionform.getDeptname();
	...
	public ActionForward xquery(ActionMapping mapping, ActionForm form,
		...
		if (searchType == null || "".equals(searchType)) {
			searchType = AssEvaConstants.TASK_PUBLISHED;
		}
		...
		for (int i = 0; i < rowList.size(); i++) {
			AssEvaTaskDetail etd = (AssEvaTaskDetail) rowList.get(i);
			if (AssEvaConstants.NODE_LEAF.equals(etd.getLeaf())) {
				taskList = assEvaKpiInstanceMgr
						.getKpiInstanceListByTimeAndPartner(etd.getId(),
								partner, timeType, startTime, endTime,
								searchType);
				for (int j = 0; j < taskList.size(); j++) {
					AssEvaKpiInstance edi = (AssEvaKpiInstance) taskList.get(j);
		...
		for (int i = 0; i < taskDetailList.size(); i++) {
			AssEvaKpiInstance eki = (AssEvaKpiInstance) taskDetailList.get(i);
			AssEvaKpiInstanceForm ekif = (AssEvaKpiInstanceForm)convert(eki);
			AssEvaTaskDetail etd = taskDetailMgr.getTaskDetailById(eki.getTaskDetailId());
			AssEvaTask et = taskMgr.getTaskById(etd.getTaskId());
