...
public class CodeTypeComsInfo {
  ...
  private void getCodeTypeInfo() throws Exception {
    ...
    typenames = (ArrayList) (staticobj.getRecordFromObj("CodeTypeInfo",
        "typenames"));
  ...
  private void setCodeTypeInfo() throws Exception {
    ...
    try {
      rt.execute(
          "select distinct dict_type,dict_typelabel from taw_ws_dict_type");
      while (rt.next()) {
        typeids.add(StaticMethod.null2String(rt.getString(1)));
        typenames.add(StaticMethod.nullObject2String(rt.getString(2)));
      }
    }
    catch (Exception e) {
      // writeLog(e) ;
  ...
  public String getTypeName(String id) {
    ...
    String name = "";
    try {
      name = StaticMethod.strFromDBToPage( (String) typenames.get(current_index));
    }
    catch (Exception e) {
  ...
  public String getTypeName() {
    //infomix
    return (String)typenames.get(current_index);
  }
