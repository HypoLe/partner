...
//			myforward = performYearSelect(mapping, form, request, response); // 月度
	...
	private ActionForward performMonthList(ActionMapping mapping,
		...
		String netId=request.getParameter("Mtype");
...
//		yearPlanList=tawwpMonthMgr.listYearPlanByDept(String
//				.valueOf(deptId),year);
		try {
			...
			if(switchChangeNet.equals("0")){
				monthPlanVOHash = tawwpMonthMgr.listMonthPlan(String
						.valueOf(deptId), year, month,netId);
			}else{
...
//				Collections.sort(listKey);
	...
	private ActionForward performCheckViews(ActionMapping mapping,
		...
		String monthPlanId = TawwpUtil.getRequestValue(request, "monthplanid",
				"");
		String yearPlanId = TawwpUtil.getRequestValue(request, "yearPlanId",
		"");
		// 获取月度作业计划审批信息编号
		...
		try {
			// 获取月度作业计划VO对象集合
			monthPlanVOList = tawwpMonthMgr.listMonthCheck(userId,yearPlanId);
	...
	private ActionForward performCheckListPass(ActionMapping mapping,
		...
		//add by fangyongfeng 
		String [] monthcheckid= request.getParameterValues("planid");
		String monthId = "";
		 if(monthcheckid!=null && monthcheckid.length>0){
			for(int i =0;i<monthcheckid.length;i++){
				monthId = monthId +  monthcheckid[i]+",";
			}
		 }
		 if(!"".equals(monthId)){
		...
		try {
			...
			// 通过月度作业计划审批
			tawwpMonthMgr.passMonthCheck(monthId, "", userId);
