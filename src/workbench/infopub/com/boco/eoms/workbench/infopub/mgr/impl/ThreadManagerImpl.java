...
public class ThreadManagerImpl extends BaseManager implements IThreadManager {
	...
	public void saveThread(Thread thread, List threadPermimissionOrgs) {
		...
		String createTime = dateformat.format(thread.getCreateTime());
		String createrId = thread.getCreaterId();
		ITawSystemUserManager uMgr = (ITawSystemUserManager)ApplicationContextHolder.getInstance().getBean("itawSystemUserManager");
		String createrDeptName = uMgr.getTawSystemUserByuserid(createrId).getDeptname();
		String createrName = uMgr.getTawSystemUserByuserid(createrId).getUsername();
		infoMap.put("createrName", createrName);
		...
		if (null != threadPermimissionOrgs && !threadPermimissionOrgs.isEmpty()) {
			...
			if (InfopubConstants.NO_AUDIT.equals(thread.getStatus())) {
				if ("1".equals(thread.getIsSend())) {
					sendMessage(threadPermimissionOrgs, thread.getId(), infoMap);
				}
			}
		}
	}
	...
	// 发送短信
	private void sendMessage(List threadPermimissionOrgs,String threadId,Map infoMap) {
		...
		String serviceId = InfopubUtil.audit;
		IContentManager mgr = (IContentManager)ApplicationContextHolder.getInstance().getBean("IContentManager");
		String sendContent = mgr.getSendContent(infoMap);
		if (null != threadPermimissionOrgs && !threadPermimissionOrgs.isEmpty()) {
			for (Iterator it = threadPermimissionOrgs.iterator(); it.hasNext();) {
				ThreadPermimissionOrg tpo = (ThreadPermimissionOrg) it.next();
				String type = tpo.getOrgType();
				if (StaticVariable.PRIV_ASSIGNTYPE_USER.equals(type)) {					
					msgService.sendMsg(serviceId, sendContent, threadId,
							"1," + tpo.getOrgId(), time);
					...
					if ("1".equals(tpo.getIsIncludeSubDept())) {
						...
						for (int w = 0; w < sonlist.size(); w++) {
							msgService
									.sendMsg(serviceId, sendContent,threadId, "2,"
	 ...
	 */
	public Map getThreads4Draft(Integer curPage, Integer pageSize, String userId, String sortCondition) {
		return dao.getThreads4audit(curPage, pageSize, userId, sortCondition);
	}
		...
		if (threadAuditHistoryForm.getIds() != null) {
			for (int i = 0; i < threadAuditHistoryForm.getIds().length; i++) {
				...
				if (InfopubConstants.AUDIT_PASS.equals(threadAuditHistoryForm
					...
					sendMessage(pOrgs, thread.getId(),infoMap);
				}
				// 保存审核历史信息
	...
	public Map searchThreads(Integer curPage, Integer pageSize,
		...
		return dao.listThreads(curPage, pageSize, threadForm, deptStr, null);
	}
	
	public Map searchThreadsOrderby(Integer curPage, Integer pageSize,
			ThreadForm threadForm, String deptStr, String orderby) {
		return dao.listThreads(curPage, pageSize, threadForm, deptStr, orderby);
	}
	...
	public Map getThreads4Reject(final Integer curPage, final Integer pageSize,
		...
		return dao.getThreads4Reject(curPage, pageSize, userId, sortCondition);
	}
