...
public class Pager {
  ...
  public static String generateForTab(int offset, int length, int size, String url,String param){
	    String ranName = getRandom();
	    String pref1 = "&";
	    String pref;
	    if (url.indexOf("?") > -1) {
	   ...
	   if (length % size == 0) {
	    ...
	    if (offset > 0) {
	      header.append("&nbsp;<a href=\""+url+pref+param+pref1+"pager.size="+length+"&pager.offset1="+(offset-size)+"\">"+
	                 prop.getString("label.prePage")+"</a>\n");
	    }
	    int start;
	    int radius = MAX_PAGE_INDEX/2*size;
	    ...
	    if(offset < length - size) {
	      header.append("&nbsp;<a href=\""+url+pref+param+pref1+"pager.size="+length+"&pager.offset1="+((int)offset+(int)size)+"\">"+
	                prop.getString("label.nextPage")+"</a>\n");
	    }
	    ...
	    header.append("<script language=\"javascript\">\n");
	    header.append("<!--\n");
	    header.append("function goPage"+ ranName +"()\n{window.location=\""+url + pref +param+pref1+"pager.size="+length+"&pager.offset1=\""+" + document.all.page"+ ranName +".value;}\n");
	    header.append("-->\n");
	    header.append("</script>");
