...
public class SelectBSC extends BodyTagSupport {
  ...
  public int doEndTag() throws JspTagException {
   ...
   try {
     ...
     String SQL = "select soa_get_nename(ne_id,ne_type) name,ne_id from tcc_ne_frame where ne_type=200 and region_id=-1483444163 order by name";
     ...
     while (rs.next()) {
       ...
       if (valueStr.indexOf(now_id) == -1) {
         ...
         srcStr += rs.getString("name");
         srcStr += " </option>";
       }else{
         desStr += " <option value=\""+rs.getString("ne_id")+"\">";
         desStr += rs.getString("name");
         desStr += " </option>";
       }
     }
     ...
     ls_display += " <tr>";
     ls_display += " <td width=\"60%\" align=\"center\">";
